 ...
 */
 ...
 /* 2006.03.02:  MorphOS port by Ilka Lehtoranta  */
 /* 2009.03.22:  Small fix by Leif Salomonsson  [don't patch rawdofmt] */
...
CONST_STRPTR Version = "$VER: Sashimi 1.8.1 (22.03.2009)";
...
STATIC CONST CONST_STRPTR ShellTemplate =
   "RECOVER/K,"
   "ON/S,"
   "BUFK/N,"
   "BUFFERSIZE/N,"
...
STATIC CONST_STRPTR SashimiResourceName = "sashimi.resource";
...
STATIC STRPTR (*OldVNewRawDoFmt)(CONST_STRPTR, APTR (*)(APTR, UBYTE), STRPTR, va_list args);
...
ReadFIFOChars(struct SashimiResource * sr,UBYTE * buffer,LONG maxChars)
{
   ...
   if(sr->sr_FIFOBytesStored > 0)
   {
      ...
      do
      {
         ...
         CopyMem(&sr->sr_FIFO[sr->sr_FIFOReadIndex], buffer, howMany);
...
STATIC VOID
SafeRawPutCharPPC(APTR buffer, UBYTE c)
{
...
STATIC
VOID SafeRawPutChar(void)
{
   ULONG old_a0, old_a1;
   old_a0 = REG_A0;
   old_a1 = REG_A1;

   SafeRawPutCharPPC(NULL, REG_D0);
...
extern struct EmulLibEntry NewSafeRawPutCharTrap;


/****************************************************************************/

STATIC CONST struct TagItem NewSetFuncTags[]     = { { SETFUNCTAG_TYPE, SETFUNCTYPE_SYSTEMV }, { TAG_DONE } };
STATIC struct EmulLibEntry NewRawIOInitTrap      = { TRAP_LIBNR, 0, (void (*)())&NewRawIOInit };
STATIC struct EmulLibEntry NewRawMayGetCharTrap  = { TRAP_LIB, 0, (void (*)())&NewRawMayGetChar };
STATIC struct EmulLibEntry NewSafeRawPutCharTrap = { TRAP_LIBNR, 0, (void (*)())&SafeRawPutChar };


STATIC STRPTR
NewVNewRawDoFmt(CONST_STRPTR FmtString, APTR (*PutChProc)(APTR, UBYTE), STRPTR PutChData, va_list args)
{
   if (PutChProc == (APTR)RAWFMTFUNC_SERIAL)
   {
      CONST_STRPTR fmt;
      STRPTR data;
      va_list va;
      ...
      OldVNewRawDoFmt(FmtString, PutChProc, PutChData, va);

      va_end(va);

      FmtString = fmt;
      PutChProc = (APTR)&SafeRawPutCharPPC;
      PutChData = data;
   }

   return OldVNewRawDoFmt(FmtString, PutChProc, PutChData, args);
}
...
RemovePatches(VOID)
{
    ...
    *
    * Doesnt really matter in MorphOS where user interrupts run in the user land... -itix
    */
   Disable();
    ...
    */
   res = SetFunction((struct Library *)SysBase, -504, (ULONG (*)())OldRawIOInit);
   if(res != (APTR)&NewRawIOInitTrap)
      SetFunction((struct Library *)SysBase, -504, (ULONG (*)())res);

   res = SetFunction((struct Library *)SysBase, -510, (ULONG (*)())OldRawMayGetChar);
   if(res != (APTR)&NewRawMayGetCharTrap)
      SetFunction((struct Library *)SysBase, -510, (ULONG (*)())res);

   res = SetFunction((struct Library *)SysBase, -516, (ULONG (*)())OldRawPutChar);
   if(res != (APTR)&NewSafeRawPutCharTrap)
      SetFunction((struct Library *)SysBase, -516, (ULONG (*)())res);

   res = NewSetFunction((struct Library *)SysBase, OldVNewRawDoFmt, -820, (struct TagItem *)&NewSetFuncTags);
   if(res != (APTR)&NewVNewRawDoFmt)
      NewSetFunction((struct Library *)SysBase, res, -820, (struct TagItem *)&NewSetFuncTags);
...
InstallPatches(VOID)
{
   ...
   OldVNewRawDoFmt   = ((struct LibVector *)(((UBYTE *)SysBase) - 820))->func;

   OldRawIOInit      = SetFunction((struct Library *)SysBase, -504, (ULONG (*)())&NewRawIOInitTrap);
   OldRawMayGetChar   = SetFunction((struct Library *)SysBase, -510, (ULONG (*)())&NewRawMayGetCharTrap);
   OldRawPutChar      = SetFunction((struct Library *)SysBase, -516, (ULONG (*)())&NewSafeRawPutCharTrap);
   OldVNewRawDoFmt   = NewSetFunction((struct Library *)SysBase, (APTR)&NewVNewRawDoFmt, -820, (struct TagItem *)&NewSetFuncTags);
...
AddSashimiResource(ULONG bufferSize,struct SashimiResource ** resourcePtr)
{
   ...
   /* This doesnt really work on Pegasos. OF clears the first 256MB and Sashimi buffer is very likely
    * lost. We could cure problem by using MEMF_REVERSED but this introduces new problems... bad luck!
    * Also removed MEMF_PUBLIC flag which is no-op in AmigaOS3 and MorphOS.
    */

   sr = AllocMem(sizeof(struct MemChunk) + sizeof(*sr) + bufferSize-1,MEMF_ANY);
   if(sr != NULL)
   {
...
SaveBuffer(const STRPTR name,struct SashimiResource * sr,LONG mode)
{
    ...
    * buffer data in.
    */
   buffer = AllocVecTaskPooled(sr->sr_FIFOTotalSize);
   if(buffer != NULL)
   {
      LONG bytesInBuffer;
      BOOL wrapped;
      ...
      if(wrapped)
      {
          ...
          */
         CopyMem(&sr->sr_FIFO[sr->sr_FIFOWriteIndex], buffer, oldBytes);
         CopyMem(sr->sr_FIFO, &buffer[oldBytes], sr->sr_FIFOWriteIndex);

         bytesInBuffer = sr->sr_FIFOTotalSize;
      }
      else
      {
         CopyMem(sr->sr_FIFO, buffer, sr->sr_FIFOWriteIndex);
...
Recover(const STRPTR fileName)
{
   ...
   {
      /* Skip inexistant chip memory on Pegasos */

...
*	MungFriend
*	version 3.7 (rev 17)
*	© 1995-98 Szymon Pura and David Zaroski
...
**  V3.5    22.12.97    Introduced checks against trashed KickTags.       **
**                      (thanks, Dave!)                                   **
**  V3.6    27.02.98    Great deal of bugs fixed                          **                                                                        **
**  V3.7    09.09.98    Yet another bug fixed...                          **
****************************************************************************


	include	MOOPS2:EInclude.m
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-
	...
	STRUCT  mfrn_allocabspad,8	; system memory node here
...
; special procs
	...
	APTR	mfrn_OldVector	* original _LVORawPutChar address
...
frem_3	JSRL	Enable
	move.l	MFNode(pc),a0
;dz
	addq.l	#mfrn_Resident,a0
;dz
	JSRL	Forbid
	lea	KickTagPtr(a6),a3
	move.l 	(a3),a2
frem_0	move.l	(a2),a1
	move.l	a1,d0
	beq	frem_1
...
frem_A	bclr	#31,d0
	...
	move.l	d0,a2
	bra	frem_0
frem_2: move.l	MFNode(pc),a0
	move.l	mfrn_Succ(a0),d1
	move.l	d1,(a3)
	bclr	#7,KickTagPtr(a6)
...
; now we need to remove MF node from the ResModules list
	lea	MyNameString(pc),a1
	JSRL	FindResident
	tst.l	d0
	beq	4$	; oh, a pre-reset situation :)
	lea	ResModules(a6),a3
	move.l	(a3),d1
	beq	4$	; this should never happen, but just in case of fire
	move.l	d1,a0
5$	move.l	(a0),d1
	beq	4$	; now that's strange
	bclr	#31,d1
	beq	7$	; new array
	move.l	a0,a3
	move.l	d1,a0
...
7$	cmp.l	d0,d1
	...
	bra	5$
6$	move.l	4(a0),(a0)+
	bne	6$	; now we're ready
	move.l	(a3),d0
	bclr	#1,d0
	move.l	d0,a0
	tst.l	(a0)
	bne	4$
	clr.l	(a3)	; let's delete the whole table
4$	JSRL	Permit
;	move.l	MFNode(pc),a0	; memory won't be freed until the next reset :-(
;	bsr	FreeResNode
	bra	QuitOk
NoMemory
...
f_Install
	...
	move.l	28(a2),a0
	move.l	(a0),d0
;	cmpi.l	#10239,d0		; is this really necessary?
;	bhi	1$
...
finst_3	movea.l	4.w,a6
	...
	beq.s	nokicktags
	bset	#31,d0
	move.l	d0,mfrn_Succ(a3)
nokicktags:
	lea.l	mfrn_MyResident(a3),a0			;dz
	move.l	a0,KickTagPtr(a6)				;dz
...
FindResNode	* () -> resnode/NULL
	movem.l	a2/a3/a6,-(sp)
	CALLEXEC	Forbid
	lea	MyNameString(pc),a3
	move.l	KickTagPtr(a6),a2
	move.l	a2,d0				
	beq	1$					;must check for null
3$	move.l	(a2)+,d0
	beq	1$
	bpl	2$
	bclr	#31,d0
...
1$	JSRL	Permit
	tst.l	d0
	beq	5$
	subq.l	#mfrn_Resident,d0	; the mfrn_allocabspad
5$	movem.l	(sp)+,a6/a2/a3
	rts
FreeResNode	* (node)(a0)
	move.l	a6,-(sp)
...
mppr_0	btst	#MFRNB_FLASH,mfrn_Flags(a0)
	...
	lea	$dff000,a1	; this probably won't work on GFX boards
...
MyNameString
	...
	dc.b	'$VER: '
MyIDString
	dc.b	'MungFriend 3.7  (09.09.98) BLABLA PRODUCT',CR,LF,0
MyTemplate
...
NoCommand
	dc.b	$1b,'[32mMungFriend 3.7',$1b,'[31m   by Szymon Pura, David Zaroski. FREEWARE - BLABLA PRODUCT',LF
	dc.b	'USAGE:',LF

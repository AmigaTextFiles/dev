{ Auslesen des Konfigurationsstrings (Config.p)
...
CONST WIZARDFILE               = "config.wizard";
      WINDOW_MAIN              = 1;
      WINDOW_MAIN_GADGETS      = 5;
      MAIN_BUTTON_FIRST        = 3;
      MAIN_BUTTON_SECOND       = 4;
      MAIN_ARGS_CONFIGDISPLAY  = 2;
                    ...
                    THEN
                     BEGIN
                      ...
                      CASE Msg^.Class OF
                       ...
                       IDCMP_IDCMPUPDATE:
                        CASE GetTagData(GA_ID,0,p_tagitem(msg^.iaddress)) OF
                         MAIN_BUTTON_FIRST:
                          BEGIN
                           tags2[0]:=TagItem(WARGSA_Arg0,long(WZ_GadgetConfig(winhandle,gads[MAIN_BUTTON_FIRST])));
                           dummy:=SetGadgetAttrsA(gads[MAIN_ARGS_CONFIGDISPLAY],window,NIL,^tags2);
                          END;

                         MAIN_BUTTON_SECOND:
                          BEGIN
                           tags2[0]:=TagItem(WARGSA_Arg0,long(WZ_GadgetConfig(winhandle,gads[MAIN_BUTTON_SECOND])));

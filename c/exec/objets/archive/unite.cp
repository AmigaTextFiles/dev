...
Unite::Unite(TEXTE *nomServ, OCTET priServ):
             (&ServeurES, "", 0, AP_IGNORE),
             Abonnes(0), ServeurActif(),
             ServeurES(nomServ, priServ) {
#if DEBOGUE
   sode->puisJe();
...
#if DEBOGUE
   sode->puisJe();
   *sode<<"Unite: "<<long(moiMeme)<<" recoit ReqESmin: "<<long(reqES)<<"\n";
   sode->vasY();
#endif
...
Unite *Unite::ouvre(ReqESmin *reqES, LONGN options) {
   ...
   *sode<<long(this)<<": Unite.ouvre("<<long(reqES)<<", "<<options<<") {\n";
   ...
   si ( ++Abonnes == 1 ) {
      SignalAssocie.bourre()= AP_SIGNAL;
      ServeurES.ajoute();
   }
#if DEBOGUE
   sode->puisJe();
...
#endif
   si ( --Abonnes == 0 ) {
      SignalAssocie.bourre()= AP_IGNORE;
      ServeurES.enleve();
   }
#if DEBOGUE
   sode->puisJe();

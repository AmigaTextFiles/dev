...
void main__(void)
{
	...
	printf__("Berechnung von PI\n");
	for (;;)
	{
		printf__("Stellenanzahl (0 für Ende):"); fflush(stdout);
		scanf__("%ld",&n);
		if (n < 1)
			exit(0);
		klen = (n * 10) / 3;
		...
		drop();
		timeend = clock();
		printf__("Zeitbedarf für %ld Stellen: %lds\n",n,

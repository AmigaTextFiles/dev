...
void main(void)
{
	...
	__printf("Berechnung von PI\n");
	for (;;)
	{
		__printf("Stellenanzahl (0 für Ende):"); fflush(stdout);
		scanf("%ld",&n);
		if (n < 1)
			exit(0);
		klen = (n * 10) / 3;
		...
		drop();
		timeend = clock();
		__printf("Zeitbedarf für %ld Stellen: %lds\n",n,

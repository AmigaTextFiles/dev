...
@d banner "This is mCWEAVE (Version 1.1)\n"
...
@ \.{mCWEAVE} should be able to handle both, old style web files and
...
|web_file_name|, |change_file_name|, |tex_file_name| and the like
...
you have to change them also in the file |"mcommon.w"|.
...
@d buf_size 256 /* maximum length of input line, plus one */
...
@d max_toks 60000 /* number of symbols in \CEE/ texts being parsed;
  must be less than 65536 */
@d max_texts 10000 /* number of phrases in \CEE/ texts being parsed;
  must be less than 10240 */
@d max_scraps 40000 /* number of tokens in \CEE/ texts being parsed */
...
As explained in \.{mcommon.w}, the field of a |name_info| structure
...
|ext_ref| is a pointer to an |external_reference| structure which
contains the name and chapter number of the referenced book.
There are two special values for |ext_ref|, |own_shared| and |own_export|
which denote the chapter's own shared and export file, respectively.
@d own_shared ((struct external_reference *)-1L)
@d own_export ((struct external_reference *)-2L)
@<Type...@>=
typedef struct xref_info {
  sixteen_bits num; /* section number plus zero or |def_flag| */
  struct xref_info *xlink; /* pointer to the previous cross-reference */
  struct external_reference *ext_ref; /* pointer to book name and chapter number */
} xref_info;
...
name_pointer p;
{
  ...
  if (q != xmem) {
   ...
   /* reference to same book or reference to own files */
      if (n==m || n==m+def_flag) {
  ...
  xref_ptr->ext_ref=ext_ref;  /* current external reference or |NULL| */
  return;
}
...
@<Store all special command identifiers@>;

@ In addition, the special commands introduced by '\.{@@\_}' are stored
...
@d special_command 0220 /* control code for '\.{@@$\_$}' */
@d macro_arg_open 0221 /* control code for `\.{@@[}' */
@d macro_arg_close 0222 /* control code for `\.{@@]}' */
@d trace 0223 /* control code for `\.{@@0}', `\.{@@1}' and `\.{@@2}' */
@d translit_code 0224 /* control code for `\.{@@l}' */
@d output_defs_code 0225 /* control code for `\.{@@h}' */
@d autodoc_code 0226  /* control code for '\.{@@a}' */
@d example_code 0227 /* control code for '\.{@@e}' */
@d format_code 0230 /* control code for `\.{@@f}' and `\.{@@s}' */
@d definition 0231 /* control code for `\.{@@d}' */
@d begin_C 0232 /* control code for `\.{@@c}' */
@d section_name 0233 /* control code for `\.{@@<}' */
@d new_section 0234 /* control code for `\.{@@\ }' and `\.{@@*}' */
...
@.String too long@>
  ...
  if(sharp_include_line && (phase==1 || parsing_exp_file))
    remember_include_file();    /* string is an include file name */
  return(string);
}
...
@<Get control code and possible section name@>= {
  if(parsing_exp_file) /* this is an ordinary \CEE/ file, no control codes */
    return '@@';
  c=*loc++;
...
{
  static int bal=0,par=0;    /* brace/paranthesis balance in \CEE/ code */
  static name_pointer typedef_name; /* the name of a new type */
  name_pointer p;
  while (next_control<format_code || next_control==spec_ctrl) {
	...
	p=id_lookup(id_first, id_loc,next_control-identifier); new_xref(p);
	@<Examine identifier to find out new types@>;
      }
    }
    else if(next_control==special_command) {
      next_control=get_next();
      if(next_control==identifier) {
	if(spec_ctrl==ignore) { /* if not enclosed in '\.{\v}' */
	  @<Special command seen in phase one@>;
	}
      }
    ...
    else if(next_control==';' && !bal && typedefing) { /* typedefing ends */
      @<end of |typedef| construct reached@>;
    }
...
@ The following flag says, that we are scanning a |typedef| construct.
...
@ The original version of \.{CWEAVE} only knew user defined types from
...
There are two different kinds of things we want to be of ilk |raw_int|
rather than |normal|. The first are identifiers following a |struct_like|
name. Since they immediatly follow the |struct_like| keyword, this is easy.
As we distinguish between \CEE/ and \CPLUSPLUS/ now, we will only do that
if |Cxx| is |true|.

The other case is type definitions using |typedef|. If we encounter a
|typedef_like| keyword, we raise the |typedefing| flag.
...
can assume it's the the new type keyword and change its ilk to |raw_int|.
Otherwise, we change the ilk of the last identifier before the final
'\.{;}' of the |typedef| construct.
@d Cxx flags['+']
@<Examine identifier to find out new types@>=
{
  int label_ilk;
  if(typedefing && !bal && !preprocessing) { /* identifier outside of braces */
    if(par) {  /* \.{(} open: a function pointer like |typedef void (*fptr)();| */
      typedefing=0;
      p->ilk=raw_int;
    }
    else typedef_name=p;  /* remember last identifier */
  }
  if(p->ilk==struct_like) { /* if followed by identifier, make its ilk |raw_int| */
    next_control=get_next();
    if(next_control==identifier) {
      if(Cxx) label_ilk=raw_int;   /* only if \CEE/++ is true */
    ...
    typedefing=1;
    par=bal=0;          /* count braces/paranthesis starting from here */
    typedef_name=NULL;
  }
}

@ At the end of the |typedef| construct, |typedef_name| holds the
last identifier seen. We change its ilk to |raw_int|.
@<end of |typedef| construct reached@>=
{
  if(typedef_name)   /* if we found the name of a type, change its ilk */
...
@.Use @@l in limbo...@>
    ...
    case noop: case section_name:
      ...
      break;
    case special_command: @<Special command seen in \TeX{} part (phase one)@>;
      break;
    case example_code: is_example=!is_example;break;
    case autodoc_code: @<Skip autodoc in phase one@>;break;
  }
...
@ Special commands are commands introduced by '\.{@@$\_$}' followed by an
identifier giving the name of the command.
@<Special command seen in \TeX{} part (phase one)@>=
{
  next_control=get_next();
  if(next_control==identifier) {
...
boolean per_cent,carryover;
{
  ...
  if (! per_cent) /* remove trailing blanks */
    while (j>out_buf && *j==' ') j--;
  if(is_adoc) {
    @<Flush buffer to autodoc section@>;
  }
  else {
...
@ Autodoc sections are one of the new features of \.{mCWEAVE}.
...
if(book_type && *chapter_name) {  /* it is a chapter */
  char *cp;
...
@ The prefix that should be prepended to {\tt mcwebmac.tex} is stored in
the following string array. This makes it possible to use localized versions
of {\tt mcwebmac}.
@<Glo...@>=
extern char mcwebmac_prefix[];

@ General purpose buffer for filenames that are not needed for a long time.
@<Glo...@>=
char a_file_name[max_file_name_length]; /* file name buffer for various purposes */
...
text_pointer p;
{
  ...
  for (j=*p; j<*(p+1); j++) {
    ...
    switch (*j/id_flag) {
      case 1: case 2:
        if(j[1]=='\\' && j[2]=='D' && j[3]=='C')  /* identifier followed by |"::"| */
	  break;
        return j;
...
if (cat1==lbrace) {
  ...
  if (cat2==lbrace || cat2==semi) {
    if(cat2==lbrace)  /* only if real definition */
      make_underlined(pp+1);
    if(Cxx)  /* only if \CEE/++, make structure label a reserved word */
      make_reserved(pp+1);
...
if (cat1==decl) {
  ...
  @<Don't export formal parameters@>;
...
|ext_ref|s but the first one to |NULL|. The first |ext_ref!=NULL| corresponds
to the function name which should stay in the export index, all others are
parameters.
@<Don't export formal param...@>=
{
  ext_ref_seen=0;   /* no |ext_ref| seen yet */
  struct_like_seen=0;
...
@ %mine
Remove all but the first external definition of the given |text p|
by setting the |ext_ref| field of the corresponding |xref| to |NULL|.
@c
void
reset_ext_refs(p)
text_pointer p;
{
  token_pointer j; /* token being looked at */
  sixteen_bits r; /* remainder of token after the flag has been stripped off */
  xref_pointer xp; /* cross reference where name defined name found */
  if (p>=text_ptr) confusion("find_first_ident");
  for (j=*p; j<*(p+1); j++) {
    r=*j%id_flag;
    switch (*j/id_flag) {
      case 1:
        if(j[1]=='\\' && j[2]=='D' && j[3]=='C')  /* identifier followed by |"::"| */
	  break;
	if(struct_like_seen) struct_like_seen=0;  /* ignore this identifier */
	...
	if(name_dir[r].ilk==struct_like && j<*(p+1) && j[1]/id_flag==1)
	  struct_like_seen=1;  /* ignore next identifier */
	break;
      case 4: case 5: /* |tok_flag| or |inner_tok_flag| */
        reset_ext_refs(tok_start+r);
      default: ; /* char, |section_flag|, fall thru: move on to next token */
        if (*j==inserted) return; /* ignore inserts */
    }
  }
}

@ %mine
Checks if the name |p| is defined in the current section |section_count|
and, if so, returns the |xref|.
@c
xref_pointer
defined_here(p)
name_pointer p;
...
switch (next_control) {
  case special_command:
    next_control=get_next();
    @<Special command seen in \CEE/ part (phase two)@>;
    goto got_next_one;
  case section_name:
...
@<Special command seen in \CEE/ part (phase two)@>=
{
  if(next_control==identifier) {
    name_pointer p=id_lookup(id_first,id_loc,normal);
    if(p==id_global || p==id_shared || p==id_export) {  /* export command */
      app(res_flag+(int)(p-name_dir));
      app_scrap(raw_int,maybe_math);
      break;
    }
    if(p==id_from)
      @<\&{from} command seen while parsing@>@;
    else if(p==id_import)
      @<\&{import} command seen while parsing@>@;
    else if(p==id_mark) {  /* ignore \&{mark} and string following */
      next_control=get_next();
      if(next_control==string) break;
      err_print("! Name of copy buffer expected");
      @.Name of copy buffer expected@>
    }
    else if(p==id_copy) break;  /* ignore \&{copy} */
    else if(p==id_paste) {
      next_control=get_next();
      if(next_control==string) {
	*id_loc=0;
...
@.Illegal special command in \CEE/ text@>
  }
}
...
@<\&{from} command seen while parsing@>=
{
  ...
  app(res_flag+(int)(p-name_dir));  /* display \&{from} */
  next_control=get_next();
  if(next_control==identifier) {
	...
	if(next_control==identifier) {
	  ...
	  if(p==id_import) {
	    ...
	    app(res_flag+(int)(p-name_dir));
	    next_control=get_next();
	    if(next_control==identifier) {  /* optional \&{transitively} */
	      p=id_lookup(id_first,id_loc,normal);
	    ...
	    if(next_control==string) {  /* statement is valid now */
...
{
  app(force);@+ app(preproc_line);@+ app_str("\\#");
  app(res_flag+(int)(p-name_dir));  /* display \&{import} */
  next_control=get_next();
  if(next_control==identifier) {
    p=id_lookup(id_first,id_loc,normal);
    if(p==id_transitively) {   /* optional \&{transitively} */
      app(' ');@+ app(res_flag+(int)(p-name_dir));
      next_control=get_next();
      if(next_control==identifier)
	p=id_lookup(id_first,id_loc,normal);
	...
	app(force);    /* import statement complete */
      }
    }
  }
}
...
through \.{mCTANGLE}.
The global variable |is_example| is set, if we are parsing example
...
@ Example \CEE/ code can occur in \TeX{} text (and autodocs, see below).
...
process_example()
{
  ...
  next_control=get_next();
  if(next_control!=definition) {  /* is code following? */
    outer_parse();
    finish_C(1);
  }
  while(next_control==definition) {  /* translate all definitions */
...
@.Closing @@e of example section...@>
  out_str("\\setsec\n");  /* return to \TeX{} section settings */
  finish_line();
  is_example=0;
}
...
@.You can't do that...@>
    ...
    case special_command: @<Special command seen in \TeX{} part (phase two)@>; break;
  }
} while (next_control<format_code);
...
@<Special command seen in \TeX{} part (phase two)@>=
{
  next_control=get_next();
  if(next_control==identifier) {   /* get name of command */
    name_pointer p=id_lookup(id_first,id_loc,normal);
    if(p==id_paste) {
      next_control=get_next();
      if(next_control==string) {
...
@.Name of copy buffer expected@>
    ...
    else if(p==id_mark) next_control=get_next(); /* skip it */
    else if(p!=id_copy)
      err_print("! Illegal special command in TeX text");
@.Illegal special command in \TeX\ text@>
  }
  else err_print("! Name of special command expected");
@.Name of special command expected@>
}
...
The following function returns 1, if the name |p| is defined externally
  ...
  name_pointer p;
{
  ...
  if(xref==xmem) return 0;  /* no reference at all? */
  if(!xref->ext_ref || xref->ext_ref==own_shared || xref->ext_ref==own_export)
    return 0;  /* not an external reference */
  if(xref->num & def_flag)   /* yes, it's a definition */
    if(xref->xlink==xmem)    /* and nothing follows */
      return 1;
  return 0;
}
...
@.\\[@>
  if(cur_xref->ext_ref)
    @<Output footnote index to book referenced from |cur_xref|@>;
  cur_xref=cur_xref->xlink;
} while (cur_xref!=xmem);
out('.'); finish_line();
...
@<Output footnote index to book referenced from |cur_xref|@>=
{
  ...
  if(cur_xref->ext_ref==own_shared) {
    if(cur_val>=def_flag) {   /* only if defined */
      dag_seen=1;
      sprintf(ref_nr,"${}^{\\dag}$");
      out_str(ref_nr);
    }
...
@ %mine
Output all books we find references to in the list |first_ext_ref|.
@c
void
output_referenced_books()
{
...
@ The environment variable |DEPDIR| gives us a reference point from
...
@*1Parse special commands in phase one.
...
@<Special command seen in phase one@>=
{
  ...
  p=id_lookup(id_first,id_loc,normal);
  if(p==id_from) {
    @<\&{from} seen in phase one@>;
  }
  else if(p==id_import) {
    @<\&{import} seen in phase one@>;
  }
  else if(p==id_mark) {
    next_control=get_next();
    if(next_control==string) {
...
@ If we see a |@_from| import command, we parse it and call
|remember_import_file| for every import file name we get from it.
@<\&{from} seen in phase one@>=
{
  char *ch_name;
  next_control=get_next();
  if(next_control==identifier) {
    ...
    if(p==id_program || p==id_library) {
      next_control=get_next();
      if(next_control==string) {
	len=id_loc-id_first-2;  /* length excl.~quotes */
  ...
  goto got_next_one;  /* already having next token */
}

@ In |name| (which is of length |len|),
...
@ If we encounter an import command in phase one, we have to take a look
...
@<\&{import} seen in phase one@>=
{
  ...
  if(next_control==identifier) {
	...
	strncpy(name,id_first+1,len);
	name[len]=0;
	if(p!=id_chapter) @<|name|$\leftarrow$|name|\.{/}|name|@>;
...
struct imported_file {
  ...
  int tangled_file;   /* 1 if from other book, 2 if from our book */
  char *given_name;   /* as given after \&{\#include}, without path */
  char file_name[2];  /* full name of file to be read (incl. path) */
};

@ We need a variable that always points to the first imported file in
order to find the head of the list of imported files.
In addition, we need a |current_imported_file| pointer that keeps track
...
@
@<Set...@>=
first_imported_file=NULL;  /* |NULL| means: no files in list yet */
current_imported_file=NULL;  /* |NULL| means: append at the end of the list */
...
@ This function is called for every file that has to be read because
...
If |exp_file==1| then we are handling an export file which might be relative
to \.{DEPDIR}. If |tangled_file!=0| the file was created by \.{mCTANGLE},
otherwise it's an include file.
|tangled_file==1| means that we are dealing with an export file of another
chapter, maybe even of another book,
|tangled_file==2| indicates that it's our own shared file and
|tangled_file==3| means our own export file.
@^system dependencies@>
...
@.No memory@>
  ...
  imf->given_name=NULL;    /* only used by include files */
  imf->tangled_file=tangled_file;
  if(current_imported_file) { /* append right after |current_imported_file| */
    imf->next_imported_file=current_imported_file->next_imported_file;
    current_imported_file->next_imported_file=imf;
  }
  else {      /* append at end of list (i.e.~after |limf|) */
    imf->next_imported_file=NULL;
    if(!limf) first_imported_file=imf;  /* first and only node in list */
    else limf->next_imported_file=imf;  /* append at end of list */
  }
  return imf;
}

@ Include files are special import files because they have names relative
...
|id_first|--|id_loc| by searching it in each of the
colon
...
remember_include_file()
{
  ...
  strncpy(full_name,id_first+1,id_len);   /* this is the given include file name */
  full_name[id_len]=0;
  @<If |full_name| equal a |given_name| in the import file list, |return|@>@;
  @<Handle case that include file is in current directory, |return| if yes@>;
  @<Try if include file is in |DEPDIR|, |return| if yes@>;
...
@ If we find the name given after the \&{\#include} statement in the
|given_name| field of the list of imported files, we already have that
...
@ If the file can be found in the current directory, there is no need
to search it in the directories given by the \.{INCLUDE} environment
variables.
@<Handle case that include file is in current directory...@>=
{
  if(!stat(full_name,&s)) {
...
@ We also always try to locate the include file relative to the
...
@ We keep a pointer to the first referenced book.
@<Global...@>=
...
@ For each new reference to a chapter of another book, we allocate
a new |external_reference| structure and append it to the list of
referenced books.
  ...
  char *bookname;
{
  ...
  boolean must_alloc_book_name=1;  /* |bookname| must be copied to allocated memory */
  ...
  for(ref=first_ext_ref;ref;ref=ref->next_ext_ref) {
    ...
    if(!c) {
      ...
      if(ref->chapter==chapter_no) return ref; /* already exists */
...
@.No memory@>
  if(!last_ref) {  /* |ref| should be first node in list */
    ref->next_ext_ref=first_ext_ref;
    first_ext_ref=ref;
  }
  else {   /* |ref| will be appended after |last_ref| */
...
@ In phase one we have collected a lot of imported files.
They are processed right at the end of phase one. During processing,
new imported files may be found and added to the list.
This happens, when there are new |#include| statements in the imported
files.
@d report_include flags['i']
@c
void
process_imported_files()
{
...
@ Note that we are now parsing \CEE/ files and no web files, since
...
It is sorted by |token_ptr| which means that all tokens whose address
|token_ptr| is
|token_sec_info[i]->token_ptr <= token_ptr < token_sec_info[i+1]->token_ptr|
come from section |token_sec_info[i]->section_count|.
@d max_token_sec_info 300
@<Glo...@>=
struct token_section {
  token_pointer token_ptr;  /* this token and all following tokens */
  int section_count;        /* are from this section */
} token_sec_info[max_token_sec_info];
struct token_section *token_sec_ptr;  /* next unused |token_section| */
extern boolean parsing_exp_file;  /* 1 if we are parsing an imported file */

@
@<Predecl...@>=
void new_token_section();

@ The following function stores the current token pointer |tok_ptr|
and the current section number |section_count| in the array described
above.
@c
void
new_token_section()
...
@ In order to find out which section a given token pointer |tk| belongs to,
...
@
@<Glo...@>=
static char foreign_book_name[max_file_name_length];  /* book we are importing from */
int foreign_chapter;   /* which chapter of that book */
struct external_reference *ext_ref;   /* current |external_reference| */

@
@<Predecl...@>=
void parse_imported_file();

@ Here is the routine that actually does the parsing of the imported
file. It opens the file, makes a list of scraps and then translates it.
Note, that a translation is also done each time a |typedef| or |extern| is
  ...
  struct imported_file *imf;
{
  ...
  current_imported_file=imf;  /* remember where we last were */
...
@#
  parsing_exp_file=1;
  ext_ref=NULL;     /* no current external reference to a book */
  token_sec_ptr=token_sec_info;    /* reset token section list */
  sec_cnt_sp=0;       /* reset section counter stack */
...
@ Translate the current scraps into \TeX\ code and reset the
texts/tokens/scraps.
@c
void
translate_and_reset()
{
  ...
  token_sec_ptr=token_sec_info;    /* reset token section list */
  section_count=sec;
  new_token_section();      /* remember from where first token comes */
}
...
@ Since we might read files created by \.{mCTANGLE}, we have to parse
  ...
  int tangled_file;
{
  ...
  if(tangled_file) {  /* if an export file rather than an include file */
    if(!strncmp(loc,"Section:",8)) {
      ...
      if(tangled_file==2) ext_ref=own_shared;  /* parsing own shared file */
      else if(tangled_file==3) ext_ref=own_export; /* parsing own export file */
  ...
  @<Simply skip the comment@>;
}

@ After parsing the comment we simply skip it.
%"
@<Simply skip the comment@>=
  for(;;) {
    if(loc>limit)
...
@ Here comes the section counter stack.
@d max_section_nest 16
@<Glo...@>=
...
@ Each time we find a new section in the comments, we push the
...
@ When the comments in the export file tell us that the current section ends,
...
@ Again, this is done by parsing \.{mCTANGLE}'s output. We simply read
...
remember_export_file()
{
  ...
  if(fd=fopen(name,"r")) {
    fclose(fd);
    remember_import_file(name,0,2);  /* 2 means: our own shared file */
  }
  if(book_type) {  /* if we are weaving a whole book rather than a simple \.{CWEB} file */
    strcpy(name,dep_dir);   /* export file name is relative to |DEPDIR| */
    len=strlen(name);
    if(len) name[len++]=file_name_separator;
    strcpy(name+len,book_name);  /* file name is preceded by book name */
    len=strlen(name);
    name[len++]=file_name_separator;
    strcpy(name+len,file_name[0]);   /* file name is same as chapter name */
    dot=file_name_ext(name+len);
    if(dot) *dot=0;
    strcat(name+len,".exp");   /* but with extension |".exp"| */
    if(fd=fopen(name,"r")) {
      fclose(fd);
      remember_import_file(name,1,3);   /* 3 means: our own export file */
    }
  }
}
...
@ Each line in the book file starts with a keyword. They have one
of the following tokens.
@d no_book 0
@d book_program 1    /* book keywords */
@d book_library 2
@<Glob...@>=
int book_type;   /* what kind of book do we have (|no_book| if old style) */
char book_file_name[max_file_name_length];/* name of book file */
char book_name[max_file_name_length];     /* name of book */
char chapter_name[max_file_name_length];  /* name of current chapter */
char out_file_name[max_file_name_length]; /* name of \TeX\ file */
...
@ Here, we do the check for a book file. Books have the file extension
of \.{.prg}.
@<Check for book...@>=
{
  int ret_val=0;
...
@ Now we read the book file line by line. All characters are copied to
...
while(get_line()) {
  ...
  while(loc<limit) {
    ...
    if(c=='@@') {
      switch(*loc++) {
      case '@@':out('@@');break;
      case 'c': @<Remember chapter@>;
	@<Output {\tt\BS input} for chapter@>;@+break;
      case 'm': @<Skip Makefile@>;@+break;
...
@ Here, we store all chapter names and their change and output file names,
...
@ So, at each occurrence of a \.{@@c}, we store the following names in the
above array.
@<Remember chapter@>=
{
  char *cp;
...
@ For each chapter, we construct an \.{\BS input} command in order
...
@<Output {\tt\BS input}...@>=
{
  ...
  else {   /* derive name from web name */
...
@ When we have scanned the whole book file, we can finally start weaving
...
@ For now, the makefile part is only skipped.
@<Skip Makefile@>=
{
...
@ Weave the current chapter. We allocate space for a new argument vector
...
@.No memory@>
  ...
  *argv_ptr=chapter_name;   /* our chapter is the first argument */
  ...
  if(change_exists) {
    ...
    *argv_ptr=change_file_name;  /* optional change file as next argument */
    if(out_exists) {
      if(argv_out) argv_ptr=argv+(argv_out-av);
      else argv_ptr=&argv[argc++];
      *argv_ptr=out_file_name;   /* \TeX\ output file is last argument */
    }
  }

  history=0;
  ret_val|=weave_file();  /* weave it with these arguments */

  @<Remember \TeX\ file name of current chapter@>;

  free(new_argv);
}

@ The \TeX\ file names of all chapters
are stored for the final table of contents.
@<Glo...@>=
char *ch_TeX_name[max_chapters];

@
@<Remember \TeX\ file name of current ch...@>=
{
  int i=chapter_no;
...
are defined separated by |'\t'| from each other.
@c
void
make_xid_file(file_extension,ref)
  char *file_extension;   /* |".xid"| or |".sid"| */
  struct external_reference *ref;  /* |own_export| or |own_shared| */
{
  name_pointer p;
  int len,i;
  xref_pointer xp;
  boolean name_written;
  FILE *xid_file=NULL;   /* not opened yet */
  char *cp;
...
@ At the end of the book, when all chapters will have been processed,
  ...
  char *file_extension;
{
  ...
  init_common_ptrs();  /* clear symbol table */
  xref_ptr=xmem;
...
@ Making the \.{iid} file is a little bit like making the \.{xid} file,
...
read_iid_files()
{
  ...
  for(ch=chapter_no-1;ch>=0;ch--) {
	...
	if((cp=strchr(buffer,'\t'))!=NULL) {
	  *cp++=0;
	  if(c=='*') {  /* indicates a new chapter of a book */
	    sscanf(cp,"%d",&c);
	  ...
	  else {  /* indicates an identifier we have references to */
	    sscanf(cp,"%d(%d)",&sec,&c);
	    section_count=(sixteen_bits)sec;
...
@ With all the information gained from the \.{xid}, \.{sid} and
...
@.Cannot create index file for book@>
...
@ The following variable determines, if index entries of identifiers,
...
@ Sort the index and output it, including those entries that are only
...
@ Since we want to group autodocs by their class, we need a
|adoc_class|-structure that contains a linked list of
all autodocs of the same class.
@<Type...@>=
struct adoc_class {
  struct adoc_class *next;  /* next autodoc class */
  struct adoc *first_adoc;  /* first autodoc entry of this class */
  char class_name[2];     /* name of the class */
};

@ Autodoc entries are sorted by name and have the following form:
@<Type...@>=
struct adoc {
  struct adoc *next;   /* next autodoc entry of same class */
  char *description;   /* explaining text */
  char name[2];        /* name of autodoc entry */
};

@ |first_adoc_class| points to the head of the autodoc class list.
@<Glob...@>=
...
@ The following functions looks up an autodoc class. The class name starts
at |name_start| and ends at |name_end|.
If the class is not already part of the |first_adoc_class| list, a new entry
...
@ The linked list of autodocs in an autodoc class is always kept sorted.
...
@ If we encounter a '\.{@@a}' command in the \TeX\ part of a section during
...
@.Autodoc class name ... expected@>
    return;
  }
  for(cp=loc;;) {
    if(loc>=limit) {
      err_print("! '}' for autodoc class missing");
@.$\}$ for autodoc class missing@>
      return;
    }
    c=*loc++;
    if(c=='}') break;
  ...
  @<Read autodoc name |name_start|, |name_end|@>;
  a=insert_adoc(ac,name_start,name_end);
  @<Read autodoc description |desc|@>;
  a->description=desc;
}

@ The class is followed by a name, which is also enclosed in braces.
We now read this name and store its start and end position in
|name_start| and |name_end|, respectively.
@<Read autodoc name...@>=
{
  do c=get_next();while(c=='\n');
  if(c!='{') {
    err_print("! '{' for autodoc name expected");
@.$\{$ for autodoc name expected@>
    return;
  }
  for(name_start=loc;;) {
    if(loc>=limit) {
      err_print("! '}' for autodoc name missing");
@.$\}$ for autodoc name missing@>
      return;
    }
    c=*loc++;
    if(c=='}') break;
...
@ Each description is first stored in the static memory pool |desc_mem|
until it has been read in and its real length is known. Then we can
allocate memory for it and copy it there.

In phase one, |desc_mem| is also used for copy buffers.
@d max_desc_size 10240
@<Glob...@>=
char desc_mem[max_desc_size];
char *desc_mem_end=desc_mem+max_desc_size;
char *desc_ptr;
boolean is_adoc;  /* causes output to be written to |desc_mem| */

@ Read autodoc description which starts with an opening brace and ends with
the corresponding closing brace. The variable |desc| will point to
allocated memory which contains the whole description.
@d app_adoc(c) {if(desc_ptr>=desc_mem_end) overflow("autodoc description");
if(c) *desc_ptr++=(c);}
@<Read autodoc description...@>=
{
  int braces;
...
@.$\{$ for autodoc description expected@>
    return;
  }
  is_adoc=1;
  braces=1;
  ...
  for(;;) {
	...
	@<Special command seen in autodoc section (phase two)@>;
	next_control=ignore;
      }
      else if(next_control!='@@') {
	err_print("! Only @@@@, @@_ or @@e allowed in autodoc section");
@.Only @@@@, @@_ or @@e allowed in auto...@>
	return;
      }
    }
...
@ The only special commands that are admitted in autodoc sections are
mark/copy/paste.
@<Special command seen in autodoc section (phase two)@>=
{
  c=get_next();
...
@.Name of copy buffer expected@>
    ...
    else if(p==id_mark) c=get_next();  /* skip string */
    else if(p!=id_copy) {
      err_print("! Not allowed in autodoc");
@.Not allowed in autodoc@>
      return;
    }
  }
}
...
@ We now have a complete set of autodoc entries and want to output
them to \TeX\ files, one for each class. The \TeX\ files have the
same name as the autodoc class and the file name extension |".adc"|.
...
@.Cannot create autodoc file@>
  ...
  for(ac=first_adoc_class;ac;ac=ac->next)
	...
	printf(" %s",a_file_name); update_terminal;
      ...
      @<Output all autodocs of autodoc class |ac|@>;
      fclose(adoc_file);
    }
...
@ The lists is already sorted, so we only have to write the
entries to |adoc_file| in the order they appear. Entries with the same
name are concatenated and put into one single autodoc entry.
@<Output all autodocs of autodoc class |ac|@>=
{
  for(a=ac->first_adoc;a;a=a->next)
    if(a->description) {
      if(*a->name=='*' && a->name[1]==0) continue; /* see below */
      if(!*a->name) fprintf(adoc_file,"%s",a->description);
      else {
	fprintf(adoc_file,"\\adoc{%s}\n",a->name);
	for(;;) {
	  fprintf(adoc_file,"%s",a->description);
	  if(!a->next || strcmp(a->name,a->next->name))  /* next one same name? */
	    break;
	  a=a->next;   /* yes, just append it */
	}
...
@ The autodoc entry with name \.{*} is special in that it is output before the
...
@ If we are in phase one rather than phase two, we skip autodocs so that
no references go to the symbol table. We simply skip three blocks
enclosed in curly braces.
@<Skip autodoc in phase one@>=
{
  int braces_opened=0;   /* how many braces still open? */
  int braces_closed=0;   /* how many braces closed up to now? */
  do {
...
The actual copying is done in the input function |get_line| in \.{mcommon.w}.
@<Glob...@>=
extern char *copy_ptr,*copy_end; /* from \.{mcommon} */
extern boolean copy_from_buffer,copy_to_buffer;
extern char *rest_after_paste;

@ For each named copy, we have a structure of the following form:
@<Type...@>=
struct copy_buffer {
...
@ These structures are linked together and |first_copy_buffer| points to
...
@ |mark| is called when we encounter the \&{mark} command followed by a name
enclosed in quotes in phase one.
It creates a new |copy_buffer| and makes it the
|current_copy_buffer|. All characters of the input buffer |buffer| starting
from the \&{mark} command are copied.

We collect all characters up to the corresponding \&{copy} in |desc_mem|.
|desc_mem| is also used for autodoc descriptions, but since these are processed
...
{
  int len;
  if(copy_to_buffer) {   /* \&{mark} and \&{copy} pairs may not be nested */
    err_print("! Still in copy mode, nesting not allowed");
@.Still in copy mode, nesting...@>
    return;
  }
  copy_ptr=desc_mem;    /* use same buffer as for autodocs */
...
@.No memory@>
  ...
  len=limit-loc;  /* copy rest of buffer */
  memcpy(copy_ptr,loc,len);
  copy_ptr+=len;
  *copy_ptr++=0;
}

@ When we see \&{copy} (again in phase one), we stop copying the input buffer
to |desc_mem|. We now finally know the size of the copied text and allocate
...
copy()
{
  ...
  int size;
  if(copy_to_buffer && current_copy_buffer) {  /* are we copying? */
    copy_to_buffer=0;
    cb=current_copy_buffer;
    copy_ptr-=limit-loc+1;   /* truncate after \&{copy} command */
    copy_ptr-=6;  /* 6 characters for \.{@@$\_$copy} */
    *copy_ptr++=0;
...
@ All copy buffers have a name which has been given with the \&{mark} command.
We can find each copy buffer in the list |first_copy_buffer| with the
following function.
@c
struct copy_buffer *
...
@ Unlike |mark| and |copy|, |paste| is called in phase two, when we
actually want to insert stuff from a copy buffer. It switches on
|copy_from_buffer|, thus causing |get_line| in \.{mcommon.w} to read from
the copy buffer.

Since we want to keep the rest of the line after \&{paste}, we copy it to
|rest_after_paste| in order to append it to the input buffer, when the
copy buffer ends.
@c
void
paste(name)
...
@.Copy buffer not found@>
    return;
  }
  copy_ptr=cb->start;
  copy_end=cb->end;
  copy_from_buffer=1;
  len=limit-loc;         /* copy rest of input buffer to |rest_after_paste| */
  rest_after_paste=malloc(len+1);
  strncpy(rest_after_paste,loc,len);
  rest_after_paste[len]=0;
  loc=limit;            /* skip rest of input buffer */
}

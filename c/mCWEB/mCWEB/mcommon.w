...
\noindent
Copyright \copyright\ 1996 Markus Öllinger
\bigskip\noindent
Permission is granted to make and distribute verbatim copies of this
document provided that the copyright notice and this permission notice
are preserved on all copies.
...
@ We will store all names in a symbol table. The following function will
...
{
  restart:
  @<Handle reading from copy buffer@>;
...
|get_line|, which is responsible for fetching the next line of input to
|buffer| now can write the input buffer to a copy buffer if |copy_to_buffer!=0|
or read from a copy buffer if |copy_from_buffer!=0|.
@<Definitions...@>=
char *copy_ptr;  /* next character to read/write */
char *copy_end;  /* end of copy buffer */
boolean copy_from_buffer,copy_to_buffer;
char *rest_after_paste;  /* rest of input buffer after \&{paste} command */

@ We must write all lines we read in |get_line| to the copy buffer,
if |copy_to_buffer!=0|.
The location in the copy buffer to write to is |copy_ptr|.
|copy_ptr| may never grow beyond |copy_end|.
@<Handle writing to copy buffer@>=
{
  if(copy_to_buffer) {
...
@ On the other hand, if |copy_from_buffer!=0|, we read from the copy buffer
instead of the input file. Again, |copy_ptr| points to the current location
in the copy buffer. The buffer ends at |copy_end|. If we reach the end
of the copy buffer, we switch back to the input file (|copy_from_buffer=0|)
and append the rest of the line after the \&{paste} command to the current
input buffer. This was saved in |rest_after_paste| by whatever routine switched
|copy_from_buffer| on.
@<Handle reading from copy buffer@>=
{
  if(copy_from_buffer) {
    if(copy_ptr>=copy_end) copy_from_buffer=0;
      ...
      copy_ptr+=len+1;
      limit=buffer+len;
      if(copy_ptr>=copy_end) {  /* end of buffer reached */
	if(rest_after_paste) {
...
|init_node| procedure, which is defined differently in \.{mcweave.w}
and \.{mctangle.w}.
...
/* are we parsing an exported file (export or shared file)? */
...
@<Make |web_file_name|...@>=
{
  if(!argv_web) argv_web=argv;   /* when first time, remember position of web file in |argv| */
  if (s-*argv > max_file_name_length-5)
    @<Complain about argument length@>;
  if (dot_pos==NULL)
    sprintf(web_file_name,"%s.w",*argv);
  else {   /* \.{.wpp} switches \CPLUSPLUS/ mode on, if not explicitly switched off */
    if(!Cxx_seen && (!strcmp(dot_pos,".wpp") || !strcmp(dot_pos,".wxx")))
      flags['+']=1;        /* these file extensions imply \CPLUSPLUS/ */
  ...
  if(dot_pos) *dot_pos='.';   /* restore dot for next call of |scan_args| */
  found_web=1;
}

@ @<Make |change_file_name|...@>=
{
  if(!argv_web) argv_change=argv;  /* remember position of change file name */
...
@ @<Override...@>=
{
  if(!argv_web) argv_out=argv;  /* the position of the output file name */
  if (s-*argv > max_file_name_length-5)
    @<Complain about argument length@>;
  if (dot_pos==NULL) {
...
@ @<Handle flag...@>=
{
  ...
  if(argv[0][1]=='l') @<Store name of localized {\tt mcwebmac}@>@;
  else
...
@ Usually, if no {\tt+l} option is given, the name of the macro file

/* Empfangen von Menü-Help Nachrichten
...
void main(void)
{
	...
	if ((surface=WZ_OpenSurface("menuhelp.wizard",0L,TAG_DONE)))
	{
		...
		if ((screen=LockPubScreen(0L)))
		{
			...
			if ((winhandle=WZ_AllocWindowHandle(screen,0L,surface,TAG_DONE)))
			{
																							...
																							TAG_DONE)))
				{
					...
					// Das Tag WA_MenuHelp um Hilfe-Nachrichten zu bekommen.
						...
						do
						{
							...
							if ((msg=(struct IntuiMessage *)GetMsg(window->UserPort)))
							{
								...
								switch(msg->Class)
								{
									...
									case IDCMP_MENUHELP:
										{
											SetWindowTitles(window,WZ_MenuHelp(winhandle,msg->Code),(STRPTR)-1L);
										}
										break;
								}

/* Simple programme affichant les hunks et un désassembleur 680x0   */

/*
   ATTENTION : Les hunks 'hunk_reloc32_short' et 'hunk_relative_reloc' n'ont
   pas été testé. Si vous trouvez un programme avec ce type de hunk, et que
   ce programme ne marche pas, contactez les auteurs du programme pour que
   soient fixés les bugs.
*/

/* erreur qui reste encore:
   - mauvais liens désassembleur (link disasms wrong)
   - bset devrait donner <illegal ea> with bit > 31
*/
...
PROC process()
  ...
  PrintF('Liste de Hunk du fichier "\s" (\d octets)\n\n',fname,flen)
  WHILE o<end
    ...
    SELECT hunkid
      ...
      CASE HUNK_CODE;
        ...
        ELSE
          PrintF(': \d octets\n', skip())
        ENDIF
      CASE HUNK_DATA;        PrintF('\thunk_data: \d octets\n',skip())
      CASE HUNK_BSS;         PrintF('\thunk_bss: \d octets\n',Mul(o[]++,4))
      ...
      CASE HUNK_DEBUG;       PrintF('\thunk_debug: \d octets\n',skip())
      ...
      CASE HUNK_LIB;         PrintF('\tlibrary_hunk: \d octets\n',Mul(o[]++,4)); hunknr:=0
      CASE HUNK_INDEX;       PrintF('\tlibrary_index: \d octets\n\n',skip())
...
PROC overlay()
  DEF m,ts
  ts:=o[]++
  PrintF('\t  taille de table = \d\n',ts)
  m:=o[]++-2
  PrintF('\t  niveau max. d'arbres overlay utilisé = \d\n',m)
  o:=ts*4+o
  hunknr:=1; f:=TRUE
ENDPROC
...
PROC symbol()
  ...
  WHILE l
    ...
    IF t<EXT_NEWCOMMON                  /* sym def */
      ...
      PrintF('\t  \s = $\h\n',s,c)
    ELSEIF t=EXT_COMMON OR t=EXT_NEWCOMMON  /* ref commune */
      s:=o; o:=l*4+o; c:=o[]++; PutChar(o-4,0); r:=o[]++
      PrintF('\t  \s (\d ref\s) taille commone = \d\n',s,r,
...
PROC reloc(size)
  ...
  WHILE a
    IF CtrlC() THEN error(ER_BREAK)
    PrintF('\t  \d d'entrée\s reloc pour le hunk #\d\n',a,
           IF a=1 THEN '' ELSE 's',o[]++)
    o:=a*size+o
    a:=o[]++
  ENDWHILE
ENDPROC
...
PROC error(nr)
  ...
  PutStr('\n')
  SELECT nr
    CASE ER_FILE;     PrintF('Ne peut lire le fichier "\s" !\n',fname)
    CASE ER_MEM;      PutStr('Pas de mémoire pour les hunks!\n')
    CASE ER_USAGE;    PutStr('USAGE: ShowHunk <exe/objfile>\n')
    CASE ER_HUNKID;   PrintF('Hunk illegal hunk id: $\h !\n',hunkid)
    CASE ER_BREAK;    PutStr('** BREAK: ShowHunk\n')
    CASE ER_FILETYPE; PutStr('N''est pas un éxécutable ou un fichier object.\n')
  ENDSELECT
  CleanUp()
ENDPROC

PROC illegal()
  PrintF('< opcode illégal: $\h>\n',pc[])
ENDPROC
  ...
  SELECT mode
    ...
    CASE 7;
      SELECT reg
        ...
        CASE 1;  immed(Long(pc+2)); pc:=pc+4      /* bug! était: immed(^pc++) */
        CASE 2;
          PutStr('(')
          immed(pc[1]++)
          PutStr(',PC)')
...
PROC code1000()
  IF Shr(pc[],4) AND %11111 = %10100   /* bug!: était  Shr(five2three,1) = 0 */
    IF eight
      IF seven2six
        SELECT seven2six
          CASE 1;  PutStr('pack\t')
...
PROC code()
  ...
  WHILE (pc < number)
    ...
    ELSE
      SELECT hibyte
        CASE 0;                 /* Manipulation de bit/MOVEP/immédiat */
          code0000()
        CASE 4;                 /* Général */
          code0100()
        ...
        CASE 14;                /* Shift/Rotate/Bit Field */
          code1110()
        CASE 15;                /* Interface Coprocesseur */
          illegal()
      ENDSELECT
    ENDIF
    pc++

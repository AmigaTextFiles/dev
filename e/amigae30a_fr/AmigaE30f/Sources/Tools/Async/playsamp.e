/*
   ...
   Un player de raw sample sur disque, écrit en AmigaE.
...
IF rdargs:=ReadArgs('Name/A/M,Rate/N',args,0)
   ...
   -> Y a-t-il un moyen plus simple de faire ça ?
    MOVE.L  args,A0
    MOVE.L  4(A0),D0
    BEQ.S   rate_def
    MOVE.L  D0,A0
            ...
            IF reply1:=CreateMsgPort()
                IF reply2:=CreateMsgPort()
                    IF arequest1:=CreateIORequest(reply1, SIZEOF ioaudio)
                        ...
                        /* alloue et utilise n'importe quel canal avec OpenDev() */
                        ...
                        IF (0=OpenDevice('audio.device', 0, arequest1, 0))
                            ...
                            /* attend request1 une première fois */
                            mnode:=arequest1
                            go:=1

                            /* jusqu'à ce que soit fait,continue à envoire des données au device */
                            WHILE (go)

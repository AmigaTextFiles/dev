...
EXPORT PROC create(str, label=NIL, min=0, sfont=NIL:PTR TO textfont,
  ...
  self.hgt:=self.base+self.sfont.ysize-self.sfont.baseline
  self.label:=label
  NEW textrp
  ...
  IF label
    ...
    self.base:=Max(self.base, self.lfont.baseline)
    self.hgt:=Max(self.hgt, self.base+self.lfont.ysize-self.lfont.baseline)

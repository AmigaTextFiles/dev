...
/*	es wird solange gesucht, bis das ende der liste erreicht ist oder der
	...
	UNTIL (StrCmp(ListItem(regelzeiger, elementnummer), ListItem(regelliste, regelnummer),ALL) AND (StrLen(ListItem(regelzeiger, elementnummer))>1)) OR (elementnummer=ListLen(regelliste))

...
/*	es wird solange gesucht, bis das ende der liste erreicht ist oder der
	...
	UNTIL (StrCmp(zkette, ListItem(regelliste, regelnummer),ALL)) OR (elementnummer=ListLen(regelliste))
					/* das Wort */					/* indexelement */

/*	War der Vergleich erfolgreich, ist ·regelnummer· kleiner als die Länge
	der Liste: */

	IF (regelnummer<ListLen(regelliste)) AND (zkette[1]<>0) /* mit strlen ging's ja nicht... */

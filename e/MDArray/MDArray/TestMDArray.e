...
PROC main()
  ...
  mda_Set(chessboard)
  piece:=chessboard[mda_Index([2,4]:INT)]
  offset:=mda_Offset([2,4]:INT)
  maxindexes:=mda_MaxIndex()
  length:=mda_Length()
  ...
  mda_Set(biny)
  FOR i:=0 TO mda_MaxIndex()
    biny[i]:=i
  ENDFOR

  bin1:=biny[mda_Index([1,0,1]:INT)]
  bin2:=biny[mda_Index([1,1,0]:INT)]
  biny[mda_Index([0,0,0]:INT)]:=0
  biny[mda_Index([1,1,1]:INT)]:=7
  binoff:=mda_Offset([1,1,0]:INT)
  binlen:=mda_Length()
  ...
  mda_Set(longarray)
  FOR i:=0 TO 5
    longarray[mda_Index([Mod(i,2),Mod(i,3),Mod(i,4),i]:INT)]:=i
  ENDFOR
  long1:=longarray[mda_Index([1,2,3,4]:INT)]
  long2:=longarray[mda_Index([1,0,3,2]:INT)]

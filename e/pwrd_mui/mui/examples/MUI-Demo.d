...
**     Translation into D 2000 by Martin Kuchinka
**
*/
...
*/
	...
	IFN MUIMasterBase:=OpenLibrary(MUIMASTER_NAME,MUIMASTER_VMIN) THEN Raise(ER_MUILIB)
...
*/
	...
	menu:=[
		...
		NM_ITEM , 'About...' ,'?',0,0,ID_ABOUT,
		NM_ITEM , NM_BARLABEL, 0 ,0,0,0,
		NM_ITEM , 'Quit'	  ,'Q',0,0,MUIV_Application_ReturnID_Quit,
		NM_END  , NIL	     , 0 ,0,0,0]:NewMenu
...
*/
	...
	in_Master:=
		'\tWelcome to the MUI demonstration program. '+
	...
	in_Notify:=
		'\tMUI objects communicate with each other '+
	...
	in_Frames:=
		'\tEvery MUI object can have a surrounding frame. '+
		'Several types are available, all adjustable with the preferences program.'

	in_Images:=
		'\tMUI offers a vector image class, that allows '+
	...
	in_Groups:=
		'\tGroups are very important for MUI. Their '+
	...
	in_Backfill:=
		'\tEvery object can have its own background, '+
	...
	in_Listviews:=
		'\tMUI\as list class is very flexible. A list can '+
	...
	in_Cycle:=
		'\tCycle gadgets, radios buttons and simple lists '+
	...
	in_String:=
		'\tOf course, MUI offers a standard string gadget class '+
...
EXCEPT
	...
	SELECT exception
	CASE ER_MUILIB;	PrintF('Failed to open \s.\n',MUIMASTER_NAME)
	CASE ER_APP;		PrintF('Failed to create application.\n')
	ENDSELECT
ENDPROC

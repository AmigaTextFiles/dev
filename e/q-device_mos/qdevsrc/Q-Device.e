...
-> LS 2007: Now compiles also with ECX.
-> Also added aslrequester code and commented out the reqtools stuff.

MODULE  '*modules'  -> contains target specific module strings
...
DEF sock, sain:PTR TO sockaddr_in, received=0, returncode=0, hst:PTR TO hostent,
    ...
    ELSE
        ...
        CopyMem(hst.addr_list[0], address, SIZEOF in_addr) -> ECX fix
    ENDIF
...
DEF entry:PTR TO listentry, driver[99]:STRING, unit[4]:STRING
    ...
    IF entry.driver <> NIL
        ...
        doMethodA(mui_output_lst, [MUIM_List_Clear])
        query(driver, Val(unit), [SCSI_INQUIRY, 0, 0, 0, 36, 0]:cdb6, SIZEOF cdb6, AFLG_INQUIRY_VERBOSE)
    ENDIF
ENDPROC
...
/*
** Procedure to open an asl file requester (LS 2007)
*/
PROC aslopenfreq(window) HANDLE
DEF req:PTR TO filerequester,
    ...
    req := Mui_AllocAslRequest(ASL_FILEREQUEST,
                        [ASL_HAIL, 'Choose logfile',
                         ASL_DIR, 'RAM:',
   ...
   IF req = NIL THEN Raise("AASL")

   IF Mui_AslRequest(req, [NIL]) = NIL THEN Raise("ASLR")

     StrCopy(logpath, req.drawer)
     q:=EstrLen(logpath)
     IF (logpath[q-1] <> 47)
         IF (logpath[q-1] <> 58) THEN StrAdd(logpath, '/', ALL)
     ENDIF
...
EXCEPT DO
   ...
   IF req THEN Mui_FreeAslRequest(req)

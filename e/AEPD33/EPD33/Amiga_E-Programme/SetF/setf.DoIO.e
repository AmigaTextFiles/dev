/* setf.DoIO.e -- a program to monitor calls to DoIO() */

OPT OSVERSION=37

MODULE 'dos/dos', 'exec/ports', 'exec/tasks', 'exec/nodes',
       'exec/memory', 'exec/io'

CONST OFFSET=$fe38  /* execbase offset of DoIO() */
...
PROC printmsgs()
  DEF msg:PTR TO mymsg, s:PTR TO io
  WHILE msg:=GetMsg(port)
    IF s:=msg.s
      WriteF('Task \s sent: dev $\h, unit $\h, com $\h, flg $\h\n',
             IF msg.t THEN msg.t ELSE '*unnamed*',
             s.device, s.unit, s.command, s.flags)
    ELSE
      WriteF('Task \s sent NIL msg\n',
             IF msg.t THEN msg.t ELSE '*unnamed*')
    ENDIF
    ReplyMsg(msg)
    Dispose(msg.s)
...
PROC sendmsg()
  DEF m:PTR TO mymsg, s:PTR TO io, tsk:tc, l:ln, i, p, q
  MOVE.L A1, s
  /* Allocate a new message */
  m:=New(SIZEOF mymsg)
  IF q:=s
    p:=New(SIZEOF io)
    FOR i:=1 TO SIZEOF io DO p[]++:=q[]++
    m.s:=p
  ENDIF

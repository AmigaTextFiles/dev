...
* just like the default trackers.
	...
	STRUCTURE TrackedObject,0
		CPTR  tr_Resource		; whatever
		LONG  tg_Verify			; for use during TRAK_GENERIC
		LABEL TrackedObject_SIZEOF

	STRUCTURE TrackedExtra,0
		APTR  tg_Function		; function to call for TRAK_GENERIC
		CPTR  tr_Window2		; for TRAK_WINDOW
		LABEL TrackedExtra_SIZEOF
...
* on user needs, etc.  This reflects the default.
		STRUCT	tr_Object,TrackedObject_SIZEOF	; The thing being tracked
		STRUCT	tr_Extra,TrackedExtra_SIZEOF	; only needed sometimes
		LABEL	trk_SIZEOF		; trk_ fixes COLLISION WITH TMPRAS...

tg_Value EQU tg_Verify
...
* See CloseWindowSafely() for more info.  If only one window, set this to NULL.
	...
	STRUCTURE DefaultTracker,-2
		...
		STRUCT  dt_Object,TrackedObject_SIZEOF	; the object being tracked
		STRUCT	dt_Extra,TrackedExtra_SIZEOF
		LABEL	dt_SIZEOF
...
*------------
	...
	STRUCTURE ProcessControlBlock,0
		...
		STRUCT	pcb_Console,pcbConsole_SIZEOF

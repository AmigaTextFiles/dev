...
-> Special OS4 version: See line 900. Major changes in eexe module.
...
MODULE 'tools/clonescreen', 'tools/macros', 'tools/arexx',
       ...
       'tools/iterators',
       '*eexe_os4', 'class/sc', '*sctext', '*schex', '*screg', 'other/sendexplorer',
...
PROC dovar(vx,vy,qual)
  DEF var[50]:STRING,v,vptr,reg=0,bp:PTR TO INT,con
  ...
  ELSEIF vy>=0
    ...
    IF v>0
      bp:=currentwin.src.findpc(vy,exe)
      setbreak(bp)
      con:=SetStdOut(ocon)
      PrintF('Breakpoint: $\h, Contents: \h[4] \h[4] \h[4]\n',bp,bp[],bp[1],bp[2])
      SetStdOut(con)
...
PROC step(fr:PTR TO stackframe)
  ...
  ELSE
    ...
    pc:=fr.returnpc-6              -> Modification: Now substracts 6 instead of 2
                         ->         to make up for extra 32-bits.

...
--8<--------- Cut this Speed up RexxMast Processing a little ---------------
   ...
   Compile current file using EC2.1b ,EPP1.4d and MAC2E4.0 if it must
...
Compile:
   /* ----------------------- Modules processing ------------------------ */
    ...
    IF (rc==0) THEN DO
       if ~exists("ram:epp") then address command 'copy EDEVICE:EPP RAM:'
       errhdlr = 2                          /* Bool for Error processing */
       load = 1                             /* Bool for right source error pos */
       'REQUEST STATUS=" Linking current source with extern modules ..."'
       signal on error
       address command 'RAM:Epp >T:Eerror -n 'name' 'nameo'_lnk'
       signal off error
       name=''nameo'_lnk'
    end
    'FIND STRING="'fstr'" FIRST QUIET COUNT' /* Replace old FIND string */
    'GOTO LINE='olne' COLUMN='col''
    ...
    signal off error
    if name~=nameo then do                     /* Name of final executable */
      if exists(nameo) then address command 'Delete 'nameo' QUIET'
      address command 'Rename 'name' 'nameo' QUIET'
    end
    'REQUEST STATUS=""'
...
select
  ...
  when errhdlr = 0 then do
    'REQUEST STATUS=""'                       /* EC Error Handler      */
    if optcdisk=='FALSE' then 'NAME NEW 'oldname''  /* restore old file name */
    ok = open(filehandler,'T:Eerror','Read')
    if ok = 0 then 'REQUEST STATUS="Could not read E Compiler error output"'
    else do
      linestring=''
      ...
      if linestring~='' then errortext=''errortext': 'linestring''
      if errorstring~='' then errortext=''errortext'|Error: 'errorstring''
      if withstring~='' then errortext=''errortext'|With : 'withstring''
      if load=1 then 'OPEN NEW NAME="'name'.e"'
      'GOTO LINE='errorline''
      errortext=compress(errortext,'"')
      'REQUEST TITLE="ECompiler 2.1b error" BODY="'errortext'"'
      ok = close(filehandler)
      'UNLOCK'
      exit
    end
  ...
  when errhdlr = 2 then do                  /* EPP Error handler     */
    'REQUEST STATUS=""'
    if optcdisk=='FALSE' then 'NAME NEW 'oldname''  /* restore old file name */
    ok = open(filehandler,'T:Eerror','Read')
    if ok = 0 then 'REQUEST STATUS="Could not read EPP error output"'
    else do
      errorstring=''
      errortext=''
      do while ~eof(filehandler)
        line = readln(filehandler)
        if index(line,'***')~=0 then parse var line "***" errorstring ": " errortext
      end
      errortext=''errorstring' ERROR WITH 'errortext''
      'REQUEST TITLE="EPP error message..." BODY="'errortext'"'
      ok = close(filehandler)
      'UNLOCK'
      exit 0
    end

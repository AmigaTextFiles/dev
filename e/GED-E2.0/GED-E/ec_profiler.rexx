  ...
  if upper(right(name,2))='.E' then do
    ...
    if exists(name) then do
      if exists('ENV:ECopt') then do
        ok = open(filehandle,'ENV:ECOpt','read')
        ecopt=readln(filehandle)
        ok = close(filehandle)
        if (find(ecopt,'SYM')~=0 | find(ecopt,'LINEDEBUG')~=0) then do
          options failat 101
          address command 'EDEVICE:AProf 'name''
          options failat 10
        end
        else 'REQUEST STATUS=" Recompile with SYMBOLS and/or LINEDEBUG"'
      end
      else 'REQUEST STATUS=" Recompile with SYMBOLS and/or LINEDEBUG"'
    end

...
DEF dev_audio_channelmap:PTR TO CHAR,
    dev_audio_unit,
    dev_unit_flags
...
PROC init() OF audio
  dev_audio_channelmap := [%1111]:CHAR
ENDPROC

PROC select(optionlist,index) OF audio
...
DEF item
  ...
  SELECT item
    CASE "cmap"
      dev_audio_channelmap := ListItem(optionlist,index)
      INC index
  ENDSELECT
...
PROC open() OF audio
  self.open_audio(dev_audio_channelmap)
  RETURN self.io
ENDPROC

PROC open_audio(channelmap=NIL:PTR TO CHAR,unit=0,flags=0) OF audio

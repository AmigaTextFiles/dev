...
/* 2.1 (LS 2004) */
/* - addq #8 /subq #8 was wrong. */
/* - added bsr.l + extb.l */
/* - fixed link */
/* - removed some ^ operators */
/* - some "PROC xxx() RETURN <exp>" changed to "PROC xxx() IS <exp>" */
/* - On MorphOS, compiled with ECX to PPC was about 12 ! */
/*   times faster than compiled with EC 68k. Thank mostly */
/*   custom stringformatting. test: showhunk PPaint disasm >t:temp */
/* 2.1b (LS 2004) */
/* - muls / mulu was mixed up */
...
PROC main()
  ...
  PutStr('ShowHunk 2.1b by LS 2004\n')
  PutStr('ShowHunk v2.0 Copyright (c) 1993 Jim Cooper\n')
  PutStr('ShowHunk v0.1 (c) 1992 $#%!\n\n')
...
PROC opsize(bit) IS ListItem(["b","w","l","?"],bit)

PROC bitsize(bit) IS IF bit THEN "l" ELSE "w"

PROC immed(val)
  PrintF(IF val < 16 THEN '\d' ELSE '$\h',val)
ENDPROC

PROC ccode(val,b) IS ListItem([IF b THEN 'ra' ELSE 't',
  ...
  ELSE
    ...
    ELSE
      ...
      ELSE
        ...
        immed(IF seven2six < 2 THEN pc[1]++ ELSE Long((pc := pc + 4) - 4))
        PutStr(',')
        ea(five2three,two2zero,0)
        PutStr('\n')
      ENDIF
...
PROC code0100()
  ...
  ELSE
    IF eight = 1
      IF bitsix = 1
        IF (bitseven = 1) AND (five2three=0)
           PrintF('extb.l\td\d\n',two2zero)
        ELSE
           PutStr('lea\t')
           ea(five2three,two2zero,1)
...
PROC code0101()
  IF seven2six < 3
    ...
    PrintF('\c\t#\d,',opsize(seven2six),ListItem([8,1,2,3,4,5,6,7],eleven2nine))
    ea(five2three,two2zero,0)
    PutStr('\n')
  ELSE
    IF five2three = 1
    ...
    ELSEIF five2three = 7
      ...
      IF two2zero < 4
        IF two2zero AND 1 THEN PrintF('.w\t#\d',pc[1]++) ELSE PrintF('.l\t#\d',Long((pc:=pc+4)-4))
      ENDIF
      PutStr('\n')
    ELSE
...
PROC code1100()
  IF seven2six = 3
    PutStr(IF eight = 1 THEN 'muls.w\t' ELSE 'mulu.w\t')
    ea(five2three,two2zero,1)
    PrintF(',d\d\n',eleven2nine)
  ELSE

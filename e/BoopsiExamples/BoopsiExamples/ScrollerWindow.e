/* ScrollerWindow.e
   ...
   Translated by Wouter from excellent example scrollerwindow.c
   by Christoph Feck, TowerSystems (feck@informatik.uni-kl.de)

   needs E v2.1b_v39 with fixed icclass.m to compile
...
PROC max(x,y) RETURN IF x>y THEN x ELSE y
PROC min(x,y) RETURN IF x<y THEN x ELSE y
PROC rassize(w,h) RETURN Shr(w+15,3) AND $FFFE * h
...
PROC bitmapdepth(bm:PTR TO bitmap) RETURN IF v39 THEN GetBitMapAttr(bm,BMA_DEPTH) ELSE bm.depth
PROC sysisize() RETURN IF screen.flags AND SCREENHIRES THEN SYSISIZE_MEDRES ELSE SYSISIZE_LOWRES

PROC newimageobject(which) RETURN NewObjectA(NIL,'sysiclass',
  [SYSIA_DRAWINFO,dri,SYSIA_WHICH,which,SYSIA_SIZE,sysisize(),NIL])

PROC newpropobject(freedom,taglist) RETURN NewObjectA(NIL,'propgclass',
  [ICA_TARGET,ICTARGET_IDCMP,PGA_FREEDOM,freedom,PGA_NEWLOOK,TRUE,
   PGA_BORDERLESS,(dri.flags AND DRIF_NEWLOOK) AND (dri.depth<>1),
   TAG_MORE,taglist])

PROC newbuttonobject(image:PTR TO object,taglist) RETURN NewObjectA(NIL,'buttongclass',
...
PROC openscrollerwindow(taglist)
  ...
  gap:=1
  IF (horizgadget:=newpropobject(FREEHORIZ,
    [GA_LEFT,rw+gap,
     ...
     NIL]))=NIL THEN RETURN
  IF (vertgadget:=newpropobject(FREEVERT,
    [GA_RELRIGHT,bw-gw+3,
     GA_TOP,topborder+rh,
     GA_WIDTH,gw-bw-bw-4,
     ...
     NIL]))=NIL THEN RETURN
  IF (leftgadget:=newbuttonobject(leftimage,
    [GA_RELRIGHT,(1)-leftimage.width-rightimage.width-gw,
     ...
     GA_ID,LEFT_GID,
     NIL]))=NIL THEN RETURN
  IF (rightgadget:=newbuttonobject(rightimage,
    [GA_RELRIGHT,(1)-rightimage.width-gw,
     ...
     GA_ID,RIGHT_GID,
     NIL]))=NIL THEN RETURN
  IF (upgadget:=newbuttonobject(upimage,
    [GA_RELRIGHT,(1)-upimage.width,
     ...
     GA_ID,UP_GID,
     NIL]))=NIL THEN RETURN
  IF (downgadget:=newbuttonobject(downimage,
...
PROC recalchvisible() RETURN window.width-window.borderleft-window.borderright
PROC recalcvvisible() RETURN window.height-window.bordertop-window.borderbottom

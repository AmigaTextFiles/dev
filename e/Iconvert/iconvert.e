...
/* Changes made by Thomas Nokielski (see lines 213 to 221) */

OBJECT def
  type:INT,name:PTR TO CHAR
ENDOBJECT
...
DEF defs[MAX_DEF]:ARRAY OF def,
    ...
    handle=NIL,flen,buf,bbuf,defn=0,line=0,mode=0,a

PROC main()
  WriteF('Iconvert v0.1 1992 $#%!\n\n')
  readinfile()
  collectidents()
  makeasmfile()
  invokea68k()
...
PROC makemodule()
  DEF v:PTR TO LONG,work[3]:ARRAY OF INT,d:PTR TO def,l,x,obj=NIL, i
  v:=bbuf+28; d:=defs
  ...
  FOR a:=1 TO defn
    IF d.type=T_CONST
      ...
      PutLong(work+2,v[]++)
      Write(handle,work,6)
      /* Only make uppercase until "_" */
      i:=0
      WHILE d.name[i]
        EXIT d.name[i]="_"
        d.name[i]:=d.name[i] AND %11011111
        INC i
      ENDWHILE
      ->UpperStr(d.name)
      /* ------------------------------- */

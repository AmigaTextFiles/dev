...
/* here is set the precision of representation */
PROC print_float()
  WriteF('\tf is \s\n', RealF(s, !f, 5))
ENDPROC
...
PROC main()
  WriteF('Next 2 lines should be the same toi\n')
  f:=2.75; i:=!f!
  print_both()
  f:=2.75; i:=convert_to_integer(!f)
  print_both()

  WriteF('Next 2 lines should be the same tof\n')
  i:=10;  f:=i!
  print_both()
  i:=10;  f:=convert_to_float(i)
  print_both()

  WriteF('f and i should be the same square\n')
  i:=square_integer(i)
  f:=square_float(f)
  print_both()

  WriteF('Next 2 lines should be the same tan\n')
  f:=!Ftan(.8)
  print_float()
  f:=my_tan(.8)
  print_float()

  WriteF('Next 2 lines should be the same exp\n')
  f:=.35
  print_float()
  f:=inaccurate(f)
  print_float()

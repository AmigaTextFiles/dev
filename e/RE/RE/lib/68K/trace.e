/*
** This will output the current source line and traced variables values
*/
OPT NOHEAD,NOEXE
...
PROC TraceLine(curs:PTR TO CHAR,str=0,lst=0:PTR TO LONG,file=0,end=0)
  ...
  IFN source
    ...
    tcon:=Open('con:0/11/640/80/RE debug output.<Esc> to quit',1006)
  ENDIF
  curs += source
  p:=curs
...
PROC loadfile(filename)(LONG,LONG)
  ...
  IF file:=Open(filename,OLDFILE)
    ...
    IF mem := AllocMem(filelen+4+4+1,MEMF_PUBLIC + MEMF_CLEAR)

...
PROC main() HANDLE
	...
	IF declarations
		...
		IF environment AND application THEN genfile.put_header( application )
	...
	IF notifications
		...
		genfile.put_notifications( muistrings_contents() )
		IF environment THEN genfile.put_init_notifications_end()

	ENDIF

	IF environment AND application THEN genfile.put_aux_funcs( locale , getstring_func )
...
** String version **
*******************/
CHAR '$VER: GenCodeE 1.7 (21.11.94)' , 0

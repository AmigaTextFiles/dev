...
DEF gadtoolsbase, gle:gle, gads[1]:ARRAY OF LONG, gads_h[1]:ARRAY OF LONG
...
PROC gadtoolsWindow() HANDLE
  ...
  gle.gle(mysc, gads, gads_h)

  ->vi:=GetVisualInfoA(mysc, [NIL])
  -> GadTools gadgets require this step to be taken
  ->gad:=CreateContext({glist})

  ->gle.addGad(BUTTON_KIND, 0, -1, 1, 'Click Here')

  ->gle.eEventHandler({button0}, 0)
...
PROC button0(gle, gad, gadval) IS WriteF('Button was pressed\n')

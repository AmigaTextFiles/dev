...
-> YAEC1.9 : output to t:temp measured to be 11 times faster !!!
-> thanks to custom stringformatting.
...
PROC main() HANDLE
  ...
  error(ER_NONE)
EXCEPT
   IF exception <> 0 THEN PrintF('unknown exception! : \d\n',exception)
ENDPROC
...
PROC process()
  ...
  PrintF('Hunk layout of file \q\s\q (\d bytes)\n\n',fname,flen)
  WHILE o<end
...
PROC error(nr)
  ...
  PutStr('\n')
  SELECT nr
    CASE ER_FILE;     PrintF('Could not read file \q\s\q !\n',fname)
  ...
  SELECT mode
    ...
    CASE 7;
      SELECT reg
        ...
        CASE 1;  immed(Long(pc+2)); pc:=pc+4      /* bug! was: immed(Int(pc++)) */
        CASE 2;
          PutStr('(')
          immed(pc[1]++)
          PutStr(',PC)')
  ...
  ELSE
    ...
    ELSE
      ...
      ELSE
        ...
        immed(IF seven2six < 2 THEN pc[1]++ ELSE Int(pc++))
        PutStr(',')
        ea(five2three,two2zero,0)
        PutStr('\n')
      ENDIF
...
PROC code0101()
  ...
  ELSE
    ...
    ELSEIF five2three = 7
      ...
      IF two2zero < 4
        IF two2zero AND 1 THEN PrintF('.w\t#\d',pc[1]++) ELSE PrintF('.l\t#\d',Int(pc++))
      ENDIF
      PutStr('\n')
    ELSE
...
PROC code()
  ...
  WHILE (pc < number)
    ...
    PrintF('L\z\r\h[8]:',pc-o-4)                  /* for offsets */

...
**
** Converted to PowerD by Martin Kuchinka, 1999
*/
...
#define LAWN_FlushLeft    TAG_USER+14000  /* NOTEZ-BIEN: TAG_USER+99 = WA_Dummy and can crash */
 ...
 */
ENUM  INCREMENTERMSG_Decrement=-1,  /* Decrement value */
    INCREMENTERMSG_Initial,     /* Initial value passed upon gadget creation */
    INCREMENTERMSG_Increment    /* Increment value */
 ...
 */
OBJECT HelpMsg
  Handle:PTR TO LayoutHandle,   /* Window layout handle */
  ObjectID:LONG             /* ID of the object, -1 for full window */
 ...
 * a LEVEL_KIND object.
 */
ENUM  TICKS_None,   /* No ticks please */
    TICKS_Left,   /* Place ticks left of the slider (FREEVERT only) */
    TICKS_Both    /* Place ticks on both sides of the slider */

CONST TICKS_Above=TICKS_Left  /* Place ticks above the slider (FREEHORIZ only) */
...
OBJECT LayoutHandle
  ...
  VisualInfo:APTR,
  AmigaGlyph:PTR TO Image,
  CheckGlyph:PTR TO Image,
  UserData:APTR,          /* Requires gtlayout.library V9 */
  Menu:PTR TO Menu        /* Requires gtlayout.library V13 */
...
/* Useful macros */
#define LT_GetString(Handle,Code) LT_GetAttributesA((Handle),(Code),NIL)
...
#define LT_GetDrawInfo(Handle)    (IF Handle THEN (Handle).DrawInfo ELSE NIL
#define LT_GetVisualInfo(Handle)    (IF Handle THEN (Handle).VisualInfo ELSE NIL
#define LT_GetScreen(Handle)      (IF Handle THEN (Handle).Screen ELSE NIL

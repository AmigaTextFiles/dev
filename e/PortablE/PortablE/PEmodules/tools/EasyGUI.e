...
MODULE 'std/pCallback', 'graphics', 'gadtools', 'wb', 'intuition', 'exec', 'dos'
...
PRIVATE
  ...
  menuitem:ARRAY OF PTR TO menuitem
#ifdef EASY_EXTRAS
...
#define KINDTAB [0,0,0,0,0,0, 1,2,3,4,5,7,8,9,11,12,13,6, 1, 0, 0,0,0,0, 1,12]:CHAR
-> Mapping of gadget type to number of required arguments.
#define MINARGS [2,2,2,2,2,2, 3,5,5,9,6,4,7,7, 9, 6, 5,5, 3, 3, 1,1,1,1, 3,6]:CHAR
...
CONST WIN_FNOSIZE=WIN_FBASIC OR WFLG_DRAGBAR OR WFLG_DEPTHGADGET OR WFLG_CLOSEGADGET
...
CONST WIN_IDCMP_NS=GAD_IDCMP OR IDCMP_REFRESHWINDOW OR IDCMP_MOUSEBUTTONS OR IDCMP_MENUPICK OR IDCMP_CLOSEWINDOW OR IDCMP_RAWKEY OR IDCMP_ACTIVEWINDOW OR IDCMP_INACTIVEWINDOW OR IDCMP_INTUITICKS OR IDCMP_CHANGEWINDOW OR IDCMP_VANILLAKEY
...
#define SysISize(flags) (IF (flags) AND SCREENHIRES THEN SYSISIZE_MEDRES ELSE SYSISIZE_LOWRES)
...
OBJECT g
  ...
  type,list:ILIST,flags,mid
ENDOBJECT

/************ plugin ************/
EXPORT CLASS plugin PRIVATE
  -> Gadget Data, Link
...
PROC gtrender(gl:PTR TO gadget,vis,ta,x,y,xs,ys,win) OF plugin IS gl
/************ plugin end ************/
...
PROC optattr(list:ILIST,n) IS IF HasATTR(list,n) THEN ATTR(list,n) ELSE 0

PROC optdefattr(list:ILIST,n)
  DEF res=-1
...
PROC optindex(i:ILIST,index) IS IF index THEN OptATTR(i,index) ELSE 0
#endif
...
/********** setdisabled **********/
EXPORT PROC setdisabled(gh,gad:PTR TO VALUE,disabled=TRUE)
  DEF index
...
PROC fillbox(rport:PTR TO rastport,pen,x,y,x2,y2)
  SetAPen(rport,pen)
  RectFill(rport,x,y,x2,y2)
ENDPROC

-> Create new gadget data.
PROC newg(xs,ys,type,list:ILIST,flags=0,mid=NO_MID) IS NEW [0,0,0,xs,ys,type,list,flags,mid]:g
...
/********** easyguiA() **********/
EXPORT PROC easyguiA(title:ARRAY OF CHAR,gui:ILIST,tags=NIL:PTR TO tagitem) HANDLE
...
/********** easygui_fallbackA() **********/
EXPORT PROC easygui_fallbackA(title:ARRAY OF CHAR,gui:ILIST,tags=NIL:PTR TO tagitem) RETURNS ret
  RETURN easyguiA(title,gui,tags)
EXCEPT
  IF exception="bigg"
    easyguiA(title,gui,[EG_FONT,['topaz.font',8,0,0]:textattr,
                               TAG_MORE,tags])
    exception := 0
  ENDIF
  ReThrow()
...
-> Init font.
PROC initfont(gh:PTR TO guihandle,tattr:PTR TO textattr)
  gh.tattr:=tattr
  gh.ta_scr:=(tattr=NIL)
ENDPROC

-> Init screen.
PROC initscr(gh:PTR TO guihandle,scr:PTR TO screen)
  gh.scr:=scr
  gh.is_wb:=(scr=NIL)
ENDPROC
...
PROC setgui(gh:PTR TO guihandle)
  DEF base:PTR TO g,s:PTR TO screen,w:PTR TO window,xsize,ysize,
      cm,h=NILA:ARRAY OF VALUE,vpe=NIL:PTR TO viewportextra,x,y
  s:=gh.scr
  w:=gh.pwnd
...
/********** guiinitA() **********/
#ifdef EASY_EXTRAS
EXPORT PROC guiinitA(title:ARRAY OF CHAR,gui:ILIST,tags=NIL:PTR TO tagitem) IS addmultiA(NIL,title,gui,tags)
#endif
#ifndef EASY_EXTRAS
EXPORT PROC guiinitA(title:ARRAY OF CHAR,gui:ILIST,tags=NIL:PTR TO tagitem) HANDLE
  DEF gh=NIL:PTR TO guihandle
  gh:=makehandle(title,gui,NIL,tags)
EXCEPT
  -> Stop the user cleanup in this case.
  IF gh THEN gh.onclean:=NIL
  cleangui(gh)
  ReThrow()
ENDPROC gh
#endif

PROC openlibrary(s:ARRAY OF CHAR)
  DEF lib
  IF (lib:=OpenLibrary(s,37))=NIL THEN RaiseX("LIB",613,s)
ENDPROC lib

-> Make a handle and initialise from the tags.
PROC makehandle(title:ARRAY OF CHAR,gui:ILIST,mh:PTR TO multihandle,tags:PTR TO tagitem) HANDLE
  DEF gh=NIL:PTR TO guihandle, temp:ARRAY OF VALUE, isopen=FALSE
  NEW gh
...
#ifdef EASY_EXTRAS
/********** changescreen() **********/
EXPORT PROC changescreen(gh:PTR TO guihandle,scr=NIL:PTR TO screen)
  IF gh.wnd=NIL THEN initscr(gh,scr)
ENDPROC

/********** changefont() **********/
EXPORT PROC changefont(gh:PTR TO guihandle,tattr=NIL:PTR TO textattr)
...
/********** changetitle() **********/
EXPORT PROC changetitle(gh:PTR TO guihandle,windowtitle=NILA:ARRAY OF CHAR)
  IF gh.wnd
...
/********** changemenus() **********/
EXPORT PROC changemenus(gh:PTR TO guihandle,newmenus=NIL:PTR TO newmenu) HANDLE
...
/********** changegui() **********/
EXPORT PROC changegui(gh:PTR TO guihandle,gui:ILIST)
  IF gui
...
PROC setwinlimits(w:PTR TO window,minx,miny,maxx,maxy)
  DEF i
  DEF exit
  FOR i:=0 TO 3
    exit := WindowLimits(w,minx,miny,maxx,maxy)
    IF exit = FALSE THEN Delay(1)
  ENDFOR IF exit
ENDPROC
...
PROC appwmessage(port:PTR TO mp) HANDLE
  DEF ret,data,list:ILIST,amsg=NIL:PTR TO appmessage,gh:PTR TO guihandle,
      pl:PTR TO plugin
  DEF exit
  IF port
    WHILE amsg:=GetMsg(port)
      ...
      pl:=gh.plugins
      WHILE pl
        IF OptATTR(pl.base.list,PLG_APPW)=NIL
          exit := FALSE
        ELSE
          exit := pl.appmessage(amsg,gh.pwnd)
        ENDIF
        IF exit = FALSE THEN pl:=pl.next
      ENDWHILE IF exit
      ret:=NIL
      data:=NIL
      ...
      ENDIF
      -> Call the awproc.
      IF ret THEN call3empty(ret, data,gh.info,amsg)
...
PROC menumessage(gh:PTR TO guihandle,code) HANDLE
  ...
  -> Stop if action value.
    ...
    IF exit = FALSE
      gh.menuitem:=ADDRESSOF item
      -> Call action function.
      call3empty(ret, NIL,NIL,gh.info)
      ret:=GUI_CONT
      exit := item=NIL
      IF exit = FALSE THEN menunum:=Unsigned(item.nextselect)
    ENDIF
  ENDWHILE IF exit
EXCEPT DO
  gh.menuitem:=NIL
...
PROC gtmessage(port:PTR TO mp) HANDLE
  DEF ret=GUI_CONT,mes=NIL:PTR TO intuimessage,type,gh:PTR TO guihandle,
      gs:PTR TO gadget,list:ILIST,code,pl:PTR TO plugin,qual
  DEF exit
...
->WriteF('type=$\h, code=$\h, gad=$\h\n', type, code, gs)
      ...
      ELSEIF type=IDCMP_CLOSEWINDOW
        ...
        IF IsActionFun(ret)
#ifdef EASY_EXTRAS
          call2empty(ret, gh.mh,gh.info)
#endif
#ifndef EASY_EXTRAS
          call2empty(ret, NIL,gh.info)
#endif
          ret:=GUI_CONT
        ENDIF
      -> Menu choice(s).
...
/********** addmultiA() **********/
EXPORT PROC addmultiA(mh:PTR TO multihandle,title:ARRAY OF CHAR,gui:ILIST,tags=NIL:PTR TO tagitem) HANDLE
  DEF gh=NIL:PTR TO guihandle
...
/********** multiforall() **********/
/*# multieval() is not supported
EXPORT PROC multiforall(varaddr:PTR TO VALUE,mh:PTR TO multihandle,expr) IS multieval(varaddr,mh,expr,FALSE)
*/

/********** multiexists() **********/
/*# Eval() is not supported
EXPORT PROC multiexists(varaddr:PTR TO VALUE,mh:PTR TO multihandle,expr) IS multieval(varaddr,mh,expr,TRUE)

-> Evaluate and maybe stop...
PROC multieval(varaddr:PTR TO VALUE,mh:PTR TO multihandle,expr,exists)
...
PROC message(wndport:PTR TO mp,awport:PTR TO mp) HANDLE
  DEF ret=-1
#ifdef EASY_APPWINDOW
  appwmessage(awport)
#endif
...
/********** quitgui() **********/
EXPORT PROC quitgui(ret=NILA:ARRAY OF CHAR) IS Throw("QUIT",ret)
...
PROC testintuimsg(msg:PTR TO intuimessage,data)
ENDPROC msg.idcmpwindow=data

#ifdef EASY_APPWINDOW
-> Check if is this an appmessage for the AppWindow.
PROC testappwmsg(msg:PTR TO appmessage,data)
ENDPROC msg.id=data
#endif

-> Selectively remove messages from the port.
PROC clearmsgs(mp:PTR TO mp,f:PTR,data)
...
EXPORT PROC cleangui(gh:PTR TO guihandle)
  ...
  IF gh
    ...
    closewin(gh)
    IF f:=gh.onclean THEN call1empty(f, gh.info)
    cleaninit(gh)
#ifdef EASY_EXTRAS
    IF gh.mh
...
EXPORT PROC closewin(gh:PTR TO guihandle)
  ...
  IF w:=gh.wnd
    ...
    IF gh.awport THEN clearmsgs(gh.awport,CALLBACK testappwmsg(),gh)
#endif
    IF w
      -> Must be in Forbid()/Permit() brackets.
      Forbid()
      -> Remove any last minute messages safely.
      clearmsgs(gh.wndport,CALLBACK testintuimsg(),w)
...
PROC rendergui(gh:PTR TO guihandle)
  ...
  w:=gh.pwnd
  base:=gh.base
  gh.gl:=CreateContext(ADDRESSOF glist)
...
PROC performkey(gh:PTR TO guihandle,code:CHAR)
  DEF list:ILIST,ret,val,tag,index,data,gad=NIL:PTR TO gadget,inc
...
#ifdef EASY_EXTRAS
  Gt_GetGadgetAttrsA(gad,gh.pwnd,NIL,[GA_DISABLED,ADDRESSOF ret,NIL])
...
*/
  ...
  IF tag
    ...
    IF IsActionFun(ret)
      ...
      IF tag=TRUE THEN call3empty(ret, 0,data,gh.info) ELSE call4empty(ret, 0,data,gh.info,val)
      ret:=GUI_CONT
    ENDIF
  ELSE
    ret:=GUI_CONT
...
PROC performaction(fun:PTR,gad:PTR TO gadget,info,code,l:ILIST,qual)
  DEF data,but,val
  data,but,val:=indexdata(gad.gadgetid)
  data:=OptDefATTR(l,data)
  IF but THEN call3empty(fun, qual,data,info) ELSE call4empty(fun, qual,data,info,IF val THEN ATTR(l,val) ELSE code)
ENDPROC  ->(IF but THEN call3(fun, qual,data,info) ELSE call4(fun, qual,data,info,IF val THEN ATTR(l,val) ELSE code))

/********** getstr **********/
EXPORT PROC getstr(gh:PTR TO guihandle,g:ILIST) IS gadstr(g,findgadget(gh,g))

/********** getinteger **********/
EXPORT PROC getinteger(gh:PTR TO guihandle,g:ILIST) IS gadinteger(g,findgadget(gh,g))

-> Copy buffer string to gadget E-string.
PROC gadstr(l:ILIST, gad:PTR TO gadget) IS IF gad THEN StrCopy(ATTR(l,STR_STR),GadString(gad)) ELSE 0

-> Copy buffer integer to gadget integer.
PROC gadinteger(l:ILIST, gad:PTR TO gadget)
  DEF x=0
  IF gad THEN ATTR(l,INT_VAL):=x:=GadLongInt(gad)
ENDPROC x

-> Record new gadget value.
PROC performset(gad:PTR TO gadget,code,l:ILIST)
  SELECT MAXGUI OF gad.gadgetid
...
PROC min_pal(x,y,d)
  DEF dy
  IF (x+y)=0 THEN RaiseX("Egui",1441,'Bad PALETTE gadget sizes.')
  dy:=Div(Mul(y,d),x+y)
ENDPROC Max(Mul(Shl(1,d-dy),7)+21,x), Max(Shl(4,dy)+3,y)

-> Calculate minimum size of GUI.
PROC minsize(gui:ILIST,gh:PTR TO guihandle,isinrows=TRUE,eql=FALSE)
  DEF p:PTR TO g,h,minargs,a,b:PTR TO VALUE,c,pl:PTR TO plugin,type,
      ta:PTR TO textattr
  minargs:=minARGS()
  ta:=gh.tattr
  h:=ta.ysize
  ...
  CASE TEXT,NUM
    ...
    b:=(IF type=NUM THEN Max(ATTR(gui,NUM_REL),2)*textlen('5',ta) ELSE Max(textlen(ATTR(gui,TXT_VAL),ta),ATTR(gui,TXT_REL)*h))
...
PROC foreach(guilist:ILIST,type,gh:PTR TO guihandle,row,eq)
  ...
  ENDIF
  resize:=(IF DoesXUncond(resize) THEN RESIZEX ELSE 0) OR (IF DoesYUncond(resize) THEN RESIZEY ELSE 0)
...
PROC adjustall(base:PTR TO g,x,y,xs,ys,gh:PTR TO guihandle)
...
PROC creategadget(base:PTR TO g,lxs,lys,gh:PTR TO guihandle)
  DEF tl,i:ILIST,minargs,text,flags=0,kindtab,h,textl,x,y,xs,ys,
...
PROC findxy(gh:PTR TO guihandle,x,y)
  ...
  WHILE gad
    IF gad.mutualexclude=EG_MAGIC  -> Then it's an EasyGUI AppW gadget...
      ...
      ENDIF
      IF x>=(gad.leftedge+offx) THEN IF y>=(gad.topedge+offy) THEN IF gad.leftedge+gad.width>x THEN IF gad.topedge+gad.height>y THEN RETURN gad.userdata
    ENDIF
    gad:=gad.nextgadget
  ENDWHILE
ENDPROC NIL
...
PROC setgadattr(g:PTR TO gadget,w:PTR TO window,gad:PTR TO VALUE,value,tag,index=0)
...
PROC setattr(gh:PTR TO guihandle,gad:PTR TO VALUE,value,tag,index=0) IS setgadattr(findgadget(gh,gad),gh.wnd,gad,value,tag,index)

/********** setXXXXX **********/
EXPORT PROC setcheck(gh:PTR TO guihandle,gad:PTR TO VALUE,bool) IS setattr(gh,gad,bool,GTCB_CHECKED,CHK_VAL)
EXPORT PROC setinteger(gh:PTR TO guihandle,gad:PTR TO VALUE,new) IS setattr(gh,gad,new,GTIN_NUMBER,INT_VAL)
EXPORT PROC setmx(gh:PTR TO guihandle,gad:PTR TO VALUE,active) IS setattr(gh,gad,active,GTMX_ACTIVE,MX_CURR)
EXPORT PROC setcycle(gh:PTR TO guihandle,gad:PTR TO VALUE,active) IS setattr(gh,gad,active,GTCY_ACTIVE,CYC_CURR)
EXPORT PROC setpalette(gh:PTR TO guihandle,gad:PTR TO VALUE,colour) IS setattr(gh,gad,colour,GTPA_COLOR,PAL_CURR)
EXPORT PROC setscrolltop(gh:PTR TO guihandle,gad:PTR TO VALUE,top) IS setattr(gh,gad,top,GTSC_TOP,SCR_TOP)
EXPORT PROC setscrolltotal(gh:PTR TO guihandle,gad:PTR TO VALUE,total) IS setattr(gh,gad,total,GTSC_TOTAL,SCR_TOTL)
EXPORT PROC setscrollvisible(gh:PTR TO guihandle,gad:PTR TO VALUE,visible) IS setattr(gh,gad,visible,GTSC_VISIBLE,SCR_VIS)
EXPORT PROC setslide(gh:PTR TO guihandle,gad:PTR TO VALUE,new) IS setattr(gh,gad,new,GTSL_LEVEL,SLI_CURR)
EXPORT PROC settext(gh:PTR TO guihandle,gad:PTR TO VALUE,new) IS setattr(gh,gad,new,GTTX_TEXT,TXT_VAL)
EXPORT PROC setnum(gh:PTR TO guihandle,gad:PTR TO VALUE,new) IS setattr(gh,gad,new,GTNM_NUMBER,NUM_VAL)
EXPORT PROC setlistvlabels(gh:PTR TO guihandle,gad:PTR TO VALUE,labs) IS setattr(gh,gad,labs,GTLV_LABELS,LST_LIST)
EXPORT PROC setlistvvisible(gh:PTR TO guihandle,gad,vis) IS setattr(gh,gad,vis,GTLV_MAKEVISIBLE)

EXPORT PROC setlistvselected(gh:PTR TO guihandle,gad:PTR TO VALUE,active)
  DEF g
...
EXPORT PROC setstr(gh:PTR TO guihandle,gad:PTR TO VALUE,new)
  setattr(gh,gad,new,GTST_STRING)
ENDPROC StrCopy(ATTR(gad,STR_STR),new)

/********** disposegui **********/
EXPORT PROC disposegui(gui:ILIST)
  DEF a,l
  IF gui

...
NATIVE {ASLFR_InitialShowVolumes} CONST ASLFR_INITIALSHOWVOLUMES = ASL_TB+130 /* Initially, show the volume list
                                               (V44) */
...
NATIVE {FontRequester} OBJECT fontrequester
    ...
    {fo_SpecialDrawMode}	specialdrawmode	:UBYTE/* Returned special drawmode (V50)  */
    ...
    {fo_TAttr}	tattr	:ttextattr          /* Returned TTextAttr. Since V50 it
...
NATIVE {ASLFO_InitialSpecMode} CONST ASLFO_INITIALSPECMODE = ASL_TB+139 /* Initial special drawmode (V50)   */
NATIVE {ASLFO_InitialCharSet}  CONST ASLFO_INITIALCHARSET  = ASL_TB+134 /* Initial charset (V50)            */
...
NATIVE {ASLFO_DoSpecialMode}   CONST ASLFO_DOSPECIALMODE   = ASL_TB+140 /* Display DrawMode cycle gadget    */
                                         /* with options Text,               */
                                         /* Text+Background, Text+Outline,   */
                                         /* Text+Shadow? (V50)               */
NATIVE {ASLFO_DoCharSet}       CONST ASLFO_DOCHARSET       = ASL_TB+135 /* Display CharSet selector? (V50)  */
NATIVE {ASLFO_SampleText}      CONST ASLFO_SAMPLETEXT      = ASL_TB+133 /* Text to display in font sample
                                          * area (V45) */

/* Filtering */
NATIVE {ASLFO_FixedWidthOnly}  CONST ASLFO_FIXEDWIDTHONLY  = ASL_TB+48  /* Only allow fixed-width fonts?    */
NATIVE {ASLFO_OTagOnly}        CONST ASLFO_OTAGONLY        = ASL_TB+136 /* Only allow bulletAPI fonts? (V50)*/
NATIVE {ASLFO_ScalableOnly}    CONST ASLFO_SCALABLEONLY    = ASL_TB+137 /* Only allow scalable bullet API
                                          * fonts? (V50) */
...
NATIVE {ASLFO_CSFilterFunc}    CONST ASLFO_CSFILTERFUNC    = ASL_TB+138 /* Function to filter charsets      */
...
NATIVE {FO_SPECIALMODE_NONE}    CONST FO_SPECIALMODE_NONE    = 0 /* Normal text   */
NATIVE {FO_SPECIALMODE_OUTLINE} CONST FO_SPECIALMODE_OUTLINE = 1 /* Outlined text */
NATIVE {FO_SPECIALMODE_SHADOW}  CONST FO_SPECIALMODE_SHADOW  = 2 /* Shadowed text */
...
NATIVE {ASL_LAST_TAG} CONST ASL_LAST_TAG = ASL_TB+140
...
NATIVE {AslSemaphore} OBJECT aslsemaphore
    ...
    {as_SortBy}	sortby	:UBYTE       /* File requester defaults;
                                              * name, date or size */
    {as_SortDrawers}	sortdrawers	:UBYTE  /* File requester defaults;
                                              * first, mix or last */
    {as_SortOrder}	sortorder	:UBYTE    /* File requester defaults;
...
NATIVE {ASLPOS_DefaultPosition} CONST ASLPOS_DEFAULTPOSITION = 0    /* Position is calculated according to
                                     * the builtin rules. */
NATIVE {ASLPOS_CenterWindow}    CONST ASLPOS_CENTERWINDOW    = 1    /* Centred within the bounds of the
                                     * parent window. */
NATIVE {ASLPOS_CenterScreen}    CONST ASLPOS_CENTERSCREEN    = 2    /* Centred within the bounds of the
                                     * parent screen. */
NATIVE {ASLPOS_WindowPosition}  CONST ASLPOS_WINDOWPOSITION  = 3    /* Relative to the top left corner of
                                     * the parent window, using the offset
                                     * values provided in the
                                     * as_RelativeLeft/as_RelativeTop members.
                                     */
NATIVE {ASLPOS_ScreenPosition}  CONST ASLPOS_SCREENPOSITION  = 4    /* Relative to the top left corner of the
                                     * parent screen, using the offset
                                     * values provided in the
                                     * as_RelativeLeft/as_RelativeTop members.
                                     */
...
NATIVE {ASLSIZE_DefaultSize}  CONST ASLSIZE_DEFAULTSIZE  = 0 /*(0<<4)*/ /* Size is calculated according to the
                                     * builtin rules. */
NATIVE {ASLSIZE_RelativeSize} CONST ASLSIZE_RELATIVESIZE = $10 /* Size is relative to the size of the
                                     * parent window or screen, using the
                                     * values provided in the
                                     * as_RelativeWidth/as_RelativeHeight
                                     * members. The as_RelativeWidth/
                                     * as_RelativeHeight values are taken
                                     * as percentage, i.e. a value of "50"
                                     * stands for 50% of the width/height
                                     * of the parent window/screen.
                                     */
NATIVE {ASLSIZE_MASK}         CONST ASLSIZE_MASK         = $30

/* Other options. */
NATIVE {ASLOPTION_ASLOverrides} CONST ASLOPTION_ASLOVERRIDES = $40 /* ASL determines placement and size of
                                       * requester windows; application's
                                       * choice is ignored.
                                       */

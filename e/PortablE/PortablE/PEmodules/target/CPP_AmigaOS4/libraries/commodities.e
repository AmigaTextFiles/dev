/* $VER: commodities.h 53.21 (29.9.2013) */
OPT NATIVE, PREPROCESS, INLINE
...
NATIVE {CxFilter} PROC	->CxFilter(d)         CreateCxObj(CX_FILTER,    (int32)d,      0)
NATIVE {CxSender} PROC	->CxSender(port,id)   CreateCxObj(CX_SEND,      (int32)port,   (int32)id)
NATIVE {CxSignal} PROC	->CxSignal(task,sig)  CreateCxObj(CX_SIGNAL,    (int32)task,   (int32)sig)
NATIVE {CxTranslate} PROC	->CxTranslate(ie)     CreateCxObj(CX_TRANSLATE, (int32)ie,     0)
NATIVE {CxDebug} PROC	->CxDebug(id)         CreateCxObj(CX_DEBUG,     (int32)id,     0)
NATIVE {CxCustom} PROC	->CxCustom(action,id) CreateCxObj(CX_CUSTOM,    (int32)action, (int32)id)
...
NATIVE {COF_ENABLE}    CONST COF_ENABLE    = 2  /* If set, the commodity is enabled. This flag is
                          * always set when creating a new broker.
                          */
NATIVE {COF_SHOW_HIDE} CONST COF_SHOW_HIDE = 4  /* If set, the "Show" and "Hide interface" gadgets
                          * for this broker will be selectable in Commodities
                          * Exchange. If not, they are disabled.
                          */
NATIVE {COF_CONSTANT}  CONST COF_CONSTANT  = 8  /* If set, the object cannot be removed by Exchange */
NATIVE {COF_BROADCAST} CONST COF_BROADCAST = 16 /* If set, the object accepts broadcasted commands
                          * such as CXCMD_LIST_CHG.
                          */
...
NATIVE {CX_HOOK}      CONST CX_HOOK      = 10 /* app provides hook                  */

/*****************************************************************************/

/* Commodities message types */
NATIVE {CXM_IEVENT}  CONST CXM_IEVENT  = 1 SHL 5
NATIVE {CXM_COMMAND} CONST CXM_COMMAND = 1 SHL 6
...
NATIVE {CMDE_OK}        CONST CMDE_OK        = (0)  -> Command executed successfully
NATIVE {CMDE_NOBROKER}  CONST CMDE_NOBROKER  = (-1) -> Broker was not found
NATIVE {CMDE_NOPORT}    CONST CMDE_NOPORT    = (-2) -> Broker's port was not found
NATIVE {CMDE_NOMEM}     CONST CMDE_NOMEM     = (-3) -> Out of memory
...
NATIVE {CxBrokerCopy} OBJECT cxbrokercopy
    {Node}	ln	:ln        -> Embedded Node
    {StructSize}	structsize	:ULONG  -> Size of this structure
    {Owner}	owner	:PTR TO tc       -> Task which created the broker
    {Name}	name	:CONST_STRPTR        -> Name of the broker
    {Title}	title	:CONST_STRPTR       -> Title of the broker
    {Description}	description	:CONST_STRPTR -> Short broker description
    {Flags}	flags	:ULONG       -> See COF prefixed flags above
ENDOBJECT

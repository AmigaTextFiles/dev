...
NATIVE {ON_DISPLAY}  CONST ->ON_DISPLAY  = custom.dmacon = BITSET OR DMAF_RASTER
NATIVE {OFF_DISPLAY} CONST ->OFF_DISPLAY = custom.dmacon = BITCLR OR DMAF_RASTER
NATIVE {ON_SPRITE}   CONST ->ON_SPRITE   = custom.dmacon = BITSET OR DMAF_SPRITE
NATIVE {OFF_SPRITE}  CONST ->OFF_SPRITE  = custom.dmacon = BITCLR OR DMAF_SPRITE

NATIVE {ON_VBLANK}   CONST ->ON_VBLANK   = custom.intena = BITSET OR INTF_VERTB
NATIVE {OFF_VBLANK}  CONST ->OFF_VBLANK  = custom.intena = BITCLR OR INTF_VERTB
...
NATIVE {GetOPen} PROC	->GetOPen(rp)
->Needs OS3 compatibility enabled: PROC GetOPen(rp:PTR TO rastport) IS NATIVE {GetOPen(} rp {)} ENDNATIVE !!ULONG
...
->Needs OS3 compatibility enabled: PROC Cinit(c:PTR TO ucoplist, n:VALUE) IS NATIVE {CINIT(} c {,} n {)} ENDNATIVE !!PTR TO coplist
...
->Needs OS3 compatibility enabled: PROC cwait(c:PTR TO ucoplist, a:VALUE, b:VALUE) IS NATIVE {CWAIT(} c {,} a {,} b {)} ENDNATIVE

NATIVE {CEND} CONST	->CEND(c)
#define CEND(c) Cend(c)
->Needs OS3 compatibility enabled: PROC Cend(c:PTR TO ucoplist) IS NATIVE {CEND(} c {)} ENDNATIVE


NATIVE {DrawCircle} PROC	->DrawCircle(rp,cx,cy,r) DrawEllipse(rp,cx,cy,r,r)
PROC DrawCircle(rp:PTR TO rastport, cx:VALUE, cy:VALUE, r:VALUE) IS NATIVE {IGraphics->DrawCircle(} rp {,} cx {,} cy {,} r {)} ENDNATIVE
NATIVE {AreaCircle} PROC	->AreaCircle(rp,cx,cy,r) AreaEllipse(rp,cx,cy,r,r)
PROC AreaCircle(rp:PTR TO rastport, cx:VALUE, cy:VALUE, r:VALUE) IS NATIVE {IGraphics->AreaCircle(} rp {,} cx {,} cy {,} r {)} ENDNATIVE !!LONG

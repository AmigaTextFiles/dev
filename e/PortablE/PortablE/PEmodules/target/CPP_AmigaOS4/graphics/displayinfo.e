...
NATIVE {DIPF_IS_SPRITES_CHNG_RES}  CONST DIPF_IS_SPRITES_CHNG_RES  = $00004000 /* supports variable sprite
                                              * resolution */
NATIVE {DIPF_IS_SPRITES_BORDER}    CONST DIPF_IS_SPRITES_BORDER    = $00008000 /* sprite can be displayed
                                              * in the border */
NATIVE {DIPF_IS_SCANDBL}           CONST DIPF_IS_SCANDBL           = $00020000 /* scan doubled */
NATIVE {DIPF_IS_SPRITES_CHNG_BASE} CONST DIPF_IS_SPRITES_CHNG_BASE = $00040000 /* can change the sprite
                                              * base colour */
NATIVE {DIPF_IS_SPRITES_CHNG_PRI}  CONST DIPF_IS_SPRITES_CHNG_PRI  = $00080000 /* can change the sprite
                                              * priority with respect to
                                              * the playfield(s).
                                              */
NATIVE {DIPF_IS_DBUFFER}           CONST DIPF_IS_DBUFFER           = $00100000 /* can support double buffering */
NATIVE {DIPF_IS_PROGBEAM}          CONST DIPF_IS_PROGBEAM          = $00200000 /* is a programmed beam-sync
                                              * mode */
NATIVE {DIPF_IS_FOREIGN}           CONST DIPF_IS_FOREIGN           = $80000000 /* this mode is not native
                                              * to the Amiga */
...
NATIVE {NameInfo} OBJECT nameinfo
    {Header}	qh	:qh
    {Name}	name[DISPLAYNAMELEN]	:ARRAY OF CHAR /*TEXT*/
    {reserved}	reserved[2]	:ARRAY OF ULONG          /* terminator */
ENDOBJECT

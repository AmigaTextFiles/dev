...
NATIVE {StartupMsgID}		CONST STARTUPMSGID		= (APSH_TOOL_ID+1)
NATIVE {LoginToolID}		CONST LOGINTOOLID		= (APSH_TOOL_ID+2)
NATIVE {LogoutToolID}		CONST LOGOUTTOOLID		= (APSH_TOOL_ID+3)
NATIVE {ShutdownMsgID}		CONST SHUTDOWNMSGID		= (APSH_TOOL_ID+4)
NATIVE {ActivateToolID}		CONST ACTIVATETOOLID		= (APSH_TOOL_ID+5)
NATIVE {DeactivateToolID}	CONST DEACTIVATETOOLID	= (APSH_TOOL_ID+6)
NATIVE {ActiveToolID}		CONST ACTIVETOOLID		= (APSH_TOOL_ID+7)
NATIVE {InactiveToolID}		CONST INACTIVETOOLID		= (APSH_TOOL_ID+8)
NATIVE {ToolStatusID}		CONST TOOLSTATUSID		= (APSH_TOOL_ID+9)
NATIVE {ToolCmdID}		CONST TOOLCMDID		= (APSH_TOOL_ID+10)
NATIVE {ToolCmdReplyID}		CONST TOOLCMDREPLYID		= (APSH_TOOL_ID+11)
NATIVE {ShutdownToolID}		CONST SHUTDOWNTOOLID		= (APSH_TOOL_ID+12)
...
NATIVE {AGA_HelpGroup}		CONST AGA_HELPGROUP		= (AGA_DUMMY+5)	/* (ULONG) Unique identifier */
...
NATIVE {AGA_ARexxPort}		CONST AGA_AREXXPORT		= (AGA_DUMMY+9)	/* (struct MsgPort *) Pointer to the ARexx message port */
NATIVE {AGA_ARexxPortName}	CONST AGA_AREXXPORTNAME	= (AGA_DUMMY+10)	/* (STRPTR) Used to specify the ARexx port name (not copied) */
NATIVE {AGA_Secure}  	    	CONST AGA_SECURE  	    	= (AGA_DUMMY+11)

NATIVE {AMIGAGUIDECONTEXT} CONST
TYPE AMIGAGUIDECONTEXT IS PTR
...
NATIVE {NewAmigaGuide} OBJECT newamigaguide
    ...
    {nag_ClientPort}	clientport	:/*STRPTR*/ ARRAY OF CHAR/* Name to assign to the clients ARexx port */
    ...
    {nag_Node}	node	:/*STRPTR*/ ARRAY OF CHAR	/* Node to align on first (defaults to Main) */
...
->TYPE AMIGAGUIDEHOST IS PTR TO amigaguidehost
...
/* onm_Flags */
NATIVE {HTNF_KEEP}	CONST HTNF_KEEP	= 1 SHL 0	/* Don't flush this node until database is closed */

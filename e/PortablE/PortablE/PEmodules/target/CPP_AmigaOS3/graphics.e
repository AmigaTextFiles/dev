...
PROC BltBitMap( srcBitMap:PTR TO bitmap, xSrc:VALUE, ySrc:VALUE, destBitMap:PTR TO bitmap, xDest:VALUE, yDest:VALUE, xSize:VALUE, ySize:VALUE, minterm:ULONG, mask:ULONG, tempA:PLANEPTR ) RETURNS planecnt:VALUE
	DEF destRP:rastport
	InitRastPort(destRP)
	destRP.bitmap := destBitMap
	BltBitMapRastPort(srcBitMap, xSrc, ySrc, destRP, xDest, yDest, xSize, ySize, minterm)
	planecnt := Min(GetBitMapAttr(srcBitMap, BMA_DEPTH), GetBitMapAttr(destBitMap, BMA_DEPTH))		->BltBitMapRastPort() doesn't return planecnt, so we have to fake it
	mask := 0 ; tempA := NILA	->dummy
ENDPROC

/* AmigaE module, for $VER: exec_protos.h 45.2 (6.6.1998) */
...
NATIVE {execbase} DEF execbase:NATIVE {LONG} PTR TO lib


/*------ misc ---------------------------------------------------------*/
NATIVE {Supervisor} PROC
PROC Supervisor( userFunction:PTR /*ULONG (*CONST userFunction)()*/ ) IS NATIVE {Supervisor(} userFunction {)} ENDNATIVE !!ULONG
...
PROC MakeLibrary( funcInit:APTR, structInit:APTR, libInit:PTR /*ULONG (*CONST libInit)()*/, dataSize:ULONG, segList:ULONG ) IS NATIVE {MakeLibrary(} funcInit {,} structInit {,} libInit {,} dataSize {,} segList {)} ENDNATIVE !!PTR TO lib
...
PROC SetFunction( library:PTR TO lib, funcOffset:VALUE, newFunction:PTR /*ULONG (*CONST newFunction)()*/ ) IS NATIVE {SetFunction(} library {,} funcOffset {,} newFunction {)} ENDNATIVE !!APTR
...
PROC RawDoFmt( formatString:ARRAY OF CHAR, dataStream:APTR, putChProc:PTR /*VOID (*CONST putChProc)()*/, putChData:APTR ) IS NATIVE {RawDoFmt(} formatString {,} dataStream {,} putChProc {,} putChData {)} ENDNATIVE !!APTR
NATIVE {GetCC} PROC
...
->PROC NewMinList( minlist:PTR TO mlh ) IS NATIVE {NewMinList(} minlist {)} ENDNATIVE
/*------ New AVL tree support for V45. Yes, this is intentionally part of Exec!*/
->NATIVE {AVL_AddNode} PROC
->PROC AvL_AddNode( root:ARRAY OF PTR TO avlnode, node:PTR TO avlnode, func:APTR ) IS NATIVE {AVL_AddNode(} root {,} node {,} func {)} ENDNATIVE !!PTR TO avlnode
->NATIVE {AVL_RemNodeByAddress} PROC
->PROC AvL_RemNodeByAddress( root:ARRAY OF PTR TO avlnode, node:PTR TO avlnode ) IS NATIVE {AVL_RemNodeByAddress(} root {,} node {)} ENDNATIVE !!PTR TO avlnode
->NATIVE {AVL_RemNodeByKey} PROC
->PROC AvL_RemNodeByKey( root:ARRAY OF PTR TO avlnode, key:AVLKey, func:APTR ) IS NATIVE {AVL_RemNodeByKey(} root {,} key {,} func {)} ENDNATIVE !!PTR TO avlnode
->NATIVE {AVL_FindNode} PROC
->PROC AvL_FindNode( root:PTR TO avlnode, key:AVLKey, func:APTR ) IS NATIVE {AVL_FindNode(} root {,} key {,} func {)} ENDNATIVE !!PTR TO avlnode
...
->PROC AvL_FindPrevNodeByKey( root:PTR TO avlnode, key:AVLKey, func:APTR ) IS NATIVE {AVL_FindPrevNodeByKey(} root {,} key {,} func {)} ENDNATIVE !!PTR TO avlnode
...
->PROC AvL_FindNextNodeByKey( root:PTR TO avlnode, key:AVLKey, func:APTR ) IS NATIVE {AVL_FindNextNodeByKey(} root {,} key {,} func {)} ENDNATIVE !!PTR TO avlnode

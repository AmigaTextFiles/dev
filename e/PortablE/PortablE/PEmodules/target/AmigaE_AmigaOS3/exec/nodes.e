...
NATIVE {ln} OBJECT ln
    {succ}	succ	:PTR TO ln	/* Pointer to next (successor) */
    {pred}	pred	:PTR TO ln	/* Pointer to previous (predecessor) */
    {type}	type	:UBYTE
    {pri}	pri	:BYTE		/* Priority, for sorting */
    {name}	name	:ARRAY OF CHAR		/* ID string, null terminated */
ENDOBJECT	/* Note: word aligned */

/* minimal node -- no type checking possible */
NATIVE {mln} OBJECT mln
    {succ}	succ	:PTR TO mln
    {pred}	pred	:PTR TO mln
ENDOBJECT

...
PROC main()
  ...
  DEF app:PTIO, window:PTIO, wheel:PTIO, hue, sat, sigs
  
  IF  (muimasterbase := OpenLibrary(MUIMASTER_NAME,MUIMASTER_VMIN))=NIL THEN Throw("ERR", 'Failed to open muimaster.library')
  IF (colorwheelbase := OpenLibrary('gadgets/colorwheel.gadget',0))=NIL THEN Throw("ERR", 'colorwheel boopsi gadget not available')

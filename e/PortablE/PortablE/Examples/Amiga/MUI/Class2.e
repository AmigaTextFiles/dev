...
MODULE 'exec', 'graphics', 'intuition'
...
PROC mNew(cl:PTR TO iclass, obj:PTIO, msg:PTR TO opset)
DEF data:PTR TO mydata,
  ...
  tags := msg.attrlist
  WHILE tag := NextTagItem(ADDRESSOF tags)
    IF tag.tag=MYATTR_PEN
      IF tag.data THEN CopyMem(tag.data !!ARRAY, data.penspec, SIZEOF mui_penspec)
    ENDIF
  ENDWHILE
...
PROC mDispose(cl:PTR TO iclass, obj:PTIO, msg:PTR TO msg) IS doSuperMethodA(cl, obj, msg)


PROC mSet(cl:PTR TO iclass, obj:PTIO, msg:PTR TO opset)
DEF data:PTR TO mydata,
    tags:ARRAY OF tagitem,
    tag:PTR TO tagitem
  ...
  WHILE tag := NextTagItem(ADDRESSOF tags)
    IF tag.tag=MYATTR_PEN
      IF tag.data
        CopyMem(tag.data !!ARRAY, data.penspec, SIZEOF mui_penspec)
        data.penchange := TRUE
...
PROC mGet(cl:PTR TO iclass, obj:PTIO, msg:PTR TO opget)
DEF data:PTR TO mydata, storage

  IF msg.attrid=MYATTR_PEN
    data := INST_DATA(cl, obj) ::mydata
    storage := msg.storage
    PutValue(ADDRESSOF storage, data.penspec)
    RETURN MUI_TRUE
  ENDIF
...
DEF methodID
  ...
  SELECT methodID
    ...
    CASE OM_SET          ;  RETURN mSet      (cl, obj, msg!!PTR)
    CASE OM_GET          ;  RETURN mGet      (cl, obj, msg!!PTR)
...
PROC main()
DEF app:PTIO, window:PTIO, myobj:PTIO, pen:PTIO,
  ...
  */
  IF (utilitybase := OpenLibrary('utility.library', 36))=NIL THEN Throw("ERR", 'Failed TO open utility.library')
  ...
  IF (mcc := eMui_CreateCustomClass(NIL, MUIC_Area, NIL, SIZEOF mydata, CALLBACK myDispatcher()))=NIL THEN Throw("ERR", 'Could not create custom class.')
  ...
  get(pen, MUIA_Pendisplay_Spec, ADDRESSOF startpen)
...
*/
    ...
    sigs := 0
    WHILE Not(doMethodA(app, [MUIM_Application_NewInput, ADDRESSOF sigs]) = MUIV_Application_ReturnID_Quit)

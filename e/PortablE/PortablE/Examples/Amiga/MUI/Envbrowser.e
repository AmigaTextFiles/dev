...
PROC main()
  DEF app:PTIO, bt_Edit:PTIO, bt_Delete:PTIO, bt_Save:PTIO, wi_Browser:PTIO, lv_Show:PTIO, lv_Vars:PTIO
  DEF buffer[2048]:STRING, var:ARRAY OF CHAR, running:BOOL, signal, result
  running := TRUE
  ...
  set(wi_Browser, MUIA_Window_Open,MUI_TRUE)
    ...
    WHILE running
      result := doMethodA(app, [MUIM_Application_Input, ADDRESSOF signal])
        SELECT result
               CASE MUIV_Application_ReturnID_Quit
                    running := FALSE
               CASE ID_DISPLAY
                    get(lv_Vars,MUIA_Dirlist_Path,ADDRESSOF var)

...
#define TagDone           TAG_DONE]:tagitem
...
#define DupStr(str)     StrCopy(NewString(StrLen(str)),str)
#define DupStrE(str)    StrCopy(NewString(EstrLen(str)),str)

#define ExceptionReport(pname)  exceptionReport(pname)

PROC exceptionReport(pname)
DEF str[5]:ARRAY OF CHAR,str2:PTR TO CHAR,x
  IF exception
    PutQuad(str!!VALUE!!PTR TO QUAD,exception)
    str[4]:=0
    str2:=str
    WHILE str2[]=0 DO str2++

    FOR x:=0 TO 3
      IF (str[x]<>0) AND ((str[x]<32) OR (str[x]>127)) THEN str2:=-1 !!VALUE!!PTR TO CHAR
    ENDFOR

    IF exception<1000
      Print('\s: Exception #\d recieved!\n',pname,exception)
    ELSE IF str2=-1
      Print('\s: Exception $\z\h[8]\n recieved!\n',pname,exception)
    ELSE
      Print('\s: Exception "\s" recieved!\n',pname,str2)
    ENDIF
  ENDIF
ENDPROC
...
DEF n:PTR TO ln, exit
  n:=list.head
  WHILE num>0
->    Print('name="\s"\n',IF n THEN IF n.name THEN n.name; ELSE 'NULL POINTER')
    n:=n.succ
    exit := n=NIL
    IF NOT exit THEN num--
  ENDWHILE IF exit
ENDPROC n

#define Long2Str(l)   long2str(l,[0,0])

PROC long2str(l,tmp:PTR TO LONG)
  PutLong(tmp,l!!LONG)
ENDPROC tmp

PROC getProgramName()
DEF b:STRING,c:ARRAY OF CHAR,d /*,thistask:PTR TO tc*/
  /*IF wbmessage
    thistask:=FindTask(NIL)
    d:=DupStr(thistask.ln.name)
  ELSE*/
    GetProgramName(b:=NewString(1024),1024)
    c:=FilePart(b)
    d:=DupStr(c)
    Dispose(b)
  /*ENDIF*/
...
STATIC id1 = 'macros.m 3.2 (21/4/96)\0'
STATIC id2 = '© Chris Sumner 1996\0'

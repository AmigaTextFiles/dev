/*"p_EnleveNode(ptr_list:PTR TO lh,num_node,doit,dat:PTR TO LONG)"*/
PROC p_EnleveNode(ptr_list:PTR TO lh,num_node,doit,dat:PTR TO LONG) 
/*===============================================================================
            ...
            IF doit<>0
                REPEAT
                    ...
                    IF p<>DISE
                        ...
                        ENDIF
                        IF (p=DISP)
                            IF pivadr<>0 THEN Dispose(pivadr)
                        ENDIF
                        IF (p=DISL)
                            IF pivadr<>0 THEN DisposeLink(pivadr)
                        ENDIF
                    ENDIF
                UNTIL (p=DISE)

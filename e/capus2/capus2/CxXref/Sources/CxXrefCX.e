...
PROC p_InitCx() HANDLE 
    ...
    DEF txt[80]:STRING
    dWriteF(['p_InitCx()\n'],0)
    StringF(txt,'CxXref (\s/\s)',hotkey,hotkeyguide)
    ...
    sender:=cxSender(broker_mp,EVT_HOTKEY_CXREF)
    ...
    ENDIF
    /* HotKey Guide */
    filter:=cxFilter(hotkeyguide)
    IF (errorcx:=CxObjError(filter))<>0 THEN Raise(ER_CX)
    AttachCxObj(broker,filter)
    sender:=cxSender(broker_mp,EVT_HOTKEY_GUIDE)
    AttachCxObj(filter,sender)
    translate:=cxTranslate(NIL)
    AttachCxObj(filter,translate)
    IF (errorcx:=CxObjError(filter))=0

...
#ifdef EASY_OS12
  MODULE 'tools/easygui_os12','hybrid/tagdata','hybrid/version',
         'hybrid/utility','hybrid/drawinfo'
  #define GetTagData getTagData
  #define GetScreenDrawInfo getScreenDrawInfo
  #define FreeScreenDrawInfo freeScreenDrawInfo
#endif
#ifndef EASY_OS12
  OPT OSVERSION=37
  MODULE 'utility'
...
DEF rp:REG, left, top, right, bottom, width, height, lenght:REG, pixels,
   ...
   IF ((self.oldvalue<>lenght) OR (lenght=0) OR (redraw))
      ...
      -> RST: added IF ... stuff to avoid negative sized rects (older intuition did not clip!)
      IF self.horizontal
         IF lenght<>pixels THEN RectFill(rp, left+lenght, top, right, bottom)
         SetAPen(rp, self.fillpen)
         IF lenght THEN RectFill(rp, left, top, left+lenght-1, bottom)
      ELSE
         IF lenght<>pixels THEN RectFill(rp, left, top, right, bottom-lenght)
         SetAPen(rp, self.fillpen)
         IF lenght THEN RectFill(rp, left, bottom-lenght+1, right, bottom)
      ENDIF
         ...
         IF value=TRUE
            IF self.gh<>0           /* ignore draw before gui is open */
               IF self.gh.wnd<>0    /* ignore draw when window is closed */
                  -> RST: removed -1 from self.xs-1, self.ys-1

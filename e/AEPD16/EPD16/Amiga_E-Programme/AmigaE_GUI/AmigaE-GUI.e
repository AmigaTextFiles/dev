...
:EC-Version.     EC3.0     EC2.1b (v39 patch) mit 3.0 Modulen
:OS.            > 2.0 
:PRG-Version.   1.403

Benötigt Gadtoolbox.library >= Version 33
...
FOR y:=0 TO 23
  bn[y].s:=String(250) 
ENDFOR  
VOID '$VER:AmigaE-GUI © F.Bunk  V1.403 (14.11.1994)' 
 bn[14].s:='.e'
 mpx:=1
  lade()
  mpx:=1
...
PROC openproject0window()
 ...
 IF (g:=CreateGadgetA(STRING_KIND,g,
  ...
  IF (g:=CreateGadgetA(STRING_KIND,g,
   ...
   zeiger[5]:=g
  IF (g:=CreateGadgetA(STRING_KIND,g,
    [offx+325,offy+14,195,14,'EC',tattr,19,1,visual,0]:newgadget,
    [GTST_STRING,bn[10].s,
     GTST_MAXCHARS,250,
     NIL]))=NIL THEN RETURN NOGADGET
 ...
 g.selectrender:=getfilei
  ...
  IF (g:=CreateGadgetA(BUTTON_KIND,g,
    ...
    [NIL]))=NIL THEN RETURN NOGADGET
  IF (g:=CreateGadgetA(BUTTON_KIND,g,
    [offx+100,offy+28,60,13,'HELP',tattr,16,16,visual,0]:newgadget,
    [NIL]))=NIL THEN RETURN NOGADGET
  IF (g:=CreateGadgetA(BUTTON_KIND,g,
  ...
  IF (g:=CreateGadgetA(BUTTON_KIND,g,
    ...
    [NIL]))=NIL THEN RETURN NOGADGET
  IF (g:=CreateGadgetA(BUTTON_KIND,g,
    [offx+210,offy+19,60,13,'About',tattr,21,16,visual,0]:newgadget,
    [NIL]))=NIL THEN RETURN NOGADGET
...
IF SetMenuStrip(wnd,menu)=FALSE THEN RETURN NOMENUS 
 ...
 DrawBevelBoxA(wnd.rport,offx+200,offy+10,80,30,[GT_VISUALINFO,visual,GTBB_RECESSED,NIL])
...
PROC auswerte()
 SELECT auszahl
   ...
   CASE 1023
        ...
        SystemTagList(starts,[SYS_INPUT,stdout,SYS_OUTPUT,stdout,NIL,NIL])
   CASE 1004
        EasyRequestArgs(0,[20,0,'Infos','\s','Weiter'],0,
        ['AmigaE-GUI V 1.402 ( 14.11.94 )\n © F.Bunk 1994\n Uses GadtoolBox.library '])
   CASE 1003
        IF mpx>0 THEN speicher()
   CASE 1005

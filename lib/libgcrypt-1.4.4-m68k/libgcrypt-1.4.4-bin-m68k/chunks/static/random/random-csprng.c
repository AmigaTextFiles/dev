...
/* Amiga - */
#if defined(HAVE_FCNTL) && defined(HAVE_FTRUNCATE) && !defined(HAVE_W32_SYSTEM) && !defined(__amigaos__)
...
*/
  int fdcheck;
  if ((fdcheck = open(NAME_OF_DEV_RANDOM, O_RDONLY)) >= 0)
  {
    close(fdcheck);

    if ((fdcheck = open(NAME_OF_DEV_URANDOM, O_RDONLY)) >= 0)
    {
      close(fdcheck);    

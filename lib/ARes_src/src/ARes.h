...
#define AE_NOERROR  -1L 	/* et non 0, à cause de OpenDevice() */
#define AE_NOMEMORY  0L 	/* plus de mémoire libre */
#define AE_FAILED    1L 	/* l'allocation a échoué */
...
#define RFI_REPERT	0x01	/* gadget "Fichier" désactivé */
#define RFI_COULEUR	0x02	/* fond en couleur 0 et non en couleur 3 */
#define RFI_VOLUMES	0x04	/* pas de liste des volumes */
#define RFI_ANGLAIS	0x08	/* textes de la requête en anglais */
#define RFI_ALLEMAND	0x10	/* textes de la requête en allemand */
...
{
  long		 rs_Flags   ;
  struct Screen *rs_Ecran   ;	/* écran où afficher la requête */
  long		 rs_IDCMP   ;	/* messages à attendre		*/
  long	       (*rs_fct)( struct ReqSimple * , struct IntuiMessage * )  ;
  char		*rs_Titre   ;	/* titre de la requête		*/
  char		*rs_Texte1  ;	/* première ligne de texte	*/
  ...
  char		*rs_Touche1 ;	/* touches équivalentes au 1er gadget  */
  char		*rs_Touche2 ;	/* touches équivalentes au 2nd gadget  */
  char		 rs_CFond   ;	/* couleur de fond de fenêtre (ou -1)  */
...
struct AutoAdapt
{
  ...
  char	 aa_lchar ;	/* LeftEdge  (en caractères)   */
  char	 aa_lval ;	/* LeftEdge  (pixels/diviseur) */
  char	 aa_tchar ;	/* TopEdge   (en caractères)   */
  char	 aa_tval ;	/* TopEdge   (pixels/diviseur) */
  char	 aa_wchar ;	/* Width      (en caractères)   */
  char	 aa_wval ;	/* Width      (pixels/diviseur) */
  char	 aa_hchar ;	/* Height     (en caractères)   */
...
#define AAF_RTOP	0x00000020	/* TopEdge relatif bord supérieur aa_ritem */
...
#define AAF_TLEFT	0x00000200	/* met le texte du gadget à gauche */
...
#define OEF_NEWROM	0x01	/* Kickstart 2.04 ou supérieur	*/
#define OEF_PALMODE	0x02	/* moniteur PAL au lieu NTSC	*/
#define OEF_FORCELACE	0x04	/* force un écran entrelacé	*/
#define OEF_CLONELACE	0x08	/* écran entrelacé si WB l'est  */
#define OEF_MAKEPUBLIC	0x10	/* rend public après ouverture	*/
#define OEF_CLONEWB	0x20	/* même mode écran que le WB	*/

			      ...
			      GTimeVal * abs_time)
{
  ...
  /* Amiga - ts!!! */
  end_time.ts_sec = abs_time->tv_sec;
  end_time.ts_nsec = abs_time->tv_usec * (G_NSEC_PER_SEC / G_USEC_PER_SEC);

  g_return_val_if_fail (end_time.ts_nsec < G_NSEC_PER_SEC, TRUE);

...
uint16_t *U_Utf32leToUtf16le(
   ...
   status = iconv(conv, (const char* * )src, &srclen, &dst, &dstlen);
   iconv_close(conv);
...
uint32_t *U_Utf16leToUtf32le(
   ...
   status = iconv(conv, (const char* * )src2, &srclen, &dst, &dstlen);
   iconv_close(conv);
   if(status == (size_t) -1)return(NULL);
...
uint32_t *U_Latin1ToUtf32le(
   ...
   dst2 = dst = calloc(dstlen,1);
   if(!dst)return(NULL);
   conv = iconv_open("UTF-32LE",   "LATIN1");
   if ( conv == (iconv_t) -1)return(NULL);
   status = iconv(conv, (const char* * )src2, &srclen, &dst, &dstlen);
   iconv_close(conv);
   if(status == (size_t) -1)return(NULL);
...
char *U_Utf16leToLatin1(
   ...
   if(!dst)return(NULL);
   conv = iconv_open("LATIN1//TRANSLIT",   "UTF-16LE"); // translate what can be, fill in with something close for the rest
   status = iconv(conv, (const char* * )src, &srclen, &dst, &dstlen);
   iconv_close(conv);
   if(status != (size_t) -1){
      if(len)*len=strlen(dst2);
...
char *U_Utf8ToLatin1(
   ...
   dst2 = dst = calloc(dstlen,1);
   if(!dst)return(NULL);
   conv = iconv_open("LATIN1//TRANSLIT",   "UTF-8"); // translate what can be, fill in with something close for the rest
   if ( conv == (iconv_t) -1)return(NULL);
   status = iconv(conv, (const char* * )src, &srclen, &dst, &dstlen);
   iconv_close(conv);
   if(status == (size_t) -1)return(NULL);
...
char *U_Latin1ToUtf8(
   ...
   dst2 = dst = calloc(dstlen,1);
   if(!dst)return(NULL);
   conv = iconv_open("UTF-8", "LATIN1"); // everything should translate
   if ( conv == (iconv_t) -1)return(NULL);
   status = iconv(conv, (const char* * )src, &srclen, &dst, &dstlen);
   iconv_close(conv);
   if(status == (size_t) -1)return(NULL);

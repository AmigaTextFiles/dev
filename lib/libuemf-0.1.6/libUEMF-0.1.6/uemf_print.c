...
void core2_print(const char *name, const char *contents){
   ...
   printf("\n");
   paptl = (PU_POINTL)((char *)pEmr->aPolyCounts + sizeof(uint32_t)* pEmr->nPolys);
   printf("   Points:         ");
   for(i=0;i<pEmr->cptl; i++){
...
void core10_print(const char *name, const char *contents){
   ...
   printf("\n");
   printf("   Points:         ");
   papts = (PU_POINT16)((char *)pEmr->aPolyCounts + sizeof(uint32_t)* pEmr->nPolys);
   for(i=0; i<pEmr->cpts; i++){
      printf(" [%d]:",i);  point16_print(papts[i]);
   }
   printf("\n");
...
void U_EMRSETPALETTEENTRIES_print(const char *contents){
   ...
   if(pEmr->cEntries){
      printf("      PLTEntries:");
      aPalEntries = (PU_LOGPLTNTRY) &(pEmr->aPalEntries);
  ...
  PU_EMRCOMMENT pEmr = (PU_EMRCOMMENT)(contents);
   ...
   if(cbData >= 4){
      ...
      if(     cIdent == U_EMR_COMMENT_PUBLIC       ){
         ...
         pEmrp = (PU_EMRCOMMENT_PUBLIC) pEmr;
         printf("   pcIdent:        %8.8x\n",pEmrp->pcIdent);
         src = (char *)&(pEmrp->Data);
         cbData -= 8;
      }

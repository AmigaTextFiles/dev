		...
		if (endSeg > startSeg) {
			if (IsArrowCharacter(s[startSeg])) {
				...
				if (draw) {
					...
					if (upArrow) {      // Up arrow
						QSPoint pts[] = {
    						QSPoint(centreX - halfWidth, centreY + halfWidth / 2),
    						QSPoint(centreX + halfWidth, centreY + halfWidth / 2),
    						QSPoint(centreX, centreY - halfWidth + halfWidth / 2),
						};
...
void CallTip::MouseClick(QSPoint pt) {
	clickPlace = 0;
...
PRectangle CallTip::CallTipStart(int pos, QSPoint pt, const char *defn,
                                 const char *faceName, int size,
                                 int codePage_, int characterSet, QSWindow &wParent) {
	clickPlace = 0;
	delete []val;
	val = 0;

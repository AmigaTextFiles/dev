...
protected:	// ScintillaBase subclass needs access to much of Editor
	...
	QSWindow wMain;	///< The Scintilla parent window

	/** Style resources may be expensive to allocate so are cached between uses.
	 * When a style attribute is changed, this cache is flushed. */
	bool stylesValid;
	ViewStyle vs;
	QSPoint sizeRGBAImage;
	Palette palette;
	...
	* and then the foreground. This avoids chopping off characters that overlap the next run. */
	...
	QSPoint LocationFromPosition(SelectionPosition pos);
	QSPoint LocationFromPosition(int pos);
	int XFromPosition(int pos);
	int XFromPosition(SelectionPosition sp);
	SelectionPosition SPositionFromLocation(QSPoint pt, bool canReturnInvalid=false, bool charPosition=false, bool virtualSpace=true);
	int PositionFromLocation(QSPoint pt, bool canReturnInvalid=false, bool charPosition=false);
	SelectionPosition SPositionFromLineX(int lineDoc, int x);
	int PositionFromLineX(int line, int x);
	int LineFromLocation(QSPoint pt);
	...
	virtual void NotifyDoubleClick(QSPoint pt, bool shift, bool ctrl, bool alt);
	...
	bool NotifyMarginClick(QSPoint pt, bool shift, bool ctrl, bool alt);
	void NotifyNeedShown(int pos, int len);
	void NotifyDwelling(QSPoint pt, bool state);
	void NotifyZoom();
	...
	virtual void DisplayCursor(QSWindow::Cursor c);
	virtual bool DragThreshold(QSPoint ptStart, QSPoint ptNow);
	virtual void StartDrag();
	void DropAt(SelectionPosition position, const char *value, bool moving, bool rectangular);
	/** PositionInSelection returns true if position in selection. */
	bool PositionInSelection(int pos);
	bool PointInSelection(QSPoint pt);
	bool PointInSelMargin(QSPoint pt);
	QSWindow::Cursor GetMarginCursor(QSPoint pt);
	void LineSelection(int lineCurrentPos_, int lineAnchorPos_, bool wholeLine);
	void WordSelection(int pos);
	void DwellEnd(bool mouseMoved);
	void MouseLeave();
	virtual void ButtonDown(QSPoint pt, unsigned int curTime, bool shift, bool ctrl, bool alt);
	void ButtonMove(QSPoint pt);
	void ButtonUp(QSPoint pt, unsigned int curTime, bool ctrl);
	...
	bool PointIsHotspot(QSPoint pt);
	void SetHotSpotRange(QSPoint *pt);

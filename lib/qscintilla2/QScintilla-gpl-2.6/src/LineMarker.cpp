...
void LineMarker::SetRGBAImage(QSPoint sizeRGBAImage, const unsigned char *pixelsRGBAImage) {
	delete image;
...
void LineMarker::Draw(Surface *surface, PRectangle &rcWhole, Font &fontForCharacter, typeOfFold tFold) {
	...
	if (markType == SC_MARK_ROUNDRECT) {
		...
		QSPoint pts[] = {
    		QSPoint(centreX - dimOn4, centreY - dimOn2),
    		QSPoint(centreX - dimOn4, centreY + dimOn2),
    		QSPoint(centreX + dimOn2 - dimOn4, centreY),
		};
		...
		QSPoint pts[] = {
    		QSPoint(centreX - dimOn2, centreY - dimOn4),
    		QSPoint(centreX + dimOn2, centreY - dimOn4),
    		QSPoint(centreX, centreY + dimOn2 - dimOn4),
		};
		surface->Polygon(pts, sizeof(pts) / sizeof(pts[0]),
                 		fore.allocated, back.allocated);
		...
		QSPoint pts[] = {
    		QSPoint(centreX - armSize, centreY - 1),
    		QSPoint(centreX - 1, centreY - 1),
    		QSPoint(centreX - 1, centreY - armSize),
    		QSPoint(centreX + 1, centreY - armSize),
    		QSPoint(centreX + 1, centreY - 1),
    		QSPoint(centreX + armSize, centreY -1),
    		QSPoint(centreX + armSize, centreY +1),
		...
		QSPoint pts[] = {
			QSPoint(centreX, centreY + dimOn2),
			QSPoint(centreX + dimOn2, centreY),
			QSPoint(centreX, centreY - dimOn2),
			QSPoint(centreX, centreY - dimOn4),
			QSPoint(centreX - dimOn4, centreY - dimOn4),
			QSPoint(centreX - dimOn4, centreY + dimOn4),
			QSPoint(centreX, centreY + dimOn4),
			QSPoint(centreX, centreY + dimOn2),
		};

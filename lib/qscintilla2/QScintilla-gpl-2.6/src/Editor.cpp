...
QSPoint Editor::LocationFromPosition(SelectionPosition pos) {
	QSPoint pt;
	RefreshStyleData();
...
QSPoint Editor::LocationFromPosition(int pos) {
	return LocationFromPosition(SelectionPosition(pos));
}

int Editor::XFromPosition(int pos) {
	QSPoint pt = LocationFromPosition(pos);
	return pt.x - vs.fixedColumnWidth + xOffset;
}

int Editor::XFromPosition(SelectionPosition sp) {
	QSPoint pt = LocationFromPosition(sp);
	return pt.x - vs.fixedColumnWidth + xOffset;
}

int Editor::LineFromLocation(QSPoint pt) {
...
SelectionPosition Editor::SPositionFromLocation(QSPoint pt, bool canReturnInvalid, bool charPosition, bool virtualSpace) {
	RefreshStyleData();
	if (canReturnInvalid) {
...
int Editor::PositionFromLocation(QSPoint pt, bool canReturnInvalid, bool charPosition) {
...
void Editor::MoveSelectedLines(int lineDelta) {
	...
	QSPoint currentLocation = LocationFromPosition(CurrentPosition());
...
void Editor::MoveCaretInsideView(bool ensureVisible) {
	...
	if (pt.y < rcClient.top) {
		MovePositionTo(SPositionFromLocation(
		            QSPoint(lastXChosen - xOffset, rcClient.top)),
					Selection::noSel, ensureVisible);
	} else if ((pt.y + vs.lineHeight - 1) > rcClient.bottom) {
		int yOfLastLineFullyDisplayed = rcClient.top + (LinesOnScreen() - 1) * vs.lineHeight;
		MovePositionTo(SPositionFromLocation(
		            QSPoint(lastXChosen - xOffset, rcClient.top + yOfLastLineFullyDisplayed)),
		        Selection::noSel, ensureVisible);
	}
}
...
Editor::XYScrollPosition Editor::XYScrollToMakeVisible(const bool useMargin, const bool vert, const bool horiz) {
	...
	const QSPoint pt = LocationFromPosition(posCaret);
	const QSPoint ptBottomCaret(pt.x, pt.y + vs.lineHeight - 1);
...
void Editor::PaintSelMargin(Surface *surfWindow, PRectangle &rc) {
	...
	if (bufferedDraw) {
		surfWindow->Copy(rcMargin, QSPoint(), *pixmapSelMargin);
	}
}
...
void Editor::DrawIndentGuide(Surface *surface, int lineVisible, int lineHeight, int start, PRectangle rcSegment, bool highlight) {
	QSPoint from(0, ((lineVisible & 1) && (lineHeight & 1)) ? 1 : 0);
	...
	if (rcArea.right > vs.fixedColumnWidth) {
		...
		while (visibleLine < cs.LinesDisplayed() && yposScreen < rcArea.bottom) {
			...
			if (ll) {
				...
				if (bufferedDraw) {
					QSPoint from(vs.fixedColumnWidth, 0);
...
void Editor::NotifyDoubleClick(QSPoint pt, bool shift, bool ctrl, bool alt) {
	SCNotification scn = {0};
...
bool Editor::NotifyMarginClick(QSPoint pt, bool shift, bool ctrl, bool alt) {
	int marginClicked = -1;
	int x = 0;
	for (int margin = 0; margin < ViewStyle::margins; margin++) {
...
void Editor::NotifyDwelling(QSPoint pt, bool state) {
	SCNotification scn = {0};
...
void Editor::PageMove(int direction, Selection::selTypes selt, bool stuttered) {
	...
	int bottomStutterLine =
	    pdoc->LineFromPosition(PositionFromLocation(
	                QSPoint(lastXChosen - xOffset, direction * vs.lineHeight * LinesToScroll())))
	    - caretYSlop - 1;

	if (stuttered && (direction < 0 && currentLine > topStutterLine)) {
		topLineNew = topLine;
		newPos = SPositionFromLocation(QSPoint(lastXChosen - xOffset, vs.lineHeight * caretYSlop),
			false, false, UserVirtualSpace());

	} else if (stuttered && (direction > 0 && currentLine < bottomStutterLine)) {
		topLineNew = topLine;
		newPos = SPositionFromLocation(QSPoint(lastXChosen - xOffset, vs.lineHeight * (LinesToScroll() - caretYSlop)),
			false, false, UserVirtualSpace());

	} else {
		QSPoint pt = LocationFromPosition(sel.MainCaret());

		topLineNew = Platform::Clamp(
		            topLine + direction * LinesToScroll(), 0, MaxScrollPos());
		newPos = SPositionFromLocation(
			QSPoint(lastXChosen - xOffset, pt.y + direction * (vs.lineHeight * LinesToScroll())),
			false, false, UserVirtualSpace());
	}
...
void Editor::CursorUpOrDown(int direction, Selection::selTypes selt) {
	...
	QSPoint pt = LocationFromPosition(caretToUse);
	int lineDoc = pdoc->LineFromPosition(caretToUse.Position());
	QSPoint ptStartLine = LocationFromPosition(pdoc->LineStart(lineDoc));
	...
	SelectionPosition posNew = SPositionFromLocation(
	            QSPoint(lastXChosen - xOffset, pt.y + direction * vs.lineHeight), false, false, UserVirtualSpace());
	if ((direction > 0) && (subLine >= (cs.GetHeight(lineDoc) - 1 - commentLines))) {
		posNew = SPositionFromLocation(
	           QSPoint(lastXChosen - xOffset, pt.y + (commentLines + 1) * vs.lineHeight), false, false, UserVirtualSpace());
	}
	if (direction < 0) {
		...
		QSPoint ptNew = LocationFromPosition(posNew.Position());
...
static bool Close(QSPoint pt1, QSPoint pt2) {
...
void Editor::DisplayCursor(QSWindow::Cursor c) {
	if (cursorMode == SC_CURSORNORMAL)
		wMain.SetCursor(c);
	else
		wMain.SetCursor(static_cast<QSWindow::Cursor>(cursorMode));
}

bool Editor::DragThreshold(QSPoint ptStart, QSPoint ptNow) {
...
void Editor::StartDrag() {
	...
	//DisplayCursor(QSWindow::cursorArrow);
}
...
bool Editor::PointInSelection(QSPoint pt) {
	SelectionPosition pos = SPositionFromLocation(pt, false, true);
	QSPoint ptPos = LocationFromPosition(pos);
	for (size_t r=0; r<sel.Count(); r++) {
...
bool Editor::PointInSelMargin(QSPoint pt) {
...
QSWindow::Cursor Editor::GetMarginCursor(QSPoint pt) {
	int x = 0;
	for (int margin = 0; margin < ViewStyle::margins; margin++) {
		if ((pt.x >= x) && (pt.x < x + vs.ms[margin].width))
			return static_cast<QSWindow::Cursor>(vs.ms[margin].cursor);
		x += vs.ms[margin].width;
	}
	return QSWindow::cursorReverseArrow;
}
...
void Editor::ButtonDown(QSPoint pt, unsigned int curTime, bool shift, bool ctrl, bool alt) {
...
bool Editor::PointIsHotspot(QSPoint pt) {
...
void Editor::SetHotSpotRange(QSPoint *pt) {
	if (pt) {
...
void Editor::ButtonMove(QSPoint pt) {
	...
	if (HaveMouseCapture()) {
		...
		if (hotSpotClickPos != INVALID_POSITION && PositionFromLocation(pt,true,false) != hotSpotClickPos) {
			if (inDragDrop == ddNone) {
				DisplayCursor(QSWindow::cursorText);
			}
			hotSpotClickPos = INVALID_POSITION;
		}
		...
		if (PointInSelection(pt) && !SelectionEmpty()) {
			...
			DisplayCursor(QSWindow::cursorHand);
			SetHotSpotRange(&pt);
		} else {
			DisplayCursor(QSWindow::cursorText);
			SetHotSpotRange(NULL);
...
void Editor::ButtonUp(QSPoint pt, unsigned int curTime, bool ctrl) {
...
sptr_t Editor::WndProc(unsigned int iMessage, uptr_t wParam, sptr_t lParam) {
	...
	case SCI_POINTXFROMPOSITION:
		if (lParam < 0) {
			...
			QSPoint pt = LocationFromPosition(lParam);
			return pt.x;
		}
	...
	case SCI_POSITIONFROMPOINT:
		return PositionFromLocation(QSPoint(wParam, lParam), false, false);

	case SCI_POSITIONFROMPOINTCLOSE:
		return PositionFromLocation(QSPoint(wParam, lParam), true, false);

	case SCI_CHARPOSITIONFROMPOINT:
		return PositionFromLocation(QSPoint(wParam, lParam), false, true);

	case SCI_CHARPOSITIONFROMPOINTCLOSE:
		return PositionFromLocation(QSPoint(wParam, lParam), true, true);

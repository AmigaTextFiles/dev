...
public:
	...
	explicit QSPoint(int x_=0, int y_=0) : x(x_), y(y_) {
	}

	// Other automatically defined methods (assignment, copy constructor, destructor) are fine

	static QSPoint FromLong(long lpoint);
};
...
class QSWindow;	// Forward declaration for Palette
...
public:
	...
	virtual void Polygon(QSPoint *pts, int npts, ColourAllocated fore, ColourAllocated back)=0;
	...
	virtual void Copy(PRectangle rc, QSPoint from, Surface &surfaceSource)=0;
...
#endif
public:
	QSWindow() : wid(0), cursorLast(cursorInvalid) {
#if PLAT_MACOSX
	  windowRef = 0;
	  control = 0;
#endif
	}
	QSWindow(const QSWindow &source) : wid(source.wid), cursorLast(cursorInvalid) {
#if PLAT_MACOSX
	  windowRef = 0;
	  control = 0;
#endif
	...
	virtual ~QSWindow();
	QSWindow &operator=(WindowID wid_) {
		wid = wid_;
		return *this;
	}
	...
	void SetPositionRelative(PRectangle rc, QSWindow relativeTo);
	...
	PRectangle GetMonitorRect(QSPoint pt);
#if PLAT_MACOSX
...
public:
	...
	virtual void Create(QSWindow &parent, int ctrlID, QSPoint location, int lineHeight_, bool unicodeMode_)=0;
...
public:
	...
	void CreatePopUp();
	void Destroy();
	void Show(QSPoint pt, QSWindow &w);
};

...
QSPoint QSPoint::FromLong(long lpoint)
{
    return QSPoint(Platform::LowShortFromLong(lpoint),
...
void Palette::Allocate(QSWindow &)
{
}
...
public:
    ...
    void Polygon(QSPoint *pts, int npts, ColourAllocated fore,
    ...
    void Copy(PRectangle rc, QSPoint from, Surface &surfaceSource);
...
void SurfaceImpl::Polygon(QSPoint *pts, int npts, ColourAllocated fore,
        ColourAllocated back)
{
    Q_ASSERT(painter);
...
void SurfaceImpl::Copy(PRectangle rc, QSPoint from, Surface &surfaceSource)
{
    Q_ASSERT(painter);
...
bool QSWindow::HasFocus()
{
    return PWindow(wid)->hasFocus();
}

PRectangle QSWindow::GetPosition()
{
    QWidget *w = PWindow(wid);
...
void QSWindow::SetPosition(PRectangle rc)
{
    PWindow(wid)->setGeometry(rc.left, rc.top, rc.right - rc.left,
            rc.bottom - rc.top);
}

void QSWindow::SetPositionRelative(PRectangle rc, QSWindow relativeTo)
{
...
PRectangle QSWindow::GetClientPosition()
{
    return GetPosition();
}

void QSWindow::Show(bool show)
{
    QWidget *w = PWindow(wid);
...
void QSWindow::InvalidateRectangle(PRectangle rc)
{
    QWidget *w = PWindow(wid);
...
void QSWindow::SetFont(Font &font)
{
    PWindow(wid)->setFont(*PFont(font.GetID()));
}

void QSWindow::SetCursor(Cursor curs)
{
    Qt::CursorShape qc;
...
void QSWindow::SetTitle(const char *s)
{
    PWindow(wid)->setWindowTitle(s);
}


PRectangle QSWindow::GetMonitorRect(QSPoint pt)
{
...
void QSMenu::Show(QSPoint pt, QSWindow &)
{

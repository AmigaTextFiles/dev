...
{
    sci->ContextMenu(QSPoint(e->globalX(), e->globalY()));
}
...
void QsciScintillaBase::mouseDoubleClickEvent(QMouseEvent *e)
{
    ...
    sci->ButtonDown(QSPoint(e->x(), e->y()), clickTime, shift, ctrl, alt);
...
{
    sci->ButtonMove(QSPoint(e->x(), e->y()));
}
...
void QsciScintillaBase::mouseReleaseEvent(QMouseEvent *e)
{
    if (sci->HaveMouseCapture() && e->button() == Qt::LeftButton)
    {
        ...
        sci->ButtonUp(QSPoint(e->x(), e->y()), 0, ctrl);
    }
}
...
{
    sci->SetDragPosition(
            sci->SPositionFromLocation(QSPoint(e->pos().x(), e->pos().y()),

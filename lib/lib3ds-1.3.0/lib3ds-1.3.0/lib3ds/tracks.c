...
double ersatz_fmod(double numer,double denom);
...
lib3ds_lin1_track_eval(Lib3dsLin1Track *track, Lib3dsFloat *p, Lib3dsFloat t)
{
  ...
  if (!k->next) {
    if (track->flags&LIB3DS_REPEAT) {
      nt=(Lib3dsFloat)ersatz_fmod(t - track->keyL->tcb.frame, k->tcb.frame - track->keyL->tcb.frame) + track->keyL->tcb.frame;
      for (k=track->keyL; k->next!=0; k=k->next) {
...
double ersatz_fmod(double numer,double denom)
{
double x = trunc(numer/denom);
return(numer - x * denom);
}

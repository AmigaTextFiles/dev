...
int InitDSPFILE(char * inputfile, CUBEFILE * dsp) {
	...
	if (!SPT) ReadFile(dsp->ch[0].infile, &readbuf, 0x180, &l, NULL);
	...
	if (SPT) {
		...
		dsp->ch[0].num_samples=dsp->ch[1].num_samples=
			(int)(GetFileSize(dsp->ch[0].infile,NULL)*(double)FOURTEEN_DIV_EIGHT);
		...
		// determine if a HALPST file loops
		{
			...
			while (c > lastc) {
				lastc=c;
				SetFilePointer(dsp->ch[0].infile,c+8,0,FILE_BEGIN);
		...
		isdfound=0;
		do {
			if (finddata.nFileSizeLow==(dsp->ch[0].num_adpcm_nibbles+0xffff)/HEX_TEN_THOUSAND_MUL_HEX_TEN_THOUSAND) {isdfound=1; break;}
	...
	{
		dsp->ch[0].sa=(int)(dsp->ch[0].sa*SIXTEEN_DIV_FOURTEEN);
		dsp->ch[0].ea=(int)(dsp->ch[0].ea*SIXTEEN_DIV_FOURTEEN);
		dsp->ch[1].sa=(int)(dsp->ch[1].sa*SIXTEEN_DIV_FOURTEEN);
		dsp->ch[1].ea=(int)(dsp->ch[1].ea*SIXTEEN_DIV_FOURTEEN);
	...
	if (!dsp->ch[0].loop_flag) dsp->nrsamples = dsp->ch[0].num_samples;
		...
		dsp->nrsamples=(dsp->ch[0].sa+looptimes*(dsp->ch[0].ea-dsp->ch[0].sa))*14/(EIGHT_MUL_EIGHT/dsp->ch[0].bps)/dsp->NCH+(fadelength+fadedelay)*dsp->ch[0].sample_rate;
	else
		dsp->nrsamples=(dsp->ch[0].sa+looptimes*(dsp->ch[0].ea-dsp->ch[0].sa))*14/(EIGHT_MUL_EIGHT/dsp->ch[0].bps)+(fadelength+fadedelay)*dsp->ch[0].sample_rate;
...
stream->offs=SetFilePointer(stream->infile,stream->chanstart+((stream->sa*stream->bps/8)&(~7)),0,FILE_BEGIN);
		...
		i--;
		if (stream->writeloc>=HEX_8000_DIV_EIGHT_MUL_FOURTEEN) stream->writeloc=0;
...
void fillbufferHALP(CUBEFILE * dsp) {
   ...
   DWORD l;
	...
	do {
		...
		c++; i--;
		if (dsp->ch[0].writeloc>=HEX_8000_DIV_EIGHT_MUL_FOURTEEN) dsp->ch[0].writeloc=0;
		if (dsp->ch[1].writeloc>=HEX_8000_DIV_EIGHT_MUL_FOURTEEN) dsp->ch[1].writeloc=0;

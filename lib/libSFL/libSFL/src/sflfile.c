    ...
    const char *filename)
{
    ...
    if (file_is_directory (filename))
        return (FALSE);  //  return ((file_mode (clean_path (filename)) & S_IREAD) != 0);
    else
    if (strlast (filename) == '/')
        return (FALSE);
    else
        return (FALSE);   // return ((file_mode (filename) & S_IREAD) != 0);
}
    ...
    const char *filename)
{
    ...
    if (file_is_directory (filename))
        return (FALSE);  // return ((file_mode (clean_path (filename)) & S_IWRITE) != 0);
    else
    if (strlast (filename) == '/')
        return (FALSE);
    else

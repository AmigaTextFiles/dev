...
int PDC_scr_open(int argc, char **argv)
{
    ...
    if (pdc_own_screen)
    {
        ...
        long vflgs = SDL_SWSURFACE|SDL_ANYFORMAT;

	env = getenv("PDC_SIZE");
	char sz = atoi(env);
	if (sz==1) {
	   vflgs = vflgs | SDL_RESIZABLE;
	}

	env = getenv("PDC_FSCRN");
        char fs = atoi(env);
	if (fs==1) {
	   vflgs = SDL_FULLSCREEN | SDL_HWSURFACE;
	} 

        pdc_screen = SDL_SetVideoMode(pdc_swidth, pdc_sheight, 0, vflgs);
    }
    else
    {
        if (!pdc_sheight)
    ...
    if (pdc_own_screen)
        PDC_set_title(argc ? argv[0] : "PDCurses-ns");

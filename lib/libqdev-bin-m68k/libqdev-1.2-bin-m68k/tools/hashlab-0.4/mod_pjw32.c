...
#define MOD_XXX_STRING     "mod_pjw32.hl"
...
#define ___PJW32BITS  32
#define ___PJW32THQU  ((___PJW32BITS * 3) / 4)
#define ___PJW32ONEE  (___PJW32BITS / 8)
#define ___PJW32HIGH  (0xFFFFFFFF << (___PJW32BITS - ___PJW32ONEE))
...
/*
 * P. J. Weinberger Hash Function - Same results as ELF32!
*/
LONG _HASHLAB_MODULERFUNC(void *ptr, VUQ128 *vu, UBYTE *str)
{
  ...
  while(*strreg)
  {
    hash = (hash << ___PJW32ONEE) + (*strreg++);

    if ((test = hash & ___PJW32HIGH))
    {
      hash = ((hash ^ (test >> ___PJW32THQU)) & (~___PJW32HIGH));
    }
  }

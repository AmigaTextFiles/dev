...
static const char rcsid[] =
	"$Id: r250.c 1.2 1995/10/23 21:54:32 JöG Exp JöG $";
...
*
*	HISTORY...
*		Oct 95: Made it a part of the Nucifraga support library,
*			which may mean changing the interface a bit, plus adding
*			RCS info.
*
*		Aug 95: Adjusted the module for use with my Amiga version
*			of a certain cookie program. It seems R250 asserts
*			16-bit ints and 32-bit long ints. Changed int to short.
*			Also commented out floating-point version, and added
*			prototype for the seeding function, and made it more
*			ANSI by adding 'void' here and there.
*
*			Only released as part of this cookie program.
*
*			Jörgen Grahn - Wetterlinsgatan 13e - 521 34 Falköping -
*			Sweden
*
...
/**** Static variables ****/
static short r250_index = 0;
static unsigned short r250_buffer[250] = {
...
static void mysrand(unsigned short newseed);

/**** Function: r250_init  
	Description: initializes r250 random number generator. ****/
void r250_init(short seed)
{
/*---------------------------------------------------------------------------*/
    short          j, k;
    unsigned short mask;
...
unsigned short r250(void)
{
/*---------------------------------------------------------------------------*/
    register short    j;
    register unsigned short new_rand;
/*---------------------------------------------------------------------------*/
    if (r250_index >= 147)
...
unsigned short r250n(unsigned short n)
{
/*---------------------------------------------------------------------------*/
    register short    j;
    register unsigned short new_rand, limit;
/*---------------------------------------------------------------------------*/
	limit = (65535U/n)*n;
	do 
        {new_rand = r250();

...
{
  return fileno( const_cast<FILE*>(__f) );
}

inline char* _FILE_I_begin(const FILE *__f)
{
  return reinterpret_cast<char*>(__f->buffer);
}

inline char* _FILE_I_next(const FILE *__f)
{
  return reinterpret_cast<char*>(__f->buffer) + __f->position;
}

inline char* _FILE_I_end(const FILE *__f)
{
  return reinterpret_cast<char*>(__f->buffer) + __f->size;
}

inline ptrdiff_t _FILE_I_avail(const FILE *__f)
{
  return __f->num_read_bytes;
}
...
{
  __f->position += __n;
  __f->num_read_bytes -= __n;
}

inline void _FILE_I_set(FILE *__f, char* __begin, char* __next, char* __end)
{
  __f->buffer = reinterpret_cast<unsigned char*>(__begin);
  __f->size = __end - __begin;
  __f->position = __next - __begin;
  __f->num_read_bytes = __end - __next;
}
...
inline void _FILE_O_set(FILE *__f, char* __begin, char* __next, char* __end)
{
  ...
  __f->size = __end - __begin;
  __f->position = __next - __begin;
  __f->num_write_bytes = __end - __next;
}

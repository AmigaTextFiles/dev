...
static void open_audio(){
  ...
  snprintf(namebuf, sizeof(namebuf), AUDIO_DEVICE ":C/%d/B/16/F/%ld", vi.channels, vi.rate);

  audiofd=open(namebuf, O_WRONLY);
  if(audiofd<0){
    fprintf(stderr,"Could not open audio device " AUDIO_DEVICE ".\n");
    exit(1);
  }
  
  audiofd_fragsize=(vi.rate/10)*vi.channels*2;
  audiofd_totalsize=10*audiofd_fragsize;
...
void audio_write_nonblocking(void){
  ...
  if(audiobuf_ready){
#if defined(__AMIGAOS4__)
	write(audiofd,audiobuf,audiobuf_fill);
	audiobuf_fill=0;
    audiobuf_ready=0;
#else
    audio_buf_info info;

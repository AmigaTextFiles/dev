...
void *memcpy(void *dstpp, const void *srcpp, size_t len) {
    ...
    if (len < sizeof(uint32_t)) {
        ...
        // Now we know that we have at least 4 bytes, 
        // copy until dst is word aligned
        MYCOPY_UNTIL_DEST_IS_WORD_ALIGNED(dst, src, len);
        ...
        MYCOPY_REST_WORDS(dstl, srcl, len, srcoffset4);
        ...
        MYCOPY_UNTIL_DEST_IS_ALTIVEC_ALIGNED(dstl, srcl, len, srcoffset4);
        ...
        if (len >= 4*ALTIVECWORD_SIZE) {
            if (((uint32_t)(src) & 15) == 0) {
                int blocks = (len >> 6);
                MYCOPY_LOOP_QUADWORD_ALTIVEC_ALIGNED(dstl, src, blocks);
                srcl = (uint32_t *)(src -srcoffset4);
            } else {
                int blocks = (len >> 6);
                MYCOPY_LOOP_QUADWORD_ALTIVEC_UNALIGNED(dstl, src, blocks);
                srcl = (uint32_t *)(src -srcoffset4);
            }
        }
        if (len >= 2*ALTIVECWORD_SIZE) {

    ...
    if (len < sizeof(uint32_t)) {
        ...
        if (len >= ALTIVECWORD_SIZE) {
            ...
            if (((uint32_t)(src) % 16) == 0) {
                ...
                MYMEMCCPY_SINGLE_ALTIVEC_WORD_ALIGNED(dst, dstl, src, srcl, srcoffset4, charmask, vc, c);
                dstl += 4;
                ...
                MYMEMCCPY_SINGLE_ALTIVEC_WORD_UNALIGNED(dst, dstl, src, srcl, srcoffset4, charmask, vc, c);
                dstl += 4;
                src += ALTIVECWORD_SIZE;
                len -= ALTIVECWORD_SIZE;
                srcl = (uint32_t *)(src -srcoffset4);

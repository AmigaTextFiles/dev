...
#define MYSTRLEN_UNTIL_WORD_ALIGNED(str, ptr)               \
    switch ((uint32_t)(ptr) % (sizeof(uint32_t))) {         \
        case 3:                                             \
...
#define MYSTRLEN_SINGLE_WORD(str, ptr32)                     \
    if (( *ptr32 - lomagic) & himagic) {                     \
        ...
        if (cptr[0] == '\0' ) return ptrdiff_t(ptr,str);     \
        if (cptr[1] == '\0' ) return ptrdiff_t(ptr,str) +1;  \
        if (cptr[2] == '\0' ) return ptrdiff_t(ptr,str) +2;  \
        if (cptr[3] == '\0' ) return ptrdiff_t(ptr,str) +3;  \
    }                                                        \
    ptr32++;

#define MYSTRLEN_LOOP_UNTIL_ALTIVEC_ALIGNED(str, ptr32)     \
    switch ((uint32_t)(ptr32) % ALTIVECWORD_SIZE) {         \
        case 4:                                             \
...
#define MYSTRNLEN_UNTIL_WORD_ALIGNED(ptr, len, maxlen)          \
    while (((uint32_t)(ptr) & (sizeof(uint32_t)-1)) && len--) { \
        if (*++ptr == '\0') return maxlen-len;                  \
    }

#define MYSTRNLEN_SINGLE_WORD(ptr32, len, maxlen)        \
    if (( *ptr32 - lomagic) & himagic) {            \
        uint8_t *cptr = (uint8_t *) ptr32;          \
        if (cptr[0] == '\0' ) return maxlen-len;    \
        if (cptr[1] == '\0' ) return maxlen-len +1; \
        if (cptr[2] == '\0' ) return maxlen-len +2; \
        if (cptr[3] == '\0' ) return maxlen-len +3; \
    }                                               \
    ptr32++;
...
#define MYSTRNLEN_REST_BYTES(ptr, len, maxlen)          \
    switch (len) {                                      \
        case 3:                                         \
            if (*ptr++ == '\0') return maxlen-len;      \
            if (*ptr++ == '\0') return maxlen-len+1;    \
            if (*ptr == '\0') return maxlen-len+2;      \
            break;                                      \
        case 2:                                         \
            if (*ptr++ == '\0') return maxlen-len;      \
            if (*ptr == '\0') return maxlen-len+1;      \

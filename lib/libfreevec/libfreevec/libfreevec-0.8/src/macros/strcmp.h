...
#define MYSTRCMP_SINGLE_ALTIVEC_WORD_ALIGNED(src1, src1l, src2, src2l, src2offset)  \
    {                                                                               \
        vector uint8_t  vsrc1 = (vector uint8_t) vec_ld(0, (uint8_t *)src1l),       \
                        ...
                        v0 = vec_splat_u8(0);                                       \
        int res = vec_any_eq(vsrc1, v0) | vec_any_ne(vsrc1, vsrc2);                 \
        if (res) {                                                                  \
...
#define MYSTRCMP_SINGLE_ALTIVEC_WORD_UNALIGNED(src1, src1l, src2, src2l, src2offset)    \
    {                                                                                   \
        ...
        if (vec_any_eq(vsrc1, v0) || vec_any_ne(vsrc1, vsrc2)) {                        \
            src2l = (uint32_t *)(src2 -src2offset);                                     \
            MYSTRCMP_QUADWORD(src1, src1l, src2, src2l, src2offset);                    \
        }                                                                               \
    }
...
#define MYSTRNCMP_LOOP_SINGLE_ALTIVEC_WORD_ALIGNED(src1, src1l, src2, src2l, src2offset)    \
    ...
    while (len >= ALTIVECWORD_SIZE) {                                                       \
        MYSTRNCMP_SINGLE_ALTIVEC_WORD_ALIGNED(src1, src1l, src2, src2l, src2offset)         \
...
#define MYSTRNCMP_LOOP_SINGLE_ALTIVEC_WORD_UNALIGNED(src1, src1l, src2, src2l, src2offset)  \
    ...
    vec_dst(src1, DST_CTRL(2,2,16), DST_CHAN_DEST);                                         \
    while (len >= ALTIVECWORD_SIZE) {                                                       \
        MYSTRNCMP_SINGLE_ALTIVEC_WORD_UNALIGNED(src1, src1l, src2, src2l, src2offset)       \
        src1l += 4; src2 += ALTIVECWORD_SIZE; len -= ALTIVECWORD_SIZE;                      \
    }
...
#define MYSTRNCMP2_SINGLE_WORD(src1, src1l, src2, src2l)                                            \
    if ((( *src1l - lomagic) & himagic) || *src1l != *src2l) {                                      \
        src2 = (uint8_t *) src2l;                                                                   \
        src1 = (uint8_t *) src1l;                                                                   \
        if (src1[0] == '\0' || (src1[0] != src2[0])) { VEC_DSS(); return DIFF(src1[0], src2[0]); }  \
        if (src1[1] == '\0' || (src1[1] != src2[1])) { VEC_DSS(); return DIFF(src1[1], src2[1]); }  \
...
#define MYSTRNCMP2_QUADWORD(src1, src1l, src2, src2l)       \
    int i;                                                  \
    for (i=0; i < 4; i++) {                                 \
        MYSTRNCMP2_SINGLE_WORD(src1, src1l, src2, src2l);   \
    }
    
#define MYSTRNCMP2_UNTIL_SRC1_IS_ALTIVEC_ALIGNED(src1, src1l, src2, src2l, len)     \
    while (((uint32_t)(src1l) % ALTIVECWORD_SIZE) && len >= sizeof(uint32_t)) {     \
        MYSTRNCMP2_SINGLE_WORD(src1, src1l, src2, src2l);                           \
        len -= sizeof(uint32_t);                                                    \
    }

#define MYSTRNCMP2_SINGLE_ALTIVEC_WORD_ALIGNED(src1, src1l, src2, src2l)            \
    {                                                                               \
        vector uint8_t  vsrc1 = (vector uint8_t) vec_ld(0, (uint8_t *)src1l),       \
                        vsrc2 = (vector uint8_t) vec_ld(0, (uint8_t *)src2),        \
                        v0 = vec_splat_u8(0);                                       \
        int res = vec_any_eq(vsrc1, v0) | vec_any_ne(vsrc1, vsrc2);                 \
        if (res) {                                                                  \
            MYSTRNCMP2_QUADWORD(src1, src1l, src2, src2l);                          \
        }                                                                           \
    }

#define MYSTRNCMP2_SINGLE_ALTIVEC_WORD_UNALIGNED(src1, src1l, src2, src2l)              \
    {                                                                                   \
        vector uint8_t  vsrc1 = (vector uint8_t) vec_ld(0, (uint8_t *)src1l),           \
                        vsrc2, MSQ, LSQ, vmask, v0 = vec_splat_u8(0);                   \
        vmask = vec_lvsl(0, src2);                                                      \
...
#define MYSTRNCMP2_LOOP_SINGLE_ALTIVEC_WORD_ALIGNED(src1, src1l, src2, src2l)               \
    vec_dst(src2, DST_CTRL(2,2,16), DST_CHAN_SRC);                                          \
    vec_dst(src1, DST_CTRL(2,2,16), DST_CHAN_DEST);                                         \
    while (len >= ALTIVECWORD_SIZE) {                                                       \
...
#define MYSTRNCMP2_LOOP_SINGLE_ALTIVEC_WORD_UNALIGNED(src1, src1l, src2, src2l)             \
    vec_dst(src2, DST_CTRL(2,2,16), DST_CHAN_SRC);                                          \
    vec_dst(src1, DST_CTRL(2,2,16), DST_CHAN_DEST);                                         \
    while (len >= ALTIVECWORD_SIZE) {                                                       \
        MYSTRNCMP2_SINGLE_ALTIVEC_WORD_UNALIGNED(src1, src1l, src2, src2l)                  \
        src1l += 4; src2l += 4; len -= ALTIVECWORD_SIZE;                                    \
    }

#define MYSTRNCMP2_LOOP_QUADWORD_ALTIVEC_ALIGNED(src1, src1l, src2, src2l)                  \
    vec_dst(src2, DST_CTRL(2,2,16), DST_CHAN_SRC);                                          \
    vec_dst(src1l, DST_CTRL(2,2,16), DST_CHAN_DEST);                                        \
    {                                                                                       \
        vector uint8_t  vsrc1a, vsrc1b, vsrc1c, vsrc1d,                                     \
        ...
        while (len >= 4*ALTIVECWORD_SIZE) {                                                 \
            ...
            vsrc1c = (vector uint8_t) vec_ld(32, (uint8_t *)src1l);                         \
            vsrc2c = (vector uint8_t) vec_ld(32, (uint8_t *)src2l);                         \
            vsrc1d = (vector uint8_t) vec_ld(48, (uint8_t *)src1l);                         \
            vsrc2d = (vector uint8_t) vec_ld(48, (uint8_t *)src2l);                         \
            res1 = vec_any_eq(vsrc1a, v0) |vec_any_ne(vsrc1a, vsrc2a);                      \
            res2 = vec_any_eq(vsrc1b, v0) |vec_any_ne(vsrc1b, vsrc2b);                      \
            res3 = vec_any_eq(vsrc1c, v0) |vec_any_ne(vsrc1c, vsrc2c);                      \
            res4 = vec_any_eq(vsrc1d, v0) |vec_any_ne(vsrc1d, vsrc2c);                      \
            if (res1) {                                                                     \
                MYSTRNCMP2_QUADWORD(src1, src1l, src2, src2l);                              \
            }                                                                               \
            if (res2) {                                                                     \
            ...
            src1l += 8; src2 += 8;                                                          \
            len -= 4* ALTIVECWORD_SIZE;                                                     \
            vec_dst(src2l, DST_CTRL(2,2,16), DST_CHAN_SRC);                                 \
            vec_dst(src1l, DST_CTRL(2,2,16), DST_CHAN_DEST);                                \
        }                                                                                   \
    }

#define MYSTRNCMP2_LOOP_QUADWORD_ALTIVEC_UNALIGNED(src1, src1l, src2, src2l)                \
    vec_dst(src2, DST_CTRL(2,2,16), DST_CHAN_SRC);                                          \
    vec_dst(src1l, DST_CTRL(2,2,16), DST_CHAN_DEST);                                        \
    {                                                                                       \
        vector uint8_t  vsrc1a, vsrc1b, vsrc1c, vsrc1d,                                     \
        ...
        while (len >= 4*ALTIVECWORD_SIZE) {                                                 \
            ...
            vmask = vec_lvsl(0, src2);                                                      \
            vsrc2a = (vector uint8_t) vec_ld(0, (uint8_t *)src2l);                          \
            vsrc2b = (vector uint8_t) vec_ld(15, (uint8_t *)src2l);                         \
            vsrc2c = (vector uint8_t) vec_ld(31, (uint8_t *)src2l);                         \
            vsrc2d = (vector uint8_t) vec_ld(47, (uint8_t *)src2l);                         \
            LSQ = (vector uint8_t) vec_ld(63, (uint8_t *)src2l);                            \
            vsrc2a = vec_perm(vsrc2a, vsrc2b, vmask);                                       \
            res1 = vec_any_eq(vsrc1a, v0) | vec_any_ne(vsrc1a, vsrc2a);                     \
            vsrc2b = vec_perm(vsrc2b, vsrc2c, vmask);                                       \
            res2 = vec_any_eq(vsrc1b, v0) | vec_any_ne(vsrc1b, vsrc2b);                     \
...
#define MYSTRNCMP2_REST_WORDS(src1, src1l, src2, src2l, len)    \
    while (len >= sizeof(uint32_t)) {                           \
        MYSTRNCMP2_SINGLE_WORD(src1, src1l, src2, src2l);       \
        len -= sizeof(uint32_t);                                \
    }

...
#if (USE_WINDOWS_API == 0) && (USE_AMIGAOS_API == 0)
...
#elif USE_AMIGAOS_API

#include <dos/dos.h>
#include <proto/dos.h>

static BPTR psf_open_handle (PSF_FILE * pfile) ;
static int psf_close_handle (BPTR handle) ;
static sf_count_t psf_get_filelen_handle (BPTR handle) ;
...
psf_fopen (SF_PRIVATE *psf)
{
	...
	if (psf->file.handle == ZERO)
		psf_log_syserr (psf, IDOS->IoErr ()) ;
...
{	int	error = 0 ;
	...
	{	case SFM_RDWR :
		...
		case SFM_READ :
				psf->file.handle = IDOS->Input () ;
				psf->file.do_not_close_descriptor = 1 ;
				break ;

		case SFM_WRITE :
				psf->file.handle = IDOS->Output () ;
				psf->file.do_not_close_descriptor = 1 ;
				break ;
...
/* USE_AMIGAOS_API */ void
psf_set_file (SF_PRIVATE *psf, int fd)
{	BPTR handle ;
	intptr_t osfhandle ;

	osfhandle = _get_osfhandle (fd) ;
	handle = (BPTR) osfhandle ;
...
/* USE_AMIGAOS_API */ int
psf_file_valid (SF_PRIVATE *psf)
{	return (psf->file.handle != ZERO) ? SF_TRUE : SF_FALSE ;
} /* psf_set_file */

/* USE_AMIGAOS_API */ sf_count_t
psf_get_filelen (SF_PRIVATE *psf)
{	sf_count_t	filelen ;
...
{	LONG success ;
	...
	success = IDOS->ChangeFileSize (psf->file.handle, len, OFFSET_BEGINNING) ;
...
{	LONG move_method ;
	...
	{	case SEEK_SET :
				offset += psf->fileoffset ;
				move_method = OFFSET_BEGINNING ;
				break ;

		case SEEK_END :
				move_method = OFFSET_END ;
				break ;

		case SEEK_CUR :
				move_method = OFFSET_CURRENT ;
				break ;
	...
	success = IDOS->ChangeFilePosition(psf->file.handle, offset, move_method);
...
{	sf_count_t total = 0 ;
	...
	{	/* Break the writes down to a sensible size. */
		...
		count = IDOS->Read (psf->file.handle, ((char*) ptr) + total, count) ; 
...
{	sf_count_t total = 0 ;
	...
	{	/* Break the writes down to a sensible size. */
		...
		count = IDOS->Write (psf->file.handle, ((const char *) ptr) + total, count) ;
...
{	sf_count_t pos ;
	...
	pos = IDOS->GetFilePosition (psf->file.handle);
...
{	struct ExamineData *exd ;
	...
	exd = IDOS->ExamineObjectTags (
		EX_FileHandleInput, psf->file.handle,
		TAG_END);
	if (exd == NULL) {
		psf_log_syserr (psf, IDOS->IoErr ()) ;
		/* Default to maximum safety. */
...
psf_init_files (SF_PRIVATE *psf)
{	psf->file.handle = ZERO ;
	psf->rsrc.handle = ZERO ;
	psf->file.hsaved = ZERO ;
} /* psf_init_files */
...
{	LONG access_mode;
	...
	handle = IDOS->Open (pfile->path.c, access_mode);

	return handle ;
} /* psf_open_handle */

/* USE_AMIGAOS_API */ static int
psf_close_handle (BPTR handle)
{	if (handle == ZERO)
		return 0 ;
...
/* USE_AMIGAOS_API */ static sf_count_t
psf_get_filelen_handle (BPTR handle)
{	return IDOS->GetFileSize (handle) ;
}

/* USE_AMIGAOS_API */ static void
psf_log_syserr (SF_PRIVATE *psf, int error)
{
	/* Only log an error if no error has been set yet. */
	if (psf->error == 0)
	{	psf->error = SFE_SYSTEM ;
		IDOS->Fault (error, "System error", psf->syserr, sizeof (psf->syserr)) ;
		} ;

...
// remove line below for faster (but worst) emulation
#define HIQUALITY
...
const int samplerRateOver = 0xA0DB5; // (1773447 / 176400) * 65536;
...
#define MAKESAMPLE\
	{\
		...
		{\
			ss += samplerRateOver;\
			DWORD pomd; int pomi;\
			pomd  = ((DWORD*)&switch_J3_Q_state[0])[0] & ((DWORD*)&switch_J3_Q_stateAND[0])[0];\
			...
			oldValI = oldValI + ((cutFreq2i*((pomi<<11) - oldValI))>>11);\
			delay++; if( (delay&3)==0 )\
			{\
				pomi = oldValI>>1;\
				pomi-=32767; if( pomi<-32767 ) pomi = -32767; else if( pomi>32767 ) pomi = 32767;\
				sndBuf[sndBufPtr&16383] = (WORD)pomi;\
	...
	{\
		...
		{\
			...
			{\
				...
				oldVal = output;\
				pomi = (int)output;\
				pomi *=2; pomi-=16384; if( pomi<-16384 ) pomi = -16384; else if( pomi>32767 ) pomi = 32767;\
			}\
			sndBuf[sndBufPtr&16383] = (WORD)pomi;\
			sndBufPtr = (sndBufPtr+sampleStep)&16383;\
		}\
...
funcPoint channelsDistorionTable0[8]=
	{ &channel0_0, &channel0_2, &channel0_4, &channel0_2, &channel0_8, &channel0_A, &channel0_C, &channel0_A };
funcPoint channelsDistorionTable1[8]=
	{ &channel1_0, &channel1_2, &channel1_4, &channel1_2, &channel1_8, &channel1_A, &channel1_C, &channel1_A };
funcPoint channelsDistorionTable2[8]=
	{ &channel2_0, &channel2_2, &channel2_4, &channel2_2, &channel2_8, &channel2_A, &channel2_C, &channel2_A };
funcPoint channelsDistorionTable3[8]=
	{ &channel3_0, &channel3_2, &channel3_4, &channel3_2, &channel3_8, &channel3_A, &channel3_C, &channel3_A };

#define CHANNELDISTORTION(chan)\
	switch( (audioControl_Latch[chan]&0xE0)>>5 )\
	{\
		case 0x01:\
		case 0x03:\
			signal_state_out[chan] = signal_state_out[chan] ^ poly5_b[ poly5Counter + pokeyClockCounter ];\
			break;\
		case 0x02:\
			if( poly5_b[ poly5Counter + pokeyClockCounter ]!=0 )\
				signal_state_out[chan] = poly4_b[ poly4Counter + pokeyClockCounter ];\
				break;\
			break;\
		case 0x04:\
			signal_state_out[chan] = poly17[ pokeyClockCounter & noiseAND ];\
			break;\
		case 0x05:\
		case 0x07:\
			signal_state_out[chan] = signal_state_out[chan] ^ 15;\
			break;\
		case 0x06:\
			signal_state_out[chan] = poly4_b[ pokeyClockCounter + poly4Counter ];\
			break;\
		case 0x00:\
		default:\
			if( poly5_b[ poly5Counter + pokeyClockCounter ]!=0 )\
...
int i,ch,pp,ss;
	...
	do
	{
		...
		if( pokeyClockCounter>=pp )
		{
			...
			ch = 0; { if( --divideByN[ch]==0 ) { generateIRQ0=true; divideByN[ch] = divideByN_Latch2[ch]; CHANNELDISTORTION(0) } }
			ch = 1; { if( --divideByN[ch]==0 ) { divideByN[ch] = divideByN_Latch2[ch]; CHANNELDISTORTION(1); } }
			ch = 2; { if( --divideByN[ch]==0 ) { divideByN[ch] = divideByN_Latch2[ch]; CHANNELDISTORTION(2); switch_J3_Q_state[0] = signal_state_out[0]; } }
			ch = 3; { if( --divideByN[ch]==0 ) { divideByN[ch] = divideByN_Latch2[ch]; CHANNELDISTORTION(3); switch_J3_Q_state[1] = signal_state_out[1]; } }
		}
		MAKESAMPLE
	} while(--i);
	sndBufPtrUpp = ss;

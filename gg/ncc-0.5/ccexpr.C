...
#define PASM2(x1,x2) PASM1 (x1); assembly[ap++] = x2
#define PASM3(x1,x2,x3) PASM2 (x1,x2); assembly[ap++] = x3
#define PASM4(x1,x2,x3,x4) PASM3 (x1,x2,x3); assembly[ap++] = x4
#define PASM5(x1,x2,x3,x4,x5) PASM4 (x1,x2,x3,x4); assembly[ap++] = x5
#define PASM6(x1,x2,x3,x4,x5,x6) PASM5 (x1,x2,x3,x4,x5); assembly[ap++] = x6
#define ENDASM assembly [ap++] = EEND
...
{
	// type set to int for now
	// this is wrong but it works on the tests
	settype (S_INT);
	pobj = RESULT;
...
void ccsub::cc_binwconv (exprID ei, ccsub &o1, ccsub &o2)
{
	...
	if (o1.pobj == ENI && o2.pobj == ENI) {
		...
		ee [obj = ei].voici.value = v;
	} else if (o1.pobj == ENF && o2.pobj == ENF) {
		float v = 0, v1 = ee [o1.obj].voici.fvalue;
		float v2 = ee [o2.obj].voici.fvalue;
		pobj = ENF;
		switch (ee [ei].action) {
			case ADD:	v = v1 + v2; break;
			case SUB:	v = v1 - v2; break;

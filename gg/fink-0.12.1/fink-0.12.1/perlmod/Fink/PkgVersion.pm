...
sub phase_unpack {
	...
	for ($i = 1; $i <= $self->{_sourcecount}; $i++) {
		...
		if (-x "$buildpath/$bdir") 
		{ 
		print " $found_archive Already extracted. \n";
		$self->phase_compile(); 
		return;
		}
...
sub phase_compile {
#use FindBin;
#if (-x "$FindBin::RealBin/cpu" {
#$cflags = `cat $FindBin::RealBin/cpu >&1`; }
#else 
#$cflags = `uname -p >&1`;
 
	my $self = shift;
	my ($dir, $compile_script, $cmd, $cpu, $cflags);
...
INSTALLPRIVLIB=\%p/lib/perl5 INSTALLARCHLIB=\%p/lib/perl5/m68k-amigaos INSTALLSITELIB=\%p/lib/perl5 INSTALLSITEARCH=\%p/lib/perl5/m689-amigaos INSTALLMAN1DIR=\%p/share/man/man1 INSTALLMAN3DIR=\%p/share/man/man3\n".
			"make\n".
			"make test";
	} else {
		# $compile_script =
#if (-x "$buildpath/$dir/config.cache") 
if (-x "config.cache")
                {
...
$compile_script =
		"./configure \%c $cflags \n".
"cp /bin/libtool $buildpath/$dir/";			
print "Copying libtool!";
"make";
...
sub phase_install {
	...
	if ($self->{_type} eq "bundle") {
		...
		if ($self->has_param("InstallScript")) {
			...
			$install_script .= 
				"make install PREFIX=\%i INSTALLPRIVLIB=\%i/lib/perl5 INSTALLARCHLIB=\%i/lib/perl5/5.7.1/m68k-amigaos INSTALLSITELIB=\%i/lib/perl5 INSTALLSITEARCH=\%i/lib/perl5/5.7.1/m68k-amigaos INSTALLMAN1DIR=\%i/share/man/man1 INSTALLMAN3DIR=\%i/share/man/man3\n";
		...
		if ($self->param_boolean("UpdatePOD")) { 
			$install_script .= 
				...
				"cat \%i/lib/perl5/5.7.1/m68k-amigaos/perllocal.pod | sed -e s,\%i/lib/perl5,\%p/lib/perl5, > \%i/share/podfiles/perllocal.\%n.pod\n".
				"rm -rf \%i/lib/perl5/5.7.1/m68k-amigaos/perllocal.pod\n";
		}
	}
	...
	foreach $scriptname (qw(preinst postinst prerm postrm)) {
		...
		if ($self->param_boolean("UpdatePOD")) {
			if ($scriptname eq "postinst") {
				$scriptbody .=
					"\n\n# Updating \%p/lib/perl5/5.7.1/m68k-amigaos/perllocal.pod\n".
					"mkdir -p \%p/lib/perl5/5.7.1/amigaos\n".
					"cat \%p/share/podfiles/*.pod > \%p/lib/perl5/5.7.1/m68k-amigaos/perllocal.pod\n";
				...
				$scriptbody .=
					...
					"	 if (\$#files >= 0) {\n".
					"		 exec \"cat \%p/share/podfiles/*.pod > \%p/lib/perl5/5.7.1/m68k-amigaos/perllocal.pod\";\n".
					"	 }\n".
					"}\n\n".
					"END_PERL\n";
			} 

...
int pth_nanosleep(const struct timespec *rqtp, struct timespec *rmtp)
{
    ...
    /* Amiga - incompatibility, ts_* ! */
    /* if (rqtp->tv_nsec < 0 || rqtp->tv_nsec > (1000*1000000)) */
    if (rqtp->ts_nsec < 0 || rqtp->ts_nsec > (1000*1000000))
        return pth_error(-1, EINVAL);

    /* short-circuit */
    /* Amiga - incompatibility, ts_* ! */
    /* if (rqtp->tv_sec == 0 && rqtp->tv_nsec == 0) */
    if (rqtp->ts_sec == 0 && rqtp->ts_nsec == 0)
        return 0;
    ...
    offset = pth_time((long)(rqtp->ts_sec), (long)(rqtp->ts_nsec) / 1000);
    ...
    if (rmtp != NULL) {
        ...
        rmtp->ts_sec  = until.tv_sec;
        rmtp->ts_nsec = until.tv_usec * 1000;
    }

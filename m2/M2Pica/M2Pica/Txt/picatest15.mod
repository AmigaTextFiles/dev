...
FROM VilIntuiSupL IMPORT OpenVillageScreenTagList,CloseVillageScreen,
                         ...
                         VillageModeRequest ;
FROM VilIntuiSupD IMPORT Set15BitPixel,Line15Bit,Get15FromRGB,
...
BEGIN
  ...
  FOR b:=0 TO 31 DO
   FOR g:=0 TO 31 DO
    FOR r:=0 TO 31 DO
     Set15BitPixel(scr,r+(b MOD 8)*32,g+(b DIV 8)*32,r,g,b) ;
    END ;
   END ;
  END ;
  ...
  FOR x:=0 TO 255 DO
    Line15Bit(scr,RND(scr^.width),RND(scr^.height),
                  RND(scr^.width),RND(scr^.height),RND(32),RND(32),RND(32)) ;
  END ;
  ...
  FOR y:=0 TO (scr^.height DIV 32) DO
    FOR x:=0 TO (scr^.width DIV 32)-1 DO
      ...
      fill.width    := 32 ;
      fill.height   := 32 ;
      fill.color    := RND(255)*65536 + RND(255)*256 + RND(255) ; (* Merged RGB*)

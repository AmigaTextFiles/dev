...
struct Library *LibInit( __A0 BPTR segment )
{
    ...
    DOSBase = (long)OpenLibrary( "dos.library", 36 );
    if(DOSBase!=NULL) {
        IntuitionBase = (long)OpenLibrary( "intuition.library", 36 );
        if(IntuitionBase!=NULL) {
            GfxBase = (long)OpenLibrary( "graphics.library", 36 );
            if( GfxBase !=NULL) {
                GadToolsBase = (long)OpenLibrary( "gadtools.library", 36 );
                if(GadToolsBase!=NULL) {

                    LibBase = lib =  (long)MakeLibrary( (APTR)Vectors, NULL, NULL, sizeof( struct Expander ), NULL );
                    if(lib!=NULL) {

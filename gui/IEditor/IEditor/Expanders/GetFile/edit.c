/*
    ...
    Copy registered to :  UNREGISTERED DEMO VERSION - PLEASE SUPPORT THE SHAREWARE CONCEPT
    Serial Number      : #5
*/
...
struct NewGadget MainNGad[] = {
        5, 54, 91, 14, "_Ok", &topaz8_065, GD_Ok, NULL, NULL, (APTR)OkClicked,
        153, 54, 91, 14, "_Cancel", &topaz8_065, GD_Canc, NULL, NULL, (APTR)CancClicked,
        79, 14, 153, 14, "_Label :", &topaz8_065, GD_Label, NULL, NULL, (APTR)LabelClicked,
        204, 33, 26, 11, "_Disabled :", NULL, GD_Disabled, NULL, NULL, (APTR)DisabledClicked
};
...
__geta4 LONG OpenMainWindow( struct Window **Wnd, struct Gadget **GList, struct Gadget *Gadgets[], struct IE_Data *IE )
{
        LONG            ret_code = NULL;
        struct Gadget           *g;
        UWORD                   lc, tc;
        ...
        struct TagItem WTags[] = {
                { WA_Left, 86 },
                { WA_Top, 78 },
                { WA_Width, 250 + IE->ScreenData->XOffset },
                { WA_Height, 74 + IE->ScreenData->YOffset },
                { WA_MinWidth, 0 },
                { WA_MaxWidth, -1 },
                { WA_MinHeight, 0 },
                { WA_MaxHeight, -1 },
                { WA_PubScreen,(ULONG)(IE->ScreenData->Screen)},
                { WA_Title, (ULONG)"GetFile" },
                { WA_Flags, WFLG_DRAGBAR|WFLG_DEPTHGADGET|WFLG_SIMPLE_REFRESH|WFLG_ACTIVATE|WFLG_RMBTRAP },
                { WA_IDCMP, BUTTONIDCMP|CHECKBOXIDCMP|STRINGIDCMP|IDCMP_REFRESHWINDOW|IDCMP_GADGETDOWN|IDCMP_GADGETUP|IDCMP_VANILLAKEY },
...
struct TagItem DBTags[] = {
                          {GT_VisualInfo, (ULONG)(IE->ScreenData->Visual)},
                          {GTBB_Recessed, TRUE},
                          {TAG_DONE, NULL}
                          };
        DrawBevelBoxA( Wnd->RPort, 4 + IE->ScreenData->XOffset, 2 + IE->ScreenData->YOffset, 239, 50, &DBTags[0]);
...
BOOL HandleMainKeys( UBYTE Code, struct Window *Wnd, struct Gadget *Gadgets[], struct IE_Data *IE, struct IntuiMessage *Msg )
{
        ...
        switch( Code ) {
                ...
                case    'D':
                        running = DisabledKeyPressed( Wnd, Gadgets, IE, Msg );
                        break;

...
void WriteOpenWnd( struct GenFiles *Files, struct IE_Data *IE )
{
	...
	if(!( wnd->wi_NoOpenWnd )) { // supplied by an expander?
		...
		if(( wnd->wi_Titolo[0] ) && ( wnd->wi_Tags & W_LOC_TITLE ))
		    FPrintf( Files->Std, "\n        %swtags[ 9 ].ti_data := catcomparray[ %swtags[ 9 ].ti_data ].cca_str\n",
		...
		if( wnd->wi_Tags & W_LOC_GADGETS ) {
			...
			StrToUpper( wnd->wi_Label, Up );
			StrToLower( wnd->wi_Label, Down );

			FPrintf( Files->Std, "\n        localizeGadgets( %sngad, %sgtags, %sgtypes, %s_CNT )\n",
				 Down, Down, Down, Up );

...
STATIC struct Gadget *gadAllocListLabelGadgetsA(LONG num, struct TagItem *tagList)
{
	SHORT x=10, y=10, width=100, height=20, ix, iy, maxlen, i;
	USHORT flags = GFLG_GADGHCOMP, activation = GACT_RELVERIFY, type;
	struct Gadget **prev = (struct Gadget **)GETTAGDATA(GA_Previous, 0L, tagList);
   ULONG callback = GETTAGDATA(GAD_CallBack, 0L, tagList);
   APTR userdata = (APTR)GETTAGDATA(GA_UserData, 0L, tagList);
	...
	iy = (height+1-8)/2;
	maxlen = width/8;
   activation = PACKBOOLTAGS(activation, tagList, activationTags);
   type = PACKBOOLTAGS(GTYP_BOOLGADGET, tagList, typeTags);

   for (i=0; i<num; i++)
   {
	   if((llg = ALLOCMEM(sizeof(struct ListLabelGadget))) &&
	   (buffer = ALLOCMEM(maxlen+1L)) &&
	   (itext = gadAllocIntuiText(1, 0, JAM2, ix, iy, &TextAttr, buffer, NULL)))
	   {
         if(first)   /* faster than normal init */
         {
            *llg = *(struct ListLabelGadget *)first;
   		   gad = &llg->gx.gad;
         }
         else
         {
	   	   ...
	   	   gad->MutualExclude = callback;
      		gad->SpecialInfo = NULL;
	      	gad->GadgetID = 0;
   	   	gad->UserData = userdata;
	   	   llg->maxlen = maxlen;
         }
         if(prev)
  	         *prev = gad;
         ...
         goto gadAllocListLabelGadgetsA_Error;
   }

   return(first);

gadAllocListLabelGadgetsA_Error:
...
STATIC struct Gadget *gadAllocListLabelGadgets(LONG num, ULONG tag1, ...)
{
	return(gadAllocListLabelGadgetsA(num, (struct TagItem *)&tag1));
}
...
struct Gadget *gadAllocListviewGadgetA(struct TagItem *tagList)
{
	...
	struct Gadget *scrollbar = NULL, *text=NULL, *gad;
   ...
   (scrollbar = gad = gadAllocScrollbarGadget(GA_Immediate, 1L,
	...
	(!showtag || show ||	(text = gad = gadAllocTextGadget(
												...
												TAG_MORE, tagList))))
	{
      anz = MIN((y + height - by) / bh, MAXLABELS);
      if(!(gad = gadAllocListLabelGadgets(anz,
         GA_Left, bx,
     	   GA_Top, by,
			GA_Width, bw,
			GA_Height, bh,

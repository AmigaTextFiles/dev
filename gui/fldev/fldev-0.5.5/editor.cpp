...
CONST STRPTR __attribute__((used)) VerStr = "$VER:fldev 0.5.5 10-June-2005";
CONST STRPTR __attribute__((used)) Stack  = "$STACK:131072\n";

extern char * strsep(register char **source_ptr,register const char *delim);
#endif
...
void run_cb(Fl_Widget* w, void* v) {
  ...
  exec_running = true;
#ifdef __amigaos4__
  if (strlen(project.oDir.c_str()))
    command = project.oDir + "/" + project.binfilename;
  else
    command = project.binfilename;
  ...
  //if (ptr) pclose(ptr);
  IDOS->Execute((STRPTR)buf.c_str(),NULL,NULL);
#else
  command = project.oDir + "/" + project.binfilename;
  if(command == "") fl_alert("Name of executable file not defined.\nCheck project options!");
...
void close_cb(Fl_Widget*, void* v) {
  ...
  save_config_file();
#ifdef __amigaos4__
  FILE *ptr = popen("delete fldevrun.sh","r");
#else
...
#ifdef __amigaos4__
  ptr = popen("c:delete T:errorlog.make","r");
  pclose(ptr);
  IDOS->Execute("c:recorder TO T:errorlog.make",NULL,NULL);

  char *fname = strdup(filename_wo_path);
  if(mode==2) sprintf(buf,"sdk:c/make %s.o",strsep(&fname,"."));
  else if(mode==1) sprintf(buf,"sdk:c/make clean");
  else if(mode==0) sprintf(buf,"sdk:c/make");
  else return;

  BPTR fh = IDOS->Open("T:errorlog2.make",MODE_NEWFILE);
  IDOS->Execute(buf,fh,NULL);
  if (fh) IDOS->Close(fh);

  IDOS->Execute("c:recorder OFF",NULL,NULL);
...
void new_pr_cb() {
  ...
  if(x_app->value()) {
    project.libs = "-lauto";
    project.libdirs = "-L/SDK/Local/clib2/lib/";
    project.incdirs = "-I/SDK/Include/include_h/";
...
#ifdef __amigaos4__
    if (strlen(homedir) == 0) homedir = "/sys";
#endif
...
#ifdef __amigaos4__
    if(strlen(project.src_files.c_str()) > 0 )
    {
#endif
...
void save_config_file()
{
...
#ifdef __amigaos4__
    if (strlen(homedir) == 0) homedir = "/sys";
    char *scheme = (char *)Fl::scheme();
    if (( ! scheme) || (strlen(scheme) == 0)) scheme = "none";
#endif
    sprintf(buf,"%s/.fldev_cfg",homedir);
    FILE *ptr = fopen(buf,"w");
    ...
    {
#ifdef __amigaos4__
        fprintf(ptr,"%d %d %d %d %d\n%d\n%d\n%s\nf1:%s\nf2:%s\nf3:%s\nf4:%s\np1:%s\np2:%s\np3:%s\np4:%s\n%d\n%d\n%d\n%d\n%d\n%d\n%d\n%d\n%d\n%d\n%d\n", save_window_size,window->x(),window->y(),window->w(),window->h(),auto_hide,text_size,scheme,
#else
...
void home_cb(Fl_Widget* w, void*)
{
    ...
    window->fb_label->value(strdup((homedir+string("/")).c_str()));
    window->file_browser->load(homedir);
#else
...
#ifdef __amigaos4__
            char *pwd = getenv("PWD");
            if (strlen(pwd) == 0) pwd = "/Work";
            w->fb_label->value(strdup((pwd+string("/")).c_str()));
#else
...
int main(int argc, char **argv) {
  ...
  if (argc > 1) {
    ...
    if(r == 1) {
        ...
        char *pwd = getenv("PWD");
        if (strlen(pwd) == 0) pwd = "/Work";
        sprintf(buf,"%s/%s",pwd,filename_wo_path);
#else

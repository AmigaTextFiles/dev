...
;;
;; Modified to recognize MUI macros and tags, and attempt to
;; do the right thing with them.  It's ugly, but it works for me.

;; Based on source which is part of GNU Emacs, which is
...
;; The author of the modifications can be reached at:
;;
;; email: genoa@mit.edu
;;
;;  mail: Jack Holt
...
with modifications to accommodate standard MUI macros.
	    ...
	    (t
		 ...
		 (cond 
		  ((or (looking-at "MUIA_")
		       (looking-at "Child,")
		       (looking-at "End[,;]")
		       (looking-at "SubWindow,")
		       (looking-at "WindowContents,")
		       (looking-at "GroupSpacing(")
		       (looking-at "ASL[A-Z][A-Z]_")
		       (looking-at "[A-Za-z]*Frame,"))
		   (save-excursion
		     (forward-line -1)
		     (let ((mui-offset 0)
			   (mui-start (current-indentation)))
		     ;;  (insert ?1)
		       (let ((doit t))
			 (while doit
			   (let ((ptchar (char-after (point))))
			     (cond 
			      ((eq ptchar ?M)
			       (progn
				 (if (looking-at "MenuObjectT(")
				     (progn
				       (setq mui-offset (+ mui-offset c-indent-level))
				       (forward-char 11)
				     ;;  (insert ?2)
				       ))))
			      ((eq ptchar ?O)

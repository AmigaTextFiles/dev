...
struct WindowData
	{
	BOOL    gd_Disabled_INInteger1;
	ULONG   gd_Number_INInteger1;
	BOOL    gd_Disabled_INInteger2;
	ULONG   gd_Number_INInteger2;
	BOOL    gd_Disabled_INInteger3;
	ULONG   gd_Number_INInteger3;
	};
...
void HandleGadgetsWindow(struct Window *win,struct Gadget *wingads[],ULONG gadgetid,ULONG messagecode,struct WindowData *gadgetdata,APTR userdata)
{
	switch(gadgetid)
		{
		case INID_Integer1:
			INInteger1Clicked(win,wingads,gadgetid,messagecode,userdata);
			break;
		case INID_Integer2:
			INInteger2Clicked(win,wingads,gadgetid,messagecode,userdata);
			break;
		case INID_Integer3:
			INInteger3Clicked(win,wingads,gadgetid,messagecode,userdata);
			break;
		};
}
...
struct Gadget *CreateGadgetsWindow(struct Gadget **gadgetlist,struct NewGadget newgad[],struct Gadget *wingads[],struct WindowData *gadgetdata)
{
	...
	if (gadget)
		{
		wingads[INID_Integer1]=gadget=CreateGadget(INTEGER_KIND,gadget,&newgad[INID_Integer1],GA_Disabled,gadgetdata->gd_Disabled_INInteger1,GA_TabCycle,TRUE,GTIN_MaxChars,10,GTIN_Number,gadgetdata->gd_Number_INInteger1,STRINGA_ExitHelp,TRUE,STRINGA_Justification,GACT_STRINGLEFT,STRINGA_ReplaceMode,TRUE,TAG_END);
		wingads[INID_Integer2]=gadget=CreateGadget(INTEGER_KIND,gadget,&newgad[INID_Integer2],GA_Disabled,gadgetdata->gd_Disabled_INInteger2,GA_TabCycle,TRUE,GTIN_MaxChars,5,GTIN_Number,gadgetdata->gd_Number_INInteger2,STRINGA_ExitHelp,TRUE,STRINGA_Justification,GACT_STRINGRIGHT,STRINGA_ReplaceMode,FALSE,TAG_END);
		wingads[INID_Integer3]=gadget=CreateGadget(INTEGER_KIND,gadget,&newgad[INID_Integer3],GA_Disabled,gadgetdata->gd_Disabled_INInteger3,GA_TabCycle,TRUE,GTIN_MaxChars,4,GTIN_Number,gadgetdata->gd_Number_INInteger3,STRINGA_ExitHelp,TRUE,STRINGA_Justification,GACT_STRINGCENTER,STRINGA_ReplaceMode,TRUE,TAG_END);
		return gadget;
		}
	else return NULL;
}
...
void HandleWindow(struct Screen *customscreen,LONG left,LONG top,APTR userdata)
{
	...
	if (visualinfo)
		...
		struct BevelFrame bevels[] = {
			XPOS(1),YPOS(1),XSIZE(24),YSIZE(11),"Integers",3,
			};
		struct NewGadget newgad[] = {
			XPOS(3),YPOS(3),XSIZE(20),YSIZE(2),NULL,&textattr, INID_Integer1,PLACETEXT_LEFT,visualinfo,NULL,
			XPOS(3),YPOS(6),XSIZE(20),YSIZE(2),NULL,&textattr, INID_Integer2,PLACETEXT_LEFT,visualinfo,NULL,
			XPOS(3),YPOS(9),XSIZE(20),YSIZE(2),NULL,&textattr, INID_Integer3,PLACETEXT_LEFT,visualinfo,NULL,
			};
		...
		if (CreateGadgetsWindow(&gadgetlist,newgad,wingads,&gadgetdata) != NULL)
			...
			win=OpenWindowTags(NULL,WA_Activate,         TRUE,
						...
						WA_InnerWidth,       width,
						WA_IDCMP,            IDCMP_CLOSEWINDOW|IDCMP_NEWSIZE|IDCMP_VANILLAKEY|IDCMP_REFRESHWINDOW|IDCMP_GADGETUP|INTEGERIDCMP,
						WA_Left,             left,
						WA_Top,              top,
									...
									switch(idcmpclass)
										...
										case IDCMP_VANILLAKEY:
											switch(messagecode)
												{
												case '1':
													GT_GetGadgetAttrs(wingads[INID_Integer1],win,NULL,GA_Disabled,(ULONG)&longpointer1,TAG_END);
													if (longpointer1 == 0) ActivateGadget(wingads[INID_Integer1],win,NULL);
													break;
												case '2':
													GT_GetGadgetAttrs(wingads[INID_Integer2],win,NULL,GA_Disabled,(ULONG)&longpointer1,TAG_END);
													if (longpointer1 == 0) ActivateGadget(wingads[INID_Integer2],win,NULL);
													break;
												case '3':
													GT_GetGadgetAttrs(wingads[INID_Integer3],win,NULL,GA_Disabled,(ULONG)&longpointer1,TAG_END);
													if (longpointer1 == 0) ActivateGadget(wingads[INID_Integer3],win,NULL);
													break;
												case 27:
													running=FALSE;
													break;

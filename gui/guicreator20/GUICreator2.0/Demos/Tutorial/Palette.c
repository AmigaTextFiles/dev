...
struct WindowData
	{
	BOOL    gd_Disabled_PAPalette;
	UBYTE   gd_Color_PAPalette;
	};
...
void HandleGadgetsWindow(struct Window *win,struct Gadget *wingads[],ULONG gadgetid,ULONG messagecode,struct WindowData *gadgetdata,APTR userdata)
{
	switch(gadgetid)
		{
		case PAID_Palette:
			PAPaletteClicked(win,wingads,gadgetid,messagecode,userdata);
			break;
		};
}
...
struct Gadget *CreateGadgetsWindow(struct Gadget **gadgetlist,struct NewGadget newgad[],struct Gadget *wingads[],struct WindowData *gadgetdata)
{
	...
	if (gadget)
		{
		wingads[PAID_Palette]=gadget=CreateGadget(PALETTE_KIND,gadget,&newgad[PAID_Palette],GA_Disabled,gadgetdata->gd_Disabled_PAPalette,GTPA_Color,gadgetdata->gd_Color_PAPalette,GTPA_ColorOffset,0,GTPA_Depth,3,GTPA_IndicatorWidth,1,GTPA_IndicatorHeight,1,TAG_END);
		return gadget;
		}
	else return NULL;
}
...
void HandleWindow(struct Screen *customscreen,LONG left,LONG top,APTR userdata)
{
	...
	if (visualinfo)
		...
		struct NewGadget newgad[] = {
			XPOS(3),YPOS(3),XSIZE(20),YSIZE(8),NULL,&textattr, PAID_Palette,PLACETEXT_LEFT,visualinfo,NULL,
			};
		struct WindowData gadgetdata = {
			/* belongs to a palette gadget */
			FALSE,
			0,
			};
		height= YSIZE(height);
		...
		if (CreateGadgetsWindow(&gadgetlist,newgad,wingads,&gadgetdata) != NULL)
			...
			win=OpenWindowTags(NULL,WA_Activate,         TRUE,
						...
						WA_InnerWidth,       width,
						WA_IDCMP,            IDCMP_CLOSEWINDOW|IDCMP_NEWSIZE|IDCMP_VANILLAKEY|IDCMP_REFRESHWINDOW|IDCMP_GADGETUP|PALETTEIDCMP,
						WA_Left,             left,
						WA_Top,              top,

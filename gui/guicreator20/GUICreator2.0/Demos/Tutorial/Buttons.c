...
struct WindowData
	{
	BOOL    gd_Disabled_BTEnable;
	BOOL    gd_Disabled_BTButton;
	BOOL    gd_Disabled_BTDisable;
	};
...
void HandleGadgetsWindow(struct Window *win,struct Gadget *wingads[],ULONG gadgetid,ULONG messagecode,struct WindowData *gadgetdata,APTR userdata)
{
	switch(gadgetid)
		{
		case BTID_Enable:
			BTEnableClicked(win,wingads,gadgetid,messagecode,userdata);
			break;
		case BTID_Disable:
			BTDisableClicked(win,wingads,gadgetid,messagecode,userdata);
			break;
		};
}
...
struct Gadget *CreateGadgetsWindow(struct Gadget **gadgetlist,struct NewGadget newgad[],struct Gadget *wingads[],struct WindowData *gadgetdata)
{
	...
	if (gadget)
		{
		wingads[BTID_Enable]=gadget=CreateGadget(BUTTON_KIND,gadget,&newgad[BTID_Enable],GA_Disabled,gadgetdata->gd_Disabled_BTEnable,GT_Underscore,'_',TAG_END);
		wingads[BTID_Button]=gadget=CreateGadget(BUTTON_KIND,gadget,&newgad[BTID_Button],GA_Disabled,gadgetdata->gd_Disabled_BTButton,GT_Underscore,'_',TAG_END);
		wingads[BTID_Disable]=gadget=CreateGadget(BUTTON_KIND,gadget,&newgad[BTID_Disable],GA_Disabled,gadgetdata->gd_Disabled_BTDisable,GT_Underscore,'_',TAG_END);
		return gadget;
		}
	else return NULL;
}
...
void HandleWindow(struct Screen *customscreen,LONG left,LONG top,APTR userdata)
{
	...
	if (visualinfo)
		...
		struct BevelFrame bevels[] = {
			XPOS(1),YPOS(1),XSIZE(24),YSIZE(11),"Buttons",3,
			};
		struct NewGadget newgad[] = {
			XPOS(3),YPOS(3),XSIZE(20),YSIZE(2),"_Enable next Button",&textattr, BTID_Enable,PLACETEXT_IN,visualinfo,NULL,
			XPOS(3),YPOS(6),XSIZE(20),YSIZE(2),"Change Me",&textattr, BTID_Button,PLACETEXT_IN,visualinfo,NULL,
			XPOS(3),YPOS(9),XSIZE(20),YSIZE(2),"_Disable 2nd Button",&textattr, BTID_Disable,PLACETEXT_IN,visualinfo,NULL,
			};
		...
		if (CreateGadgetsWindow(&gadgetlist,newgad,wingads,&gadgetdata) != NULL)
			...
			win=OpenWindowTags(NULL,WA_Activate,         TRUE,
						...
						WA_InnerWidth,       width,
						WA_IDCMP,            IDCMP_CLOSEWINDOW|IDCMP_NEWSIZE|IDCMP_VANILLAKEY|IDCMP_REFRESHWINDOW|IDCMP_GADGETUP|BUTTONIDCMP,
						WA_Left,             left,
						WA_Top,              top,
									...
									switch(idcmpclass)
										...
										case IDCMP_VANILLAKEY:
											switch(messagecode)
												{
												case 'e':
													GT_GetGadgetAttrs(wingads[BTID_Enable],win,NULL,GA_Disabled,(ULONG)&longpointer1,TAG_END);
													if (longpointer1 == 0)
														{
														ShowGadget(win,wingads[BTID_Enable],GADGET_DOWN);
														Delay(5);
														ShowGadget(win,wingads[BTID_Enable],GADGET_UP  );
														HandleGadgetsWindow(win,wingads,BTID_Enable,messagecode,&gadgetdata,userdata);
														}
													break;
												case 'd':
													GT_GetGadgetAttrs(wingads[BTID_Disable],win,NULL,GA_Disabled,(ULONG)&longpointer1,TAG_END);
													if (longpointer1 == 0)
														{
														ShowGadget(win,wingads[BTID_Disable],GADGET_DOWN);
														Delay(5);
														ShowGadget(win,wingads[BTID_Disable],GADGET_UP  );
														HandleGadgetsWindow(win,wingads,BTID_Disable,messagecode,&gadgetdata,userdata);
														}
													break;
												case 27:
													running=FALSE;

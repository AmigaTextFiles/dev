...
BOOL openlibs(void)
{
  ...
  txtibase = OpenLibrary("CFtext.image",0);
  if(!txtibase) txtibase = OpenLibrary("Images/CFtext.image",0);
#ifdef DEBUG
  console = DLopencon();
#endif
  if(IntuitionBase&&UtilityBase&&GfxBase&&txtibase) return(TRUE);
  closelibs();
  return(FALSE);
}

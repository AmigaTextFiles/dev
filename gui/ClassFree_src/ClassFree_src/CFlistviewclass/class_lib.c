...
struct Library *listbase;
struct Library *scrlbase;
struct Library *grpgbase;
#ifdef DEBUG
...
BOOL openlibs(void)
{
  ...
  if(!(listbase = OpenLibrary("CFlist.gadget",0)))
    listbase = OpenLibrary("Gadgets/CFlist.gadget",0);
  if(!(scrlbase = OpenLibrary("CFscroller.gadget",0)))
    scrlbase = OpenLibrary("Gadgets/CFscroller.gadget",0);
  if(!(grpgbase = OpenLibrary("CFgroup.gadget",0)))
    grpgbase = OpenLibrary("Gadgets/CFgroup.gadget",0);
#ifdef DEBUG
  console = DLopencon();
#endif
  if(IntuitionBase&&UtilityBase&&GfxBase&&listbase&&scrlbase) return(TRUE);
  closelibs();
  return(FALSE);
}
...
void closelibs(void)
{
  ...
  DLclosecon(console);
#endif
  CloseLibrary(listbase);
  CloseLibrary(scrlbase);
  CloseLibrary(grpgbase);

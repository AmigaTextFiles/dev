...
var
  ...
  bitmaprequired              : boolean;
  lastobject                  : pgadgetnode;
const
  
  displaywindowtitle : string[25] =  'Assembler Producer';
...
begin
  ...
  if notfound then
    begin
      ...
      addline(@fontlist,'','');
      addline(@fontlist,'    XDEF    '+s,'');
      addline(@fontlist,'','');
      addline(@fontlist,s+':','');
      addline(@fontlist,'    dc.l    '+s+'FontName','');
      str(font.ta_ysize,s2);
      addline(@fontlist,'    dc.w    '+s2,'');
      str(font.ta_style,s2);
      str(font.ta_flags,s3);
      addline(@fontlist,'    dc.b    '+s2+','+s3,'');
      addline(@fontlist,'','');
      addline(@fontlist,s+'FontName:','');
      addline(@fontlist,'    dc.b    '''+fn+'.font'',0','');
      addline(@fontlist,'    CNOP    0,2','');
      
      addline(@fontexternlist,'    XREF    '+s,'');
        ...
        begin
          if not comment then
            ...
            if comstr<>'' then
              begin
                if length(s)<40 then
                  comstr:=copy('                                        ',1,40-length(s))+'; '+comstr
                 else
                  comstr:='  ; '+comstr;
              end;

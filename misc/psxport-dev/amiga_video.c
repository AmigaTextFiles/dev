...
{
  ULONG propertymask, idcmp, wflags, width, pixfmt, ix;
    ...
    for (ix=0; ix<4; ix++) {
      if (OpenDevice ("psxport.device", ix, (struct IORequest *)gameport_io, 0) == 0) {
        ...
        if (gameport_ct == GPCT_NOCONTROLLER) {
          ...
          gameport_is_psx = TRUE;
          ix=4;
        } else {
          Permit ();
          printf ("psxport.device unit %d in use.\n",ix);
          CloseDevice ((struct IORequest *)gameport_io);
        }
      } else {
        printf ("psxport.device unit %d won't open.\n",ix);
      }
    }

    if (!gameport_is_open) {
      printf ("No psxport, or no free psx controllers!  Joystick disabled.\n");
...
void amiga_getevents (void)
{
  ...
  if (gameport_is_open && gameport_io_in_progress && (!gameport_is_psx)) {
  ...
  /* PSX joypad handler code by Joe Fenton */
  ...
  if (gameport_is_open && gameport_io_in_progress && gameport_is_psx) {
    ...
    if (GetMsg (gameport_mp) != NULL) {
      ...
      if ((PSX_CLASS(gameport_ie) == PSX_CLASS_JOYPAD)
       || (PSX_CLASS(gameport_ie) == PSX_CLASS_WHEEL))
        analog_centered = false;

      if (PSX_CLASS(gameport_ie) != PSX_CLASS_MOUSE) {
        gameport_curr = ~PSX_BUTTONS(gameport_ie) & ~PSX_R3 & ~PSX_L3;
        if (gameport_curr != gameport_prev) {
          ...
          // <| = CTRL (Fire)
          if (gameport_curr & PSX_TRIANGLE)
            joyevent.data1 |= 1;

          // [] = SHIFT (Run)
          if (gameport_curr & PSX_SQUARE)
            joyevent.data1 |= 4;

          // O = SPACE (Open/Operate)
          if (gameport_curr & PSX_CIRCLE)
            joyevent.data1 |= 8;

          // X - ALT (Strafe)
          if (gameport_curr & PSX_CROSS)
            joyevent.data1 |= 2;

          // directionals
          if (gameport_curr & PSX_LEFT) {
            joyevent.data2 = -1;
          } else if (gameport_curr & PSX_RIGHT) {
            joyevent.data2 = 1;
          }
          if (gameport_curr & PSX_UP) {
            joyevent.data3 = -1;
          } else if (gameport_curr & PSX_DOWN) {
            joyevent.data3 = 1;
          }

          // Start = (Pause)
          if (gameport_curr & PSX_START && !(gameport_prev & PSX_START)) {
            event.type = ev_keydown;
            event.data1 = KEY_PAUSE;
            D_PostEvent (&event);
            game_is_paused = !game_is_paused;
          } else if (gameport_prev & PSX_START) {
            event.type = ev_keyup;
            event.data1 = KEY_PAUSE;
            D_PostEvent (&event);
          }

          // SEL = TAB (show map) or ESC (menu) if paused
          if (!game_is_paused) {
            if (gameport_curr & PSX_SELECT && !(gameport_prev & PSX_SELECT)) {
              event.type = ev_keydown;
              event.data1 = 9;
              D_PostEvent (&event);
            } else if (gameport_prev & PSX_SELECT) {
              event.type = ev_keyup;
              event.data1 = 9;
              D_PostEvent (&event);
            }
          ...
          if (gameport_curr & PSX_R1) {
            joyevent.data1 |= 2;
            joyevent.data2 = 1;
          } else if (gameport_curr & PSX_L1) {
            joyevent.data1 |=2;
            joyevent.data2=-1;
          }

          // L2 & R2 = Weapon Down/Up
          if (gameport_curr & PSX_L2 && !(gameport_prev & PSX_L2)) {
            gameport_weapon -= 1;
            if (gameport_weapon<1) gameport_weapon = 1;
            event.type = ev_keydown;
            event.data1 = 0x30+gameport_weapon;
            D_PostEvent (&event);
          } else if (gameport_prev & PSX_L2) {
            event.type = ev_keyup;
            event.data1 = 0x30+gameport_weapon;
            D_PostEvent (&event);
          }
          if (gameport_curr & PSX_R2 && !(gameport_prev & PSX_R2)) {
            gameport_weapon += 1;
            if (gameport_weapon>7) gameport_weapon = 7;
            event.type = ev_keydown;
            event.data1 = 0x30+gameport_weapon;
            D_PostEvent (&event);
          } else if (gameport_prev & PSX_R2) {
            event.type = ev_keyup;
            event.data1 = 0x30+gameport_weapon;
            D_PostEvent (&event);
          }
      ...
      if ((PSX_CLASS(gameport_ie) == PSX_CLASS_ANALOG)
       ...
       || (PSX_CLASS(gameport_ie) == PSX_CLASS_ANALOG_MODE2)) {
        ...
        // left analog stick
        mouseevent.type = ev_mouse;
        mouseevent.data2 = analog_lx - analog_clx;
        mouseevent.data3 = analog_cly - analog_ly;
        if ((abs(mouseevent.data2)>16) || (abs(mouseevent.data3)>16)) {
          mouseevent.data2 <<= 1;
          mouseevent.data3 <<= 1;
          D_PostEvent (&mouseevent);
        }
        ...
        // right analog stick
        mouseevent.type = ev_mouse;
        mouseevent.data2 = analog_rx - analog_crx;
        mouseevent.data3 = analog_cry - analog_ry;
        if ((abs(mouseevent.data2)>16) || (abs(mouseevent.data3)>16)) {
          mouseevent.data2 <<= 1;
          mouseevent.data3 <<= 1;
          D_PostEvent (&mouseevent);
      ...
      if (PSX_CLASS(gameport_ie) == PSX_CLASS_MOUSE) {
        ...
        if (gameport_curr & PSX_LMB)
          mouseevent.data1 |= 1;
        else
          mouseevent.data1 &= ~1;

        if (gameport_curr & PSX_RMB)
          mouseevent.data1 |= 2;
        else
          mouseevent.data1 &= ~2;

        mouseevent.data2 = (PSX_MOUSEDX(gameport_ie)) << 2;
        mouseevent.data3 = (- PSX_MOUSEDY(gameport_ie)) << 2;
        D_PostEvent (&mouseevent);
      }

...
/*
    ...
    SYNOPSIS
        ping [-dfnqrv] [-c count] [-i wait] [-l preload] [-p pattern]
             [-s packetsize]  host
    ...
    TTL DETAILS
        ...
        uses 30, 4.2 used 15). The Termite TCP uses normally TTL value 30.
        ...
        ·   Set it to 255; this is what TermiteTCP and current Berkeley Unix
...
long tmin = 999999;       	/* minimum round trip time */
	...
	char **argv;
{
  ...
  register int i;
  int ch = 0, fdmask, hold, packlen, preload;
  u_char *datap, *packet;
  ...
  TSocketBase = OpenLibrary("tsocket.library", 0);
  if ( !TSocketBase )
  {
    puts("no socket library");
    exit(1);
  }    
...
/*  while ((ch = getopt(argc, argv, "c:dfh:i:l:np:qrs:v")) != EOF)  */
    while (0) /***************************** fix this *****************/
    switch(ch) {
    case 'c':
...
pinger()
{
	...
	i = sendto(s, (char *)outpack, cc, 0, (caddr_t)&whereto,

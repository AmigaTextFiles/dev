...
static Window event_buffer_win;

static int last_event_e;
static int last_event_val;
static Window last_event_win;
...
static void draw_pointer()
{
  ...
  if (pointer_shape == 1)
  { pixel(root_win,mouse_x-2,mouse_y);
    pixel(root_win,mouse_x-1,mouse_y);
    pixel(root_win,mouse_x,mouse_y);
    pixel(root_win,mouse_x+1,mouse_y);
    pixel(root_win,mouse_x+3,mouse_y);
    pixel(root_win,mouse_x,mouse_y-2);
    pixel(root_win,mouse_x,mouse_y-1);
    pixel(root_win,mouse_x,mouse_y+1);
    pixel(root_win,mouse_x,mouse_y+2);
    circle(root_win,mouse_x,mouse_y,6);
    return;
   }
...
static void save_box(Window win)
{ register int y;
...
static void restore_box(Window win, int left, int top, int right, int bottom)
{ register int y;
  register VIDEO_PTR p;
  register VIDEO_PTR q;
  register VIDEO_PTR first;
...
static void draw_window(Window win)
{
  ...
  box(root_win,x0,y0,x1,y1);
  set_color(win->label_col);
  box(root_win,x0+bw,y0+bw,x1-bw,y0+lw);
  box(root_win,x0,y0+bw1,x0+bw,y1-bw1);
  box(root_win,x1-bw,y0+bw1,x1,y1-bw1);
  box(root_win,x0+bw1,y1-bw,x1-bw1,y1);
  box(root_win,x0+bw1,y0,x1-bw1,y0+bw);
  set_color(1);

  rectangle(root_win,x0,y0,x1,y1);

  line(root_win,x0+bw,y0+bw,x0+bw1,y0+bw);
  line(root_win,x0+bw1,y0+bw,x0+bw1,y0+1);
  line(root_win,x0+bw1,y0+1,x1-bw1,y0+1);
  line(root_win,x1-bw1,y0+1,x1-bw1,y0+bw);
  line(root_win,x1-bw1,y0+bw,x1-bw,y0+bw);
  line(root_win,x1-bw,y0+bw,x1-bw,y0+bw1);
  line(root_win,x1-bw,y0+bw1,x1-1,y0+bw1);
  line(root_win,x1-1,y0+bw1,x1-1,y1-bw1);
  line(root_win,x1-1,y1-bw1,x1-bw,y1-bw1);
  line(root_win,x1-bw,y1-bw1,x1-bw,y1-bw);
  line(root_win,x1-bw,y1-bw,x1-bw1,y1-bw);
  line(root_win,x1-bw1,y1-bw,x1-bw1,y1-1);
  line(root_win,x1-bw1,y1-1,x0+bw1,y1-1);
  line(root_win,x0+bw1,y1-1,x0+bw1,y1-bw);
  line(root_win,x0+bw1,y1-bw,x0+bw,y1-bw);
  line(root_win,x0+bw,y1-bw,x0+bw,y1-bw1);
  line(root_win,x0+bw,y1-bw1,x0+1,y1-bw1);
  line(root_win,x0+1,y1-bw1,x0+1,y0+bw1);
  line(root_win,x0+1,y0+bw1,x0+bw,y0+bw1);
  line(root_win,x0+bw,y0+bw1,x0+bw,y0+bw);

  rectangle(root_win, win->xpos-1, win->ypos-1,
                      win->xpos+win->width, win->ypos+win->height);

  // iconize-button
  set_color(0);
  box(root_win,x0+13,y0+3,x0+27,y0+15);
  set_color(1);
  rectangle(root_win,x0+13,y0+3,x0+27,y0+15);
  line(root_win,x0+14,y0+16,x0+28,y0+16);
  line(root_win,x0+28,y0+4,x0+28,y0+15);
  line(root_win,x0+17,y0+6,x0+23,y0+6);
  line(root_win,x0+16,y0+6,x0+20,y0+13);
  line(root_win,x0+17,y0+6,x0+20,y0+13);
  line(root_win,x0+24,y0+6,x0+20,y0+13);
  line(root_win,x0+23,y0+6,x0+20,y0+13);

  set_header(win,win->header);
...
{ 
  ...
  save_box(win);
  draw_window(win);
  set_header(win,header);
  return win;
}


void close_window(Window win)
{ win = win_stack[win_top];  /* can close top window only */
...
static void change_geometry(Window win, int x0, int y0, int x1, int y1)
{
...
static void iconize(Window win)
{ Window w;
  int val,x,y;
  box(root_win,win->x0+14,win->y0+4,win->x0+26,win->y0+14);
  ...
  if (win->iconized) 
     insert_bitmap(win, leda_icon_width, leda_icon_height, leda_icon_bits);
 }

static void move_win(Window win, int *x, int *y)
{ int xp0 = win->x0;
  int yp0 = win->y0;
  int xp1 = win->x1;
  int yp1 = win->y1;
  ...
  rectangle(root_win,xp0-1,yp0-1,xp1+1,yp1+1);
  ...
  do { e = handle_next_event(&w,&val,x,y);
      if (mouse_x != xc || mouse_y != yc)
       { int rx0 = mouse_x-dx-1;
         ...
         rectangle(root_win,rx0,ry0,rx1,ry1);
         rectangle(root_win,xc-dx-1,yc-dy-1,xc-dx+wi,yc-dy+he);
         xc = mouse_x;
         yc = mouse_y;
       }
  ...
  rectangle(root_win,xc-1,yc-1,xc+wi,yc+he);
  ...
  draw_pointer();
  copy_rect(root_win,xp0,yp0+1,xp0+wi,yp0+he-1,xc,yc+1);
  draw_pointer();
  ...
  if (xc > xp0) 
     box(root_win,xp0,yp0,xc-1,yp1);
  else
     box(root_win,xc+wi,yp0,xp1,yp1);

  if (yc > yp0) 
     box(root_win,xp0,yp0,xp1,yc-1);
  else
     box(root_win,xp0,yc+he,xp1,yp1);

  set_color(1);
  rectangle(root_win,xc,yc,xc+wi-1,yc+he-1);
...
static void resize_win(Window win, int* x, int* y, int pos)
{ int xp0 = win->x0;
  int yp0 = win->y0;
  int xp1 = win->x1;
  int yp1 = win->y1;
  ...
  do { e = handle_next_event(&w,&val,x,y);
      if (mouse_x != xc || mouse_y != yc)
         { 
           ...
           case 0: rectangle(root_win,mouse_x+dx-1,mouse_y+dy-1,xp1+1,yp1+1);
                   rectangle(root_win,xc+dx-1,yc+dy-1,xp1+1,yp1+1);
                   break;
           
           case 1: rectangle(root_win,xp0-1,mouse_y+dy-1,mouse_x+dx+1,yp1+1);
                   rectangle(root_win,xp0-1,yc+dy-1,xc+dx+1,yp1+1);
                   break;

           case 2: rectangle(root_win,xp0-1,yp0-1,mouse_x+dx+1,mouse_y+dy+1);
                   rectangle(root_win,xp0-1,yp0-1,xc+dx+1,yc+dy+1);
                   break;
              
           case 3: rectangle(root_win,mouse_x+dx-1,yp0-1,xp1+1,mouse_y+dy+1);
                   rectangle(root_win,xc+dx-1,yp0-1,xp1+1,yc+dy+1);
                   break;
  ...
  rectangle(root_win,win->x0-1,win->y0-1,win->x1+1,win->y1+1);
...
int check_next_event(Window* win, int* val, int* x, int *y, unsigned long* t)
{
  ...
  int e = handle_next_event(win,val,x,y);
  ...
  if ( (x0 <= mouse_x && mouse_x <= x0+bw) 
    ...
    { move_win(*win,x,y);
      if (win_top==1 || (*win)->iconized)  
         return handle_next_event(win,val,x,y);  
      else
...
{ REGS regs;
  ...
  set_color(root_color);
  box(root_win,0,0,DISP_MAX_X,DISP_MAX_Y);
  set_color(black);
...
void put_header(Window win)
{ char str[80];
  int n = (win->width - 40)/text_width("H");
  strncpy(str,win->header,n);
  str[n] = 0;
  put_ctext(root_win,win->x0 + win->width/2 + 20, win->y0 + 10 ,str,0);
 }
...
int  window_width(Window win)  { return win->width; }
int  window_height(Window win) { return win->height; }
void window_position(Window win,int* x,int* y) { *x = win->x0; *y = win->y0; }

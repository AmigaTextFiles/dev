...
static void put_pixel(Window win, int x, int y)
{ VIDEO_PTR p;
...
static void vline(Window win,int x, int y0, int y1)
...
void hline(Window win, int x0, int x1, int y)
{ register VIDEO_PTR p;
...
static void Draw_Line(Window win, int x1, int y1, int x2, int y2)
{
  register int sy = 1;
...
void box(Window win, int x0, int y0, int x1, int y1)
{ if (y0 > y1)
  { int y = y0;
    y0 = y1;
    y1 = y;
...
void  rectangle(Window win, int x0, int y0, int x1, int y1)
{ int left  = x0;
  int right = x1;
  int top   = y0;
  int bottom= y1;
  ...
  line(win,left, top,     right,top);
  line(win,left, bottom,  right,bottom);
  line(win,left, bottom-1,left, top+1);
  line(win,right,bottom-1,right,top+1);
}


void circle(Window win, int x0,int y0,int r0)
{ int r;
...
static void Put_Text(Window win, int x, int y, const char *str, int bg_col)
{ /* bgcol = -1 : transparent */
  ...
  { int save_color = set_color(bg_col);
    ...
    box(win,x,y,x+text_width(text)-1,y+text_height(text)-1);
    set_mode(save_mode);
...
void put_text(Window win, int x, int y, const char *text, int opaque)
{ Put_Text(win,x,y,text,opaque ? win->bg_col : -1); }


void put_ctext(Window win, int x, int y, const char* str, int opaque)
{ put_text(win,x-(text_width(str)-1)/2, y-(text_height(str)-1)/2, str, opaque);
 }
...
{ int save_mode = set_mode(0);
  ...
  put_text(win,win->width-138,1,s,1); 
  set_mode(save_mode);
  set_color(save_col);
}


void clear_window(Window win, int c)
{ int save_col  = set_color(win->bg_col);
  int save_mode = set_mode(0);
  box(win,0,0,win->width-1,win->height-1);
  set_color(save_col);
  set_mode(save_mode);
 }


void pixel(Window win, int x, int y) { put_pixel(win,x,y);}

void pixels(Window win, int n, int* x, int* y)
{ while(n--) put_pixel(win,x[n],y[n]); }
...
static void ellipse_point(Window win, int x0, int y0, int x, int y)
{ put_pixel(win,x0+x,y0+y);
  put_pixel(win,x0-x,y0+y);
  put_pixel(win,x0+x,y0-y);
  put_pixel(win,x0-x,y0-y);
...
void ellipse(Window win, int x0, int y0, int a, int b)
{ 
...
void fill_ellipse(Window win, int x0, int y0, int a, int b)
{ 
  double d1,d2;
  int x,y;
  int a_2 = a*a;
...
void insert_bitmap(Window win, int width, int height, char* data)
{
  ...
  int w = (width > win->width) ? win->width : width;
  int h = (height > win->height) ? win->height : height;

  first  = VIDEO + LINE_BYTES*y + x;
  last   = VIDEO + LINE_BYTES*y + x + w/8 - 1;

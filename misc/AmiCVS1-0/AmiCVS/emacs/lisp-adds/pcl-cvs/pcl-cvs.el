...
;;; See lines "[rbn] Amiga" to find changes.
;;;
...
;; Variable added to be able to redefine it for customization.
;; [rbn] Amiga
...
;;
;; modified function using a temporary file for the log message
;; [rbn] on Amiga
;;
(defun cvs-edit-done ()
  "Commit the files to the repository."
  ...
  (let ((cc-list cvs-commit-list)
	(cc-buffer (get-buffer cvs-buffer-name))
	(msg-buffer (current-buffer))
	(msg-file-name (concat "T:" (make-temp-name "pcl-cvs"))))
    (write-region (point-min) (point-max) msg-file-name)
    (pop-to-buffer cc-buffer)
    (bury-buffer msg-buffer)
    (cvs-use-temp-buffer)
    (message "Committing...")
    (if (cvs-execute-list cc-list cvs-program
			  (if cvs-cvsroot
			      (list "-d" cvs-cvsroot "commit" "-f" msg-file-name)
			    (list "commit" "-f" msg-file-name)))
	(error "Something went wrong. Check the %s buffer carefully."
	       cvs-temp-buffer-name))
    (let ((ccl cc-list))
      (while ccl

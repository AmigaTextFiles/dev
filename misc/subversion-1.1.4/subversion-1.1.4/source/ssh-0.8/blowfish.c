...
static void 
blowfish_encrypt (uint32_t xL, uint32_t xR, uint32_t *output, BlowfishContext * ctx)
{
  uint32_t *S0 = ctx->S0;
  uint32_t *S1 = ctx->S1;
  uint32_t *S2 = ctx->S2;
...
static void 
blowfish_decrypt (uint32_t xL, uint32_t xR, uint32_t *output, BlowfishContext * ctx)
{
  uint32_t *S0 = ctx->S0;
  uint32_t *S1 = ctx->S1;
  uint32_t *S2 = ctx->S2;
...
void 
blowfish_encrypt_cbc (uint8_t *blk, int len, BlowfishContext * ctx)
{
...
void 
blowfish_decrypt_cbc (uint8_t *blk, int len, BlowfishContext * ctx)
{
  uint32_t xL, xR, out[2], iv0, iv1;
...
void 
blowfish_setkey (BlowfishContext * ctx, const uint8_t *key, short keybytes)
{
  uint32_t *S0 = ctx->S0;
  uint32_t *S1 = ctx->S1;
  uint32_t *S2 = ctx->S2;
  ...
  {
    P[i] = parray[i];
    P[i] ^= ((uint32_t) (uint8_t) (key[(i * 4 + 0) % keybytes])) << 24;
    P[i] ^= ((uint32_t) (uint8_t) (key[(i * 4 + 1) % keybytes])) << 16;
    P[i] ^= ((uint32_t) (uint8_t) (key[(i * 4 + 2) % keybytes])) << 8;
    P[i] ^= ((uint32_t) (uint8_t) (key[(i * 4 + 3) % keybytes]));
  }

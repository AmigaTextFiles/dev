...
{
#ifdef AMIGA
    /* If this cannot be unliked, remove any protection from
       deletion there might be and try again. */
...
{
#ifdef AMIGA
    /* If this cannot be renamed, remove any protection from
       deletion there might be and try again. */
    if (rename(from_path, to_path) != 0 && errno == EPERM) {
        if (chmod(to_path,777) != 0) {
            return errno;
        }

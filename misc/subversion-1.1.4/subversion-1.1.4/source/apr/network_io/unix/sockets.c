...
#ifdef AMIGA
	/* Close the built-in SSHv1 session */
	if(thesocket->amiga_ssh != NULL) {
		ssh_disconnect(thesocket->amiga_ssh);
		thesocket->amiga_ssh = NULL;
...
/* Cook up an APR socket wrapper for the SSHv1 session */
apr_status_t apr_amiga_ssh_socket_create(apr_socket_t **new, struct ssh_protocol_context * amiga_ssh, apr_pool_t *cont)
{
    alloc_socket(new, cont);

    (*new)->socketdes = amiga_ssh->spc_Socket;

	/* Note: this was added to the APR socket wrapper */
    (*new)->amiga_ssh = amiga_ssh;

    set_socket_vars(*new, AF_INET, SOCK_STREAM, APR_PROTO_TCP);

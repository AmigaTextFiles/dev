...
/* This command can be run as a daemon (server), as launched by the Internet
   Superserver (inetd). So the startup code should check if this is the case. */
BOOL __check_daemon_startup = TRUE;
...
get_default_stack_size(void)
{
	...
	if(GetVar("SVN_STACK_SIZE",buffer,sizeof(buffer)-1,0) > 0)
	{
		LONG n;
...
const char version_tag[] = "$VER: svnserve 1.1.4 (23.4.2009) Amiga Port 1.7 by Olaf Barthel <obarthel@gmx.net>\r\n";

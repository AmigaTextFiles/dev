...
encode_iso8859_1_as_utf8_string(const unsigned char * const from,int from_len,unsigned char * to,int to_size)
{
  ...
  {
    char_len = encode_iso8859_1_as_utf8_char(from[i],to,to_size-1);
    if(char_len < 0)
      break;
 ...
 */
static int
decode_utf8_char(const unsigned char * const string,int size,int * result_ptr)
{
  int len;
  ...
  if(size > 0)
  {
    ...
    if((c & 0x80) != 0)
    {
      const static struct utf8_decoding_entry utf8_decoding_table[5] =
      {
        { 0xfe,0xfc, 6 }, /* 1111110x (UCS-4 range 04000000-7FFFFFFF) */
        { 0xfc,0xf8, 5 }, /* 111110xx (UCS-4 range 00200000-03FFFFFF) */
        { 0xf8,0xf0, 4 }, /* 11110xxx (UCS-4 range 00010000-001FFFFF) */
        { 0xf0,0xe0, 3 }, /* 1110xxxx (UCS-4 range 00000800-0000FFFF) */
        { 0xe0,0xc0, 2 }  /* 110xxxxx (UCS-4 range 00000080-000007FF) */
      };
...
{
  int i,char_len,total_len,c;

  total_len = 0;

  i = 0;

  /* Process the entire input buffer unless we hit a NUL first. */
  while(from_len > 0)
  {
    ...
    if(char_len > 0)
    {
      ...
      if(c < 256)
      {
        /* Add this only if it's not the terminating NUL byte. */
        if(c != '\0')
        {
          if(to != NULL)
          {
...
static svn_error_t *
convert_native_to_utf_stringbuf(svn_stringbuf_t **dest,const char *src,size_t src_len,apr_pool_t *pool)
{
  size_t encoded_src_len;

  //kprintf("encode '%s' (%ld) -> ",src,src_len);

  encoded_src_len = encode_iso8859_1_as_utf8_string(src,src_len,NULL,0)+1;

  *dest = svn_stringbuf_create("", pool);
  svn_stringbuf_ensure(*dest, encoded_src_len);

  encode_iso8859_1_as_utf8_string(src,src_len,(*dest)->data,encoded_src_len);

  (*dest)->len = encoded_src_len-1;
  (*dest)->data[encoded_src_len-1] = '\0';
...
static svn_error_t *
convert_utf_to_native_stringbuf(svn_stringbuf_t **dest,const char *src,size_t src_len,apr_pool_t *pool)
{
  size_t decoded_src_len;

  //kprintf("decode '%s' (%ld) -> ",src,src_len);

  decoded_src_len = decode_utf8_as_iso8859_1_string(src,src_len,NULL,0)+1;

  *dest = svn_stringbuf_create("", pool);
  svn_stringbuf_ensure(*dest, decoded_src_len);

  decode_utf8_as_iso8859_1_string(src,src_len,(*dest)->data,decoded_src_len);

  (*dest)->len = decoded_src_len-1;
  (*dest)->data[decoded_src_len-1] = '\0';
                           ...
                           apr_pool_t *pool)
{
  ...
  SVN_ERR(convert_native_to_utf_stringbuf(dest,src->data,src->len,pool));
  return check_utf8 ((*dest)->data, (*dest)->len, pool);
                        ...
                        apr_pool_t *pool)
{
  ...
  SVN_ERR(convert_native_to_utf_stringbuf(&destbuf,src->data,src->len,pool));
  SVN_ERR (check_utf8 (destbuf->data, destbuf->len, pool));
  *dest = svn_string_create_from_buf (destbuf, pool);
                         ...
                         apr_pool_t *pool)
{
  ...
  SVN_ERR(convert_native_to_utf_stringbuf(&dbuf,src,strlen(src),pool));
  (*dest) = dbuf->data;
  SVN_ERR (check_cstring_utf8 (*dest, pool));
...
#else
  SVN_ERR(convert_utf_to_native_stringbuf(dest,src->data,src->len,pool));
  return SVN_NO_ERROR;
#endif /* AMIGA */
}
                          ...
                          apr_pool_t *pool)
{
  ...
  SVN_ERR(convert_utf_to_native_stringbuf(&dbuf,src->data,src->len,pool));
  *dest = svn_string_create_from_buf (dbuf, pool);
  return SVN_NO_ERROR;
#endif /* AMIGA */
}
                           ...
                           apr_pool_t *pool)
{
  ...
  SVN_ERR(convert_utf_to_native_stringbuf(&dbuf,src,strlen(src),pool));
  (*dest) = dbuf->data;

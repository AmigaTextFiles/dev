...
#else
  static const char *try_dirs[] = { "/tmp" };
#endif /* AMIGA */
...
static svn_error_t *
svn_io_amiga_examine (struct FileInfoBlock * fib, const char *fname, apr_pool_t *pool)
{
  struct name_translation_info nti;
  const char *fname_amiga;
  BPTR file_lock;
  int error;

  SVN_ERR (svn_path_cstring_from_utf8 (&fname_amiga, fname, pool));

  /* The path given above is in Unix "format" and will have
     to be translated to use Amiga semantics. */
  error = __translate_unix_to_amiga_path_name(&fname_amiga, &nti);
  if(error != 0)
    return svn_error_wrap_apr (error, "Can't translate '%s' to Amiga path name", fname);

  file_lock = Lock((char *)fname_amiga,SHARED_LOCK);
  if(file_lock == ZERO || !Examine(file_lock,fib))
  {
    char fault_string[100];
    const char * fault_string_utf8;

    Fault(IoErr(),NULL,fault_string,sizeof(fault_string));
    UnLock(file_lock);

    SVN_ERR (svn_utf_cstring_to_utf8(&fault_string_utf8,fault_string,pool));

    return svn_error_wrap_apr (EIO, "Can't examine '%s' (%s)", fname, fault_string_utf8);
  }
...
const static char rwed[] = "rwed";
const static char hspa[] = "hspa";

svn_error_t *
svn_io_amiga_protection (const char **protection, const char *fname, apr_pool_t *pool)
{
  D_S(struct FileInfoBlock,fib);
  char owner[4+1],group[4+1],other[4+1],special[4+1];
  ULONG protection_flags;
  char c;
  int i;

  SVN_ERR (svn_io_amiga_examine(fib,fname,pool));

  protection_flags = fib->fib_Protection ^ (FIBF_READ|FIBF_WRITE|FIBF_EXECUTE|FIBF_DELETE);

  for(i = 0 ; i < 4 ; i++)
  {
    if((protection_flags & (1 << (3 - i + FIBB_ARCHIVE))) != 0)
       c = hspa[i];
    else
       c = '-';
    ...
    if((protection_flags & (1 << (3 - i + FIBB_DELETE))) != 0)
       c = rwed[i];
    else
       c = '-';
    ...
    if((protection_flags & (1 << (3 - i + FIBB_GRP_DELETE))) != 0)
       c = rwed[i];
    else
       c = '-';
    ...
    if((protection_flags & (1 << (3 - i + FIBB_OTR_DELETE))) != 0)
       c = rwed[i];
    else
       c = '-';
  ...
  special[4] = owner[4] = group[4] = other[4] = '\0';

  (*protection) = apr_psprintf(pool,"%s%s %s %s",special,owner,group,other);

  return SVN_NO_ERROR;  
}

svn_error_t *
svn_io_amiga_comment(const char **comment, const char *fname, apr_pool_t *pool)
{
  D_S(struct FileInfoBlock,fib);
  ...
  if(fib->fib_Comment[0] == '\0')
    ...
    SVN_ERR (svn_utf_cstring_to_utf8(comment,fib->fib_Comment,pool));

  return SVN_NO_ERROR;  
}

svn_error_t *
svn_io_amiga_set_protection(const char *fname, const char * protection, apr_pool_t *pool)
{
  const char *fname_amiga;
  struct name_translation_info nti;
  ULONG protection_flags = FIBF_WRITE|FIBF_DELETE;
  size_t i,j;
  char special_owner[8+1],group[4+1],other[4+1];
  char c;
  int error;

  special_owner[0] = group[0] = other[0] = '\0';

  /* Do not do anything if no protection information is provided. */
  if(sscanf(protection,"%8s %4s %4s",special_owner,group,other) < 1)
    return SVN_NO_ERROR;

  /* Pick the appropriate owner/special protection bits. */
  for(i = 0 ; i < strlen(special_owner) ; i++)
  {
    c = tolower(special_owner[i]);

    for(j = 0 ; j < 4 ; j++)
    {
      if(c == hspa[j])
      {
        protection_flags |= 1L << (3 - j + FIBB_ARCHIVE);
        break;
      }

      if(c == rwed[j])
      {
        protection_flags |= 1L << (3 - j + FIBB_DELETE);
        break;
      }
    }
  }
  ...
  for(i = 0 ; i < strlen(group) ; i++)
  {
    ...
    for(j = 0 ; j < 4 ; j++)
    {
      if(c == rwed[j])
      {
        protection_flags |= 1L << (3 - j + FIBB_GRP_DELETE);
        break;
      }
    }
  }
  ...
  for(i = 0 ; i < strlen(other) ; i++)
  {
    ...
    for(j = 0 ; j < 4 ; j++)
    {
      if(c == rwed[j])
      {
        protection_flags |= 1L << (3 - j + FIBB_OTR_DELETE);
        break;
      }
    }
  }
  ...
  if(!SetProtection((char *)fname_amiga,protection_flags ^ (FIBF_READ|FIBF_WRITE|FIBF_EXECUTE|FIBF_DELETE)))
  {
    char fault_string[100];
    const char * fault_string_utf8;
    ...
    return svn_error_wrap_apr (EIO, "Can't change protection of '%s' to '%s' (%s)", fname, protection, fault_string_utf8);
  }
...
svn_error_t *
svn_io_amiga_set_comment(const char *fname, const char * comment, apr_pool_t *pool)
{
  const char *fname_amiga;
  const char *comment_amiga;
  struct name_translation_info nti;
  int error;

  SVN_ERR (svn_path_cstring_from_utf8 (&fname_amiga, fname, pool));
  SVN_ERR (svn_utf_cstring_from_utf8(&comment_amiga,comment,pool));
  ...
  if(!SetComment((char *)fname_amiga,(char *)comment_amiga))
  {
    char fault_string[100];
    const char * fault_string_utf8;
    ...
    return svn_error_wrap_apr (EIO, "Can't change comment of '%s' to '%s' (%s)", fname, comment, fault_string_utf8);
  }

...
typedef struct
{
  ...
  /* when amiga_protection is not NULL the hash contains amiga:protection */
  const char * amiga_protection;

  /* when amiga_comment is not NULL the hash contains amiga:comment */
  const char * amiga_comment;
...
#if defined(AMIGA)
          else if (strcmp (property, "amiga:protection") == 0)
            autoprops->amiga_protection = this_value;
          else if (strcmp (property, "amiga:comment") == 0)
            autoprops->amiga_comment = this_value;
#endif /* AMIGA */
        }
                            ...
                            apr_pool_t *pool)
{
...
#if defined(AMIGA)
  autoprops.amiga_protection = NULL;
  autoprops.amiga_comment = NULL;
#endif /* AMIGA */
  ...
  *mimetype = autoprops.mimetype;
  ...
  if (! autoprops.amiga_protection)
    {
      SVN_ERR (svn_io_amiga_protection (&autoprops.amiga_protection, path, pool));
      if (autoprops.amiga_protection)
        apr_hash_set (autoprops.properties, "amiga:protection",
                      strlen ("amiga:protection"), 
                      svn_string_create (autoprops.amiga_protection, pool));
    }

  if (! autoprops.amiga_comment)
    {
      SVN_ERR (svn_io_amiga_comment (&autoprops.amiga_comment, path, pool));
      if (autoprops.amiga_comment)
        apr_hash_set (autoprops.properties, "amiga:comment",
                      strlen ("amiga:comment"), 
                      svn_string_create (autoprops.amiga_comment, pool));
    }

  ...
  for (hi = apr_hash_first (pool, dirents); hi; hi = apr_hash_next (hi))
    {
      ...
      if (*type == svn_node_dir)
      ...
      else if (*type == svn_node_file)
        {
...
#if defined(AMIGA)
          svn_string_t *amiga_protection, *amiga_comment;
#endif /* AMIGA */
          const char *eol = NULL;
          ...
          amiga_protection = apr_hash_get (props, "amiga:protection", APR_HASH_KEY_STRING);
          if (amiga_protection)
            SVN_ERR (svn_io_amiga_set_protection(copy_to, amiga_protection->data, iterpool));

          amiga_comment = apr_hash_get (props, "amiga:comment", APR_HASH_KEY_STRING);
          if (amiga_comment)
            SVN_ERR (svn_io_amiga_set_comment(copy_to, amiga_comment->data, iterpool));
...
#if defined(AMIGA)
  const svn_string_t *amiga_comment_val;
  const svn_string_t *amiga_protection_val;
#endif /* AMIGA */
                  ...
                  apr_pool_t *pool)
{
  ...
  else if (strcmp (name, "amiga:protection") == 0)
    fb->amiga_protection_val = svn_string_dup (value, fb->pool);

  else if (strcmp (name, "amiga:comment") == 0)
    fb->amiga_comment_val = svn_string_dup (value, fb->pool);
            ...
            apr_pool_t *pool)
{
  ...
  if (fb->amiga_protection_val)
    SVN_ERR (svn_io_amiga_set_protection(fb->path, fb->amiga_protection_val->data, pool));

  if (fb->amiga_comment_val)
    SVN_ERR (svn_io_amiga_set_comment(fb->path, fb->amiga_comment_val->data, pool));

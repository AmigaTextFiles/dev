...
#ifdef AMIGA
	/* Use the built-in SSHv1 client code. */
	if(strcmp(args[0],"ssh") == 0)
	{
		struct ssh_protocol_context * ssh;

		/* This opens the SSHv1 tunnel and starts the svnserve command. */
		ssh = amiga_ssh(args);
		if(ssh != NULL)
		{
			apr_socket_t * sock;

			/* Cook up an APR socket wrapper which contains the SSH session handle. */
			status = apr_amiga_ssh_socket_create(&sock,ssh,pool);
			if(status != APR_SUCCESS)
				return svn_error_wrap_apr(status, NULL, "Can't create ssh tunnel wrapper");
		...
		{
			/* ZZZ probably not such a good error code. */
			return SVN_ERR_RA_NOT_AUTHORIZED;
		}
	}
#endif /* AMIGA */

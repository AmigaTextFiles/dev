                 ...
                 apr_pool_t *pool)
{
  ...
  if (strcmp (name, "amiga:protection") == 0)
    {
      /* If you remove the effects of the "amiga:protection" property,
         then the file access permissions set by other properties
         should remain. */
      if(value != NULL)
        SVN_ERR (svn_io_amiga_set_protection(path, value->data, pool));
    }

  if (strcmp (name, "amiga:comment") == 0)
    {
      /* A NULL comment string is treated like an empty comment string. */
      SVN_ERR (svn_io_amiga_set_comment(path, (value != NULL) ? value->data : "", pool));
    }

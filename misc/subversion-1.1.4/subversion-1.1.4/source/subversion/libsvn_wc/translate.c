...
svn_error_t *
svn_wc__maybe_set_amiga (svn_boolean_t *did_set,
                         const char *path,
                         svn_wc_adm_access_t *adm_access,
                         apr_pool_t *pool)
{
  ...
  SVN_ERR (svn_wc_prop_get (&propval, "amiga:protection", path, adm_access,
                            pool));

  if (propval != NULL)
    {
      SVN_ERR (svn_io_amiga_set_protection (path, propval->data, pool));
      if (did_set)
        *did_set = TRUE;
    }

  SVN_ERR (svn_wc_prop_get (&propval, "amiga:comment", path, adm_access,
                            pool));

  if (propval != NULL)
    {
      SVN_ERR (svn_io_amiga_set_comment (path, propval->data, pool));
      if (did_set)
        *did_set = TRUE;
    }

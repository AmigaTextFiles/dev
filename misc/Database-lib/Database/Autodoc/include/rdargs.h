@DATABASE "rdargs.h"
@MASTER   "include:joinOS/dos/rdargs.h"
...
@{"CSource" LINK File 106}  @{"RDArgs" LINK File 146}


@{b}#defines@{ub}

@{"ARG_BUFFER_SIZE" LINK File 63}  @{"MAX_MULTIARGS" LINK File 73}  @{"MAX_TEMPLATE_ITEMS" LINK File 69}  @{"RDAB_DATE_CDN" LINK File 174}
@{"RDAB_DATE_DEF" LINK File 176}    @{"RDAB_DATE_INT" LINK File 170}  @{"RDAB_DATE_USA" LINK File 172}       @{"RDAB_FUTURE" LINK File 178}
@{"RDAB_NOALLOC" LINK File 158}     @{"RDAB_NOPROMPT" LINK File 160}  @{"RDAB_STDIN" LINK File 156}          @{"RDAF_DATE_CDN" LINK File 175}
@{"RDAF_DATE_DEF" LINK File 177}    @{"RDAF_DATE_INT" LINK File 171}  @{"RDAF_DATE_USA" LINK File 173}       @{"RDAF_FUTURE" LINK File 179}
@{"RDAF_NOALLOC" LINK File 159}     @{"RDAF_NOPROMPT" LINK File 161}  @{"RDAF_STDIN" LINK File 157}          
 ...
 * parameters are only available for the @{"ParseArgs()" LINK "/joinOS.guide/ParseArgs"} function NOT for
 * ReadArgs().
 * So use @{"ParseArgs()" LINK "/joinOS.guide/ParseArgs"}, @{"FreeArguments()" LINK "/joinOS.guide/FreeArguments"}, @{"ParseItem()" LINK "/joinOS.guide/ParseItem"}, and @{"FindArgument()" LINK "/joinOS.guide/FindArgument"} instead
 ...
 *       structure passed to @{"ParseArgs()" LINK "/joinOS.guide/ParseArgs"}:
 *          @{"RDAF_DATE_INT" LINK File 171} - international format "yy-mm-dd" (e.g. "01-05-12")
 *          @{"RDAF_DATE_USA" LINK File 173} - US-American format "mm-dd-yy" (e.g. "05-12-01")
 *          @{"RDAF_DATE_CDN" LINK File 175} - Canadian format "dd-mm-yy" (e.g. "12-05-01")
 *          @{"RDAF_DATE_DEF" LINK File 177} - default format of current locale used (currently
 *                         not supported, defaults to DOS format).
 *       As a result of this, a date string has to be entered always in DOS
 *       format if no RDArgs structure is passed to @{"ParseArgs()" LINK "/joinOS.guide/ParseArgs"}.
 *
 ...
 *
 * Look for the Autodoc of @{"ParseArgs()" LINK "/joinOS.guide/ParseArgs"} or ReadArgs() for details about the
 ...
 * passed as @{"struct CSource" LINK File 106}* is defined as follows:
 ...
 * used:
 *
 * long CS_ReadChar (@{"struct CSource" LINK File 106} *CSource)
 * {
 ...
 * }
 *
 * BOOL CS_UnReadChar (@{"struct CSource" LINK File 106} *CSource)
 * {
 ...
 * }
 *
 * To initialize a @{"struct CSource" LINK File 106}, you set CSource->CS_Buffer to a string which
 ...
 * additional space of @{"MAX_MULTIARGS" LINK File 73} * 4 bytes.
 *
...
struct RDArgs
{
   @{"struct CSource" LINK File 106} RDA_Source;    /* Select input source */
...
#define RDAF_STDIN (1L<<@{"RDAB_STDIN" LINK File 156})  /* set, this is the default behaviour) */
#define RDAB_NOALLOC    1  /* RDA_Buffer is allocated from user-application */
#define RDAF_NOALLOC    (1L<<@{"RDAB_NOALLOC" LINK File 158})
#define RDAB_NOPROMPT   2     /* don't prompt template or extended help */
#define RDAF_NOPROMPT   (1L<<@{"RDAB_NOPROMPT" LINK File 160})

#endif      /* _AMIGA */

/* The next flags are specifying, how @{"ParseArgs()" LINK "/joinOS.guide/ParseArgs"} (NOT ReadArgs()) will parse
...
#define RDAF_DATE_INT (1L<<@{"RDAB_DATE_INT" LINK File 170})
#define RDAB_DATE_USA   26 /* dates must be passed in format "mm-dd-yy" */
#define RDAF_DATE_USA (1L<<@{"RDAB_DATE_USA" LINK File 172})
#define RDAB_DATE_CDN   27 /* dates must be passed in format "dd-mm-yy" */
#define RDAF_DATE_CDN (1L<<@{"RDAB_DATE_CDN" LINK File 174})
#define RDAB_DATE_DEF   28 /* date format from locale (currently not supported*/
#define RDAF_DATE_DEF (1L<<@{"RDAB_DATE_DEF" LINK File 176})    /* , defaults to DOS date format)*/
#define RDAB_FUTURE     29 /* weekdays refer to future */
#define RDAF_FUTURE (1L<<@{"RDAB_FUTURE" LINK File 178})

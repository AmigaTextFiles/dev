 ...
 *	    All Rights Reserved
 *
 * Modified for use on Windoof systems 2001 Peter Riede
 */
 ...
 *
 * FUNCTION
 *		Display an alert with the according alert-number.
 *		If the desired alert is of type AT_DeadEnd or the user selects "Cancel"
 *		("Reboot" on AmigaOS) the application terminates.
 *
 * INPUT
 *		AlertNumber - the number of the alert, that should be displayed to the
 *							user, if the most significant bit is set, the alert is of
 *							the type DeadEnd and the application terminates after the
 *							alert window is closed.
 *							See "Alerts.h" for predefined alert numbers.
 * RESULT
 *		The function has no result, in most cases the function never returns.
 *
 * NOTE 
 *		All open FileLocks (and on AmigaOS FileHandles) stays open if the program
 *		terminates via an alert, so it's better to reboot the machine after an
 *		alert occured.
 */

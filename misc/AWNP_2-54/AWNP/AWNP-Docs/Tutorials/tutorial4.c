...
/*We pass 'GP' alot but it allows for multiple GUI's in a single aplication*/
 ...
 int topipe(struct GUIpipe * GP, UBYTE * data,...);
 ...
 int showtx(int delay,UBYTE * text,...);
 int iconify(struct GUIpipe * GP);
 int appdrop(struct GUIpipe * GP);
 int readfile(UBYTE * fn);
 int setenv(struct GUIpipe * GP);
 int setenvarc(struct GUIpipe * GP);
 int unsetenvarc(struct GUIpipe * GP);
 int readenv(VOID);
...
/*NEW FOR TUTORIAL4 remember if the gui is iconified or not*/
short iconified;
...
/*for env snapshots*/
UBYTE * envname="env:GUITutorial4", * envarcname="envarc:GUITutorial4";
int wintop,winleft,winhigh,winwide;
       ...
       switch(GP->str1[1]+(GP->str1[0]<<8))
        {
         ...
         break;
         case('cl'):
         showtx(200,"User Closed Window or used CTRl\\");
         stop=1;
         break;
        }
      }
...
/* Open pipe 'tut3' with GUI creation option '/xc' */
   if(!( GP->file=Open("awnpipe:tut4/xc",MODE_OLDFILE))) return(1);
...
childlabel (chl) keyword. */
   ...
   topipe(GP," le\n");
   topipe(GP," menu gt \"Project|Window|$@SSnapshot|$@UUnSnapshot|-|About\"\n");
...
/* disable the save gadget if we do not have a name, enable it if we do*/
   ...
   if(a==cangad)
    {
     showtx(200,"User Canceled\n");
     return(1);
    }
   if(a==dongad)
    {
     showtx(0,"name: %s*n age: %ld*n sex: %ld*nknowledge: %ld*n basic: %ld arexx: %ld c: %ld asm: %ld ",
 name,age,sex,knowledge,basic,arexx,c,asm);
     return(1);
    }
   return(0);
  }
 ...
 int menu(struct GUIpipe * GP)
{
   if(GP->val2==0)
  {
if (GP->val3==2)showtx(500," Tutorial 4 for AWNP *n  by William Parker");
if (GP->val3==0){
if (GP->val4==0) setenvarc(GP);
if (GP->val4==1) unsetenvarc(GP);
}
}
   if(GP->val2==1)
    {
     if(GP->val3==0)
      {
     showtx(200,"name: %s*n age: %ld*n sex: %ld*nknowledge: %ld*n basic: %ld arexx: %ld c: %ld asm: %ld ",
 name,age,sex,knowledge,basic,arexx,c,asm);
      }
     ...
     if(GP->val3==1)
      {
       if(GP->val4==0)
        {
         showtx(200,"name %s age %ld sex %ld", name,age,sex);
        }
       if(GP->val4==1)
        {
         showtx(200,"knowledge %ld basic %ld arexx %ld c %ld asm %ld",
 ...
 int saveform()
  {
   UBYTE buf[200];
   BPTR fh;
   if (strlen(name)&&strlen(name)<192){
   strcat(strcat(strcpy(buf,"t:"),name),".tut3");
...
{
BPTR win;
if(win= Open("awnpipe:tut4txt/xc",MODE_READWRITE))
{
FPrintf(win,"db dg \"Tutorial 4\" q cg cm m a so si\nlabel lj gt \"");
VFPrintf(win,text,(APTR)(4+(int)(&text)) );
FPrintf(win,"\"\nopen\n");
if (delay)FPrintf(win,"tick %ld\n",delay);
else FPrintf(win,"m\n");
Close(win);
}
return(0);
...
/* Make sure we have a quoted file name (should always be true..but)
and skip/remove the quotes.*/
if(fn[strlen(fn)-1]='\"'){
fn[strlen(fn)-1]=0;
fn++;
}
...
{
iconified=GP->val2;
if (GP->val2==1) topipe(GP,"id 0 s 32\n");
else  topipe(GP,"id 0 s 64\n");
return(0);
}

/* we do not use topipe() since we dont want error checking on the special
return from this modify command. This is a special case */
int  setenv(struct GUIpipe * GP)
{
BPTR fh;
   FPrintf(GP->file,"id 0 read\n");
   if (!getline(GP)) {
  if( fh=Open(envname,MODE_NEWFILE)){
Write(fh,GP->buf,strlen(GP->buf));
  Close(fh);
return(1);
}
}
...
BPTR fh;
if(setenv(GP)){
/* gp->buf already holds the info we want to save*/
 if( fh=Open(envarcname,MODE_NEWFILE)){
Write(fh,GP->buf,strlen(GP->buf));
  Close(fh);
return(1);
}
...
UBYTE buf[100];
int in;
 if( fh=Open(envname,MODE_OLDFILE)){
in=Read(fh,buf,99);
buf[in]=0;
Close(fh);
return(sscanf(buf,"%ld %ld %ld %ld",&winleft,&wintop,&winhigh,&winwide));
}
return(0);
}

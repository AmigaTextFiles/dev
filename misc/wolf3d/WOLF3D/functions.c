...
extern struct xBUFFTYPE xBUFFER[ViewRight+1];
...
void CreateXbuffer( word zCount)
{
    ...
    for(index = ViewLeft; index <= ViewRight; index++) xBUFFER[index].height = 0;
	...
	for(wall = N; wall <= W; wall++)
	{
		...
		if( ((cX*aZ)+(cZ*aX)) > 0)
		{
		    ...
		    if(z > 0)
		    {
			x = ( x<< 9)/(z+z+z) + ViewCenterX;
			lastY = (40<<14)/(z+z+z);
		    }
		    else
		    {
			x = x + ViewCenterX;
			lastY = ViewBottom-ViewCenterY;
		    };
			...
			if(z > 0)
			{
				...
				if(xBUFFER[loop].height < lastY)
				{
				    ...
				    xBUFFER[loop].bitmap = (long *)Wall[lastBMP];
				};
			    };
			};

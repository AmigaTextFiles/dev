...
main (int argc, char **argv)
{
  ...
  #ifdef AMIGA
  {
    char * base_name = FilePart(program_name);

    if(strcasecmp(base_name,"aid") == 0)
    {
      ignore_case_flag = REG_ICASE;
      pattern_style = ps_literal;
      delimiter_style = ds_substring;
    }
    else if (strcasecmp(base_name,"eid") == 0)
    {
      result_style = parse_result_style ("edit");
    }
    else if (strcasecmp(base_name,"gid") == 0)
    {
      result_style = parse_result_style ("grep");
    }
  }
  #endif /* AMIGA */
...
editit:
  ...
  switch (fork ())
	...
	*(char const **) argv = editor;
	...
	parms = p = MALLOC(char *,total_size);
	...
	for(i = 0 ; argv[i] != NULL ; i++)
	{
	  ...
	  name = argv[i];
	  if(__translate_unix_to_amiga_path_name(&name,&nti) != 0)
	    error (0, errno, _("can't translate `%s'"), argv[i]);
...
alloca_trap(void)
{
	fprintf(stderr,"%s: %s\n",__program_name,strerror(ENOMEM));
	abort();
}

/******************************************************************************/

void (*__alloca_trap)(void) = alloca_trap;
...
get_default_stack_size(void)
{
	...
	if(GetVar("IDUTILS_STACK_SIZE",buffer,sizeof(buffer)-1,0) > 0)
	{
		LONG n;
...
char * __stack_size_string = "$STACK: 50000";
unsigned int __stack_size = 50000;

/******************************************************************************/

const char version_tag[] = "$VER: lid 3.2 (28.11.2005) Amiga Port 1.2 by Olaf Barthel <olsen@sourcery.han.de>\r\n";

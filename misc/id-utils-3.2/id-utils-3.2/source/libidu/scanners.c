...
static char *
get_language_map_file_name(void)
{
  static char map_file_name[1024];

  if(map_file_name[0] == '\0')
  {
    if(GetVar("ID-LANG.MAP",map_file_name,sizeof(map_file_name)-1,0) <= 0)
      strcpy(map_file_name,LANGUAGE_MAP_FILE);
  }
...
#else
  if (file_name == 0)
    file_name = get_language_map_file_name();
#endif /* AMIGA */
...
#else
	    next_ptr = parse_language_map_file (get_language_map_file_name(), next_ptr);
#endif /* AMIGA */
	  else
	    {

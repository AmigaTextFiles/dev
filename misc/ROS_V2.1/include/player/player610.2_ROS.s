...
* modified for use with ros.library by TIK/RETIRE

* P61 initialization bug fixed, thanks to PABLO/RETIRE

**
**	$VER: player610.2_ROS.s 1.21 (05.02.96)
**	Includes Release 2.0
**


********************************
* do not change system flag!!! *
********************************
...
lev6 = 1	;0 = NonLev6
	...
	include player/Player61.i

*-----------------------------------------------*

	printt	""
	printt	"P61A options used:"
	printt	"------------------"
	ifd	start
...
; parameters modified for ROS usage
...
.modok	basereg	P61_cn,a3
	...
	move.w	d1,-(sp)		; timer a/b, modified for ROS usage
	ifne	CIA
	move	d0,-(sp)
	endc
...
;	ifeq	opt020
	...
	move.l	(a5,d4),d6
;	else
;	add	d4,d4			; forget this 4 cycle optimization
;	move.l	(a5,d4*8),d6
;	endc
	move.l	d6,-4(a4)
	move	4(a5,d4),d4		; buggy offset if 020 optimization
	sub.l	d4,a6
	sub.l	d4,a6
	bra.b	P61_jatk
...
.cl	move	d1,(a1)+
	...
	moveq	#channels-1,d0
.l	move.l	a2,P61_Sample(a1)
	add.w	#Channel_Block_SIZE,a1
	dbf	d0,.l
...
;P61_gottimer
	...
	move.w	(sp)+,d3		; timer a/b
	add.w	d3,d3
	add.w	d3,d3

	lea	P61_craddr+8(pc),a0
...
P61_periods
	ifne	P61_ft
	incbin	player/periods
	else
	incbin	player/periods.nft
	endc
...
P61_vibtab	incbin	player/vibtab
	endc

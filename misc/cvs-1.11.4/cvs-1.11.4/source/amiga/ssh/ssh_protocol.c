 ...
 * Olaf Barthel <olsen@sourcery.han.de> and
 * Jens Langner <Jens.Langner@light-speed.de>
 *
 * While some of this code is Amiga-specific, namely the use of the
 * shared 'bsdsocket.library', it is quite portable all in itself.
 * Just remove those Amiga-specific header files and you should be
 * able to use this code in just about any other configuration.
 *
 *
...
#define select(num_fds,read_fds,write_fsd,except_fsd,timeval) \
	WaitSelect(num_fds,read_fds,write_fsd,except_fsd,timeval,NULL)

char *amiga_strerror(int n);

#define strerror(n) amiga_strerror(n)
...
exchange_identification(struct ssh_protocol_context *spc)
{
	...
	{
		fprintf(stderr,"SSH: Error reading public key packet (%d, %s).\n",errno,strerror(errno));
		result = n;
		goto out;
	}
...
create_keys(struct ssh_protocol_context *spc)
{
	...
	makekey(spc->spc_PacketIn.body + 8 + i, &hostkey, &keystr2);
...
write_packet(struct ssh_protocol_context *spc)
{
	...
	crc = crc32(spc->spc_PacketOut.data + 4, biglen - 4);

	spc->spc_PacketOut.data[biglen + 0] = (unsigned char) ((crc >> 24) & 0xFF);
	spc->spc_PacketOut.data[biglen + 1] = (unsigned char) ((crc >> 16) & 0xFF);
	spc->spc_PacketOut.data[biglen + 2] = (unsigned char) ((crc >>  8) & 0xFF);
	spc->spc_PacketOut.data[biglen + 3] = (unsigned char) ( crc        & 0xFF);

...
LONG FileLength(file)  BPTR file;  {
    ...
    if (file > 0)  {
	IDOS->Seek(file, 0, OFFSET_END); /* Seek to end of file.*/
	fileLength = IDOS->Seek(file, 0, OFFSET_CURRENT);
...
IFFP OpenRIFF(file0, new0, clientFrame)
	...
	IDOS->Seek(file, 0, OFFSET_BEGINNING);  /* Go to file start. */
...
IFFP SkipFwd(context, bytes)   GroupContext *context;  LONG bytes; {
    ...
    if (bytes > 0) {
	if (-1 == IDOS->Seek(context->file, bytes, OFFSET_CURRENT))
...
ID GetChunkHdr(context0)   GroupContext *context0;  {
    ...
    /* Read the chunk header (finally). */
        ...
        switch (
	    IDOS->Read(context->file, (BYTE *)&context->ckHdr, sizeof(ChunkHeader))
	    ) {
...
IFFP IFFReadBytes(context, buffer, nBytes)
	...
	switch ( IDOS->Read(context->file, buffer, nBytes) ) {

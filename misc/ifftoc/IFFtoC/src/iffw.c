...
IFFP OpenWIFF(file, new0, limit)  BPTR file; GroupContext *new0; LONG limit; {
    ...
    if (0 > IDOS->Seek(file, 0, OFFSET_BEGINNING))	  /* Go to start of the file.*/
	iffp = DOS_ERROR;
...
IFFP PutCkHdr(context0, ckID, ckSize)
    ...
    if (0 >
	IDOS->Write(context->file, (BYTE *)&context->ckHdr, sizeof(ChunkHeader))
	)
	return(DOS_ERROR);
...
IFFP IFFWriteBytes(context0, data, nBytes)
    ...
    if (0 > IDOS->Write(context->file, data, nBytes))
	return(DOS_ERROR);
...
IFFP PutCkEnd(context0)  GroupContext *context0; {
	...
	if ( 0 >
IDOS->Seek(context->file, -(context->bytesSoFar + sizeof(LONG)), OFFSET_CURRENT) ||
	     0 >
IDOS->Write(context->file, (BYTE *)&context->bytesSoFar, sizeof(LONG))  ||
	     0 >
IDOS->Seek(context->file, context->bytesSoFar, OFFSET_CURRENT)  )
	    return(DOS_ERROR);
	}
	...
	if ( 0 > IDOS->Write(context->file, (BYTE *)&zero, 1) )
	    return(DOS_ERROR);
	context->position += 1;
	};

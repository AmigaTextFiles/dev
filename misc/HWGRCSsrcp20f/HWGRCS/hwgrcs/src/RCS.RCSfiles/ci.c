...
1.3
date	96.03.25.06.24.44;	author heinz;	state Exp;
branches;
next	1.2;
...
1.3
log
@A true RCS 5.7 bug. SEEK_CUR instead of SEEK_SET was used.
@
text
...
#ifdef _AMIGA
char HWGversion[] = "$" "VER: " PRGPREFIX "ci " PRGVERSION "." PRGREVISION " (" PRGDATE ")" PRGVERTEXT;
#endif /* _AMIGA */
mainProg(ciId, "ci", "$Id: ci.c 1.2 1996/03/03 10:57:16 heinz Exp heinz $")
{
...
#ifdef _AMIGA
/* For the newdelta, we will increment the C= style revision number
   unless the keepflag is set or we don't keep alocked working file.
*/
        CBMRevIncDelta = (!forceciflag || !lockflag || keepflag) ? NULL : &newdelta;
#endif /* _AMIGA */
...
#		    if !has_fflush_input && !(large_memory && maps_memory)
#ifdef _AMIGA
/* BUG: We want to reinstate the position that we got a few lines above.
   This means that we have to use SEEK_SET! SEEK_CUR is wrong.
*/
			if (lseek(wfd, wfd_off, SEEK_SET) == -1)
#else
...
#ifdef _AMIGA
/* This is tricky for reference directory support! If we have a reference
   directory set, we behave like "-u" or "-l" was specified. This leaves us
   with a working copy that we can move to our reference directory and make
   read only. After we are done with that, we check keepworkingfile again
   and delete it then if necessary.
*/

        if (!keepworkingfile && !AMIGA_refdirptr) { /* } Fool brace matching */
#else
        if (!keepworkingfile) {
#endif /* _AMIGA */
		Izclose(&workptr);
		...
		if (changework  ||  (r=fixwork(newworkmode,mtime)) != 0) {
			...
			case 1:
			    ...
			    aflush(exfile);
#ifdef _AMIGA
                /* Now this is a little tricky. With the new keyword
                   'AmigaProtection', the expansion of the workfile can
                   change the filemode in RCSstat due to a use change. So
                   we need to reevaluate the newworkmode now to use any new
                   settings found by the keyword expansion.
                */
                newworkmode = WORKMODE(RCSstat.st_mode,
                        !   (Expand==VAL_EXPAND  ||  lockthis < StrictLocks)
                );
...
#ifdef _AMIGA
/* This is the second part of reference directory support! Here we actually
   create the file in the reference directory and remove the working file
   if necessary.
*/
        if(AMIGA_refdirptr)
        {
            /* Here we need to create the reference file! We do it in a
               rather simple fashion for now. I know this can fail in
               various ways, but hacking further into RCS seems to be
               extremely dangerous! */
            char buf[BUFSIZ];
            FILE *fpin = fopen(workname, "r");
            ...
            if(fpin)
            {
                ...
                /* Create the tmp destination filename! A little hack! */
                strcpy(buf, AMIGA_refdirptr);
                l = strlen(buf);
                if(l && buf[l - 1] != '/' && buf[l - 1] != ':')
                {
                    strcat(buf, "/");
                } /* if */
                strcat(buf, basefilename(workname));
                strcat(buf, "_");
                ...
                if(strlen(buf) > 1)
                {
                    ...
                    if(fpout)
                    {
                        ...
                        /* Rather simple, ain't it? Due to a large buffer
                           forced by hwgunix.lib, this should be acceptable. */
                        ...
                        /* The buffer needs to hold two filenames.
                           The temp name with the '_', and the destination
                           name without it!
                        */
                        l = strlen(buf);
                        s = buf + l + 1;
                        ...
                        /* Now rename it to the real reference file! */
                        r = chnamemod(NULL, buf, s, 1, WORKMODE(RCSstat.st_mode, 0), mtime);
                    }
                    else
                    {
                        warn("could not create file in reference directory!");
                    } /* if */
                }
                else
                {
                    warn("could not create temporary name for file!");
                } /* if */
            ...
            {
                warn("could not access workfile");
            } /* if */

            /* Honor the users preference! */
            if(!keepworkingfile && !r)
            {
                r = un_link(workname);
            } /* if */
        } /* if */
...
d287 1
a287 1
mainProg(ciId, "ci", "$Id: ci.c 1.1 1996/03/02 16:37:23 heinz Exp heinz $")
d741 6
d748 1
@

...
libId(utilId, "$Id: rcsutil.c 1.1 1996/03/02 16:37:23 heinz Exp heinz $")
...
#ifdef _AMIGA
/* BUG! ptrdiff_t != int */
            VOID write(STDERR_FILENO, buf, (int)(b - buf));
#else
...
/* As the Amiga does not have fork(), we have to do it differently. I use
   ...
   SAS/C, escpecially with I/O and stderr handling. If you like it, try it.
*/
        ...
        wstatus = AMIGArunv(infd, outname, &args[1]);
#else
#if has_spawn
	int in, out;
	char const *file;
...
#define RCSINITNAME     "RCS/INIT"
        if(!q)
        {
            ...
            strcat(buf, "_");
            strcat(buf, basefilename(argv[0]));

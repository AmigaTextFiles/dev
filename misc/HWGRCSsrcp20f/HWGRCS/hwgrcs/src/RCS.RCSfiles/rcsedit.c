...
libId(editId, "$Id: rcsedit.c 1.1 1996/03/02 16:37:23 heinz Exp heinz $")
...
/* For CBMVersion, we don't need to add anything as this is handled
   separately in expandline(). This is ugly stuff here.
*/
            if (exp != VAL_EXPAND && marker != CBMVersion)
#else
	    if (exp != VAL_EXPAND)
#endif /* _AMIGA */
		afputc(' ', out);
...
/* rcssuffix may well return 0 and this is not caught without
   ...
   So we need to do something about it. We chose the least destructive
   option.
*/
        if(!x)
        {

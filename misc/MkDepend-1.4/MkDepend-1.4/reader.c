/* $Id: reader.c,v 1.4 1997/11/09 23:42:46 lars Exp $ */
...
**---------------------------------------------------------------------------
** 10-Sep-95 [lars]
** 13-Oct-95 [lars] reader_copymake2() new, to copy the file remainder.
** 04-Feb-96 [lars] reader_openrw() now allows immediate writing.
**---------------------------------------------------------------------------
** $Log: reader.c,v $
** Revision 1.4  1997/11/09  23:42:46  lars
...
** from the data.
**
** Revision 1.3  1997/11/08  19:02:01  lars
** Added the patches and bugfixes submitted by Flavio Stanchini.
** Updated my address.
**
** Revision 1.2  1996/02/25  20:43:33  lars
** Put under RCS.
**
**---------------------------------------------------------------------------
*/
...

{
  ...
  if (pIName)
  {
    if (pch > pLimit)
      pch = pLimit;
    memcpy(aBuffer, pIName, pLimit-pIName);
    pch -= (pLimit-pIName);
  }
  else
    pch = aBuffer;
  iRead = read(fdFile, pch, aBuffer+BUFSIZE-pch);
  if (iRead >= 0)
  {
    pLimit = pch+iRead;
    *pLimit = '\n';
...

{
  ...
  assert(fdFile2 < 0);
  reader_init();
  fdFile2 = open(pNameW, O_WRONLY|O_CREAT|O_TRUNC);
  if (fdFile2 < 0)
    return 1;
  if (pNameR)
  {
 ...
 *   errno set to an error code in case.
 */
...
#if 0
  /* For debugging purposes: the states as printable strings. */
  char   *stab[]
    = {
        "Start "
      , "InLine"
...
{
  if (pch > aBuffer && write(fdFile2, aBuffer, pch-aBuffer) != pch-aBuffer)
    return 1;
  pch = aBuffer;
  return 0;
}
...
/*-------------------------------------------------------------------------*/
int
reader_writen (const char *pText, int len)
...

{
  assert(pText);
  assert(fdFile2 >= 0);
  if (pLimit-pch < len && reader_writeflush())
    return 1;
...
/*-------------------------------------------------------------------------*/
int
reader_copymake (const char * pTagline, int bExclude)

/* Copy the file from fdFile to fdFile2 up to the tagline.
 * If <bExclude> is FALSE, the tagline is copied as well, of if it wasn't
...

{
  ...
  while (!END_OF_FILE() && count < len)
  {
    if (END_OF_BUF())
    {
      if (write(fdFile2, aBuffer, pch-aBuffer) != pch-aBuffer)
        return 1;
      if (fillbuffer())
        return 1;
      if (END_OF_FILE())
  ...
  if (!END_OF_FILE())
  {
    ...
    if (pState)
    {
      ...
      memcpy(pState->buf, pch, pState->len);
    }
  }

  /* If the tagline was found, but is to be excluded, backoff */
  if (bExclude)
    pch -= len;
  ...
  if (count < len && !bExclude && reader_writen(pTagline, len))
    return 1;
...
/*-------------------------------------------------------------------------*/
int
reader_copymake2 (const char * pTagline, int bExclude)

/* Copy the remainder from fdFile to fdFile2, starting with the tagline
 * if bExlude is FALSE, or starting after the tagline if bExclude is true.
 * The tagline MUST end in a newline character!
 * Return 0 on success, non-0 on error (errno holds the error code then).
 */

{
  ...
  if (!bExclude && reader_write(pTagline))
    return 1;

  /* Flush the buffer before we destroy it by restoring the old state */
  ...
  memcpy(pch, pState->buf, pState->len);
  free(pState);
  pState = NULL;
  pIName = NULL;
  ...
  do
  {
    if (!END_OF_BUF() && write(fdFile2, pch, pLimit-pch) != pLimit-pch)
      return 1;

...
static const char VerString[] = "\0$VER: example_68kgate.library " PROGRAM_VER " "COMPILE_DATE;
static const char LibName[]   = "example_68kgate.library";
...
struct Library  *LibrarySysVBase = NULL;
...
static BPTR LIB_Close(void)
{
	...
	if (LibBase->Library.lib_OpenCnt == 0)
	{
		CloseLibrary(LibrarySysVBase);

		LibBase->Initialized = 0;
		LibrarySysVBase      = NULL;
...
static struct Library *LIB_Open(void)
{
	...
	if (LibBase->Initialized == 0)
	{
		if (LibrarySysVBase = OpenLibrary("example_sysv.library", 0))
		{
			LibBase->Initialized = 1;
		}
		else
		{
			LibBase->Library.lib_OpenCnt--;
	...
	return LibBase->Initialized ? &LibBase->Library : NULL;
}

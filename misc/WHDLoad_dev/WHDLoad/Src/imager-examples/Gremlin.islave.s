		...
		; Gremlin Imager (Barfly assembler source)
		;
		; Written by Codetapper/Action in 1999!
		;
		; Gremlin uses the same disk format on the majority of
		; their games. Examples include Super Cars 1 and 2,
		; Switchblade 2, Zool, Zool 2 and many more.
		;
		; Track 1 of disk 1 is often a copy protection track, 
		; usually with the sync $4124. On some games it is a
		; standard Dos track. Change line 68 as required for
		; each game as you analyse it to include/skip tracks 0-1.
		;
		; Disk 1: Tracks 000-000: Sync ($4489), length $1600 bytes (Dos)
		;	  Tracks 001-001: Sync ($4124), length $1800 bytes (Protection or Dos)
...
.passchk
		...
		dc.b	"$VER: "
Text		dc.b	"Gremlin imager V1.0",10

/*
sc:c/sc opt txt/DecodeMACE.c
*/

#include "Decode.h"

/* /// "Daten" */
ushort MACEtab1[] = { 0xfff3, 0x0008, 0x004c, 0x00de, 0x00de, 0x004c, 0x0008, 0xfff3 };

ushort MACEtab3[] = { 0xffee, 0x008c, 0x008c, 0xffee };
...
/* /// "Exp1to3()" */
void Exp1to3(const uchar *inBuffer,
             void *outBuffer,
             ulong cnt,
             ulong numChannels,
             ulong whichChannel)
...
*/
   ...
   while (cnt>0) {
     pkt=inBuffer[0];
     chomp3( pkt       & 7, MACEtab1, MACEtab2);
     chomp3((pkt >> 3) & 3, MACEtab3, MACEtab4);
     chomp3( pkt >> 5     , MACEtab1, MACEtab2);
     pkt=inBuffer[1];
     chomp3( pkt       & 7, MACEtab1, MACEtab2);
     chomp3((pkt >> 3) & 3, MACEtab3, MACEtab4);
     chomp3( pkt >> 5     , MACEtab1, MACEtab2);
...
*/
   ...
   while (cnt>0) {
     ...
     chomp6( pkt >> 5     , MACEtab1, MACEtab2);
     chomp6((pkt >> 3) & 3, MACEtab3, MACEtab4);
     chomp6( pkt       & 7, MACEtab1, MACEtab2);
...
/* /// "DecodeMACE3Mono()" */
__asm ulong DecodeMACE3Mono(REG(a0) uchar *from,
                            REG(a1) uchar *toL,
                            REG(a2) uchar *toR,
                            REG(d0) ulong size,
                            REG(a3) struct MACEData *spec)
{
  Exp1to3(from,toL,size/6,1,1);
  return size;
}
/* \\\ */

/* /// "DecodeMACE3Stereo()" */
__asm ulong DecodeMACE3Stereo(REG(a0) uchar *from,
                              REG(a1) uchar *toL,
                              REG(a2) uchar *toR,
                              REG(d0) ulong size,
                              REG(a3) struct MACEData *spec)
{
  Exp1to3(from,toL,size/6,2,1);
  Exp1to3(from,toR,size/6,2,2);
  return size;
}
/* \\\ */

/* /// "DecodeMACE6Mono()" */
__asm ulong DecodeMACE6Mono(REG(a0) uchar *from,
                            REG(a1) uchar *toL,
                            REG(a2) uchar *toR,
                            REG(d0) ulong size,
                            REG(a3) struct MACEData *spec)
{
  Exp1to6(from,toL,size/6,1,1);
  return size;
}
/* \\\ */

/* /// "DecodeMACE6Stereo()" */
__asm ulong DecodeMACE6Stereo(REG(a0) uchar *from,
                              REG(a1) uchar *toL,
                              REG(a2) uchar *toR,
                              REG(d0) ulong size,
                              REG(a3) struct MACEData *spec)
{
  Exp1to6(from,toL,size/6,2,1);
  Exp1to6(from,toR,size/6,2,2);
  return size;
}
/* \\\ */

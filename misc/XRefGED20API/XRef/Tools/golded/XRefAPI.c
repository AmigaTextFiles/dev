...
** $HISTORY:
**
** 02.04.95 : 001.005 : changed to GoldED2.0 API client style
...
#define REVISION 5
#define DATE "2.4.95"
#define VERS "XRefAPI 1.5"
#define VSTRING "XRefAPI 1.5 (2.4.95)\r\n"
#define VERSTAG "\0$VER: XRefAPI 1.5 (2.4.95)"
/*FE*/ /* $ENDDEFINE:   */
...
LONG RexxCmd_XRefPhrase(struct GlobalData *gd,struct APIMessage *msg)
{
   ...
   if(column && (buf[--column] > '@'))
   {
      ...
      len = 1;
      for(begin = buf + column; column && (*(begin - 1) > '@') ; --column, --begin)
         len++;
      ...
      if((XRefBase = OpenLibrary("xref.library",1)))
      {
                                  ...
                                  TAG_DONE))
         {
            if(gd->gd_Found > 1 && gd->gd_Match == MATCH_SELECT)
            {
               ...
               if((screen = LockPubScreen(msg->api_Global->F_ScrnName)))
               {
                  struct Node *node;
            ...
            if(gd->gd_Found == 1)
            {
               ...
               strncpy(gd->gd_Buffer,config->CurrentBuffer,column);
               strncpy(&gd->gd_Buffer[column],gd->gd_Name,newlen);
               strcpy(&gd->gd_Buffer[column + newlen],config->CurrentBuffer + config->Column);
               gd->gd_Modify.mr_Next   = NULL;
               gd->gd_Modify.mr_Line   = config->Line;
               gd->gd_Modify.mr_Column = column + newlen;
               gd->gd_Modify.mr_Data   = gd->gd_Buffer;
               gd->gd_Modify.mr_Size   = column + newlen + (config->CurrentLen - config->Column);

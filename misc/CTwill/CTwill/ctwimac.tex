...
\xdef\fmtversion{\fmtversion+CTWILL3.0}
...
\outer\def\NNN#1#2#3.{\gdepth=#1\gtitle={#3}\def\secno{#2}% starred section
  ...
  \else\global\setbox\partialpage=\vbox{
          \vbox to44pc{\unvbox\partialpage\vfill}\break}
...
\newcount\nn \newcount\mm \newcount\pp
\newdimen\pagethresh \pagethresh=88pc
\newdimen\nsize \newdimen\msize
...
\def\flushout{\ifvoid\partialpage\else
    ...
    \setbox0=\vbox{\eightpoint \hsize=11pc
...
\def\addneweref#1 #2 {\nn=221 \begintest{#1 #2}\anr\endtest\]#1 #2 }
...
\def\makeref #1 #2 #3#4 {\nn=#2 \hangindent=1em
  \ifsorted\else\ttoks={#1 #2 #3#4}\immediate\write\reffile{+ \the\ttoks}\fi
...
\def\readrefs{\the\prefs \mm=\secno \advance\mm-\baseno \advance\mm200
  {\def\lmda##1{\global\let##1\relax}\pp=200

...
\outer\def\M#1.{\def\modno{#1}\stsec\ignorespaces}
\outer\def\N{\ifvoid\partialpage\lefttrue
  \else\ifdim\ht\partialpage<44pc \leftfalse\else\lefttrue\fi\fi\NNN}
\outer\def\NN{\ifvoid\partialpage\leftfalse
  \else\ifdim\ht\partialpage<44pc \lefttrue\else\leftfalse\fi\fi\NNN}
\def\NNN#1.#2.{\def\modno{#1}% beginning of starred section
  \ifleft \flushout
    \gdef\rlhead{\let\i=I\uppercase{\ignorespaces#2}} % running left headline
    \global\let\rrhead=\rlhead % running right headline
  \else\global\setbox\partialpage=\vbox{
          \vbox to44pc{\box\partialpage\vfill}\break}
      \gdef\rrhead{\let\i=I\uppercase{\ignorespaces#2}}
  \fi
...
\def\twillout{\setbox\newmod=\vbox{\unvbox255}
  ...
  \ifdim\nsize>\pagethresh \flushout \message{\modno}\fi
  \addtopartialpage}
...
\newif\ifpagesaved \newif\iftitle
\def\leftheadline{\hbox to35pc{\vbox to10pt{}%
  \iftitle\global\titlefalse\else\ninesl\rhead\fi
  \hfil\eightrm\folio}}
\def\rightheadline{\hbox to35pc{\vbox to10pt{}%
  \eightrm\folio\hfil\ninesl\rhead\/}}
...
\def\preinx{\tenpoint \hsize=35pc
  \output{\ifpagesaved\normaloutput{\box\sbox}\fi
    \global\setbox\sbox=\page \global\pagesavedtrue}
  \rightskip0pt \tolerance 200
  \pagesavedfalse}
\def\inx{\vskip 15pt plus 1fil

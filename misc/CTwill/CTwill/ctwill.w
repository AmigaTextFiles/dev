...
nor even to fully update the program documentation below.

The ``banner line'' defined here should be changed whenever \.{CTWILL} is
modified. The version number parallels the corresponding version of \.{CWEAVE}.

@d banner "This is CTWILL (Version 3.0)\n"
...
in the current section (unless a temporary meaning is assigned, in
which case the \.{@@-} is redundant).
...
(End of user manual. Resume the program for \.{CWEAVE}, with occasional
...
{ char *first=id_first;
  ...
  if (*loc++!=' ')
    ...
    t=title_lookup();
    if (*(loc-1)=='}')
      while (isdigit(*loc)) n=10*n+(*loc++)-'0';
    if (*loc++!=' ')

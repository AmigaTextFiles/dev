	...
	char			*result )
{
	...
	/* If the timeout counter is still going, abort it */
...
void CloseDownARexxHost( struct RexxHost *host )
{
	...
	if( host->port )
	{
		...
		/* MsgPort leeren */
		Forbid();  	/* Forbit/Permit added by Jeremy to hopefully reduce Enforcer hitting! */
		while( rexxmsg = (struct RexxMsg *) GetMsg(host->port) )
			ReplyRexxCommand( rexxmsg, -20, (long) "Host closing down", NULL );
...
static void DoRXCommand( struct RexxHost *host, struct RexxMsg *rexxmsg )
{
	...
	BProcessResults    = TRUE;	/* signal to check args */
		
drc_cleanup:
	/* Save all info into global state struct */
	RexxState.rexxmsg  = rexxmsg;
	RexxState.rc       = rc;
	RexxState.rc2	   = rc2;
	RexxState.result   = result;
	RexxState.host     = host;
	RexxState.cargstr  = cargstr;
	RexxState.array    = array;
	RexxState.argb     = argb;
	RexxState.rxc      = rxc;	
	RexxState.resarray = resarray;
	RexxState.argarray = argarray;

	/* Nur RESULT, wenn weder VAR noch STEM */
	if (PState.uwRexxWaitMask == 0)	ReplyAndFreeRexxMsg(BProcessResults);
	return;	
}
...
void ARexxDispatch( struct RexxHost *host )
{
	...
	while(((nCount++) < 2)&&( rexxmsg = (struct RexxMsg *) GetMsg(host->port) ))
	{	

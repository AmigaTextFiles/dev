...
int main (int argc, char *argv[])
{
  ...
  nn = Neural_net2_default_constr (2,3,3,1,0,1,1.0);
  Neural_net2_set_variance (nn,atof (argv [1]));
  ...
  for (x = 0; x < num_examples; ++x)
    {
      curr_error += Neural_net2_calc_forward (nn,input [x],
                                              &desired_output [x],
                                              &number_wrong,
                                              &skip,0,&printed);
    }
  ...
  while ( (number_wrong > 0) && (epochs < 10000) )
    {
      ...
      if ( Neural_net2_generate_gaussian_offsets (nn) )
        {
          ++skip;
          continue;
        }
      ...
      for (x = 0; x < num_examples; ++x)
        {
          error += Neural_net2_calc_forward_rand_opt (nn,input [x],
                                             &desired_output [x],
                                             &number_wrong, 1);
                                       /* Direction flag  ^^^ */
        }
      ...
      if ( error < curr_error  )
        {
          ...
          Neural_net2_update_weights_with_offset (nn,1);
        }
          ...
          if ( error < curr_error )
            {
              ...
              Neural_net2_update_weights_with_offset (nn,0);
  ...
  for (x = 0; x < num_examples; ++x)
    {
      Neural_net2_calc_forward_test (nn,input [x],&desired_output [x],1,0.1,0.3);
    }
  printf ("\n\n");
}

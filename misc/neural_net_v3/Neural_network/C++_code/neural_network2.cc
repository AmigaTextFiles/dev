...
void Neural_network2::allocate_weight_matrices ()
{
...
void Neural_network2::allocate_bp_matrices ()
{
  if ( bp_flag )
    {
      ...
      hidden1_momentum = new double [num_hidden2 * num_hidden1];
      hidden2_momentum = new double [num_outputs * num_hidden2];
      ...
      hidden2_sum_delta_weight = new double [num_hidden2];
    }
...
void Neural_network2::allocate_rand_opt_matrices ()
{
  ...
  if ( rand_opt_flag )
    {
      ...
      hidden1_weights_g_offset = new double [num_hidden2 * num_hidden1];
      hidden2_weights_g_offset = new double [num_outputs * num_hidden2];

      input_weights_bias = new double [num_hidden1 * num_inputs];
      hidden1_weights_bias = new double [num_hidden2 * num_hidden1];
      hidden2_weights_bias = new double [num_outputs * num_hidden2];
    }
  else // Set them all to zero
    {
...
void Neural_network2::initialize_learning_matrices ()
{
  int    x,y;
      ...
      for (y = 0; y < num_hidden2; ++y)
        {
          if ( bp_flag )
            {
              ...
              hidden2_learning_rate [x * num_hidden2 + y] = _learning_rate;
              hidden2_momentum [x * num_hidden2 + y] = 0.0;
          ...
          if ( rand_opt_flag )
            {
              hidden2_weights_g_offset [x * num_hidden2 + y] = 0.0;
              hidden2_weights_bias [x * num_hidden2 + y] = 0.0;
            }
...
void Neural_network2::initialize_weight_matrices (double range)
{
  int    x;
  ...
  for (x = num_hidden2 * num_hidden1 - 1; x >= 0; --x)
    {
  ...
  for (x = num_outputs * num_hidden2 - 1; x >= 0; --x)
    {
...
void Neural_network2::deallocate_bp_matrices ()
{
...
void Neural_network2::deallocate_rand_opt_matrices ()
{
...
void Neural_network2::deallocate_all_matrices ()
{
...
Neural_network2::Neural_network2 (int number_inputs, int number_hidden1,
...
Neural_network2::Neural_network2 (String& filename, int& file_error,
                                int backpropagation_flag,
...
int Neural_network2::read_weights (String& filename)
{
  ifstream fp;
  int      x;
  long     iter;
  ...
  /* and output nodes. */
  fp >> num_inputs >> num_hidden1 >> num_hidden2 >> num_outputs;
...
int Neural_network2::save_weights (String& filename)
{
  ofstream fp;
  int      x;
  ...
  /* and output nodes. */
  fp << num_inputs << " " << num_hidden1 << " " << num_hidden2 << " ";
  fp << num_outputs << "\n";
...
void Neural_network2::set_size_parameters (int number_inputs,
...
int Neural_network2::backpropagation_flag (int new_flag)
{
...
int Neural_network2::random_optimization_flag (int new_flag)
{
...
double Neural_network2::learning_rate (double learning_rate)
{
  int    x;
...
void Neural_network2::set_standard_dbd_parameters ()
{
  _alpha = 0.4;
  _beta = 1.0;
  _epsilon = 0.1;
...
void Neural_network2::backpropagation (double input [],
  ...
  for (x = num_hidden2 - 1; x >= 0; --x)
    {
      ...
      /* Formula delta = SUM (previous deltas*weight)
         ...
         it for next layer.
      */
      delta = hidden2_sum_delta_weight [x] * hidden2_act [x] *
...
double Neural_network2::calc_forward (double input [], double desired_output [],
...
void Neural_network2::update_weights ()
{
  int     x,y;
  int     size;
  ...
  for (x = num_outputs - 1; x >= 0; --x)
    {
      ...
      p_momentum = &hidden2_momentum [x * size];
      p_sum_delta = &hidden2_weights_sum_delta [x * size];
...
int Neural_network2::calc_forward_test (double input [],
                                       double desired_output [],
...
void Neural_network2::kick_weights (double range)
{
  int    x;
  double variation;

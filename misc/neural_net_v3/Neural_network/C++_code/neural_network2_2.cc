...
double Neural_network2::calc_forward_rand_opt (double input [],
  ...
  for (x = num_hidden2 - 1; x >= 0; --x)
    {
      ...
      for (y = num_hidden1 - 1; y >= 0; --y)
        {
          hidden2_act [x] += (hidden1_act [y] * (weight [y] + dir * offset [y]));
        }
  ...
  for (x = num_outputs - 1; x >= 0; --x)
    {
      ...
      offset = &hidden2_weights_g_offset [x * size];
      for (y = num_hidden2 - 1; y >= 0; --y)
        {
          output_act [x] += hidden2_act [y] * (weight [y] + dir * offset [y]);
        }
...
int Neural_network2::generate_gaussian_offsets ()
{
  int x;
...
void Neural_network2::update_weights_with_offset (int direction)
{
  int    x;

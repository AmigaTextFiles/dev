...
VRMLSaver::VRMLSaver(SaveVRMLParams *par) {
    f=NULL;
    sp=par;
    nb=0;
    puts("VRMLSaver object created");
...
void VRMLSaver::WriteVRML_V1(FILE *fd,VRMLNode *n) {
    f=fd;
    VRMLState state=VRMLState();
    n->Browse(&state);

    SetAttrs((Object *) TX_Msg, MUIA_Text_Contents,(ULONG) "Saving VRML V1.0 ascii file");
...
void VRMLSaver::SaveNode(VRMLNode *n,int tab) {
    switch (n->ID) {
...
void VRMLSaver::WriteTabs(int t) {
    for (int i=0;i<t;i++) {
	fprintf(f,"\t");
    };
}
 ...
 **************/
// AsciiText
void VRMLSaver::WriteAsciiText(AsciiText *a,int tab) {
    int i=0,size=a->Size();
...
// Cone
void VRMLSaver::WriteCone(Cone *c,int tab) {
    WriteTabs(tab);
...
// Coordinate3
void VRMLSaver::WriteCoordinate3(Coordinate3 *c,int tab) {
    Vertex3d *cv=NULL;
    int size=c->Size();
...
// Cube
void VRMLSaver::WriteCube(Cube *c, int tab) {
    WriteTabs(tab);
    if (strcmp(c->GetName(),"NONE")) {
	fprintf(f,"DEF %s ",c->GetName());
    };
...
// Cylinder
void VRMLSaver::WriteCylinder(Cylinder *c, int tab) {
    WriteTabs(tab);
    if (strcmp(c->GetName(),"NONE")) {
	fprintf(f,"DEF %s ",c->GetName());
    };
...
// DirectionalLight
void VRMLSaver::WriteDirectionalLight(DirectionalLight *dl, int tab) {
    WriteTabs(tab);
...
// FontStyle
void VRMLSaver::WriteFontStyle(FontStyle *fs, int tab) {
    WriteTabs(tab);
...
// Group
void VRMLSaver::WriteGroup(Group *g, int tab) {
    int i;
	...
	SaveNode(g->children.Get(i),tab+1);
    };
    WriteTabs(tab);
...
// IndexedFaceSet
void VRMLSaver::WriteIFS(IndexedFaceSet *ifs, int tab) {
    int index,i,j,pos;
    int size=ifs->Size();
    WriteTabs(tab);
    Face *cf;
...
// IndexedLineSet
void VRMLSaver::WriteILS(IndexedLineSet *ils, int tab) {
    int index,i,j,pos;
    int size=ils->Size();
    WriteTabs(tab);
    Face *cf;
...
// Info
void VRMLSaver::WriteInfo(VInfo *in, int tab) {
    WriteTabs(tab);
...
// LOD
void VRMLSaver::WriteLOD(LOD *l, int tab) {
    int i,n=0;
    double x,y,z;
...
// Material
void VRMLSaver::WriteMaterial (Material *m, int tab) {
    float r,g,b;
    int i=0;
    int size=m->Size();
    Mat *cm;
...
// MaterialBinding
void VRMLSaver::WriteMaterialBinding(MaterialBinding *mb, int tab) {
    WriteTabs(tab);
...
// MatrixTransform
void VRMLSaver::WriteMatrixTransform(MatrixTransform *mt, int tab) {
...
// Normal
void VRMLSaver::WriteNormal(Normal *no, int tab) {
    int i,n;
    int size=no->Size();
    // float x,y,z;
    Vertex3d *cp;
...
// NormalBinding
void VRMLSaver::WriteNormalBinding(NormalBinding *nb, int tab) {
    WriteTabs(tab);
...
// OrthographicCamera
void VRMLSaver::WriteOC(OrthographicCamera *oc, int tab) {
    WriteTabs(tab);
...
// PerspectiveCamera
void VRMLSaver::WritePC(PerspectiveCamera *pc, int tab) {
    WriteTabs(tab);
...
// PointLight
void VRMLSaver::WritePointLight(PointLight *pl, int tab) {
    WriteTabs(tab);
...
// PointSet
void VRMLSaver::WritePointSet(PointSet *ps, int tab) {
    WriteTabs(tab);
...
// Rotation
void VRMLSaver::WriteRotation(Rotation *r, int tab) {
    double x,y,z,a;
...
// Scale
void VRMLSaver::WriteScale(Scale *s, int tab) {
    double x,y,z;
...
// Separator
void VRMLSaver::WriteSeparator (Separator *s, int tab) {
    WriteTabs(tab);
    if (strcmp(s->GetName(),"NONE")) {
	fprintf(f,"DEF %s ",s->GetName());
    };
    fprintf(f,"Separator {\n");
    WriteTabs(tab+1);
    fprintf(f,"renderCulling ");
    switch (s->renderCulling) {
	case CULLING_AUTO:fprintf(f,"AUTO\n");break;
	case CULLING_ON:fprintf(f,"ON\n");break;
	case CULLING_OFF:fprintf(f,"OFF\n");break;
    };
...
// ShapeHints
void VRMLSaver::WriteShapeHints(ShapeHints *sh, int tab) {
    WriteTabs(tab);
...
// Sphere
void VRMLSaver::WriteSphere(Sphere *s, int tab) {
    WriteTabs(tab);
    puts("==>WriteSphere");
    if (strcmp(s->GetName(),"NONE")) {
	fprintf(f,"DEF %s ",s->GetName());
...
// SpotLight
void VRMLSaver::WriteSpotLight(SpotLight *sl, int tab) {
    WriteTabs(tab);
...
// Switch
void VRMLSaver::WriteSwitch(Switch *sw, int tab) {
    int i;
...
// Texture2
void VRMLSaver::WriteTexture2(Texture2 *t, int tab) {
    WriteTabs(tab);
...
// Texture2Transform
void VRMLSaver::WriteTexture2Transform(Texture2Transform *tt, int tab) {
    double x,y;
...
// TextureCoordinate2
void VRMLSaver::WriteTextureCoordinate2(TextureCoordinate2 *tc, int tab) {
    int size=tc->Size();
    Vertex2d *cp;
...
// Transform
void VRMLSaver::WriteTransform(Transform *t, int tab) {
    double x,y,z,a;
...
// TransformSeparator
void VRMLSaver::WriteTransformSeparator(TransformSeparator *ts, int tab) {
    WriteTabs(tab);
...
// Translation
void VRMLSaver::WriteTranslation(Translation *t, int tab) {
    double x,y,z;
    WriteTabs(tab);
    if (strcmp(t->GetName(),"NONE")) {
	fprintf(f,"DEF %s ",t->GetName());
...
// WWWAchor
void VRMLSaver::WriteWWWAnchor(WWWAnchor *www, int tab) {
    int i=0;
...
// WWWInline
void VRMLSaver::WriteWWWInline(WWWInline *www, int tab) {
    double x,y,z;
...
// USE
void VRMLSaver::WriteUSE(USE *u, int tab) {
    WriteTabs(tab);

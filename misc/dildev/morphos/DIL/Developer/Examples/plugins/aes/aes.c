/*
 * aes.dilp - AES cipher plugin for DIL
 * Copyright ©2004-2009 Rupert Hausberger <naTmeg@gmx.net>
 * All rights reserved.
 *
 * Please see "License.readme" for the terms of this file
 *
 *
 * Based on 7z v4.58, 2008-03-26 Igor Pavlov
 * Ported to MorphOS, 2008-10-01 Rupert Hausberger <naTmeg@gmx.net>
 */

#include <proto/alib.h>
#include <proto/exec.h>

#include "aes.h"

//-----------------------------------------------------------------------------

#define GetUi16(p) \
			 (((const UBYTE *)(p))[0] | \
	((UWORD)((const UBYTE *)(p))[1] << 8))

#define GetUi32(p) \
			 (((const UBYTE *)(p))[0] | \
	((ULONG)((const UBYTE *)(p))[1] << 8 ) | \
	((ULONG)((const UBYTE *)(p))[2] << 16) | \
	((ULONG)((const UBYTE *)(p))[3] << 24))

#define GetUi64(p) ((UQUAD)GetUi32(p) | (UQUAD)GetUi32(((const UBYTE *)(p)) + 4) << 32)

#define SetUi32(p, d) { ULONG _x_ = (d); \
	((UBYTE *)(p))[0] = (UBYTE)_x_; \
	((UBYTE *)(p))[1] = (UBYTE)(_x_ >> 8); \
	((UBYTE *)(p))[2] = (UBYTE)(_x_ >> 16); \
	((UBYTE *)(p))[3] = (UBYTE)(_x_ >> 24); }

#define xtime(x) ((((x) << 1) ^ (((x) & 0x80) != 0 ? 0x1B : 0)) & 0xFF)

#define Ui32(a0, a1, a2, a3) ((ULONG)(a0) | ((ULONG)(a1) << 8) | ((ULONG)(a2) << 16) | ((ULONG)(a3) << 24))

#define gb0(x) ( (x)				& 0xFF)
#define gb1(x) (((x) >> ( 8)) & 0xFF)
#define gb2(x) (((x) >> (16)) & 0xFF)
#define gb3(x) (((x) >> (24)) & 0xFF)

#define HT(i, x, s) (T + (x << 8))[gb ## x(s[(i + x) & 3])]
#define HT4(m, i, s, p) m[i] = \
		HT(i, 0, s) ^ \
		HT(i, 1, s) ^ \
		HT(i, 2, s) ^ \
		HT(i, 3, s) ^ w[p + i]
#define HT16(m, s, p) \
		HT4(m, 2, s, p); \
		HT4(m, 0, s, p); \
		HT4(m, 3, s, p); \
		HT4(m, 1, s, p); \

#define FT(i, x) Sbox[gb ## x(m[(i + x) & 3])]
#define FT4(i) dest[i] = Ui32(FT(i, 0), FT(i, 1), FT(i, 2), FT(i, 3)) ^ w[i];

#define HD(i, x, s) (D + (x << 8))[gb ## x(s[(i - x) & 3])]
#define HD4(m, i, s, p) m[i] = \
		HD(i, 0, s) ^ \
		HD(i, 1, s) ^ \
		HD(i, 2, s) ^ \
		HD(i, 3, s) ^ w[p + i];
#define HD16(m, s, p) \
		HD4(m, 0, s, p); \
		HD4(m, 2, s, p); \
		HD4(m, 3, s, p); \
		HD4(m, 1, s, p); \

#define FD(i, x) InvS[gb ## x(m[(i - x) & 3])]
#define FD4(i) dest[i] = Ui32(FD(i, 0), FD(i, 1), FD(i, 2), FD(i, 3)) ^ w[i];

//-----------------------------------------------------------------------------

static const UBYTE Rcon[11] = { 0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36 };

static const UBYTE Sbox[256] = {
	 };

static UBYTE InvS[256];

static ULONG T[256 * 4];
static ULONG D[256 * 4];

//-----------------------------------------------------------------------------

void AES_Init(void)
{
	unsigned i;

	for (i = 0; i < 256; i++)
		InvS[Sbox[i]] = (UBYTE)i;
	for (i = 0; i < 256; i++)
	{
		{
			ULONG a1 = Sbox[i];
			ULONG a2 = xtime(a1);
			ULONG a3 = xtime(a1) ^ a1;
			T[			 i] = Ui32(a2, a1, a1, a3);
			T[0x100 + i] = Ui32(a3, a2, a1, a1);
			T[0x200 + i] = Ui32(a1, a3, a2, a1);
			T[0x300 + i] = Ui32(a1, a1, a3, a2);
		}

		{
			ULONG a1 = InvS[i];
			ULONG a2 = xtime(a1);
			ULONG a4 = xtime(a2);
			ULONG a8 = xtime(a4);
			ULONG a9 = a8 ^ a1;
			ULONG aB = a8 ^ a2 ^ a1;
			ULONG aD = a8 ^ a4 ^ a1;
			ULONG aE = a8 ^ a4 ^ a2;
			D[			 i] = Ui32(aE, a9, aD, aB);
			D[0x100 + i] = Ui32(aB, aE, a9, aD);
			D[0x200 + i] = Ui32(aD, aB, aE, a9);
			D[0x300 + i] = Ui32(a9, aD, aB, aE);
		}
	}
}

//-----------------------------------------------------------------------------

void AES_SetKeyEncode(AESContext *p, const UBYTE *key, unsigned keySize)
{
	unsigned i, wSize;
	ULONG *w;

	keySize /= 4;
	p->numRounds2 = keySize / 2 + 3;

	wSize = (p->numRounds2 * 2 + 1) * 4;
	w = p->rkey;

	for (i = 0; i < keySize; i++, key += 4)
		w[i] = Ui32(key[0], key[1], key[2], key[3]);

	for (; i < wSize; i++)
	{
		unsigned rem = i % keySize;
		ULONG t = w[i - 1];

		if (rem == 0) 
			t = Ui32(Sbox[gb1(t)] ^ Rcon[i / keySize], Sbox[gb2(t)], Sbox[gb3(t)], Sbox[gb0(t)]);
		else if (keySize > 6 && rem == 4)
			t = Ui32(Sbox[gb0(t)], Sbox[gb1(t)], Sbox[gb2(t)], Sbox[gb3(t)]);

		w[i] = w[i - keySize] ^ t;
	}
}

void AES_SetKeyDecode(AESContext *p, const UBYTE *key, unsigned keySize)
{
	unsigned i, num;
	ULONG *w;

	AES_SetKeyEncode(p, key, keySize);
	num = p->numRounds2 * 8 - 4;
	w = p->rkey + 4;
	for (i = 0; i < num; i++)
	{
		ULONG r = w[i];
		w[i] =
			D[			 Sbox[gb0(r)]] ^
			D[0x100 + Sbox[gb1(r)]] ^
			D[0x200 + Sbox[gb2(r)]] ^
			D[0x300 + Sbox[gb3(r)]];
	}
}

//-----------------------------------------------------------------------------

static void AesEncode32(ULONG *dest, const ULONG *src, const ULONG *w, unsigned numRounds2)
{
	ULONG s[4];
	ULONG m[4];

	s[0] = src[0] ^ w[0];
	s[1] = src[1] ^ w[1];
	s[2] = src[2] ^ w[2];
	s[3] = src[3] ^ w[3];
	w += 4;
	for (;;)
	{
		HT16(m, s, 0);
		if (--numRounds2 == 0)
			break;
		HT16(s, m, 4);
		w += 8;
	}
	w += 4;
	FT4(0); FT4(1); FT4(2); FT4(3);
}

static void AesDecode32(ULONG *dest, const ULONG *src, const ULONG *w, unsigned numRounds2)
{
	ULONG s[4];
	ULONG m[4];

	w += numRounds2 * 8;
	s[0] = src[0] ^ w[0];
	s[1] = src[1] ^ w[1];
	s[2] = src[2] ^ w[2];
	s[3] = src[3] ^ w[3];
	for (;;)
	{
		w -= 8;
		HD16(m, s, 4);
		if (--numRounds2 == 0)
			break;
		HD16(s, m, 0);
	}
	FD4(0); FD4(1); FD4(2); FD4(3);
}

void AES_Encode32(const AESContext *p, const ULONG *src, ULONG *dest)
{
	AesEncode32(dest, src, p->rkey, p->numRounds2);
}

void AES_Decode32(const AESContext *p, const ULONG *src, ULONG *dest)
{
	AesDecode32(dest, src, p->rkey, p->numRounds2);
}

//-----------------------------------------------------------------------------

void AES_InitCbc(AESContextCbc *p, const UBYTE *iv)
{
	unsigned i;

	for (i = 0; i < 4; i++)
		p->prev[i] = GetUi32(iv + i * 4);
}

void AES_InitCbc4(AESContextCbc *p, const ULONG *iv)
{
	unsigned i;

	for (i = 0; i < 4; i++)
		p->prev[i] = iv[i];
}

ULONG AES_EncodeCbc(AESContextCbc *p, const UBYTE *src, UBYTE *dst, ULONG size)
{
	ULONG i;

	if (size == 0 || size % AES_BLOCK_SIZE != 0)
		return 0;
	else if (size < AES_BLOCK_SIZE) {
		CopyMem((const APTR)src, (APTR)dst, size);
		return size;
	}
	size -= AES_BLOCK_SIZE;
	for (i = 0; i <= size; i += AES_BLOCK_SIZE, src += AES_BLOCK_SIZE, dst += AES_BLOCK_SIZE)
	{
		p->prev[0] ^= GetUi32(src);
		p->prev[1] ^= GetUi32(src + 4);
		p->prev[2] ^= GetUi32(src + 8);
		p->prev[3] ^= GetUi32(src + 12);
	
		AesEncode32(p->prev, p->prev, p->aes.rkey, p->aes.numRounds2);
	
		SetUi32(dst,		p->prev[0]);
		SetUi32(dst + 4,	p->prev[1]);
		SetUi32(dst + 8,	p->prev[2]);
		SetUi32(dst + 12,	p->prev[3]);
	}
	return i;
}

ULONG AES_DecodeCbc(AESContextCbc *p, const UBYTE *src, UBYTE *dst, ULONG size)
{
	ULONG i;
	ULONG in[4], out[4];

	if (size == 0 || size % AES_BLOCK_SIZE != 0)
		return 0;
	else if (size < AES_BLOCK_SIZE) {
		CopyMem((const APTR)src, (APTR)dst, size);
		return size;
	}
	size -= AES_BLOCK_SIZE;
	for (i = 0; i <= size; i += AES_BLOCK_SIZE, src += AES_BLOCK_SIZE, dst += AES_BLOCK_SIZE)
	{
		in[0] = GetUi32(src);
		in[1] = GetUi32(src + 4);
		in[2] = GetUi32(src + 8);
		in[3] = GetUi32(src + 12);
	
		AesDecode32(out, in, p->aes.rkey, p->aes.numRounds2);
	
		SetUi32(dst,		p->prev[0] ^ out[0]);
		SetUi32(dst + 4,	p->prev[1] ^ out[1]);
		SetUi32(dst + 8,	p->prev[2] ^ out[2]);
		SetUi32(dst + 12,	p->prev[3] ^ out[3]);
	
		p->prev[0] = in[0];
		p->prev[1] = in[1];
		p->prev[2] = in[2];
		p->prev[3] = in[3];
	}
	return i;
}

//-----------------------------------------------------------------------------


...
newbn (int length)
{
  ...
  b = malloc ((length + 1) * sizeof (unsigned short));
  if (b != NULL)
    b[0] = length;
 ...
 */
static void 
bigmul (unsigned short *a, unsigned short *b, unsigned short *c, int len)
{
  int i, j;
  unsigned long ai, t;
 ...
 */
static void 
bigmod (unsigned short *a, unsigned short *m, int len)
{
...
int 
makekey (unsigned char *data, R_RSAKey * key, unsigned char **keystr)
{
...
int
rsaencrypt (unsigned char *data, int length, R_RSAKey * key)
{
  Bignum b1, b2;
  int w, i;
  unsigned char *p;

...
static void 
MD5_Core_Init (MD5_Core_State * s)
{
  s->h[0] = 0x67452301;
  s->h[1] = 0xefcdab89;
  s->h[2] = 0x98badcfe;
...
static void 
MD5_Block (MD5_Core_State * s, unsigned long *block)
{
...
void 
MD5Update (struct MD5Context *s, unsigned char const *p, unsigned len)
{
  unsigned char *q = (unsigned char *) p;
  unsigned long wordblock[16];
  unsigned long lenw = len;
  int i;
  ...
  else
  {
    ...
    while (s->blkused + len >= BLKSIZE)
    {
      ...
      {
        wordblock[i] =
          (((unsigned long) s->block[i * 4 + 3]) << 24) |
          (((unsigned long) s->block[i * 4 + 2]) << 16) |
          (((unsigned long) s->block[i * 4 + 1]) << 8) |
          (((unsigned long) s->block[i * 4 + 0]) << 0);
      }
...
void 
MD5Final (unsigned char output[16], struct MD5Context *s)
{
  int i;
  unsigned pad;
  unsigned char c[64];

...
bitsel (unsigned long *input, const int *bitnums, int size)
{
  unsigned long ret = 0;
  while (size--)
  {
...
void 
des_set_key (unsigned char *key, DESCon * sched)
{
  unsigned long key_msw = GET_32BIT_MSB_FIRST (key);
  unsigned long key_lsw = GET_32BIT_MSB_FIRST (key + 4);
...
static const unsigned long SPboxes[8][64] =
{
...
static void 
des_encipher (unsigned long *output, unsigned long L, unsigned long R, DESCon * sched)
{
  unsigned long swap, s0246, s1357;
...
static void 
des_decipher (unsigned long *output, unsigned long L, unsigned long R, DESCon * sched)
{
  unsigned long swap, s0246, s1357;
...
static void 
des_cbc_encrypt (unsigned char *dest, const unsigned char *src, unsigned int len, DESCon * sched)
{
  unsigned long out[2], iv0, iv1;
  unsigned int i;
...
static void 
des_cbc_decrypt (unsigned char *dest, const unsigned char *src, unsigned int len, DESCon * sched)
{
  unsigned long out[2], iv0, iv1, xL, xR;
  unsigned int i;
...
void 
des_3cbc_encrypt (unsigned char *dest, const unsigned char *src, unsigned int len, DESCon * scheds)
{
...
void 
des_3cbc_decrypt (unsigned char *dest, const unsigned char *src, unsigned int len, DESCon * scheds)
{

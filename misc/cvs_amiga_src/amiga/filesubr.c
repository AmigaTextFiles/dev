    ...
    const char *to;
{
	...
	free (source);
#else
	error (1, 0, "cannot copy symbolic links on this system (%s)", from);
#endif
	return;
    }
    ...
    const char *file2;
{
	...
	return result;
#else
	error (1, 0, "cannot compare symbolic links on this system (%s and %s)",
	       file1, file2);
#endif
    }
#endif
    ...
    char **filename;
{
    ...
    fn = xmalloc (tmplen + 11);
    if (Tmpdir[tmplen-1]==':' || Tmpdir[tmplen-1]=='/')
    ...
    const char *file;
{
    ...
    char *path = xmalloc (dirlen + 1 + strlen(file) + 1);
    ...
    char ***pargv;
{
    ...
    if (*pargc = amiga_expandwild(argc,argv,NULL)) {
        if (*pargv = xmalloc(*pargc * sizeof(char *))) {
           if (amiga_expandwild(argc,argv,*pargv))
                return;
           else
                free(*pargv);
        }

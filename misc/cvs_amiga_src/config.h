/* config.h  Manually created for Amiga cvs port using vbcc + PosixLib */
...
#define FILENAMES_CASE_INSENSITIVE
...
#define PACKAGE_BUGREPORT "frank@phoenix.owl.de"
...
#define ISABSOLUTE(s) (strchr(s,':')!=0)

/* For the Amiga port we don`t allow the EXT Method because it doesn`t work as it should */
#define NO_EXT_METHOD 1

#define START_SERVER_RETURNS_SOCKET 1
#define NO_SOCKET_TO_FD 1

#define START_SERVER amiga_start_server
#define SHUTDOWN_SERVER 1
#define SHUTDOWN_SERVER_INPUT amiga_shutdown_server_input
#define SHUTDOWN_SERVER_OUTPUT amiga_shutdown_server_output
...
extern void START_SERVER (int *tofd, int *fromfd,
           ...
           char *server_host,
           char *server_cvsroot);
extern void SHUTDOWN_SERVER_INPUT(int fd);
extern void SHUTDOWN_SERVER_OUTPUT(int fd);

#define same_file(a,b) (-1)

/* lstat cannot return symbolic links on the Amiga, use stat instead */
#define CVS_LSTAT stat

/* OS specific init function */
extern void amiga_init(int *,char ***);
#define SYSTEM_INITIALIZE amiga_init

/* Dice: 1> dcc -l0 -mD dpk.o tags.o Rabbit.c -o Rabbit
**
** Blits a Rabbit to screen using a bob structure.  Control using
** the joystick.
*/

#include <proto/dpkernel.h>

BYTE *ProgName      = "Rabbit";
...
void main(void) {
  ...
  if (bobpic = Get(ID_PICTURE)) {
     bobpic->Source = &bobfile;
     bobpic->Bitmap->MemType = MEM_BLIT;

   if (Init(bobpic,NULL)) {

    if (screen = Get(ID_SCREEN)) {
     if (palette = CloneMemBlock(bobpic->Bitmap->Palette,MEM_DATA)) {
        screen->Bitmap->Palette = palette;
       ...
       if (restore = InitTags(screen,
        ...
        if (Rabbit = InitTags(screen,
         ...
         if (joydata = Get(ID_JOYDATA)) {
            joydata->Port = 2;     /* Forces joystick control */
...
void Demo(void)
{
  ...
  } while (!(joydata->Buttons & JD_FIRE1));
}

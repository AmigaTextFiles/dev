/* SAS/C: sc Enneper.c opt math=standard INCDIR=INCLUDES:
**
** This is a demo of a rotating Enneper's Surface.  The object is
** pre-calculated then rotated in real time for speed, although things could
** be faster than this.
**
** Improve: Replace doubles with longs/words if possible.
*/

#include <proto/dpkernel.h>
#include <math.h>

BYTE *ProgName      = "Enneper's Surface";
...
#define AMTDOTS 1000       /* The amount of dots in the Object. */

#define MAXZ 1.96          /* MAXZ defined by 1.4*1.4 = 1.96 */
...
void Demo(void)
{
  ...
  for (i=0; i<AMTDOTS; i++) {
    u = ((double)FastRandom(28000-1)+1)/10000-1.4;  /*  -1.4 < u < 1.4  */
    v = ((double)FastRandom(28000-1)+1)/10000-1.4;  /*  -1.4 < v < 1.4  */
    object[i].X  = u*(1+v*v)-u*u*u;
    object[i].Y  = v*(1+u*u)-v*v*v;
    object[i].Z  = u*u-v*v;
  }
...
//      Y2 = Y2*cosine[anglez] + temp*sine[anglez];
      ...
      screen->Bitmap->DrawPixel(screen->Bitmap,(WORD)X2+offsetx,(WORD)Y2+offsety,colour);
    }

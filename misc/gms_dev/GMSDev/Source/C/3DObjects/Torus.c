/* SAS/C: sc Torus.c opt math=standard INCDIR=INCLUDES:
**
** This is a demo of a spinning torus consisting entirely of dots.
...
void Demo(void)
{
  ...
  for (i=0; i<AMTDOTS; i++) {
    u = ((double)FastRandom(6283))/1000;
    v = ((double)FastRandom(6283))/1000;
    object[i].X  = cos(u)*(2+cos(v));
    object[i].Y  = sin(u)*(2+sin(v));
    object[i].Z  = sin(v);
  }

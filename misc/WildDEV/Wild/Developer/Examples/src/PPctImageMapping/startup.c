...
#define OpenLib(base,name,ver) \
 if (!(base=(((struct Library *)OpenLibrary(name,ver))))) \
  {return(FALSE);} 
...
static int openlibs(void)
{
  ...
  OpenLib(IntuitionBase,"intuition.library",37);
  OpenLib(GraphicsBase,"graphics.library",37);
  OpenLib(IffParseBase,"iffparse.library",37);
  OpenLib(DOSBase,"dos.library",37);
  OpenLib(WildBase,"wild.library",1);
}

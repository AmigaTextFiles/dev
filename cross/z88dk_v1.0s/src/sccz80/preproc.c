 ...
 *
 *      $Id: preproc.c 1.3 1999/03/18 01:14:26 djm8 Exp $
 */
...
void preprocess()
{
        ...
        while ( ch() ) {
                if ( ch()==' ' || ch()=='\t' ) {
                        ...
                        {
                                        warning("Typedef not supported by compiler");
                                        junk();
                                        vinline();
                                        if (eof) break;
                        }
        ...
        keepch(0);
        if ( mptr >= MPMAX ) error("Input line too long") ;
        strcpy(line, mline) ;
        lptr = 0 ;
}

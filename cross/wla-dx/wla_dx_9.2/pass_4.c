...
int pass_4(void) {
  ...
  if (output_format == OUTPUT_LIBRARY && test_mode == OFF) {
    ...
    while (label_tmp != NULL) {
      if (label_tmp->alive == ON) {
	fprintf(final_ptr, "%s%c%c%c", label_tmp->label, 0x0, label_tmp->section_id, label_tmp->filename_id);
  ...
  /* object file output */
    ...
    while (label_tmp != NULL) {
      if (label_tmp->alive == ON) {
	fprintf(final_ptr, "%s%c%c%c%c", label_tmp->label, 0x0, label_tmp->slot, label_tmp->filename_id, label_tmp->section_id);

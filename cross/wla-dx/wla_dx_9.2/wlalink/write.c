...
int fix_references(void) {
  ...
  while (r != NULL) {
	...
	l = get_closest_anonymous_label(&r->name[1], x, r->file_id, l);
      }
	...
	l = get_closest_anonymous_label(r->name, x, r->file_id, l);
      }
...
int write_symbol_file(char *outname) {
...
int parse_stack(struct stack *sta) {
  ...
  while (g != sta->stacksize) {
	...
	if (is_label_anonymous(&si->string[1]) == SUCCEEDED) {
	  l = get_closest_anonymous_label(&si->string[1], sta->linenumber, sta->file_id, l);
	  k = l->address;
	}
	...
	if (is_label_anonymous(si->string) == SUCCEEDED) {
	  l = get_closest_anonymous_label(si->string, sta->linenumber, sta->file_id, l);
	  k = l->address;
	}
...
struct label *get_closest_anonymous_label(char *name, int rom_address, int file_id, struct label *l) {
  ...
  if (strcmp(name, "_b") == 0 || strcmp(name, "_B") == 0) {
    while (l != NULL) {
      if (strcmp("__", l->name) == 0 && file_id == l->file_id) {
  ...
  /* -, --, +, ++, ... */
  while (l != NULL) {
    if (strcmp(name, l->name) == 0 && file_id == l->file_id) {
      if (name[0] == '-') {
	e = rom_address - l->rom_address;
	if (e >= 0 && e < d) {
	  closest = l;

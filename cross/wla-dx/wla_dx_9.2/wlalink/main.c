...
#ifdef AMIGA
char version_string[] = "$VER: WLALINK 5.5 (30.07.2004)";
#endif
...
int romsize, rombanks, banksize, verbose_mode = OFF, section_overwrite = OFF, symbol_mode = OFF;
...
int main(int argc, char *argv[]) {
  ...
  if (i == FAILED) {
    printf("\nWLALINK GB-Z80/Z80/6502/65C02/6510/65816/HUC6280/SPC-700 WLA Macro Assembler Linker v5.5\n");
    printf("Written by Ville Helin in 2000-2004\n");
    printf("USAGE: %s [-bdrvs] <LINK FILE> <OUTPUT FILE>\n", argv[0]);
    printf("Options:\n");
  ...
  if (symbol_mode == ON) {
    if (write_symbol_file(argv[argc - 1]) == FAILED)
      return FAILED;
  }

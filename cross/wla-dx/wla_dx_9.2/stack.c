...
int stack_calculate(char *in, int *value) {
  ...
  /* check for un-named labels, and  */
  if (q == 1) {
    if (si[0].type == STACK_ITEM_TYPE_OPERATOR) {
      /* - */
      if (si[0].value == SI_OP_MINUS) {
	si[0].type = STACK_ITEM_TYPE_STRING;
	si[0].sign = SI_SIGN_POSITIVE;
	si[0].string[0] = '-';
	si[0].string[1] = 0;
      }
      /* + */
      else if (si[0].value == SI_OP_PLUS) {
	si[0].type = STACK_ITEM_TYPE_STRING;
	si[0].sign = SI_SIGN_POSITIVE;
	si[0].string[0] = '+';
	si[0].string[1] = 0;
...
int resolve_stack(struct stack_item s[], int x) {
  ...
  while (x > 0) {
	...
	if (s->string[1] == '@' && s->string[2] == 0) {
	  ...
	  if (b > macro_runtime_current->macro->arguments) {
	    sprintf(xyz, "MACRO argument number %d is out of range [1, %d].\n", b, macro_runtime_current->macro->arguments);

...
int pass_1(void) {
  ...
  while ((t = get_next_token()) == SUCCEEDED) {
	...
	/* do we use names for the arguments? */
	if (m->nargument_names > 0) {
      ...
      if (p > m->arguments) {
	sprintf(emsg, "Too many arguments to macro \"%s\".\n", m->name);
	print_error(emsg, ERROR_INP);
	return FAILED;
      }
      if (p < m->arguments) {
	sprintf(emsg, "Too few arguments to macro \"%s\".\n", m->name);
	print_error(emsg, ERROR_INP);
	return FAILED;
      }
...
int parse_directive(void) {
  ...
  if (strcmp(cp, "DB") == 0 || strcmp(cp, "BYT") == 0 || strcmp(cp, "BYTE") == 0 || strcmp(cp, "ASC") == 0) {
    strcpy(bak, cp);
  ...
  if (strcmp(cp, "MEMORYMAP") == 0) {
	...
	if (slotsize_defined == 0) {
	  print_error("SLOTSIZE must be defined prior to SLOT.\n", ERROR_DIR);
	  return FAILED;
	}
	...
	if (q == FAILED)
	  return FAILED;
	if (q != SUCCEEDED || d < 0) {
	  print_error("SLOT needs a non-negative value as an ADDRESS.\n", ERROR_DIR);
	  return FAILED;
	}

	slots[o].address = d;
	slots[o].size = slotsize;
	slots_amount++;
	s++;
      }
      else {
  ...
  if (strcmp(cp, "MACRO") == 0) {
    ...
    unsigned char e, *argz;
    int d, k, macro_start, a;
    ...
    if (k != a) {
      ...
      if (m->arguments - k == 1)
	fprintf(stderr, "%s:%d: WARNING: Argument ", get_file_name(active_file_info_last->filename_id), active_file_info_last->line_current);
      else
	fprintf(stderr, "%s:%d: WARNING: Arguments ", get_file_name(active_file_info_last->filename_id), active_file_info_last->line_current);

      /* display the skipped arguments */
      for (d = 0, k = 0; k < a; k++) {
	if (argz[k] == 0) {
	  if (d == 0)
	...
	fprintf(stderr, " are");
      fprintf(stderr, " not used in MACRO \"%s\".\n", m->name);
      return FAILED;
    }

    if (m->nargument_names > 0 && k > m->nargument_names) {
      sprintf(emsg, "Not all arguments have names in MACRO \"%s\".\n", m->name);
      print_error(emsg, ERROR_WRN);
    }
  ...
  if (strcmp(cp, "ENDM") == 0) {
	...
	i = macro_stack[0].macro_end;

	/* free the arguments */
	if (macro_stack[0].supplied_arguments > 0) {
	  for (d = 0; d < macro_stack[0].supplied_arguments; d++)
	    free(macro_stack[0].argument_data[d]);
	  free(macro_stack[0].argument_data);
	  macro_stack[0].argument_data = NULL;
	}

	/* free the argument definitions */
	for (q = 0; q < macro_stack[0].macro->nargument_names; q++)
	  undefine(macro_stack[0].macro->argument_names[q]);

	if ((extra_definitions == ON) && (active_file_info_last->filename_id != macro_stack[0].macro_end_filename_id)) {
	  redefine("WLA_FILENAME", 0.0, get_file_name(macro_stack[0].macro_end_filename_id), DEFINITION_TYPE_STRING,
		   strlen(get_file_name(macro_stack[0].macro_end_filename_id)));
	  redefine("wla_filename", 0.0, get_file_name(macro_stack[0].macro_end_filename_id), DEFINITION_TYPE_STRING,
		   strlen(get_file_name(macro_stack[0].macro_end_filename_id)));
	}

	active_file_info_last->filename_id = macro_stack[0].macro_end_filename_id;
	active_file_info_last->line_current = macro_stack[0].macro_end_line;

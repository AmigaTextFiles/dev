 ...
 */
static char rcsid[] = "$Id: lcc.c,v 1.1.1.1 1998/10/01 16:08:32 felber Exp $";
...
//		tempdir[i-1] = '\0';
	...
	if (errcnt == 0 && !Eflag && !Sflag && !cflag && llist[1]) {
		compose(ld, llist[0], llist[1],
			append(outfile ? outfile : concat("Cartridge", first(suffixes[4])), 0));
		if (callsys(av))
			errcnt++;
	}
	rm(rmlist);	
...
printf("\nbuf=\"%s\"\n",buf);
*/
	status=SystemTagList(buf,TAG_DONE);
	if (status&0377) {
...
char *tempname(char *suffix) {
	static int n;
	char last_temp = tempdir[strlen(tempdir)-1];

	char *name = stringf("%s%slcc%d%d%s", tempdir,
		((last_temp == '/') || (last_temp == ':')) ? "" : "/",
		FindTask(NULL), n++, suffix);

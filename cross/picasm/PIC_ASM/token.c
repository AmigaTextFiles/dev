 ...
 */
  if(tok_char == '_' || isalpha(tok_char))
	 {
		line_buf_flag = (line_buf_ptr == &line_buffer[1]);

		token_string[0] = tok_char;
		tp = 1;
		read_src_char();
		while(tp < TOKSIZE-1 && (tok_char == '_' || isalnum(tok_char)))
	{
	  token_string[tp++] = tok_char;
	  read_src_char();
	}

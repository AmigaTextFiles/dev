...
expr_element(void)
{
  ...
  switch(token_type)
	...
	if((sym = lookup_symbol(token_string)) == NULL)
	  val = EXPR_FALSE;
	...
	if((sym = lookup_symbol(token_string)) == NULL)
	  {
	    error(1, "Undefined symbol '%s'", token_string);
	    expr_error = 1;
	  }
	else
	  {
...
get_expression(void)
{
  ...
  val1 = expr_add_sub();
  while(!expr_error && ((op = token_type) == TOK_EQUAL || op == TOK_EQ ||
		       op == TOK_NOT_EQ ||

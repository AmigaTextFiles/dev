...
BEGIN
     ...
     IF info.getPpidImplemented THEN
        p := si.GetPpid(info); IF (p # -1) THEN d.PrintF("ppid: %ld\n",p) END;
     ELSE
        d.PrintF("ppid: unknown\n");
     END;
     IF info.getPgrpImplemented THEN
        p := si.GetPgrp(info); IF (p # -1) THEN d.PrintF("pgrp: %ld\n",p) END;
     ELSE
        d.PrintF("pgrp: unknown\n");
     END;
           ...
           FOR i := 0 TO 9 DO
               ...
               LOOP
                  ...
                  IF info.loadAvgType = si.loadAvgFixedPnt THEN
                     ...
                     IF info.loadAvgTime1 # 0 THEN WriteReal (load.load1 / info.fScale, 1, 2) ELSE d.PrintF(" N/A") END;
                     IF info.loadAvgTime2 # 0 THEN WriteReal (load.load2 / info.fScale, 1, 2) ELSE d.PrintF(" N/A") END;
                     IF info.loadAvgTime3 # 0 THEN WriteReal (load.load3 / info.fScale, 1, 2) ELSE d.PrintF(" N/A") END;
                     d.PrintF("\n");
                  END;
               END; (* LOOP *)
           END; (* FOR *)

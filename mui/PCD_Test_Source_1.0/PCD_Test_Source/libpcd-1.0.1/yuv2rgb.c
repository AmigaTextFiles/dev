		     ...
		     unsigned char *dest, int type, int scale)
{
    ...
    case PCD_TYPE_RGB:
	{
	    ...
	    for (x = 0; x < maxx; x++, dest += bytes) {
		double luma_x = (double)((LUT_gray_int[luma[x << scale]]+10.0));	
		double red_x = (double) LUT_gray_int[red[x]]; 	
		double blue_x = (double) (LUT_gray_int[blue[x]] * 1.1);
			
			//KPrintF("luma: %g red/Green: %g blue/yellow: %g |\n",luma_x,red_x,blue_x);
			
		gray = LUT_gray_int[luma[x << scale]];
		dest[0] =  (unsigned char)(red_x / 255.0 * luma_x);                             //GET_RED ;
		dest[1] =  (unsigned char)(((red_x - (255.0 - blue_x)) * 1.1) / 255.0 * luma_x);                         //GET_GREEN;
		dest[2] = (unsigned char)(blue_x / 255.0 * luma_x);                                                          //ET_BLUE;
	    }
	}
	break;
    case PCD_TYPE_BGR:
		      ...
		      unsigned char *dest, int type, int scale)
{
    ...
    case PCD_TYPE_RGB:
	{
	    ...
	    for (x = 0; x < maxx; x++, dest += bytes) {
		double luma_x = (double)((LUT_gray_int[luma[x << scale][y << scale]]+10.0));	
		double red_x = (double) LUT_gray_int[red[x]]; 	
		double blue_x = (double) (LUT_gray_int[blue[x]] * 1.1);
		...
		dest[0] =  (unsigned char)(red_x / 255.0 * luma_x);                             //GET_RED ;
		dest[1] =  (unsigned char)(((red_x - (255.0 - blue_x)) * 1.1) / 255.0 * luma_x);                         //GET_GREEN;
		dest[2] = (unsigned char)(blue_x / 255.0 * luma_x);     
	    }
	}
	break;
    case PCD_TYPE_BGR:

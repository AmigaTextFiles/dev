...
pcd_open(struct PCD_IMAGE *img, char *filename)
{
    ...
    if (0 == fd) {
	sprintf(pcd_errmsg, "open %s: %s", filename,"I can't open the file");
	return -1;
    }

      img->mmap = (unsigned char*)AllocVec(sizeof(unsigned char) * img->size,MEMF_ANY | MEMF_CLEAR);
      FRead(fd,img->mmap,1,img->size); 
    //img->size = lseek(fd, 0, SEEK_END);
    //lseek(fd, 0, SEEK_SET);
    //img->mmap = mmap(NULL, img->size, PROT_READ, MAP_SHARED, fd, 0);
    if ((void *) -1 == img->mmap) {
	sprintf(pcd_errmsg, "mmap %s: %s", filename,"I can't allocate the memory");
	pcd_close(img);
	return -1;
    }
    //close(fd);
    Close(fd);
    if (0 == Strnicmp("PCD_OPA", img->mmap, 7)) {

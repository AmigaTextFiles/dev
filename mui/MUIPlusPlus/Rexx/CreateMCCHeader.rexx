...
headerName = mccName'_mcc.hpp'    		/* Name of file generated */
...
do while ~eof(inheritance)
    ...
    if abstractClass = 'n' then do
        ...
        /* Constructor that uses tags. We first check to ensure that some
    ...
    /* Create file to store obsolete methods for this class */
    ...
    if obsoleteInlinesExist = 'y' then do
...
WriteAttributesAndMethods: PROCEDURE expose output indent nl obsoleteName classPrefix obsoleteInlinesFile obsoleteInlinesFile obsoleteInlinesExist startVarArgs
    parse arg className , superClassName
    ...
    do while ~eof(defs)
        ...
        if word(line,1) = 'attribute' then do
            ...
            if obsolete = 'O' then do
                ...
                obsoleteInlinesExist = 'y'
            end

            else do
                fileForInlines = output
            end
            ...
            if substr(usage,3,1) = 'G' then do
                ...
                call writeln(fileForInlines,indent || type name '(void) const')
                call writeln(fileForInlines,indent'{')
                call writeln(fileForInlines,indent || indent "return ("type")GetAttr ("tag");")
                call writeln(fileForInlines,indent'}'nl)
            end
            ...
            if substr(usage,2,1) = 'S' then do
                ...
                call writeln(fileForInlines,indent'void' name '('type 'value)')
                call writeln(fileForInlines,indent'{')
                call writeln(fileForInlines,indent || indent "SetAttr ("tag", (ULONG)value);")
                call writeln(fileForInlines,indent'}'nl)
            end
        end
        ...
        /* Write member functions (methods) */
            ...
            if variableArgs = 'n' then do
                call writeln(fileForInlines,indent'ULONG' name '('paramlist')')
                call writeln(fileForInlines,indent'{')
                call writeln(fileForInlines,indent || indent'return DoMethod ('tag || arglist');')
                call writeln(fileForInlines,indent'}'nl)
            end
                ...
                call writeln(fileForInlines,indent'ULONG' name '('paramlist')')
                call writeln(fileForInlines,indent'{')
                call writeln(fileForInlines,indent || indent'sva.methodID = 'tag';')
                call writeln(fileForInlines,indent || indent'return DoMethodA ((Msg)&sva);')
                call writeln(fileForInlines,indent'}'nl)
            end
        end
    end

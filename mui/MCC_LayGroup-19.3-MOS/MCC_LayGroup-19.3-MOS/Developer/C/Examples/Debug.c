...
void SetupDebug(const char *class_name, const char *class_version)
{
  ...
  kprintf("** %s v%s startup ********************\n",
          class_name, class_version);
  kprintf("Initializing runtime debugging:\n");

  snprintf(var_name, 64, "%s.debug", class_name);
  if(GetVar(var_name, var, sizeof(var), 0) > 0)
  {
    char *tok;
    char *debug = var;
  ...
  kprintf("set debug classes/flags (env:%s): %08lx/%08lx\n",
          var_name, debug_classes, debug_flags);

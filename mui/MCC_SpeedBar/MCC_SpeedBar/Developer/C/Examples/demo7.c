...
main(int argc,char **argv)
{
    ...
    {
        Object *app, *win, *sb, *viewMode, *borderless, *sunny,
               *raising, *scaled, *lPos, *barSpacer, *underscore,
               *defaults, *update;
        char   *s;

        if (app = ApplicationObject,
                MUIA_Application_Title,       "SpeedBar Demo7",
                MUIA_Application_Version,     "$VER: SpeedBarDemo7 16.1 (3.12.2002)",
                MUIA_Application_Copyright,   "Copyright 1999-2002 by Alfonso Ranieri",
                MUIA_Application_Author,      "Alfonso Ranieri <alforan@tin.it>",
                MUIA_Application_Description, "Speed(Bar|Button).mcc test",
                MUIA_Application_Base,        "SPEEDBARTEST",

                SubWindow, win = WindowObject,
                    MUIA_Window_ID,    MAKE_ID('M','A','I','N'),
                    MUIA_Window_Title, "SpeedBar Demo7",
                    ...
                    WindowContents, VGroup,
                        ...
                        Child, HGroup,
                            Child, defaults = obutton("De_faults"),
                            Child, HSpace(0),
            ...
            End)
        {
            ...
            DoMethod(defaults,MUIM_Notify,MUIA_Pressed,FALSE,app,2,MUIM_Application_ReturnID,TAG_USER);
            DoMethod(update,MUIM_Notify,MUIA_Pressed,FALSE,app,2,MUIM_Application_ReturnID,TAG_USER+1);
            ...
            while ((id = DoMethod(app,MUIM_Application_NewInput,&sigs))!=MUIV_Application_ReturnID_Quit)
            {
                switch(id)
                {
                    case TAG_USER:
                        ...
                        DoMethod(app,MUIM_Application_ReturnID,TAG_USER+1);
                        break;

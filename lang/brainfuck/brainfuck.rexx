...
parse arg $                                      /*allow user to specify a BrainF*ck pgm*/
                                                 /* â”Œ--â—„-- No program? Then use default;*/
if $=''  then $=,                                /* â†“      it displays:  Hello, World!  */
  ...
  "> + .                  display '!'    which is  ASCII  33 (decimal)    "
                                                 /* [â†‘]   note the  Brainf*ck  comments.*/
     do !=1  while  !~==0  &  !<=length($)       /*keep executing  BF  as long as IP ¬ 0*/
     x=substr($,!,1)/*parse var  $  =(!)  x  +1*//*obtain a  Brainf*ck instruction  (x),*/
       ...
       when x== .   then call writech stdout/*charout*/ , d2c(@.p) /*display a  "tape"  cell to terminal. */
...
/*--------------------------------------------------------------------------------------*/
forward:  if @.p~==0  then return !;  c=1        /*C: â—„---  is the   [   nested counter.*/
          ...
          return k
/*--------------------------------------------------------------------------------------*/
backward: if @.p==0   then return !;  c=1        /*C: â—„---  is the   ]   nested counter.*/

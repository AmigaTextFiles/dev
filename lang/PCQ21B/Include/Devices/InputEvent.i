...
Type
   iepp_Position_Struct = Record
    x,y : Short;
   END;

   IEPointerPixel = Record
    iepp_Screen : ScreenPtr;   { pointer to an open screen }
    iepp_Position : iepp_Position_Struct;
   END;
   IEPointerPixelPtr = ^IEPointerPixel;
 ...
 * IEQUALIFIER_RELATIVEMOUSE is not supported for IESUBCLASS_TABLET.
   ...
   iept_Range_Struct = Record
    x,y : Short;
   END;
   iept_Value_Struct = Record
    x,y : Short;
   END;


   IEPointerTablet = Record
    iept_Range : iept_Range_Struct;     { 0 is min, these are max      }
    iept_Value : iept_Value_Struct;     { between 0 AND iept_Range     }

    iept_Pressure : Short;  { -128 to 127 (unused, set to 0)  }
   END;
   IEPointerTabletPtr = ^IEPointerTablet;

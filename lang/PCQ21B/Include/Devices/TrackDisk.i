...
Type
    ...
    TDU_PublicUnit = record
        ...
        tdu_CurrTrk     : Short;        {    track the heads are over... }
                                        {    ONLY ACCESS WHILE UNIT IS STOPPED! }
        tdu_CalibrateDelay : Integer;   {    time to wait after stepping }
                                        {    during a recalibrate }
        tdu_Counter     : Integer;      {    counter for disk changes... }
                                        {    ONLY ACCESS WHILE UNIT IS STOPPED! }

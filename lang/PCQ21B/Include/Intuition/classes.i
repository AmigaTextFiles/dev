...
FUNCTION INST_DATA(cl : IClassPtr; o : _ObjectPtr) : ADDRESS;
 External;
 

{ sizeof the instance data for a given class }
FUNCTION SIZEOF_INSTANCE(cl : IClassPtr) : Integer;
 External;


{ convenient typecast  }
FUNCTION _OBJ(o : _Objectptr) : _ObjectPtr;
 External;

{ get "public" handle on baseclass instance from real beginning of obj data }
FUNCTION BASEOBJECT(_obj : _ObjectPtr) : ADDRESS;
 External;

{ get back to object data struct from public handle }
FUNCTION __OBJECT(o : _ObjectPtr) : _ObjectPtr;
 External;

{ get class pointer from an object handle      }
FUNCTION OCLASS(o : Address) : IClassPtr;
 External;

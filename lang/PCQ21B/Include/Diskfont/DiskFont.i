...
{$I   "Include:Diskfont/DiskFontBaseVar.i"}
...
Type
   ...
   TFontContents = Record
    ...
    tfc_TagCount  : Short;       { including the TAG_DONE tag }
    {
    ...
    tfc_Flags     : Byte;
   END;
   TFontContentsPtr =^TFontContents;
...
Type
    ...
    TAvailFonts = Record
        taf_Type        : Short;           { MEMORY, DISK, OR SCALED }
        taf_Attr        : TTextAttr;       { text attributes for font }
    END;
    TAvailFontsPtr = ^TAvailFonts;
...
FUNCTION NewScaledDiskFont(srcFont : TextFontPtr; destTextAttr : TTextAttrPtr) : DiskFontHeaderPtr;
    External;
...
FUNCTION OpenDiskFontLib(vers : Integer): Boolean;
external;

PROCEDURE CloseDiskFontLib;
external;

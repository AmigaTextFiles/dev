...
{$I   "Include:Libraries/GadToolsBaseVar.i"}
...
Type
   NewMenu = Record
    ...
    nm_Label          : String;            {  Menu's label }
    nm_CommKey        : String;            {  MenuItem Command Key Equiv }
    nm_Flags          : Short;             {  Menu OR MenuItem flags (see note) }
    nm_MutualExclude  : Integer;           {  MenuItem MutualExclude word }
...
FUNCTION GTMENU_USERDATA(m : MenuPtr) : Integer;
 External;

FUNCTION GTMENUITEM_USERDATA(mi : MenuItemPtr) : Integer;
 External;
...
{ structure of LV_DRAW messages, object is a (struct Node *) }
 LVDrawMsg = Record
    ...
    lvdm_State          : Integer;     { how to render     }
 end;
 LVDrawMsgPtr = ^LVDrawMsg;
...
FUNCTION CreateGadgetA(kind : Integer; gad, ng : GadgetPtr; taglist : Address) : GadgetPtr;
 External;

PROCEDURE FreeGadgets(gad : GadgetPtr);
 External;

PROCEDURE GT_SetGadgetAttrsA(gad : GadgetPtr; win : WindowPtr; req : RequesterPtr; taglist : Address);
 External;

{ Menu functions }
FUNCTION CreateMenusA(nm : NewMenuPtr; taglist : ADDRESS) : MenuPtr;
 External;

PROCEDURE FreeMenus(m : MenuPtr);
 External;

FUNCTION LayoutMenuItemsA(firstitem : MenuItemPtr; vi : Address; taglist : Address) : Boolean;
 External;

FUNCTION LayoutMenusA(firstmenu : MenuPtr; vi, taglist : Address) : Boolean;
 External;

{ Misc Event-Handling Functions }
FUNCTION GT_GetIMsg(iport : MsgPortPtr) : IntuiMessagePtr;
 External;
...
FUNCTION GT_FilterIMsg(imsg : IntuiMessagePtr) : IntuiMessagePtr;
 External;

FUNCTION GT_PostFilterIMsg(imsg : IntuiMessagePtr) : IntuiMessagePtr;
 External;

FUNCTION CreateContext( PtrToEmptyGadgetRecord : Address ) : GadgetPtr;
 External;

{ Rendering Functions }
PROCEDURE DrawBevelBoxA(rport : RastPortPtr;left,top,width,height : Short; taglist : Address);
 External;

{ Visuals Functions }
FUNCTION GetVisualInfoA(Scr : ScreenPtr; taglist : ADDRESS) : ADDRESS;
 External;

PROCEDURE FreeVisualInfo(vi : Address);
 External;


{--- functions in V39 or higher (Release 3) ---}

FUNCTION GT_GetGadgetAttrsA(gad : GadgetPtr; Win : WindowPtr;
                            req : RequesterPtr; taglist : Address) : Integer;
    External;

{ some extra functions to open and close the lib }

FUNCTION OpenGadToolsLib(vers : Integer): Boolean;
external;

PROCEDURE CloseGadToolsLib;
external;
...
{$C+}
FUNCTION CreateGadget(kind : INTEGER; gad,ng : GadgetPtr; ...): GadgetPtr;
EXTERNAL;

PROCEDURE DrawBevelBox(rport : RastPortPtr; left,top,width,height : Short; ...);
EXTERNAL;

FUNCTION LayoutMenus(firstmenu : MenuPtr; vi : ADDRESS; ...): BOOLEAN;
EXTERNAL;

PROCEDURE GT_SetGadgetAttrs(gad : GadgetPtr; Win : WindowPtr;
                           req : RequesterPtr; ...);
EXTERNAL;

FUNCTION LayoutMenuItems(firstitem : MenuItemPtr; vi : ADDRESS; ...): BOOLEAN;
EXTERNAL;

FUNCTION GT_GetGadgetAttrs(gad : GadgetPtr; Win : WindowPtr;
                           req : RequesterPtr; ...): INTEGER;
EXTERNAL;

FUNCTION GetVisualInfo(Scr : ScreenPtr; ...): ADDRESS;
EXTERNAL;

FUNCTION CreateMenus(nm : NewMenuPtr; ...): MenuPtr;
EXTERNAL;
{$C-}

...
type
       FileRequester = Record
        ...
        rf_File        : String;        { Filename pointer             }
        rf_Dir         : String;        { Directory name pointer       }
        rf_Reserved1   : Array[0..9] of Byte;
        rf_LeftEdge,rf_TopEdge,         { Preferred window pos }
        rf_Width,rf_Height : WORD;      { Preferred window size  }
        rf_Reserved2   : Array[0..1] of Byte;
        rf_NumArgs     : Integer;       { A-la WB Args, for multiselects }
        rf_ArgList     : WBArgListPtr;
        rf_UserData    : Address;       { Applihandle (you may write!!) }
        rf_Reserved3   : Array[0..7] of Byte;
        rf_Pat         : String;      { Pattern match pointer }
...
Type
 FontRequester = Record
    ...
    fo_FrontPen,                                { Returned front pen               }
    fo_BackPen,                                 { Returned back pen                }
    fo_DrawMode         : Byte;                 { Returned drawing mode            }
    fo_Reserved1        : Byte;
    fo_UserData         : APTR;                 { You can store your own data here }
    fo_LeftEdge,                                { Coordinates of requester on exit }
    fo_TopEdge,
    fo_Width,
    fo_Height           : WORD;
    fo_TAttr            : TTextAttr;            { Returned TTextAttr               }
 end;
 FontRequesterPtr = ^FontRequester;
...
Type
 ScreenModeRequester = Record
    sm_DisplayID        : Integer;  { Display mode ID                  }
    sm_DisplayWidth,                { Width of display in pixels       }
    sm_DisplayHeight    : Integer;  { Height of display in pixels      }
    sm_DisplayDepth,                { Number of bit-planes of display  }
    sm_OverscanType     : WORD;     { Type of overscan of display      }
    sm_AutoScroll       : Boolean;  { Display should auto-scroll?      }

    sm_BitMapWidth,                 { Used to create your own BitMap   }
    sm_BitMapHeight     : Integer;

    sm_LeftEdge,                    { Coordinates of requester on exit }
    sm_TopEdge,
    sm_Width,
    sm_Height           : WORD;

    sm_InfoOpened       : Boolean;  { Info window opened on exit?      }
    sm_InfoLeftEdge,                { Last coordinates of Info window  }
    sm_InfoTopEdge,
    sm_InfoWidth,
    sm_InfoHeight       : WORD;
 ...
 ScreenModeRequesterPtr = ^ScreenModeRequester;
 ...
 * allowed.
 ...
 DisplayMode = Record
    ...
    dm_PropertyFlags : Integer;         { applicable properties }
 end;
 DisplayModePtr = ^DisplayMode;
...
FUNCTION AllocAslRequest(Typ : Integer; TagList : Address) : Address;
    External;

FUNCTION AllocFileRequest : FileRequesterPtr;
    External;

FUNCTION AslRequest(Req : Address; TagList : Address) : Boolean;
    External;

PROCEDURE FreeAslRequest(Req : Address);
    External;

PROCEDURE FreeFileRequest(Req : FileRequesterPtr);
    External;

FUNCTION RequestFile(Req : FIleRequesterPtr) : Boolean;
    External;

{ some extra functions to open and close the lib }

FUNCTION OpenAslLib(vers : Integer): Boolean;
external;

PROCEDURE CloseAslLib;
external;
...
{$C+}
FUNCTION AllocAslRequestTags(Typ : INTEGER; ...): address;
EXTERNAL;

FUNCTION AslRequestTags(Req : Address; ...): BOOLEAN;
EXTERNAL;
{$C-}

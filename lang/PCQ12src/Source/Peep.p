...
begin
	...
	if IsDataRegister(Peephole[Op1].Operand1) and
	   streq(Peephole[Op1].Operand2,Peephole[Op2].Operand1) then begin
		...
		Peephole[Op2].Deleted := True; Inc(Count[4]);
	    end else begin
		Peephole[Op2].Operand1 := Peephole[Op1].Operand1; Inc(Count[3]);
	    end;
	    Changed := True;

...
{$endif MAKELIB}
  ...
  (***********************************************************************)
  (* PROCEDURE CloseFile - this routine is called each time              *)
  (*  a file is closed, it updates the linked list of opened files       *)
  (***********************************************************************)
    procedure CloseFile(var f:file);
  (***********************************************************************)
  (* PROCEDURE CloseAll                                                  *)
  (*  This routine will close all outstanding opened files when the      *)
  (*  compiler aborts - only implemented in TBufferedFile currently.     *)
  (***********************************************************************)
    Procedure CloseAll;
    ...
    if InOutRes = 0 then
     Begin
       if FileList = nil then
         Begin
           new(FileList);
           FileList^.Handle := filerec(f).handle;
           Move(f,FileList^.FileInfo,sizeof(f));
           FileList^.Closed := FALSE;
           FileList^.next := nil;
           exit;
         end;
       hp:=FileList;
      ...
      hp^.next:=hp1;
      hp1^.next:=nil;
      Move(f,hp1^.FileInfo,sizeof(f));
      hp1^.handle:=filerec(f).handle;
      hp1^.closed:=FALSE;
    end;
{$IFDEF EXTDEBUG}
    WriteLn('OPENED: ',filerec(f).name);
{$ENDIF}
 end;
 ...
 Begin
    ...
    while assigned(hp) do
     Begin
        {-------------------------------------------------------}
        { This is a three step process, first is it the same    }
        { file handle, if so then check if it is the same name  }
        { this is possible because the file might have been     }
        { closed, and the file handle assigned a new name,      }
        { finally check if the file is already closed, in case  }
        { we have a file opened more then once with the same    }
        { handle.                                               }
        { we could add more checking by rescanning if a file is }
        { closed more then once.                                }
        {-------------------------------------------------------}
        if (hp^.handle = filerec(f).handle) and
           (strcomp(filerec(hp^.FileInfo).name,filerec(f).name) = 0) and
           (hp^.closed=FALSE) then
         Begin
           hp^.closed:=TRUE;
           break;
         end;
 ...
 Begin
    ...
    while assigned(hp) do
     Begin
        ...
        Begin
             Move(hp^.FileInfo,f,sizeof(f));
{$IFDEF EXTDEBUG}
             WriteLn('FINAL CLOSE: ',filerec(f).name);
{$ENDIF EXTDEBUG}
             Close(f);
             hp^.closed:=TRUE;
        end;

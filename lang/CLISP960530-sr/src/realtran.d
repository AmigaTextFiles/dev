  ...
  local object pi_F_float_F(f)
    ...
    { floatcase(f,
     ...
     {var reg6 object pi = O(LF_pi);
      var reg7 uintC f_len = TheLfloat(f)->len; # gewünschte Länge von Pi
      var reg4 uintC oldlen = TheLfloat(pi)->len; # vorhandene Länge
      var reg8 uintC newlen;
      if (f_len < oldlen) { return LF_shorten_LF(pi,f_len); }
      if (f_len == oldlen) { return pi; }
      # TheLfloat(O(LF_pi))->len um mindestens einen konstanten Faktor
      ...
      #   (/ (expt a 2) t)
      # )
      {var reg3 uintC len = newlen + 1; # Arbeite mit Long-Floats mit len Digits
       if (uintCoverflow(len)) { fehler_LF_toolong(); }
       {var reg2 uintL uexp_limit = LF_exp_mid - intDsize*(uintL)newlen; # LF_exp_mid - n
...
# Generiert eine Funktion wie F_atanx_F
  #define GEN_F_atanx(name,Fixnum_plusminus1,F_plusminus_F)                            \
    local object CONCAT3(F_,name,_F) PARM1(x,                                          \
      var reg3 object x)                                                               \
...
# F_atanhx_F : mit x -> x+sqrt(x^2+1), a = x^2
  GEN_F_atanx(atanhx,Fixnum_minus1,_EMA_)
      ...
      # evtl. Vorzeichenwechsel oder Vertauschen:
       {var reg2 object q = STACK_(2+1);
        switch (I_fixnump(q) # q mod 4, je nachdem ob q Fixnum oder Bignum
                ? (as_oint(q) >> oint_addr_shift) & (bit(2)-1)
                : TheBignum(q)->data[(uintP)(TheBignum(q)->length)-1] & (bit(2)-1)
               )
          { case 0:
      ...
      loop
        { if (eq(a,Fixnum_1)) break; # a=1 -> Rekursion zu Ende
          if (I_I_comp(a,b) >=0)
            ...
            { swap(reg3 object, a, b);
              swap(reg3 uintL, uc, ud); swap(reg3 uintL, vc, vd);
            }
        }
      # a=1 -> c=0,d=1 -> Ergebnis ud/vd
      pushSTACK(UL_to_I(ud)); # ud als Integer
     {var reg5 object y = UL_to_I(vd); # vd als Integer

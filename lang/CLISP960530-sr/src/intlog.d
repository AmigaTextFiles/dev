    ...
    { # Nun sind unterhalb von destptr n Digits Platz.
      ...
      if (I_fixnump(obj))
        ...
        { var reg2 uintC len = TheBignum(obj)->length;
          # Pointer bestimmen:
          var reg1 uintD* ptr = &TheBignum(obj)->data[(uintP)len];
          n -= len;
  ...
  local object I_lognot_I(x)
    ...
    { if (I_fixnump(x)) # Fixnum -> ganz einfach:
        ...
        { SAVE_NUM_STACK # num_stack retten
          ...
          var reg5 uintC n;
          BN_to_NDS(x, MSDptr=,n=,_EMA_); # NDS zu x bilden
    ...
    { if (I_fixnump(x))
        ...
        if (I_fixnump(y))
          ...
          { var reg6 uintD* xMSDptr;
            ...
            var reg6 uintC ylen;
            BN_to_NDS_nocopy(x, xMSDptr=,xlen=,_EMA_);
            BN_to_NDS_nocopy(y, yMSDptr=,ylen=,_EMA_);
  ...
  local boolean I_oddp(x)
    ...
    { if (I_fixnump(x))
        ...
        { var reg1 Bignum x_ = TheBignum(x);
          if (x_->data[(uintP)(x_->length)-1] & bit(0))
            return TRUE;
            else
            return FALSE;
    }   }
     ...
     {SAVE_NUM_STACK # num_stack retten
      if (!(R_minusp(y)))
        ...
        if (I_bignump(y) # y ein Bignum
            || ((log2_intDsize+intCsize < oint_data_len) # intDsize*2^intCsize < 2^oint_data_len ?
                && (as_oint(y) >= as_oint(fixnum(intDsize*bitc(intCsize)))) # ein Fixnum > Bitlänge aller Integers
           )   )
          # y so groß, daß selbst (ASH 1 y) einen Überlauf gäbe.
          goto badamount;
          else
          { var reg2 uintL y_ = (as_oint(y)-as_oint(Fixnum_0))>>oint_data_shift; # Wert von y, >=0, <intDsize*2^intCsize
        ...
        if (I_bignump(y)) goto sign; # y ein Bignum -> Vorzeichen von x zurück
          ...
          { var reg2 uintL y_ = ((as_oint(Fixnum_minus1)-as_oint(y))>>oint_data_shift)+1; # Wert von -y, >0
            var reg2 uintL i = y_%intDsize; # i = (-y) mod intDsize, >=0, <intDsize
            var reg2 uintL k = floor(y_,intDsize); # k = (-y) div intDsize, >=0
            # DS zu x bilden:
            var reg5 uintD* MSDptr;
            var reg6 uintC len;
            I_to_NDS(x, MSDptr=,len=,_EMA_); # DS zu x bilden.
      ...
      badamount:
        ...
        fehler(error, GETTEXT("~: too large shift amount ~"));
    }}
  ...
  local object I_logcount_I(x)
    ...
    { if (I_fixnump(x))
        ...
        { var reg6 uintD* MSDptr;
          var reg3 uintC len;
          BN_to_NDS_nocopy(x, MSDptr=,len=,_EMA_); # DS zu x bilden, len>0.
         {var reg4 uintL bitcount = 0; # Bitzähler
          ...
          if (log2_intDsize+intCsize<=oint_data_len) # intDsize*2^intCsize <= 2^oint_data_len ?
...
#elif defined(GNU) && defined(I80Z86) && !defined(NO_ASM)
  #define integerlength16(digit,size_zuweisung)  \
    { var reg1 uintW one_position; # Position der führenden 1               \
...
#elif defined(GNU) && defined(__ibm032__) && !defined(NO_ASM) # RT/ROMP
  #define integerlength32(digit,size_zuweisung)  \
    { var reg2 uintL x32 = (uint32)(digit);                \
      if (x32 >= bit(16))                                  \
  ...
  global uintL I_integer_length(x)
    ...
    { if (I_fixnump(x))
        ...
        { var reg4 uintD* MSDptr;
          var reg5 uintC len;
          BN_to_NDS_nocopy(x, MSDptr=,len=,_EMA_); # normalisierte DS zu x bilden.

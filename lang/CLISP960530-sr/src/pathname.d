...
# Dasselbe, rekursiv wie mit SUBST:
...
#if defined(PATHNAME_UNIX) || defined(PATHNAME_OS2) || defined(PATHNAME_RISCOS) || defined(PATHNAME_AMIGAOS)
  ...
  local object common_case(string)
    ...
    { if (!simple_string_p(string))
        return string;
     {var reg7 uintL len = TheSstring(string)->length;
      # Suche, ob Groß- oder Kleinbuchstaben (oder beides) vorkommen:
      var reg5 boolean all_upper = TRUE;
      var reg4 boolean all_lower = TRUE;
      { var reg2 uintB* ptr = &TheSstring(string)->data[0];
  ...
  local object subst_common_case(obj)
    ...
    { if (atomp(obj))
      ...
      { var reg2 object new_car = subst_common_case(Car(obj));
        pushSTACK(new_car);
      }
      # rekursiv für den CDR aufrufen:
      { var reg2 object new_cdr = subst_common_case(Cdr(STACK_1));
    ...
    { if (eq(host,unbound)) { return NIL; } # nicht angegeben -> NIL
      ...
      badhost:
        { pushSTACK(host);
          ...
          fehler(error, GETTEXT("~: illegal hostname ~"));
        }
    }
...
local object parse_logical_word(z,subdirp)
  ...
  { var zustand startz; startz = *z; # Start-Zustand
   {var reg4 uintB ch;
    ...
    var reg8 boolean last_was_star = FALSE;
    var reg9 boolean seen_starstar = FALSE;
    loop
      { if (z->count == 0) break;
   ...
   {var reg5 uintL len = z->index - startz.index;
    if (subdirp)
    ...
    if (len==0)
      ...
      { var reg6 object result = allocate_string(len);
        # und füllen:
        {var reg2 uintB* ptr1 = &TheSstring(STACK_2)->data[startz.index];
         var reg3 uintB* ptr2 = &TheSstring(result)->data[0];
  ...
  { # Host-Specification parsen:
    ...
    loop
      { # Versuche, ein weiteres Unterdirectory zu parsen.
        var reg2 object subdir = parse_logical_word(&z,TRUE);
    ...
    # Name parsen:
    { var reg1 object name = parse_logical_word(&z,FALSE);
      ...
      if ((z.count > 0) && (TheSstring(STACK_2)->data[z.index]=='.'))
        { var zustand z_name; z_name = z;
          ...
          z.index++; z.FNindex = fixnum_inc(z.FNindex,1); z.count--;
          # Typ parsen:
         {var reg2 object type = parse_logical_word(&z,FALSE);
          ...
          if (!nullp(type))
            { if ((z.count > 0) && (TheSstring(STACK_2)->data[z.index]=='.'))
                { var zustand z_type; z_type = z;
                  ...
                  z.index++; z.FNindex = fixnum_inc(z.FNindex,1); z.count--;
                  # Version parsen:
                 {var reg3 object version = parse_logical_word(&z,FALSE);
...
# CLTL S. 414
  { # Stackaufbau: thing, host, defaults, start, end, junk-allowed.
    ...
    var reg7 boolean parse_logical = FALSE;
    ...
    { var reg2 object host;
      ...
      if (nullp(host))
        { # host := (PATHNAME-HOST defaults)
          var reg1 object defaults = test_default_pathname(STACK_2);
          #ifdef LOGICAL_PATHNAMES
    ...
    { var reg5 object thing = STACK_4;
      ...
      if (!stringp(thing))
        { if (!symbolp(thing)) { fehler_thing(thing); }
          ...
          if (!parse_logical)
            {
              #if defined(PATHNAME_UNIX) || defined(PATHNAME_OS2) || defined(PATHNAME_RISCOS)
       ...
       {var object string; # String thing
        ...
        if (parse_logical)
          { pushSTACK(allocate_logpathname());
           ...
           {var reg1 uintL remaining = parse_logical_pathnamestring(z);
        ...
        #endif
          {
              ...
              if ((!(z.count==0)) && (TheSstring(STACK_0)->data[z.index] == '<'))
                { var zustand startz = z; # Start-Zustand
                 ...
                 {var reg4 object envvar;
                  { var reg3 uintL len = z.index - startz.index - 2;
                    if (len==0) goto no_envvar;
                    envvar = allocate_string(len+1);
                    # und füllen:
                   {var reg2 uintB* ptr1 = &TheSstring(STACK_0)->data[startz.index+1];
                    var reg1 uintB* ptr2 = &TheSstring(envvar)->data[0];
                    dotimesL(len,len, { *ptr2++ = *ptr1++; });
                    *ptr2 = '\0';
                  }}
                  # Dessen Wert holen:
                   begin_system_call();
                  {var reg1 const char* envval = getenv(TheAsciz(envvar));
                   end_system_call();
                   if (envval==NULL)
                     { pushSTACK(envvar);
                       ...
                       fehler(error, GETTEXT("~: there is no environment variable ~"));
                     }
                   pushSTACK(asciz_to_string(envval)); # Wert der Variablen als String
                 }}
                  # Reststück bilden:
                  { var reg4 uintL len = z.count;
                    var reg3 object tail = allocate_string(len);
                    var reg2 uintB* ptr1 = &TheSstring(STACK_1)->data[z.index];
                    var reg1 uintB* ptr2 = &TheSstring(tail)->data[0];
                    dotimesL(len,len, { *ptr2++ = *ptr1++; } );
                    pushSTACK(tail);
                  }
                  # Beides zusammenhängen, thing ersetzen:
                  { var reg2 uintL envval_len = TheSstring(STACK_1)->length;
                    var reg1 object new_thing = string_concat(2);
                    ...
                    z.index = 0; z.count = TheSstring(new_thing)->length; z.FNindex = Fixnum_0;
                  }
                  goto envvar_ok;
                 ...
                 var reg1 uintB ch;
                   ...
                   if (ch=='-')
                     { # '-' übergehen:
                       ...
                       # Host-String bilden:
                       { var reg4 uintL len = z.index - startz.index - 1;
                         if (len==0) goto no_hostspec;
                         host = allocate_string(len);
                         # und füllen:
                        {var reg2 uintB* ptr1 = &TheSstring(STACK_1)->data[startz.index+1];
                         var reg3 uintB* ptr2 = &TheSstring(host)->data[0];
                         dotimesL(len,len, { *ptr2++ = *ptr1++; });
                       }}
                     }
            ...
            #endif
            {
                     ...
                     {var reg1 struct passwd * userpasswd = getpwnam(TheAsciz(username));
                      ...
                      if (userpasswd == (struct passwd *)NULL) # erfolglos?
                        { if (!(errno==0)) { OS_error(); } # Error melden
                          ...
                          fehler(error, GETTEXT("~: there is no user named ~"));
                        }
              ...
              if ((!(z.count == 0)) && (TheSstring(STACK_2)->data[z.index] == '$'))
                ...
                { # Character übergehen:
                 ...
                 {var reg6 object envval_dir;
                  ...
                  { var reg5 object envvar = allocate_string(charcount+1);
                    { var reg1 uintB* charptr2 = &TheSstring(envvar)->data[0];
                      var reg4 uintL count;
                      charptr = &TheSstring(STACK_2)->data[z.index];
              ...
              #endif
              #if defined(PATHNAME_UNIX) || defined(PATHNAME_MSDOS) || defined(PATHNAME_OS2)
              ...
              # Stackaufbau: ..., Datenvektor, Pathname, (last (pathname-directory Pathname)),
              #              string.
              split_name_type(sym_nullp(S(read_pathname_p))); # String STACK_0 in Name und Typ aufspalten
              after_name_type:
              # Stackaufbau: ..., Datenvektor, Pathname, (last (pathname-directory Pathname)),
              #              name, type.
              # Name und Typ in Pathname eintragen:
        ...
        if (!junk_allowed)
          ...
          if (!(z.count == 0))
            { pushSTACK(z.FNindex); # letzter Index
              ...
              fehler(error, GETTEXT("~: syntax error in filename ~ at position ~"));
            }
        #ifdef LOGICAL_PATHNAMES
        if (parse_logical)
          { if (!nullp(STACK_(3+2)))
...
# (PATHNAME-HOST pathname), CLTL S. 417, CLtL2 S. 644
  { var reg1 object pathname = coerce_xpathname(STACK_1);
    #ifdef LOGICAL_PATHNAMES
    if (logpathnamep(pathname))
      ...
      {
        #if HAS_HOST
        var reg2 object erg = ThePathname(pathname)->pathname_host;
...
# (PATHNAME-DEVICE pathname), CLTL S. 417, CLtL2 S. 644
  { var reg1 object pathname = coerce_xpathname(STACK_1);
    ...
    if (logpathnamep(pathname))
      ...
      {
        #if HAS_DEVICE
        var reg2 object erg = ThePathname(pathname)->pathname_device;
        value1 = (eq(STACK_0,S(Kcommon)) ? common_case(erg) : erg);
...
# (PATHNAME-DIRECTORY pathname), CLTL S. 417, CLtL2 S. 644
  { var reg1 object pathname = coerce_xpathname(STACK_1);
    ...
    #endif
      { var reg2 object erg = ThePathname(pathname)->pathname_directory;
...
# (PATHNAME-NAME pathname), CLTL S. 417, CLtL2 S. 644
  { var reg1 object pathname = coerce_xpathname(STACK_1);
    ...
    #endif
      { var reg2 object erg = ThePathname(pathname)->pathname_name;
        value1 = (eq(STACK_0,S(Kcommon)) ? common_case(erg) : erg);
      }
...
# (PATHNAME-TYPE pathname), CLTL S. 417, CLtL2 S. 644
  { var reg1 object pathname = coerce_xpathname(STACK_1);
    ...
    #endif
      { var reg2 object erg = ThePathname(pathname)->pathname_type;
        value1 = (eq(STACK_0,S(Kcommon)) ? common_case(erg) : erg);
      }
...
# (PATHNAME-VERSION pathname), CLTL S. 417, CLtL2 S. 644
  { var reg1 object pathname = coerce_xpathname(popSTACK());
    #ifdef LOGICAL_PATHNAMES
    if (logpathnamep(pathname))
...
# (LOGICAL-PATHNAME thing), CLtL2 S. 631
  { var reg1 object thing = popSTACK();
        ...
        loop
          { if (!nullp(shifthash(STACK_0,STACK_1,T)))
            ...
            if (nullp(TheLogpathname(STACK_1)->pathname_host))
              ...
              { var reg1 object new = allocate_logpathname();
                var reg2 object old = STACK_1;
                TheLogpathname(new)->pathname_host      = O(default_logical_pathname_host); # Default "SYS"
                TheLogpathname(new)->pathname_directory = TheLogpathname(old)->pathname_directory;
                TheLogpathname(new)->pathname_name      = TheLogpathname(old)->pathname_name;
                TheLogpathname(new)->pathname_type      = TheLogpathname(old)->pathname_type;
                TheLogpathname(new)->pathname_version   = TheLogpathname(old)->pathname_version;
                STACK_1 = new;
              }
           {var reg2 object host = TheLogpathname(STACK_1)->pathname_host;
            var reg1 object translations = gethash(host,Symbol_value(S(logpathname_translations)));
      ...
      { var reg2 object directory = ThePathname(pathname)->pathname_directory;
        ...
        if (eq(Car(directory),S(Kabsolute)))
          ...
          { directory = Cdr(directory); # übergehen
           {var reg1 object firstdir = Car(directory);
...
# verändert STACK
  ...
  local uintC nametype_namestring_parts PARM3(name,type,version,
    var reg2 object name,
    var reg1 object type,
    var reg4 object version)
  #else
  local uintC nametype_namestring_parts_ (object name, object type);
  local uintC nametype_namestring_parts_ PARM2(name,type,
    var reg2 object name,
    var reg1 object type)
...
# (FILE-NAMESTRING pathname), CLTL S. 417
  { var reg1 object pathname = coerce_pathname(popSTACK());
  ...
  local object recalc_defaults_pathname()
    {
      ...
      set_Symbol_value(S(default_pathname_defaults),value1);
      return value1;
    }
    ...
    { var reg1 object pathname = Symbol_value(S(default_pathname_defaults)); # Wert von *DEFAULT-PATHNAME-DEFAULTS*
      if (pathnamep(pathname))
        ...
        { pushSTACK(subr_self); # subr_self retten (für spätere Fehlermeldungen)
          ...
          pushSTACK(OL(defaults_warn_string));
...
# :WILD-Komponenten ersetzt.
  { var reg10 boolean wildp = !(eq(STACK_0,unbound) || nullp(STACK_0));
    skipSTACK(1);
    ...
    if (logpathnamep(STACK_1) && logpathnamep(STACK_0))
      ...
      { var reg6 object new = allocate_logpathname(); # neuen Pathname holen
        var reg8 object d = popSTACK(); # defaults
        var reg7 object p = popSTACK(); # pathname
        # Hosts matchen:
        { var reg1 object p_host = TheLogpathname(p)->pathname_host;
          var reg2 object d_host = TheLogpathname(d)->pathname_host;
          TheLogpathname(new)->pathname_host = p_host; # erstmal new-host := pathname-host
          ...
          if (wildp ? FALSE : nullp(p_host))
            { # pathname-host nicht angegeben, aber defaults-host angegeben:
              TheLogpathname(new)->pathname_host = d_host; # new-host := defaults-host
              goto lmatch_directories;
        }   }
        # Directories nicht matchen:
        { # new-directory := pathname-directory :
          TheLogpathname(new)->pathname_directory = TheLogpathname(p)->pathname_directory;
        }
        goto ldirectories_OK;
        lmatch_directories:
        # Directories matchen:
        { var reg2 object p_directory = TheLogpathname(p)->pathname_directory; # pathname-directory
          var reg3 object d_directory = TheLogpathname(d)->pathname_directory; # defaults-directory
      ...
      # Fängt pathname-subdirs mit :RELATIVE an?
          ...
          TheLogpathname(new)->pathname_directory = new_subdirs; # new-directory := new-subdirs
        }}
        ldirectories_OK:
        # Nun sind die Directories OK.
        # Name matchen:
        # Verwende pathname-name, falls angegeben, und defaults-name sonst.
        { var reg1 object p_name = TheLogpathname(p)->pathname_name;
          TheLogpathname(new)->pathname_name =
        ...
        { var reg1 object p_type = TheLogpathname(p)->pathname_type;
          TheLogpathname(new)->pathname_type =
        ...
        { var reg1 object p_version = TheLogpathname(p)->pathname_version;
          TheLogpathname(new)->pathname_version =
    ...
    #endif
   {var reg6 object new = allocate_pathname(); # neuen Pathname holen
    var reg8 object d = popSTACK(); # defaults
    var reg7 object p = popSTACK(); # pathname
    #if HAS_HOST
    # Hosts matchen:
...
# ) ) )
  { # pathname und defaults überprüfen:
   ...
   {var reg6 object new = allocate_pathname();
    pushSTACK(new);
          ...
          if (equal(p_directory,d_directory))
            ...
            { # Fängt weder pathname-subdirs noch defaults-subdirs
              ...
              if (   (!eq(Car(p_directory),S(Krelative)))
                ...
                { var reg1 object Lp = p_directory;
                  var reg2 object Ld = d_directory;
    ...
    { if (eq(obj,S(Kwild))) { return TRUE; }
     ...
     {var reg3 uintL len = TheSstring(obj)->length;
      ...
      {var reg2 uintB* charptr = &TheSstring(obj)->data[0];
       var reg4 boolean last_was_star = FALSE;
       dotimespL(len,len,
         { var reg1 uintB ch = *charptr++;
...
# CLTL S. 416, CLtL2 S. 643
  ...
  { var reg5 boolean logical = FALSE;
    var reg4 boolean convert = eq(STACK_6,S(Kcommon));
    # 1. host überprüfen:
    #ifdef LOGICAL_PATHNAMES
      ...
      if (eq(device,unbound)) # angegeben ?
        ...
        { if (convert) { STACK_4 = device = common_case(device); }
          if (nullp(device)) goto device_ok; # = NIL ?
            ...
            { var reg2 uintB* ptr = &TheSstring(device)->data[0];
              var reg3 uintL count;
              dotimesL(count,TheSstring(device)->length,
                { var reg2 uintB ch = *ptr++;
    ...
    { var reg1 object directory = STACK_3;
      ...
      elif (stringp(directory) && legal_name(directory))
        { var reg1 object new_cons = allocate_cons();
          Car(new_cons) = S(Kabsolute); Cdr(new_cons) = allocate_cons();
          Car(Cdr(new_cons)) = directory;
          STACK_3 = new_cons;
          goto directory_ok;
        }
      # Keiner der gewünschten Fälle -> Fehler:
...
# wie MAKE-PATHNAME, nur daß ein Logical Pathname gebildet wird.
  { # Ein logischer Pathname als :HOST-Argument zu MAKE-PATHNAME
    ...
    var reg1 object obj = allocate_logpathname();
...
# < ergebnis: TRUE wenn string Wildcard-Zeichen enthält
  ...
  local boolean has_word_wildcards(string)
    ...
    { var reg2 uintL len = TheSstring(string)->length;
      var reg1 uintB* charptr = &TheSstring(string)->data[0];
...
# < ergebnis: TRUE wenn (PATHNAME-DIRECTORY pathname) Wildcards enthält.
  ...
  local boolean has_directory_wildcards(pathname)
    ...
    { # Directory überprüfen:
      #ifdef LOGICAL_PATHNAMES
      if (logpathnamep(pathname))
        { var reg1 object directory = TheLogpathname(pathname)->pathname_directory;
...
# < ergebnis: TRUE wenn (PATHNAME-NAME pathname) Wildcards enthält.
  ...
  local boolean has_name_wildcards(pathname)
    ...
    { # Name überprüfen:
      #ifdef LOGICAL_PATHNAMES
      if (logpathnamep(pathname))
        { var reg1 object name = TheLogpathname(pathname)->pathname_name;
...
# < ergebnis: TRUE wenn (PATHNAME-TYPE pathname) Wildcards enthält.
  ...
  local boolean has_type_wildcards(pathname)
    ...
    { # Typ überprüfen:
      #ifdef LOGICAL_PATHNAMES
      if (logpathnamep(pathname))
        { var reg1 object type = TheLogpathname(pathname)->pathname_type;
...
# (WILD-PATHNAME-P pathname [field-key]), CLtL2 S. 623
  { var reg3 object pathname = coerce_xpathname(STACK_1);
    var reg1 object key = STACK_0;
    var reg2 boolean erg;
...
# eine Pathname-Komponente ("Muster").
  ...
  local boolean directory_match_ab(m_list,b_list,logical)
    var reg2 object m_list;
    var reg1 object b_list;
    var reg4 boolean logical;
    { # Algorithmus analog zu wildcard_match_ab.
      var reg3 object item;
      loop
...
# (PATHNAME-MATCH-P pathname wildname), CLtL2 S. 623
  { # Stackaufbau: pathname, wildname.
    var reg3 boolean logical = FALSE;
    STACK_1 = coerce_xpathname(STACK_1);
    ...
    #endif
   {var reg2 object wildname = popSTACK();
...
# 3. Zum Schluß (MERGE-PATHNAMES modifiziertes_muster2 beispiel).
  ...
  local void wildcard_diff(muster,beispiel,previous,solutions)
    var reg2 object muster;
    var reg1 object beispiel;
    var reg4 object* previous;
    var reg3 object* solutions;
  ...
  local void wildcard_diff_ab(muster,beispiel,m_index,b_index,previous,solutions)
    var reg3 object muster;
    var reg2 object beispiel;
    var reg6 uintL m_index;
    var reg5 uintL b_index;
    var reg8 object* previous;
    var reg7 object* solutions;
    { var reg4 uintB c;
      loop
        { if (m_index == TheSstring(muster)->length)
            { if (b_index == TheSstring(beispiel)->length)
                { push_solution(); }
              return;
            }
          c = TheSstring(muster)->data[m_index++];
          if (c=='*') break;
          if (b_index == TheSstring(beispiel)->length) return;
          if (c=='?')
     ...
     {var reg9 uintL b_start_index = b_index;
      loop
        { # Um weniger zu consen, die Fälle abfangen, wo wildcard_diff_ab()
          # gar nichts tut:
          if (m_index == TheSstring(muster)->length
              ? b_index == TheSstring(beispiel)->length
              : (c = TheSstring(muster)->data[m_index],
                 (c=='*') || (c=='?')
                 || (b_index < TheSstring(beispiel)->length
...
# kann GC auslösen
  ...
  local void directory_diff_ab(m_list,b_list,logical,previous,solutions)
    var reg3 object m_list;
    var reg2 object b_list;
    var reg8 boolean logical;
    var reg6 object* previous;
    var reg7 object* solutions;
    { # Algorithmus analog zu wildcard_diff_ab.
      var reg4 object item;
      if (atomp(m_list))
        { if (atomp(b_list))
            { push_solution(); }
          return;
...
# Dasselbe, rekursiv wie mit SUBST:
  ...
  local object subst_logical_case(obj)
    ...
    { if (atomp(obj))
      ...
      pushSTACK(obj);
      # rekursiv für den CAR aufrufen:
      { var reg2 object new_car = subst_logical_case(Car(obj));
        pushSTACK(new_car);
      }
      # rekursiv für den CDR aufrufen:
      { var reg2 object new_cdr = subst_logical_case(Cdr(STACK_1));
        if (eq(new_cdr,Cdr(STACK_1)) && eq(STACK_0,Car(STACK_1)))
          { obj = STACK_1; skipSTACK(2); return obj; }
          else
          # (CONS new_car new_cdr)
...
# Dasselbe, rekursiv wie mit SUBST:
  ...
  local object customary_case(string)
    ...
    { if (!simple_string_p(string))
      ...
      nstring_capitalize(&TheSstring(string)->data[0],TheSstring(string)->length);
      return string;
      #endif
    }
  local object subst_customary_case(obj)
    ...
    { if (atomp(obj))
      ...
      pushSTACK(obj);
      # rekursiv für den CAR aufrufen:
      { var reg2 object new_car = subst_customary_case(Car(obj));
        pushSTACK(new_car);
      }
      # rekursiv für den CDR aufrufen:
      { var reg2 object new_cdr = subst_customary_case(Cdr(STACK_1));
        if (eq(new_cdr,Cdr(STACK_1)) && eq(STACK_0,Car(STACK_1)))
          { obj = STACK_1; skipSTACK(2); return obj; }
          else
          # (CONS new_car new_cdr)
...
# translate_pathname(&subst,muster)
  local object translate_pathname (object* subst, object muster, boolean for_logical_p);
...
# und verkürzen subst passend. Falls nicht passend, liefert es nullobj.
  ...
  local object convert_subdir (object muster, boolean for_logical_p);
  local object translate_subdir (object* subst, object muster, boolean logical, boolean for_logical_p);
  local object translate_directory (object* subst, object muster, boolean logical, boolean for_logical_p);
  ...
  local object translate_device(subst,muster,logical)
    ...
    var reg4 boolean logical;
    {
      ...
      if (eq(muster,S(Kwild)) && mconsp(*subst))
        { if (m_simple_string_p(Car(*subst)))
            { var reg3 object erg = Car(*subst); *subst = Cdr(*subst);
              return erg;
            }
            else
            return nullobj;
  ...
  local object translate_nametype(subst,muster,logical)
    var reg6 object* subst;
    var reg3 object muster;
    var reg7 boolean logical;
    { if (eq(muster,S(Kwild)) && mconsp(*subst))
        { if (m_simple_string_p(Car(*subst)))
            { var reg3 object erg = Car(*subst); *subst = Cdr(*subst);
              return erg;
      ...
      if (simple_string_p(muster))
        { pushSTACK(muster); # muster retten
         {var reg5 object* muster_ = &STACK_0;
          var reg4 uintL len = TheSstring(muster)->length;
          var reg1 uintL index = 0;
          var reg7 uintL stringcount = 0; # Anzahl der Strings auf dem Stack
          loop
            { var reg9 uintL last_index = index;
              var reg2 uintB c;
  ...
  # kann GC auslösen
  local object convert_subdir (object muster, boolean for_logical_p);
  local object convert_subdir(muster,for_logical_p)
    var reg2 object muster; 
    var reg3 boolean for_logical_p;
    {
      #ifdef PATHNAME_EXT83
      if (for_logical_p)
        { pushSTACK(muster);
          muster = allocate_cons();
          Car(muster) = popSTACK();
          Cdr(muster) = O(leer_string);
        }
      #endif
      return muster;
    }

  local object translate_subdir(subst,muster,logical,for_logical_p)
    var reg3 object* subst;
    var reg2 object muster;
    var reg4 boolean logical;
    var reg5 boolean for_logical_p;
    {
      #ifdef LOGICAL_PATHNAMES
      if (logical)
      ...
      #endif
      #ifdef PATHNAME_EXT83
      if (atomp(muster) && !for_logical_p) return muster;
      muster = convert_subdir(muster,for_logical_p);
  ...
  local object translate_directory(subst,muster,logical,for_logical_p)
    var reg2 object* subst;
    var reg4 object muster;
    var reg6 boolean logical;
    var reg7 boolean for_logical_p;
    { var reg5 uintL itemcount = 0; # Anzahl der Elemente auf dem Stack
      # Startpoint:
      pushSTACK(Car(muster)); muster = Cdr(muster); itemcount++;
      # subdirs:
      while (consp(muster))
        { var reg3 object item = Car(muster);
          muster = Cdr(muster);
          if (eq(item,S(Kwild_inferiors)))
            { if (mconsp(*subst))
                { if (listp(Car(*subst)))
                    { var reg1 object list = Car(*subst); *subst = Cdr(*subst);
                      while (consp(list))
                        { var reg1 object obj;
                          pushSTACK(list);
                          pushSTACK(muster);
                          obj = convert_subdir(Car(list),for_logical_p);
                          muster = popSTACK();
                          list = popSTACK();
                          pushSTACK(obj);
                          list = Cdr(list);
                ...
                { 
                  pushSTACK(muster);
                  item = convert_subdir(item,for_logical_p); itemcount++; 
                  muster = popSTACK();
                  pushSTACK(item);
                }
            }
            ...
            { pushSTACK(muster); # muster retten
              item = translate_subdir(subst,item,logical,for_logical_p);
  ...
  local object translate_pathname(subst,muster,for_logical_p)
    var reg2 object* subst;
    var reg3 object muster;
    var reg4 boolean for_logical_p;
    { var reg4 boolean logical = FALSE;
      var reg1 object item;
      ...
      #endif
      item = translate_directory(subst,xpathname_directory(logical,STACK_(2+2*HAS_DEVICE)),logical,for_logical_p);
...
# :merge = NIL --> letzten MERGE-PATHNAMES Schritt überspringen
  { # Stackaufbau: beispiel, muster1, muster2, all, merge.
    var reg3 boolean logical = FALSE; # Flag, ob beispiel und muster logische Pathnames sind
    var reg4 boolean logical2 = FALSE; # Flag, ob muster2 ein logischer Pathname ist
    var reg5 boolean logical1 = FALSE;
    STACK_4 = coerce_xpathname(STACK_4);
    STACK_3 = coerce_xpathname(STACK_3);
    STACK_2 = coerce_xpathname(STACK_2);
    #ifdef LOGICAL_PATHNAMES
    if (logpathnamep(STACK_4)) logical1 = TRUE;
    ...
    while (mconsp(STACK_1)) # solutions durchgehen
      { var reg1 object solutions = STACK_1;
        STACK_1 = Cdr(solutions);
       {var reg2 object solution = reverse(Car(solutions)); # Liste solution umdrehen
        ...
        #endif
        pushSTACK(solution);
        STACK_0 = translate_pathname(&STACK_0,STACK_(2+1+2),logical1);
       }
...
# kann GC auslösen
  ...
  local object namestring_asciz(dir_namestring)
    var reg2 object dir_namestring;
...
# < ergebnis: <0 falls Fehler
      ...
      local int getwdof(buf,drive)
        var uintB* buf;
        var uintB drive;
        { var char buf__[MAXPATHLEN+1];
          var reg1 uintB *buf_;
          var unsigned int current_drive;
          var unsigned int drive_count;
          _dos_getdrive(&current_drive);
          _dos_setdrive(drive,&drive_count);
          getcwd(buf__,MAXPATHLEN);
          buf_=buf__+3;
          while (*buf_) { *buf++=*buf_++; }
          *buf='\0';
          _dos_setdrive(current_drive,&drive_count);
          return 0;
        }
    #else
...
# < boolean ergebnis: ob dieses Laufwerk existiert und ansprechbar ist
  ...
  local boolean good_drive(drive)
    var reg1 uintB drive;
    { var char RootPathName[4];
      var reg2 boolean result;
      RootPathName[0]=drive;
      RootPathName[1]=':';
      RootPathName[2]='\\';
      RootPathName[3]='\0';
      begin_system_call();
      result = GetDriveType(RootPathName) > 0;
      end_system_call();
      return result;
    }
  #endif
  ...
  local uintB default_drive()
   ...
   local uintB default_drive()
    { var char path_buffer[3+MAXPATHLEN]; # vgl. GETWD(3)
      begin_system_call();
      GetCurrentDirectory (MAXPATHLEN,path_buffer);
      end_system_call();
...
# kann GC auslösen
  ...
  local object default_directory_of(drive)
    ...
    { var char path_buffer[3+MAXPATHLEN]; # vgl. GETWD(3)
        ...
        { var reg2 char* path_end = &path_buffer[asciz_length(path_buffer)];
                ...
                { if (matomp(Cdr(STACK_0))) # newlist (bis auf :ABSOLUTE) leer ?
                  ...
                  if (eq(Car(STACK_0),S(Kwild_inferiors))) # newlist fängt mit '...\' an ?
                    { # :PARENT von "...\" aus liefert Error
                      ...
                      fehler(file_error,  GETTEXT("\"..\\\\\" after \"...\\\\\" is invalid: ~"));
                    }
                  STACK_0 = Cdr(STACK_0);
                }
...
# kann GC auslösen
  local object assure_dir_exists_ (boolean tolerantp, boolean dirtolerantp);
  local object assure_dir_exists_(tolerantp,dirtolerantp)
    var reg4 boolean tolerantp;
    var reg5 boolean dirtolerantp;
      ...
      if (!(nullp(Cdr(ThePathname(STACK_0)->pathname_directory))
         ...
         *endptr = '\0'; # '\' am Schluß durch Nullbyte ersetzen
         ...
         if (stat(TheAsciz(dir_namestring),&statbuf) < 0)
           { end_system_call();
             if (tolerantp && (errno==ENOENT)) { return nullobj; }
             OS_error();
           }
         end_system_call();
...
# kann GC auslösen
  local object assure_dir_exists (boolean tolerantp);
     ...
     {var reg1 BPTR lock = Lock("",ACCESS_READ);
      if (lock==BPTR_NULL)
        { if (!(IoErr()==ERROR_OBJECT_NOT_FOUND)) { OS_error(); }
          ...
          fehler(file_error, GETTEXT("Couldn't access current directory"));
        }
      end_system_call();
...
# UP: Stellt sicher, daß das Directory eines Pathname existiert.
# assure_dir_exists(tolerantp,dirtolerantp)
       ...
       {var reg1 object namestring = string_concat(stringcount); # zusammenhängen
        ...
        { var reg3 BPTR lock = Lock(TheAsciz(namestring),ACCESS_READ);
         ...
         {local var LONGALIGNTYPE(struct FileInfoBlock) status;
          ...
          UnLock(lock);
          end_system_call();
          if (statusptr->fib_DirEntryType > 0 && !dirtolerantp) # Ist es ein Directory?
            { # STACK_0 = Wert für Slot PATHNAME von FILE-ERROR
            ...
            { pushSTACK(namestring);
              ...
              split_name_type(1);
             {var reg1 object pathname = STACK_(0+3); # der kopierte Pathname
              ThePathname(pathname)->pathname_type = popSTACK();
              ThePathname(pathname)->pathname_name = popSTACK();
              # Fertig.
...
# (Keine Vereinfachung, da wir ja den Truename bestimmen müssen.)
  global object assume_dir_exists (void);
  global object assume_dir_exists()
    { subr_self = L(open); return assure_dir_exists(FALSE); }
...
# > STACK_0: nicht-Logical Pathname, bei dem Directory kein :RELATIVE enthält.
# > tolerantp: Flag, ob ein Fehler vermieden werden soll
# > dirtolerantp:  Don't complain if file ends being a directory
# > subr_self: Aufrufer (ein SUBR)
...
# kann GC auslösen
  ...
  local object assure_dir_exists_ (boolean tolerantp, boolean dirtolerantp);
  local object assure_dir_exists_(tolerantp,dirtolerantp)
    var reg7 boolean tolerantp;
    var reg8 boolean dirtolerantp;
        ...
        { # Truepath des Directory bestimmen:
          ...
          { var reg5 object pathname = STACK_0;
           ...
           {var reg1 object namestring = string_concat(stringcount); # zusammenhängen
            ...
            end_system_call();
            # File existiert.
            if (S_ISDIR(status.st_mode) && !dirtolerantp) # Ist es ein Directory?
              { # STACK_0 = Wert für Slot PATHNAME von FILE-ERROR
                pushSTACK(whole_namestring(STACK_0));
                pushSTACK(TheSubr(subr_self)->name);
                //: DEUTSCH "~: ~ ist ein Directory und kein File."
              ...
              { if (allowed_links==0) # keine Links mehr erlaubt?
                  { errno = ELOOP_VALUE; OS_error(); } # ja -> UNIX-Error ELOOP simulieren
                allowed_links--; # danach ist ein Link weniger erlaubt
               {var reg4 uintL linklen = status.st_size; # vermutliche Länge des Link-Inhalts
                retry_readlink:
                ...
                { var reg3 object linkbuf = allocate_string(linklen); # Buffer für den Link-Inhalt
                  ...
                  # Link-Inhalt lesen:
                  begin_system_call();
                 {var reg1 int result = readlink(TheAsciz(namestring),TheAsciz(linkbuf),linklen);
                  end_system_call();
...
# < result: absolute pathname
  ...
  local object canonicalise_dirname(pathname,dirname)
    ...
    var reg4 object dirname;
    { var reg3 uintC stringcount = host_namestring_parts(pathname); # Strings für den Host
      ...
      pushSTACK(dirname);
     {var reg1 object dir_string = string_concat(stringcount+1);
      # Punkt am Schluß durch Nullbyte ersetzen:
      TheSstring(dir_string)->data[TheSstring(dir_string)->length-1] = '\0';
      # absolut machen:
      { var char path_buffer[MAXPATHLEN];
        begin_system_call();
...
# kann GC auslösen
  ...
  local object use_default_dir(pathname)
    var reg3 object pathname;
    { var reg5 boolean resolved_root = FALSE;
      retry:
      # erst den Pathname kopieren:
      pathname = copy_pathname(pathname);
     {var reg2 object subdirs = ThePathname(pathname)->pathname_directory;
     ...
     resolved: # Stackaufbau: pathname, subdir-oldlist.
      ...
      { var reg1 object newlist = listof(2); pushSTACK(newlist); }
      # Stackaufbau: pathname, subdir-oldlist, subdir-newlist.
      while (mconsp(STACK_1)) # Bis oldlist am Ende ist:
        { var reg4 object subdir = Car(STACK_1); # nächstes subdir
     ...
     {var reg2 object subdirs = nreverse(popSTACK()); # newlist, wieder umdrehen
      skipSTACK(1);
      pathname = popSTACK();
      ThePathname(pathname)->pathname_directory = subdirs; # in den Pathname eintragen
      return pathname;
...
# > STACK_0: absoluter Pathname ohne Wildcards im Directory
# > tolerantp: Flag, ob ein Fehler vermieden werden soll
# > dirtolerantp: Don't complain if pathname is a directory
# < ergebnis:
#     falls Name=NIL: Directory-Namestring (für RISCOS, mit '.' am Schluß)
#     falls Name/=NIL: Namestring (für RISCOS, mit Nullbyte am Schluß)
#     falls tolerantp evtl.: nullobj
  ...
  local object assure_dir_exists_(tolerantp,dirtolerantp)
    ...
    { var reg4 object pathname = STACK_0;
     ...
     {var reg1 object dir_namestring = string_concat(stringcount); # zusammenhängen
      # Existenztest:
      var struct stat statbuf;
      var reg1 uintL len = TheSstring(dir_namestring)->length;
      ...
      if (namenullp(STACK_0)) # kein File angesprochen?
        ...
        { var reg3 object namestring = namestring_asciz(dir_namestring);
          # Information holen:
          local struct stat status;
          begin_system_call();
  ...
  local void prepare_create(pathname)
    ...
    { if (!nullp(ThePathname(pathname)->pathname_type))
        { # call pathname_add_subdir:
          ...
          pushSTACK(pathname);
          if (eq(assure_dir_exists(TRUE),nullobj))
...
# kann GC auslösen
  ...
  local void change_default()
    { var reg4 object pathname = STACK_0;
      ...
      stringcount += directory_namestring_parts(pathname); # Strings fürs Directory
     {var reg1 object dir_namestring = string_concat(stringcount); # zusammenhängen
      var reg3 uintL len = TheSstring(dir_namestring)->length;
      ASSERT((len > 0) && (TheSstring(dir_namestring)->data[len-1]=='.'));
      TheSstring(dir_namestring)->data[len-1] = '\0'; # '.' am Schluß durch Nullbyte ersetzen
      begin_system_call();
...
#   GEMDOS: ohne Seriennummer, mit Default-Directory
#   Unix: mit Default-Directory
  { var reg2 object flag = popSTACK(); # optionales Argument flag
    var reg1 object pathname = coerce_pathname(popSTACK()); # Argument zu einem Pathname machen
    #if defined(PATHNAME_UNIX) || defined(PATHNAME_AMIGAOS) || defined(PATHNAME_RISCOS)
    if (!eq(flag,unbound) && !nullp(flag))
      ...
      { check_no_wildcards(pathname); # mit Wildcards -> Fehler
        ...
        # (da GEMDOS/Unix/AMIGAOS das Default-Directory von LISP nicht kennt)
    ...
    if (streamp(pathname))
      ...
      { pathname = coerce_pathname(pathname); # zu einem Pathname machen
       ...
       {# Directory muß existieren:
        var reg3 object namestring = assure_dir_exists_(FALSE,TRUE); # Filename als ASCIZ-String
        if (namenullp(STACK_0))
          # Kein Name angegeben
...
# (PROBE-FILE filename), CLTL S. 424
  { var reg1 object pathname = popSTACK(); # pathname-Argument
    ...
    pushSTACK(pathname);
   {# Directory muß existieren:
    var reg3 object namestring = assure_dir_exists_(TRUE,TRUE); # Filename als ASCIZ-String
...
# (DELETE-FILE filename), CLTL S. 424
  { var reg1 object pathname = popSTACK(); # pathname-Argument
    ...
    pushSTACK(pathname);
   {# Directory muß existieren:
    var reg3 object namestring = assure_dir_exists(TRUE); # Filename als ASCIZ-String
    if (eq(namestring,nullobj))
  ...
  local void rename_file()
    { # 1. newpathname := (MERGE-PATHNAMES newname oldpathname)
      ...
      { var reg1 object oldpathname = STACK_1;
       ...
       {# Directory muß existieren:
        var reg2 object old_namestring = assure_dir_exists(FALSE); # Filename als ASCIZ-String
      ...
      { var reg1 object newpathname = coerce_pathname(STACK_2);
       ...
       {# Directory muß existieren:
        var reg2 object new_namestring = assure_dir_exists(FALSE); # Filename als ASCIZ-String
        pushSTACK(new_namestring);
      }}
      # Stackaufbau: filename, newname, oldpathname, newpathname,
      #              oldtruename, oldnamestring, newtruename, newnamestring.
      # 4. Datei umbenennen:
      #if defined(UNIX) || defined(AMIGAOS) || defined(DJUNIX) || defined(EMUNIX) || defined(WATCOM) || defined(RISCOS) || defined(WIN32_DOS) || defined(WIN32_UNIX)
      #if 0
      ...
      #endif
      #if defined(UNIX) || defined(DJUNIX) || defined(EMUNIX) || defined(WATCOM) || defined(RISCOS) || defined(WIN32_DOS) || defined(WIN32_UNIX)
      if (!( rename(TheAsciz(STACK_2),TheAsciz(STACK_0)) ==0)) { OS_error(); }
      #endif
      end_system_call();
      #endif
    }
...
# kann GC auslösen
  ...
  local object open_file(filename,direction,if_exists,if_not_exists,type,eltype_size)
    ...
    { pushSTACK(filename); # Filename retten
      ...
      { # Directory muß existieren:
        var reg3 object namestring = # Filename als ASCIZ-String
          assure_dir_exists((direction == 0) && ((if_not_exists % 2) == 0)); # tolerant nur bei :PROBE und if_not_exists = 0 oder 2
              ...
              if (!file_exists(namestring))
                ...
                { # :IF-DOES-NOT-EXIST-Argument entscheidet:
                 ...
                 {# :CREATE -> Datei mit open erzeugen und schließen:
                  ...
                  begin_system_call();
                  #if defined(DJUNIX) || defined(EMUNIX) || defined(WATCOM) || defined(WIN32_DOS)
            ...
            default: # Modus ist :OUTPUT oder :IO
              { # Defaultwert für if_not_exists ist von if_exists abhängig:
                ...
                { var reg1 sintW ergebnis = # erzeugen
                  ...
                  if (ergebnis<0) { OS_error(); } # Error melden
                  #if defined(WIN32_DOS) && 0
                  CLOSE(ergebnis);
                  ergebnis = open(TheAsciz(namestring), O_RDWR);
                  #endif
                  setmode(ergebnis,O_BINARY);
                  end_system_call();
                      ...
                      { case 1: # :ERROR -> Error
                        ...
                        case 3: case 4: # :RENAME oder :RENAME-AND-DELETE -> umbenennen:
                          ...
                          { # Truename mit "%" bzw. ".bak" bzw. "~" erweitern:
                            var reg1 object filename = STACK_0;
                            var reg3 object new_namestring;
                            if (openp(filename)) { fehler_rename_open(filename); } # Keine offenen Dateien umbenennen!
                            pushSTACK(namestring); # namestring retten
                            #if defined(UNIX) || defined(AMIGAOS) || defined(WIN32_UNIX)
                            ...
                            { var reg1 object new_name = string_concat(2);
                              filename = STACK_0;
                              ThePathname(filename)->pathname_name = new_name;
                            }
                            if (openp(filename)) { fehler_delete_open(filename); } # Keine offenen Dateien löschen!
                            new_namestring = assure_dir_exists(FALSE);
                            skipSTACK(1);
                            #endif
                ...
                { # if-exists-Handling: bei if_exists<=5 Inhalt löschen,
                  ...
                  begin_system_call();
                  ergebnis = OPEN(TheAsciz(namestring),
                                  (if_exists<=5 ? O_RDWR | O_CREAT | O_TRUNC
                                                : O_RDWR | O_CREAT
                                  ),
                                  my_open_mask
                                 );
      ...
      if (eq(arg,unbound) || eq(arg,S(string_char)) || eq(arg,S(Kdefault))) # STRING-CHAR, :DEFAULT
        ...
        { var reg2 object h = Car(arg);
          ...
          if (!(posfixnump(eltype_size) && !eq(eltype_size,Fixnum_0)
                && ((oint_data_len < log2_intDsize+intCsize) # (Bei oint_data_len <= log2(intDsize)+intCsize-1
                    # ist stets eltype_size < 2^oint_data_len < intDsize*(2^intCsize-1).)
                    || (as_oint(eltype_size) < as_oint(fixnum(intDsize*(uintL)(bitm(intCsize)-1))))
             ) )   )
            goto bad_eltype;
        }
      else
        { bad_eltype:
          ...
          fehler(error,GETTEXT("~: illegal :ELEMENT-TYPE argument ~"));
    }   }
...
# kann GC auslösen
  ...
  #
    ...
    local int findfirst(path,buf,attr)
      var reg2 const char * path;
      var reg3 struct ffblk * buf;
      var reg4 unsigned int attr;
      { var reg1 unsigned int result = _dos_findfirst(path,attr,buf);
        if (result==0)
          { return 0; } # kein Error
          else
    ...
    local int findnext(buf)
      var reg2 struct ffblk * buf;
      { var reg1 unsigned int result = _dos_findnext(buf);
        if (result==0)
          { return 0; } # kein Error
          else
          { errno = result; # = _doserrno;
  ...
  # verändert STACK_1, kann GC auslösen
    ...
    local void subdirs(pathstring)
      var reg3 object pathstring;
      {
       ...
       #elif defined(WIN32_DOS)
        # Dateisuche gemäß DOS-Konvention:
        var WIN32_FIND_DATA ffd;
        var HANDLE hFindFile;
        set_break_sem_4(); # wegen DTA-Buffer gegen Unterbrechungen sperren
        # Suchanfang, suche nach Ordnern und normalen Dateien:
        begin_system_call();
        if ((hFindFile = FindFirstFile(TheAsciz(pathstring),&ffd)) == INVALID_HANDLE_VALUE) 
            { OS_error(); }
          else # Keine Datei gefunden -> Schleife nicht durchlaufen
          loop
            { # Stackaufbau: new-pathname-list, pathname.
              end_system_call();
              # gefundene Datei untersuchen:
              if (ffd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) # sollte ein Unterdirectory sein
                if (!(ffd.cFileName[0] == '.')) # sollte nicht mit '.' anfangen
                  # (sonst ist es wohl '.' oder '..', wird übergangen)
                  { # in Name.Typ aufspalten, Default-Typ "" :
                    extract(&ffd.cFileName[0],O(leer_string));
                   {var reg1 object new_cons = allocate_cons();
                    Car(new_cons) = popSTACK(); Cdr(new_cons) = popSTACK();
                    # new_cons = (name . type)
                    pushSTACK(new_cons);
                     ...
                     if (!nullp(STACK_(0+3+1)))
                       { pushSTACK(new); # newpathname als 1. Listenelement
                         ...
                         { # Uhrzeit und Datum von DOS-Format in Decoded-Time umwandeln:
                           ...
                           var WORD date;
                           var WORD time;
                           FileTimeToDosDateTime(&ffd.ftLastWriteTime,&date,&time);
                           convert_timedate((uintW)time,(uintW)date,&timepoint);
                           pushSTACK(timepoint.Sekunden);
                           pushSTACK(timepoint.Minuten);
                           pushSTACK(timepoint.Stunden);
                           pushSTACK(timepoint.Tag);
                         ...
                         pushSTACK(new); # als 3. Listenelement
                         pushSTACK(UL_to_I(*(uintL*)(&ffd.nFileSizeLow))); # Länge als 4. Listenelement
                         new = listof(4); # 4-elementige Liste bauen
                       }
                     # new auf die Liste new-pathname-list pushen:
                      pushSTACK(new);
          ...
          loop
            { # Stackaufbau: ..., new-pathname-list, [ht,] pathname-list-rest.
             ...
             {var reg9 object pathname = Car(pathname_list_rest); # nächstes Directory
              ...
              if (!recursively)
                { switch (next_task)
                    { case 0: # Dieses pathname liefern
                        #if defined(UNIX) || defined(WIN32_UNIX)
                        assure_dir_exists(FALSE); # erst noch Links auflösen
                        #endif
                      ...
                      case 1: # In diesem pathname nach einem File sehen
                        ...
                        if (name_and_type && nullp(ThePathname(pathname)->pathname_type))
                          ...
                          { # subdirs := (butlast subdirs) = (nreverse (cdr (reverse subdirs)))
                            var reg1 object subdirs = reverse(ThePathname(pathname)->pathname_directory);
                            pathname = STACK_0;
                            ThePathname(pathname)->pathname_type = Car(subdirs);
                            ThePathname(pathname)->pathname_directory = nreverse(Cdr(subdirs));
                          }
                        #endif
                        assure_dir_exists(FALSE); # Links auflösen, File suchen
                      ...
                      case -1: # In diesem pathname nach einem Subdirectory sehen
                        { var reg2 object namestring = assure_dir_exists(FALSE); # Links auflösen, Directory-Namestring
                          pushSTACK(namestring); # Directory-Namestring
                          {var reg1 object subdir = Car(STACK_(1+(1+H+2)+1+1)); # (car subdir-list)
              ...
              {{var reg1 object dir_namestring = assure_dir_exists(FALSE); # Links auflösen, Directory-Name bilden
                ...
                if (!nullp(STACK_(1+5+(1+H+2)+2))) # ;CIRCLE-Flag abfragen
                  { # pathname in der Hash-Tabelle suchen:
                    ...
                    {var reg8 object namestring = string_concat(3); # zusammenhängen
                     ...
                     {var reg1 object new_cons = allocate_cons(); # zusammenconsen
               ...
               { var reg8 object namestring;
                 ...
                 #endif
                 #ifdef RISCOS
                 var reg10 object wildcard_mask;
                 namestring = copy_string(STACK_0); # Directory-Name
                 TheSstring(namestring)->data[TheSstring(namestring)->length-1] = '\0'; # mit Nullbyte statt '.' am Schluß
                 ...
                 { var reg1 uintB* ptr = &TheSstring(wildcard_mask)->data[0];
                   var reg2 uintL count = TheSstring(wildcard_mask)->length;
                   ...
                   { var reg2 SDIRENT* dp;
                    ...
                    {var reg4 object direntry;
                     {var reg3 uintL direntry_len;
                      ...
                      #ifndef DIRENT_RECLEN
                      #define DIRENT_RECLEN(dp) dp->d_reclen
                      #endif
                     
                      # Unter UNIX_LINUX reicht direntry_len := dp->d_reclen, aber i.a. ist
                      # direntry_len := min(dp->d_reclen,asciz_length(dp->d_name))  nötig:
                      {var reg1 const uintB* ptr = (const uintB*)(&dp->d_name[0]);
                       var reg1 uintL count;
                       direntry_len = 0;
                       dotimesL(count,DIRENT_RECLEN(dp),
                         { if (*ptr == '\0') break;
                           ptr++; direntry_len++;
                         });
                      }
                        ...
                        {var reg3 object namestring = string_concat(3); # zusammenhängen
                         ...
                         if (!( stat(TheAsciz(namestring),&status) ==0))
                           { if (!(errno==ENOENT ||errno==ELOOP_VALUE
                                   #ifdef EACCES # WIN32_UNIX, DJGPP2
                                   ||errno==EACCES
                                   #endif
                                ) ) { OS_error(); }
                             end_system_call();
                               ...
                               { if (next_task>0)
                                   {
                                     ...
                                     if (wildcard_match(STACK_(2+(1+H+2)+3),STACK_0))
                                       ...
                                       # und auf result-list pushen:
                                       {
                                         ...
                                         {var reg1 object pathname = copy_pathname(STACK_2);
                                          pushSTACK(pathname);
                                          if (name_and_type && nullp(ThePathname(pathname)->pathname_type))
                                            # Move the last subdir into the type slot of the pathname.
                                            { # subdirs := (butlast subdirs) = (nreverse (cdr (reverse subdirs)))
                           ...
                           { # name&type mit direntry matchen:
                             if (wildcard_match(STACK_(2+(1+H+2)+3),STACK_0))
                               ...
                               { pushSTACK(STACK_0); # direntry
                                ...
                                assure_dir_exists(FALSE); # Truename bilden (symbolische Links auflösen)
                                { if (!nullp(STACK_(0+5+(1+H+2)+3+2))) # :FULL gewünscht?
                                    with_stat_info(); # ja -> STACK_0 erweitern
                                  # und STACK_0 vor result-list pushen:
                                 {var reg1 object new_cons = allocate_cons();
               ...
               {var reg8 object namestring = string_concat(3); # zusammenhängen
                ...
                # Suchanfang, suche nach Ordnern und normalen Dateien:
                begin_system_call();
                if ((hFindFile=FindFirstFile(TheAsciz(namestring),&ffd)) == INVALID_HANDLE_VALUE)
                  { OS_error(); }
                 else # Keine Datei gefunden -> Schleife nicht durchlaufen
                 loop
                   { # Stackaufbau: ..., next-pathname.
                     end_system_call();
                     {# Directory-Eintrag in String umwandeln:
                      var reg4 object direntry = asciz_to_string(&ffd.cFileName[0]);
                      # "." und ".." übergehen:
                      if (!(equal(direntry,O(punkt_string))
                            || equal(direntry,O(punktpunkt_string))
                         ) )
                        { pushSTACK(direntry);
                          # Stackaufbau: ..., pathname, dir_namestring, direntry.
                          if (ffd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) # Ist es ein Directory?
                            # Eintrag ist ein Directory.
                            { if (recursively) # alle rekursiven Subdirectories gewünscht?
                                # ja -> zu einem Pathname machen und auf
                                # pathnames-to-insert pushen (wird nachher
         ...
         for (drive='A'; drive<='Z'; drive++) # alle Drives durchlaufen
           if (good_drive(drive))
             { pushSTACK(make_string(&drive,1)); # Device, einelementiger String
              {var reg2 object newpathname = copy_pathname(STACK_(2+2+1)); # Pathname kopieren
...
# (CD [pathname]) setzt das aktuelle Laufwerk und das aktuelle Directory.
  { var reg1 object pathname = popSTACK();
    ...
    pushSTACK(pathname);
    assure_dir_exists(FALSE); # Directory muß existieren
  ...
  local object shorter_directory_arg()
    { var reg1 object pathname = coerce_pathname(popSTACK()); # Argument zu einem Pathname machen
      ...
      {var reg2 object subdirs = ThePathname(pathname)->pathname_directory;
       ...
       {var reg3 object dir_namestring = assure_dir_exists(FALSE);
  ...
  global void init_pathnames()
    {
      ...
      #endif
      #endif
      #if defined(HAVE_SHELL) && !defined(AMIGAOS)
      #if defined(UNIX) || defined(WIN32_UNIX)
      ...
      { # Im Environment nach Variable SHELL suchen:
        begin_system_call();
       {var reg1 const char* shell = getenv("SHELL");
        end_system_call();
      ...
      { # Im Environment nach Variable COMSPEC suchen:
        begin_system_call();
       {var reg1 const char* shell = getenv("COMSPEC");
        end_system_call();
        if (!(shell==NULL)) # gefunden?
...
#if defined(DJUNIX) || defined(WATCOM) || defined(EMUNIX_OLD_8d)
...
#endif
#if defined(UNIX) || defined(EMUNIX_NEW_8e) || defined(RISCOS) || defined(WIN32_DOS) || defined(WIN32_UNIX)
  ...
  { var reg1 object pathname = popSTACK(); # pathname-Argument
    if (streamp(pathname))
      ...
      { pathname = coerce_pathname(pathname); # zu einem Pathname machen
        ...
        pushSTACK(pathname);
       {# Directory muß existieren:
        var reg3 object namestring = assure_dir_exists(FALSE); # Filename als ASCIZ-String
        #ifdef MSDOS
         #if defined(DJUNIX) || defined(WATCOM) || defined(EMUNIX_OLD_8d)
          # Datei öffnen:
          begin_system_call();
    ...
    { var decoded_time timepoint;
      ...
      #endif
      #if defined(UNIX) || defined(EMUNIX_NEW_8e) || defined(AMIGAOS) || defined(RISCOS) || defined(WIN32_DOS) || defined(WIN32_UNIX)
  ...
  {var reg6 object* args_pointer = rest_args_pointer STACKop 1;
   {var reg1 object* argptr = args_pointer; # Pointer über die Argumente
    ...
    { var reg2 object* file_ = &NEXT(argptr);
      ...
      pushSTACK(pathname);
     {# Directory muß existieren:
      var reg4 object namestring = assure_dir_exists(FALSE); # Filename als ASCIZ-String
      # Überprüfe, ob die Datei existiert:
      if_file_exists(namestring, ; , { fehler_file_not_exists(); } );
   ...
   #if defined(EMUNIX_PORTABEL) || (defined(WATCOM) && defined(WINDOWS)) || defined(WIN32_DOS)
   ...
   {# argv-Array aufbauen:
    var DYNAMIC_ARRAY(reg5,argv,char*,1+(uintL)argcount+1);
    ...
    {var reg2 int flags =
       ...
       #endif
       ;
     var reg1 int ergebnis = spawnv(P_WAIT|flags,argv[0],argv);
     end_system_call();
     if (ergebnis < 0) { OS_error(); } # Error melden
     # Fertig.
   ...
   { # argv-Array im Stack aufbauen und Strings in den Stack kopieren:
     var reg9 uintL argvdata_length = 0;
     { var reg1 object* argptr = args_pointer;
       var reg3 uintC count;
       dotimespC(count,argcount+1,
         { var reg2 object arg = NEXT(argptr); # nächstes Argument, ASCIZ-String
           argvdata_length += TheSstring(arg)->length;
         });
     }
    {var DYNAMIC_ARRAY(reg9,argv,char*,1+(uintL)argcount+1);
     var DYNAMIC_ARRAY(reg9,argvdata,char,argvdata_length);
     { var reg8 object* argptr = args_pointer;
       var reg7 char** argvptr = &argv[0];
       var reg1 char* argvdataptr = &argvdata[0];
       var reg5 uintC count;
       dotimespC(count,argcount+1,
         { var reg4 object arg = NEXT(argptr); # nächstes Argument, ASCIZ-String
           var reg2 char* ptr = TheAsciz(arg);
           var reg3 uintL len = TheSstring(arg)->length;
...
global int find_executable(program_name)
  var reg3 const char * program_name;
{ # Don't need to execute this more than once.
 ...
 {var reg8 boolean has_slash = FALSE;
  {var reg1 const char * p;
   for (p = program_name; *p; p++) { if (*p == '/') { has_slash = TRUE; break; } }
  }
  if (!has_slash)
    { # exec searches paths without slashes in the directory list given
      # by $PATH.
      var reg7 const char * path = getenv("PATH");
      if (!(path==NULL))
        { var reg1 const char * p;
          var reg6 const char * p_next;
          for (p = path; *p; p = p_next)
            { var reg2 const char * q;
              var reg4 uintL p_len;
              for (q = p; *q; q++) { if (*q == ':') break; }
              p_len = q-p; p_next = (*q=='\0' ? q : q+1);
              # We have a path item at p, of length p_len.
              # Now concatenate the path item and program_name.
             {var reg5 char * concat_name = (char*) malloc(p_len + strlen(program_name) + 2);
...
#if DJGPP == 2
# hack until DJGPP2 gets fixed 
# (symptom: EACCES on read of just-recreated files)
global int djgpp2_creat (CONST char *path, unsigned long mode);

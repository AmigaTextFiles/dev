...
(in-package "LISP")
(export '(doseq dohash #-(or UNIX WIN32-UNIX) *default-time-zone* default-directory dir))
(in-package "SYSTEM")
...
(defconstant least-positive-normalized-long-float least-positive-long-float)
(defconstant least-negative-normalized-long-float least-negative-long-float)
...
(progn
  (defun logical-pathname-translations (host)
    ...
    (or (gethash host sys::*logical-pathname-translations*) ; :test #'equal !
        (error 
         #L{
         DEUTSCH "~S: ~S benennt keinen Logical Host."
  ...
  (set-logical-pathname-translations "SYS"
    '#+(or DOS WIN32-DOS) (("**;*.LISP" "\\...\\*.LSP") (";**;*.LISP" "...\\*.LSP")
                             ("**;*.*" "\\...\\*.*") (";**;*.*" "...\\*.*")
                            )
     #+(or AMIGA UNIX OS/2 WIN32-UNIX) (("**;*.LISP" "/**/*.lsp") (";**;*.LISP" "**/*.lsp")
                             ("**;*.*" "/**/*.*") (";**;*.*" "**/*.*")
                            )
     #+ACORN-RISCOS         (("**;*.LISP" "$.**.*.lsp") (";**;*.LISP" "**.*.lsp")
                             ("**;*.*" "$.**.*.*") (";**;*.*" "**.*.*")
                            )
  )
)
...
(defun %time (new-real1 new-real2 new-run1 new-run2 new-gc1 new-gc2
  ...
  (macrolet ((merge-2-values (val1 val2)
               (if (< internal-time-units-per-second 1000000)
                 ...
                 `(+ (* ,val1 internal-time-units-per-second) ,val2) ; TIME_2: UNIX sonst
            )) )
...
(defun sleep (time)
  (if (and (realp time) (not (minusp time)))
    (progn
      ...
      #+(or UNIX WIN32-UNIX WIN32-DOS) ; SLEEP_2
...
; Bei vielen Betriebssystemen (nicht bei UNIX) muß die Zeitzone beim
                      ...
                      (progn
                        ...
                        (and (if (not (eql Monat3 11))
                             ...
                             (and (progn
                                    (unless Zeitzone
                                      (setq Zeitzone
                                        #-(or UNIX WIN32-UNIX) (- *default-time-zone*
                                                  (if (funcall *default-dst-check* Jahr3 Jahrtag Stunde) 1 0)
                                               )
                                        #+(or UNIX WIN32-UNIX) (default-time-zone (+ (* 24 UTTag) Stunde))
                                    ) )
...
(defun dir (&optional (pathnames #+(or DOS WIN32-DOS) '("*.*\\" "*.*")
                                 #+(or AMIGA UNIX OS/2 WIN32-UNIX) '("*/" "*")
                                 #+ACORN-RISCOS '("*." "*" "*.*")
           )          )

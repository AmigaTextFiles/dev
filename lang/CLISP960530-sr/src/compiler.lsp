...
#+CLISP (import '(sys::function-name-p sys::parse-body sys::make-load-time-eval
...
#+(and CROSS (not (or CLISP2 CLISP3)))
(eval-when (eval load compile)
  (pushnew
    (if (y-or-n-p 
         #L{
         DEUTSCH "Kann man von C aus den SP verändern?"
         ENGLISH "Can C manipulate the SP stack pointer?"
         FRANCAIS "Peut-on modifier le pointeur de pile en C?"
         }
        )
      'CLISP2
      'CLISP3
...
(defconstant *jmpbuf-size*
  (progn
    (format *query-io* 
            #L{
            DEUTSCH "~%Bitte *jmpbuf-size* eingeben: "
            ENGLISH "~%Please input *jmpbuf-size*: "
            FRANCAIS "~%Veuillez entrer *jmpbuf-size* :"
            }
    )
    (read *query-io*)
) )
...
(defconstant *big-endian*
  #+(or AMIGA SUN3 SUN4) t ; BIG-ENDIAN-Prozessor
  #+(or SUN386 PC386) nil ; LITTLE-ENDIAN-Prozessor
  #-(or AMIGA SUN3 SUN4 SUN386 PC386)
    (y-or-n-p 
     #L{
     DEUTSCH "Prozessor BIG-ENDIAN?"
     ENGLISH "processor big endian?"
     FRANCAIS "Processeur BIG-ENDIAN?"
     }
    )
)
#+CROSS
(defun version ()
  (list ' #+CLISP2 SYSTEM::CLISP2 #+CLISP3 SYSTEM::CLISP3
        *jmpbuf-size*
        *big-endian*
        '130695
) )
      ...
      (if arg
        ...
        (let ((obj (let ((*read-base* 16.)) (read stream t nil t))))
          (unless (= (length obj) arg)
            (error 
             #L{
             DEUTSCH "Falsche Länge eines Closure-Vektors: ~S"
...
(eval-when (compile load eval)
  ...
  (defconstant funtab
    '#(system::%funtabref system::subr-info
       #| svref system::%svstore |# row-major-aref system::row-major-store
       array-element-type array-rank array-dimension array-dimensions
       array-total-size adjustable-array-p bit-and bit-ior bit-xor bit-eqv
       bit-nand bit-nor bit-andc1 bit-andc2 bit-orc1 bit-orc2 bit-not
       array-has-fill-pointer-p fill-pointer system::set-fill-pointer
    ...
    (values-list
      (assoc sym
        '(; Das ist die Tabelle aller SUBRs, wie in SUBR.D.
          ...
          #+(or UNIX WIN32-UNIX) (user-homedir-pathname 0 1 nil nil nil)
...
(defun compiler-error (caller &optional where)
  (error 
   #L{
   DEUTSCH "Fehler im Compiler!! Aufgetreten in ~A~@[ bei ~A~]."
...
(defun process-declarations (declspeclist &aux (specials nil) (ignores nil) (ignorables nil))
  ...
  (dolist (declspec declspeclist)
    (if (or (atom declspec) (cdr (last declspec)))
      (c-warn 
       #L{
       DEUTSCH "Falsche Deklarationen-Syntax: ~S~%Wird ignoriert."
      ...
      (let ((declspectype (car declspec)))
        (if (and (symbolp declspectype)
          ...
          (cond ((eq declspectype 'SPECIAL)
                 (dolist (x (cdr declspec))
                   (if (symbolp x)
                     ...
                     (c-warn 
                      #L{
                      DEUTSCH "Nur Symbole können SPECIAL-deklariert werden, nicht ~S."
                ...
                ((eq declspectype 'IGNORE)
                 (dolist (x (cdr declspec))
                   (if (symbolp x)
                     ...
                     (c-warn 
                      #L{
                      DEUTSCH "Nur Symbole können IGNORE-deklariert werden, nicht ~S."
                ...
                ((eq declspectype 'IGNORABLE)
                 (dolist (x (cdr declspec))
                   (if (symbolp x)
                     ...
                     (c-warn 
                      #L{
                      DEUTSCH "Nur Symbole können IGNORABLE-deklariert werden, nicht ~S."
          ...
          (c-warn 
           #L{
           DEUTSCH "Unbekannte Deklaration ~S.~%Die ganze Deklaration ~S wird ignoriert."
...
(defun declared-notinline (fun &optional (denv *denv*))
  ...
  (loop
    (when (atom denv)
      ...
      (return (eq (get (sys::get-funname-symbol fun) 'inlinable) 'notinline))
    )
...
(defun test-list (L &optional (l1 0) (l2 nil))
  ...
  (unless (>= (length L) l1)
    (c-error 
     #L{
     DEUTSCH "Form zu kurz (zu wenig Argumente): ~S"
  ...
  (when l2
    (unless (<= (length L) l2)
      (c-error 
       #L{
       DEUTSCH "Form zu lang (zu viele Argumente): ~S"
        ...
        (let ((fun (first *form*)))
          (if (function-name-p fun)
              ...
              (if (null a)
                ...
                (let ((handler (gethash fun c-form-table)))
                  (if handler ; Behandlungsfunktion gefunden?
                    ; also (symbolp fun)
                    (if (or (special-form-p fun) (macro-function fun)
            ...
            (if (and (consp fun) (eq (car fun) 'LAMBDA))
              ...
              (c-error 
               #L{
               DEUTSCH "Das ist nicht der Name einer Funktion: ~S"
...
(defun c-VAR (symbol)
  ...
  (multiple-value-bind (a b c) (venv-search symbol)
    (when (eq a 'NIL)
      (c-warn 
       #L{
       DEUTSCH "~S ist weder deklariert noch gebunden, behandle es als SPECIAL-deklarierte Variable."
       ENGLISH "~S is neither declared nor bound, it will be treated as if it were declared SPECIAL."
       FRANCAIS "~S n'est ni déclaré ni lié, et va être traité comme étant déclaré SPECIAL."
       }
       symbol
      )
      (when *compiling-from-file*
          ...
          (make-anode :type 'VARSET
                      ...
                      :code (if (var-constantp var)
                              (progn
                                (c-warn 
                                 #L{
                                 DEUTSCH "Der Konstante ~S kann nicht zugewiesen werden.  Die Zuweisung wird ignoriert."
                                 ENGLISH "The constant ~S may not be assigned to.  The assignment will be ignored."
                                 FRANCAIS "Rien ne peut être assigné à la constante ~S.  L'assignation est ignorée."
                                 }
                                 symbol
                                )
                                '((VALUES1))
...
(defun test-argument-syntax (args applyargs fun req opt rest-p key-p keylist allow-p)
  (unless (and (listp args) (null (cdr (last args))))
    (c-error 
     #L{
     DEUTSCH "Argumentliste zu Funktion ~S ist dotted: ~S"
  ...
  (let ((n (length args))
    ...
    (unless (and (or applyargs (<= req n)) (or rest-p key-p (<= n reqopt)))
      (c-warn 
       #L{
       DEUTSCH "~S mit ~S~:[~; oder mehr~] Argumenten aufgerufen, braucht aber ~:[~:[~S bis ~S~;~S~]~;mindestens ~*~S~] Argumente."
       ENGLISH "~S called with ~S~:[~; or more~] arguments, but it requires ~:[~:[from ~S to ~S~;~S~]~;at least ~*~S~] arguments."
       FRANCAIS "~S est appelé avec ~S~[; ou plus~] d'arguments mais a besoin ~:[de ~:[~S à ~S~;~S~]~;d'au moins ~*~S~] arguments."
       }
       fun n applyargs
    ...
    (unless (evenp n)
      (c-warn 
       #L{
       DEUTSCH "Keyword-Argumente zu Funktion ~S sind nicht paarig: ~S"
    ...
    (do ((keyargs args (cddr keyargs))
        ...
        ((null keyargs)
         (if wrong-key
           (c-error 
            #L{
            DEUTSCH "Keyword ~S ist bei Funktion ~S nicht erlaubt.~%Erlaubt ~:[sind nur ~{~S~#[~; und ~S~:;, ~]~}~;ist nur ~{~S~}~]."
            ENGLISH "keyword ~S is not allowed for function ~S.~%The only allowed keyword~:[s are ~{~S~#[~; and ~S~:;, ~]~}~; is ~{~S~}~]."
            FRANCAIS "L'argument mot-clé ~S n'est pas permis pour la fonction ~S.~%Seul~:[s sont permis ~{~S~#[~; et ~S~:;, ~]~}~; est permis ~{~S~}~]."
            }
      ...
      (let ((key (first keyargs)))
        ...
        (unless (keywordp key)
          (c-warn 
           #L{
           DEUTSCH "Das Argument ~S zu Funktion ~S ist kein Keyword."
             ...
             (case fun ; fun ein SUBR
               (; Seiteneffektklasse (NIL . NIL) haben diejenigen Funktionen,
                ...
                (SYSTEM::%FUNTABREF
                 ...
                 SYSTEM::SYMBOL-MACRO-P CLOS::STD-INSTANCE-P
               ...
               (;
                ...
                (SYSTEM::SUBR-INFO
                 AREF SVREF ROW-MAJOR-AREF ARRAY-DIMENSION ARRAY-DIMENSIONS ARRAY-TOTAL-SIZE
                 ...
                 SYSTEM::%COPY-STRUCTURE SYSTEM::%STRUCTURE-TYPE-P SYSTEM::CLOSURE-NAME
                 SYSTEM::CLOSURE-CODEVEC SYSTEM::CLOSURE-CONSTS SYSTEM::MAKE-CODE-VECTOR
                 SYSTEM::%MAKE-CLOSURE CLOS::ALLOCATE-STD-INSTANCE CLOS:SLOT-EXISTS-P
        ...
        (if (and name (eq fun name)) ; beschreibt fun ein gültiges SUBR?
            ...
            ((CAR CDR FIRST REST NOT NULL CONS SVREF VALUES
             ...
             (if (and (<= req n) (or rest-p (<= n (+ req opt))))
               ...
               (progn
                 (c-warn 
                  #L{
                  DEUTSCH "~S mit ~S Argumenten aufgerufen, braucht aber ~:[~:[~S bis ~S~;~S~]~;mindestens ~*~S~] Argumente."
                  ENGLISH "~S called with ~S arguments, but it requires ~:[~:[from ~S to ~S~;~S~]~;at least ~*~S~] arguments."
                  FRANCAIS "~S est appelé avec ~S arguments mais a besoin ~:[de ~:[~S à ~S~;~S~]~;d'au moins ~*~S~] arguments."
                  }
                  fun n
          ...
          (let ((inline-lambdabody
                  (or (and *compiling-from-file*
                      ...
                      (get (sys::get-funname-symbol fun) 'sys::inline-expansion)
               )) )
...
(defun c-REQUIRE (module-name &optional (pathname nil p-given))
  (unless (member (string module-name) *compiled-modules* :test #'string-equal)
    ...
    (flet ((load-lib (file)
             (let* ((present-files
                      (search-file file (append sys::*source-file-types* '(#".lib")))
                    )
  ...
  (do ((L parameters (cdr L))
    ...
    (cond ((symbolp (car L)) (push (car L) symbols) (push nil forms))
          ...
          (t (catch 'c-error
               (c-error 
                #L{
                DEUTSCH "Falsche Syntax in LET/LET*: ~S"
    ...
    (macrolet ((err-illegal (item)
               ...
               (err-norest ()
                 `(catch 'c-error
                    (c-error 
                     #L{
                     DEUTSCH "Fehlender &REST-Parameter in der Lambdaliste: ~S"
                     ENGLISH "Missing &REST parameter in lambda list ~S"
                     FRANCAIS "Il manque le paramètre &REST dans la liste lambda ~S"
                     }
                     lambdalist
                  ) )
               )
               (err-superflu (item)
                 `(catch 'c-error
                    (c-error 
                     #L{
                     DEUTSCH "Überflüssiges Lambdalisten-Element: ~S"
                     ENGLISH "Lambda list element ~S is superfluous."
                     FRANCAIS "L'élément de liste lambda est superflu : ~S"
                     }
                     ,item
                  ) )
              ))
      ...
      (if L
        (catch 'c-error
          (c-error 
           #L{
           DEUTSCH "Eine Lambdaliste, die einen Punkt enthält, ist nur bei Macros erlaubt, nicht hier: ~S"
...
(defun lambdalist-error (item)
  (catch 'c-error
    (c-error 
     #L{
     DEUTSCH "Unzulässiges Lambdalistenelement: ~S"
     ENGLISH "Illegal lambda list element ~S"
     FRANCAIS "N'est pas permis dans une liste lambda : ~S"
     }
     item
) ) )
  ...
  (let ((sym (var-name var)))
    (if (member sym *ignores* :test #'eq)
      ...
      (if (var-specialp var)
        (c-warn 
         #L{
         DEUTSCH "Binden der Variablen ~S kann trotz IGNORE-Deklaration~%Seiteneffekte haben, weil sie SPECIAL deklariert ist."
        ...
        (if (var-usedp var)
          (c-warn 
           #L{
           DEUTSCH "Variable ~S wird trotz IGNORE-Deklaration benutzt."
      ...
      (unless (member sym *ignorables* :test #'eq)
        ...
        (unless (or (var-specialp var) (var-usedp var))
          ...
          (unless (null (symbol-package sym)) ; sym ein (gensym) ?
            ...
            (c-warn 
             #L{
             DEUTSCH "Variable ~S wird nicht benutzt.~%Schreibfehler oder fehlende IGNORE-Deklaration?"
    ...
    (progn
      (when (c-constantp symbol)
        (catch 'c-error
          (c-error 
           #L{
           DEUTSCH "Konstante ~S kann nicht gebunden werden."
...
(defun c-DECLARE ()
  ...
  (c-error 
   #L{
   DEUTSCH "Deklarationen sind an dieser Stelle nicht erlaubt: ~S"
...
(defun c-SETQ ()
  ...
  (when (evenp (length *form*))
    (c-error 
     #L{
     DEUTSCH "Ungerade viele Argumente zu SETQ: ~S"
          ...
          (if (symbolp symboli)
            ...
            (progn
              (catch 'c-error
                (c-error 
                 #L{
                 DEUTSCH "Zuweisung auf ~S unmöglich, da kein Symbol."
...
(defun c-PSETQ ()
  ...
  (when (evenp (length *form*))
    (c-error 
     #L{
     DEUTSCH "Ungerade viele Argumente zu PSETQ: ~S"
  ...
  (let ((symbols (second *form*)))
    (dolist (sym symbols)
      (unless (symbolp sym)
        (c-error 
         #L{
         DEUTSCH "Nur Symbole können Variable sein, nicht ~S"
  ...
  (do ((L (second *form*) (cdr L))
    ...
    (cond ((symbolp (car L)) (push (car L) varlist) (push nil valueslist))
          ...
          (t (catch 'c-error
               (c-error 
                #L{
                DEUTSCH "Falsche Syntax in COMPILER-LET: ~S"
...
(macrolet ((check-blockname (name)
             `(unless (symbolp ,name)
                (catch 'c-error
                  (c-error 
                   #L{
                   DEUTSCH "Blockname muß ein Symbol sein, nicht ~S"
  ...
  (let ((name (second *form*)))
    ...
    (let ((a (benv-search name)))
      (cond ((null a) ; dieser Blockname ist unsichtbar
             (c-error 
              #L{
              DEUTSCH "RETURN-FROM auf Block ~S an dieser Stelle nicht möglich."
        ...
        (if (atom item)
          (if (or (and (symbolp item) (not (null item))) (numberp item))
            ...
            (catch 'c-error
              (c-error 
               #L{
               DEUTSCH "Nur Zahlen und Symbole sind zulässige Sprungziele, nicht aber ~S"
...
(defun c-GO ()
  ...
  (let ((tag (second *form*)))
    (unless (or (and (symbolp tag) (not (null tag))) (numberp tag))
      (c-error 
       #L{
       DEUTSCH "Sprungziel muß ein Symbol oder eine Zahl sein, nicht ~S"
    ...
    (multiple-value-bind (a b) (genv-search tag)
      (cond ((null a) ; dieser Tag ist unsichtbar
             (c-error 
              #L{
              DEUTSCH "GO auf Tag ~S an dieser Stelle nicht möglich."
    ...
    (if (and (not longp) (function-name-p name))
      (multiple-value-bind (a b c) (fenv-search name)
        (case a
          ...
          (SYSTEM::MACRO
           (c-error 
            #L{
            DEUTSCH "~S ist keine Funktion, sondern ein lokal definierter Macro."
      ...
      (let ((funname (car (last *form*))))
        (if (and (consp funname) (eq (car funname) 'LAMBDA) (consp (cdr funname)))
          ...
          (c-error 
           #L{
           DEUTSCH "Nur Symbole und Lambda-Ausdrücke sind Namen von Funktionen, nicht ~S"
           ENGLISH "Only symbols and lambda expressions are function names, not ~S"
           FRANCAIS "Seuls les symboles et les expressions lambda sont des noms de fonction et non ~S"
           }
           funname
) ) ) ) ) )
...
(macrolet ((err-syntax (specform fdef)
             `(catch 'c-error
                (c-error 
                 #L{
                 DEUTSCH "Falsche Syntax einer Funktionsdefinition in ~S: ~S"
    ...
    (dolist (situation (second *form*))
      (case situation
        ...
        (T (cond ((equal situation '(NOT EVAL)) (setq load-flag t compile-flag t))
                 ...
                 (t (c-error 
                     #L{
                     DEUTSCH "Situation bei EVAL-WHEN muß EVAL, LOAD oder COMPILE sein, nicht ~S."
        ...
        (let ((clause (car clauses)))
          (if (atom clause)
            (c-error 
             ...
             FRANCAIS "Clause COND sans test : ~S"
             }
             clause
            )
            (let ((test (car clause)))
      ...
      (do ((clauses clauses))
        ...
        (let ((clause (pop clauses)))
          (if (atom clause)
            (c-error 
             #L{
             DEUTSCH "CASE-Klausel ohne Objekte: ~S"
                  ...
                  (progn
                    (when clauses
                      (catch 'c-error
                        (c-error 
                         #L{
                         DEUTSCH "~S: Die ~S-Klausel muß die letzte sein: ~S"
                  ...
                  (let ((newkeys '()))
                    (dolist (key (if (listp keys) keys (list keys)))
                      (if (not (member key allkeys :test #'eql)) ; remove-duplicates
                        ...
                        (c-warn 
                         #L{
                         DEUTSCH "Doppelt aufgeführter ~S-Fall ~S : ~S"
    ...
    (let ((r (length reqvar)) ; Anzahl der required-Argumente
      ...
      (when (and (null restvar) (> |t| (+ r s)))
        ...
        (catch 'c-error
          (c-error 
           #L{
           DEUTSCH "Zuviele Argumente für ~S"
      ...
      (when (and (null applyarglist) (< |t| r))
        ...
        (catch 'c-error
          (c-error 
           #L{
           DEUTSCH "Zuwenig Argumente für ~S"
...
(defun c-FUNCTION-CALL (funform arglist)
  (when (inline-callable-function-p funform (length arglist))
...
(defun c-APPLY ()
  ...
  (let* ((funform (second *form*))
    ...
    (when (inline-callable-function-p funform n t)
      ; Aufruf eines Lambda-Ausdrucks INLINE möglich
      (return-from c-APPLY
      ...
      (let ((type (c-constant-value typeform)) h)
        (cond ((symbolp type)
                (cond ; Test auf Property TYPE-SYMBOL:
                      ...
                      #+CLISP ; Test auf Property CLOS::CLASS:
                      ((and (setq h (get type 'CLOS::CLASS)) (clos::class-p h)
...
(defun c-FORMAT ()
  ...
  (if (stringp (third *form*))
    ...
    (let ((*form* `(FORMAT ,(second *form*) (FORMATTER ,(third *form*)) ,@(cdddr *form*))))
...
(macrolet ((c-seqop (op n)
             (let ((op-if (intern (string-concat (string op) "-IF") *lisp-package*))
                   (op-if-not (intern (string-concat (string op) "-IF-NOT") *lisp-package*))
                   (c-op-if (intern (string-concat "C-" (string op) "-IF")))
                   (c-op-if-not (intern (string-concat "C-" (string op) "-IF-NOT"))))
               `(progn
                  (defun ,c-op-if ()
                    (test-list *form* ,(+ 1 n))
                    (let ((pred-arg ,(case n (2 `(second *form*))
                                             (3 `(third *form*))
                                     )
                         ))
...
(defun pass2 (*func*)
  (when (anode-p (fnode-code *func*)) ; falls 2. Pass noch nicht durchgeführt:
    ...
    (let ((code-list (compile-to-LAP))) ; Code flachklopfen und in Stücke zerteilen,
      ...
      (let (#+CLISP3 (SPdepth (SP-depth code-list))) ; Stackbedarf bestimmen

  ...
  local object I_I_Byte(size,position)
    ...
    { if (!(I_fixnump(size) && !R_minusp(size)))
        { pushSTACK(size); # Wert für Slot DATUM von TYPE-ERROR
          ...
          fehler(type_error, GETTEXT("The arguments to BYTE must be fixnums >=0: ~, ~"));
        }
...
# Fehler, wenn Argument kein Byte.
  ...
  local void fehler_byte(bad)
    ...
    { pushSTACK(bad); # Wert für Slot DATUM von TYPE-ERROR
      ...
      fehler(type_error, GETTEXT("~ is not a BYTE specifier"));
    }
    ...
    { SAVE_NUM_STACK # num_stack retten
     ...
     {var reg7 uintD* newMSDptr;
      { var reg1 uintL i = p%intDsize; # p mod intDsize
        ...
        num_stack_need_1((uintL)len, newMSDptr=,_EMA_); # neue UDS newMSDptr/len/..
        if (i==0)
  ...
  local object mkf_extract(x,p,q)
    ...
    { SAVE_NUM_STACK # num_stack retten
     ...
     {# Platz (len Digits) für die neue UDS bereitstellen:
      ...
      num_stack_need_1((uintL)len, newMSDptr = ,_EMA_); # Platz belegen

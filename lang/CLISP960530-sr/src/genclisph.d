...
global void printf_with_args(string,argcount,args)
  ...
  var reg3 int argcount;
  var reg2 printf_arg* args;
  { while (*string)
      { if (string[0]=='%')
...
{ char* glue =
    ...
    #endif
    ;
  printf("#ifndef _CLISP_H\n");
  printf("#define _CLISP_H\n");
  printf("\n");
...
# printf("#define local  static\n");
  ...
  printf("#define maygc\n");
  #ifdef ANSI
    printf("#define _ARGS(x) x\n");
  #else
    printf("#define _ARGS(x) ()\n");
  #endif
  #ifdef GNU
    #ifdef ANSI
...
#   #ifndef alloca
#     #ifdef UNIX_OSF
#       printf("extern char* alloca _ARGS((int size));\n");
#     #else
#       printf("extern void* alloca _ARGS((int size));\n");
#     #endif
#   #endif
# #elif defined(_AIX)
...
# #endif
    ...
    printf("typedef signed char  CLISP_BYTE;\n");
  #else
    printf("typedef char         CLISP_BYTE;\n");
  #endif
  printf("typedef unsigned char  CLISP_UBYTE;\n");
  printf("typedef short          CLISP_WORD;\n");
  printf("typedef unsigned short CLISP_UWORD;\n");
  #if (long_bitsize==32)
    printf("typedef long           CLISP_LONG;\n");
    printf("typedef unsigned long  CLISP_ULONG;\n");
  #elif (int_bitsize==32)
    printf("typedef int            CLISP_LONG;\n");
    printf("typedef unsigned int   CLISP_ULONG;\n");
  #endif
  #if (long_bitsize==64)
    printf("typedef long           CLISP_LONGLONG;\n");
    printf("typedef unsigned long  CLISP_ULONGLONG;\n");
  #elif defined(HAVE_LONGLONG)
    printf("typedef long long           CLISP_LONGLONG;\n");
    printf("typedef unsigned long long  CLISP_ULONGLONG;\n");
  #endif
  ...
  printf("#define NULL  ((void*) 0L)\n");
    ...
    printf("#define pointerplus(pointer,offset)  ((void*)((CLISP_ULONG)(pointer)+(offset)))\n");
  #else
    printf("#define pointerplus(pointer,offset)  ((CLISP_UBYTE*)(pointer)+(offset))\n");
  #endif
...
# #if defined(GNU)
#   #ifdef DECALPHA
#     printf("#define DYNAMIC_ARRAY(regdecl,arrayvar,arrayeltype,arraysize)  arrayeltype arrayvar[(arraysize)+1]\n");
#   #else
#     printf("#define DYNAMIC_ARRAY(regdecl,arrayvar,arrayeltype,arraysize)  arrayeltype arrayvar[arraysize]\n");
#   #endif
...
#   printf("#define DYNAMIC_ARRAY(regdecl,arrayvar,arrayeltype,arraysize)  regdecl arrayeltype* arrayvar = (arrayeltype*)alloca((arraysize)*sizeof(arrayeltype))\n");
#   printf("#define FREE_DYNAMIC_ARRAY(arrayvar)\n");
# #else
#   #ifdef HAVE_STDLIB_H
...
#   #endif
#   printf("extern void* malloca _ARGS((size_t size));\n");
#   printf("extern void freea _ARGS((void* ptr));\n");
#   printf("#define DYNAMIC_ARRAY(regdecl,arrayvar,arrayeltype,arraysize)  regdecl arrayeltype* arrayvar = (arrayeltype*)malloca((arraysize)*sizeof(arrayeltype))\n");
#   printf("#define FREE_DYNAMIC_ARRAY(arrayvar)  freea(arrayvar)\n");
# #endif
  { int i;
    for (i=1; i<=8; i++)
      { printf("typedef CLISP_UBYTE   uint%d;\n",i);
        printf("typedef CLISP_BYTE    sint%d;\n",i);
      }
    for (i=9; i<=16; i++)
      { printf("typedef CLISP_UWORD   uint%d;\n",i);
        printf("typedef CLISP_WORD    sint%d;\n",i);
      }
    for (i=17; i<=32; i++)
      { printf("typedef CLISP_ULONG   uint%d;\n",i);
        printf("typedef CLISP_LONG    sint%d;\n",i);
      }
    #ifdef HAVE_LONGLONG
      for (i=33; i<=64; i++)
        if ((i==33) || (i==48) || (i==64))
          { printf("typedef CLISP_ULONGLONG  uint%d;\n",i);
            printf("typedef CLISP_LONGLONG   sint%d;\n",i);
          }
    #endif
  }
...
# printf("       && ((TheArray(obj)->flags & %d) == %d) \\\n",arrayflags_atype_mask,Atype_Bit);
# printf("  )   )\n");
# printf("#define m_bit_vector_p(obj)  \\\n");
# printf4("  (((mtypecode(obj) & ~%d) == %d) || (((mtypecode(obj) & ~%d) == %d) \\\n",(tint)imm_array_mask,(tint)sbvector_type,(tint)imm_array_mask,(tint)bvector_type);
# printf("       && ((TheArray(obj)->flags & %d) == %d) \\\n",arrayflags_atype_mask,Atype_Bit);
# printf("  )   )\n");
...
# printf1("#define minstancep(obj)  (mtypecode(obj)==%d)\n",(tint)instance_type);
# #ifdef case_structure
#   printf1("#define structurep(obj)  (typecode(obj)==%d)\n",(tint)structure_type);
#   printf1("#define mstructurep(obj)  (mtypecode(obj)==%d)\n",(tint)structure_type);
# #else
#   printf("#define structurep(obj)  (orecordp(obj) && (TheRecord(obj)->rectype == %d))\n",Rectype_Structure);
#   printf("#define mstructurep(obj)  (morecordp(obj) && (TheRecord(obj)->rectype == %d))\n",Rectype_Structure);
# #endif
# printf1("#define orecordp(obj)  (typecode(obj)==%d)\n",(tint)orecord_type);
# printf1("#define morecordp(obj)  (mtypecode(obj)==%d)\n",(tint)orecord_type);
# printf("#define packagep(obj)  (orecordp(obj) && (TheRecord(obj)->rectype == %d))\n",Rectype_Package);
# printf1("#define charp(obj)  (typecode(obj)==%d)\n",(tint)char_type);
# printf1("#define mcharp(obj)  (mtypecode(obj)==%d)\n",(tint)char_type);
...
# #endif
    ...
    printf("#define begin_call()  saved_STACK = STACK\n");
    printf("#define end_call()  saved_STACK = (object*)NULL\n");
    #ifdef HAVE_SAVED_REGISTERS
      printf("extern struct registers\n");
      printf("              {\n");
      printf("                long STACK_register_contents;\n");
      printf("                long mv_count_register_contents;\n");
      printf("                long value1_register_contents;\n");
      printf("                struct registers *prev;\n");
      printf("              } *callback_saved_registers;\n");
      printf("#define begin_callback()  \\\n");
      printf("  { struct registers *registers = alloca(sizeof(struct registers)); \\\n");
      printf("    registers->prev = callback_saved_registers; \\\n");
      ...
      printf("    callback_saved_registers = registers; \\\n");
      printf("    STACK = saved_STACK; end_call(); \\\n");
      printf("    }\n");
      printf("#define end_callback()  \\\n");
      printf("  { struct registers *registers = callback_saved_registers; \\\n");
      printf("    begin_call(); \\\n");
      ...
      printf("    callback_saved_registers = registers->prev;\\\n");
      printf("    }\n");
    #else
      printf("#define begin_callback()  setSTACK(STACK = saved_STACK); end_call()\n");
      printf("#define end_callback()  begin_call()\n");
    #endif
    ...
    printf("#define end_callback()\n");
  #else
    #if defined(HAVE_SAVED_SUBR_SELF)
      printf("extern object saved_subr_self;\n");
      printf("#define begin_call()  saved_subr_self = subr_self\n");
      printf("#define end_call()  subr_self = saved_subr_self\n");
      printf("#define begin_callback()  end_call()\n");
      printf("#define end_callback()  begin_call()\n");
    #else
    ...
    #endif
  #endif
# #if defined(AMIGAOS) || defined(NO_ASYNC_INTERRUPTS) || !defined(HAVE_SAVED_STACK)
...
# printf("typedef Values subr_norest_function _ARGS((void));\n");
# printf("typedef Values subr_rest_function _ARGS((reg4 uintC argcount, reg3 object* rest_args_pointer));\n");
...
#   printf("#define symbol_tab  (*symbol_tab_addr)\n");
  ...
  printf("typedef struct module_ { char* name; subr_* stab; uintC* stab_size; object* otab; uintC* otab_size; boolean initialized; subr_initdata* stab_initdata; object_initdata* otab_initdata; void (*initfunction1) _ARGS((struct module_ *)); void (*initfunction2) _ARGS((struct module_ *));");
...
# printf("extern Values eval _ARGS((object form));\n");
  ...
  #endif
  printf("#define LISPFUN_F(name,req_anz,opt_anz,rest_flag,key_flag,key_anz,keywords)  { (lisp_function)(&C_%sname), 0, 0, 0, req_anz, opt_anz, (uintB)subr_%srest_flag, (uintB)subr_%skey_flag, key_anz, },\n",glue,glue,glue);
  printf("#define LISPFUN  LISPFUN_B\n");
# printf("extern object allocate_bit_vector_0 _ARGS((uintL len));\n");
# printf("extern uintB up_case _ARGS((uintB ch));\n");
# printf("extern uintB down_case _ARGS((uintB ch));\n");
# printf("extern uintB* unpack_string _ARGS((object string, uintL* len));\n");
# printf("extern object make_list _ARGS((uintL len));\n");
# printf("extern object listof _ARGS((uintC len));\n");
# printf("typedef enum { condition, serious_condition, error, program_error, control_error, arithmetic_error, division_by_zero, floating_point_overflow, floating_point_underflow, cell_error, unbound_variable, undefined_function, type_error, package_error, print_not_readable, stream_error, end_of_file, file_error, storage_condition, warning, } conditiontype;\n");
...
# printf("extern object find_package _ARGS((object string));\n");
# printf("extern uintBWL intern _ARGS((object string, object pack, object* sym_));\n");
# printf("extern object intern_keyword _ARGS((object string));\n");
# printf("extern boolean eql _ARGS((object obj1, object obj2));\n");
# printf("extern boolean equal _ARGS((object obj1, object obj2));\n");
# printf("extern boolean equalp _ARGS((object obj1, object obj2));\n");
# printf("extern object get _ARGS((object symbol, object key));\n");
  printf("extern object L_to_I _ARGS((sint32 wert));\n");
  #if (intLsize<=oint_data_len)
    printf("#define UL_to_I(wert)  fixnum((uintL)(wert))\n");
  #else
    printf("extern object UL_to_I _ARGS((uintL wert));\n");
  #endif
  printf("extern object L2_to_I _ARGS((sint32 wert_hi, uint32 wert_lo));\n");
  printf("extern object UL2_to_I _ARGS((uint32 wert_hi, uint32 wert_lo));\n");
  #ifdef intQsize

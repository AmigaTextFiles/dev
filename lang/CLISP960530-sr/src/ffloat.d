...
# Auspacken eines Floats:
    ...
    #ifdef GNU
      #define FF_to_float(obj)  (((ffloatjanus) { explicit_: ffloat_value(obj) }).machine_float)
    #else
...
#   maybe_nan: Ergebnis unbestimmt, liefert IEEE-NaN
  #define float_to_FF(expr,ergebnis_zuweisung,maybe_overflow,maybe_subnormal,maybe_underflow,maybe_divide_0,maybe_nan)  \
    { var ffloatjanus _erg; _erg.machine_float = (expr);            \
      if ((_erg.explicit_ & ((uint32)bit(FF_exp_len+FF_mant_len)-bit(FF_mant_len))) == 0) # e=0 ? \
        { if ((maybe_underflow                                      \
            ...
            && (((~_erg.explicit_) & ((uint32)bit(FF_exp_len+FF_mant_len)-bit(FF_mant_len))) == 0) # e=255 ? \
           )                                                        \
        { if (maybe_nan && !((_erg.explicit_ << (32-FF_mant_len)) == 0)) \
        ...
        { ergebnis_zuweisung allocate_ffloat(_erg.explicit_); }      \
    }
#endif
...
#     overflow schiebe um ein weiteres Bit nach rechts, incr. Exponenten.
 #if defined(FAST_FLOAT) && !defined(I80Z86)
     ...
     {var reg7 signean sign = R_sign(x); # Vorzeichen
      ...
      {var reg4 sintL lendiff = I_integer_length(x) # (integer-length a)
       ...
       { var reg5 object zaehler;
        ...
        {var reg1 uint32 mant = ((FF_mant_len+3 <= oint_data_len)

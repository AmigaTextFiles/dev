    ...
    { var reg1 object _x = (obj);                                         \
      ...
      if (uexp==0)                                                        \
        ...
        { exp_zuweisung (sintWL)((uintWL)uexp - SF_exp_mid); # Exponent   \
          unused (sign_zuweisung R_sign(_x));                # Vorzeichen \
...
# Falls x=0.0, fertig. Sonst Vorzeichenbit umdrehen.
  local object SF_minus_SF(x)
    var reg1 object x;
    { return (eq(x,SF_0) ? x : as_object(as_oint(x) ^ wbit(vorz_bit_o)) ); }
...
# (- x1 x2) = (+ x1 (- x2))
  local object SF_SF_minus_SF(x1,x2)
    ...
    { if (eq(x2,SF_0))
        ...
        { return SF_SF_plus_SF(x1, as_object(as_oint(x2) ^ wbit(vorz_bit_o)) ); }
    }

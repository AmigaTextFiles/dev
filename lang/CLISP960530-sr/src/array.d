  ...
  local object notsimple_displace(array,index)
    ...
    { loop
      ...
      fehler_bad_index:
        ...
        fehler(error,GETTEXT("index too large")); # ausführlicher??
    }
...
# > obj: Nicht-Array
# > subr_self: Aufrufer (ein SUBR)
  nonreturning_function(local, fehler_array_displaced_to, (object obj));
  local void fehler_array_displaced_to(obj)
    var reg1 object obj;
    { 
      pushSTACK(obj); # Wert für Slot DATUM von TYPE-ERROR
      pushSTACK(S(array)); # Wert für Slot EXPECTED-TYPE von TYPE-ERROR
...
# > subr_self: Aufrufer (ein SUBR)
  ...
  local void fehler_index_range()
    { pushSTACK(TheSubr(subr_self)->name);
      ...
      fehler(error,GETTEXT("~: index ~ for ~ is out of range"));
    }
...
# (ROW-MAJOR-AREF array index), CLtL2 S. 450
  { var reg1 object array = STACK_1;
    # Array überprüfen:
    test_array(array);
    # index überprüfen:
    if (!mposfixnump(STACK_0)) fehler_index_type();
...
# = (SETF (ROW-MAJOR-AREF array index) element), CLtL2 S. 450
  { var reg2 object element = popSTACK();
    var reg1 object array = STACK_1;
    # Array überprüfen:
    test_array(array);
    # index überprüfen:
...
LISPFUNN(array_dimension,2) # (ARRAY-DIMENSION array axis-number), CLTL S. 292
  { var reg1 object axis_number = popSTACK();
    ...
    fehler_axis:
      ...
      fehler(error,GETTEXT("~: ~ is not an nonnegative integer less than the rank of ~"));
  }
    ...
    { # Hauptunterscheidung: Vektor / mehrdimensionaler Array
      ...
      fehler2: # Fehlermeldung bei (mindestens) 2 Argumenten
        { var reg1 object array1 = STACK_2;
          ...
          fehler(error,GETTEXT("~: The arguments ~ and ~ should be arrays of bits with the same dimensions"));
        }
  ...
  local uintL* get_fill_pointer(obj)
    ...
    { # obj muß ein Vektor sein:
      ...
      fehler_fillp:
        ...
        fehler(error,GETTEXT("~: vector ~ has no fill pointer"));
    }
            ...
            { # extension sollte ein Fixnum >0, <arraysize_limit sein:
              if ( (!(posfixnump(extension)))
                ...
                { pushSTACK(extension); # Wert für Slot DATUM von TYPE-ERROR
                  ...
                  fehler(type_error,GETTEXT("~: extension ~ should be a positive fixnum"));
            }   }
            else
          ...
          { var reg4 uintL newlen = len + inc; # neue Länge
            ...
            if (newlen > arraysize_limit_1)
              { # Vektor würde zu lang -> Fehlermeldung
                ...
                fehler(error,GETTEXT("~: extending the vector by ~ elements makes it too long"));
              }
            #endif
            { # Neuen Datenvektor holen. Dazu Fallunterscheidung je nach Typ:
              ...
              switch (atype)
                { case Atype_T: # array ist ein General-Vector
                    ...
                    { var reg1 object* ptr2 = &TheSvector(neuer_datenvektor)->data[0];
                      # alten in neuen Datenvektor kopieren:
                      if (len>0)
                        { var uintL index = 0;
                          var reg4 object datenvektor = array1_displace_check(array,len,&index);
                          var reg2 object* ptr1 = &TheSvector(datenvektor)->data[index];
                  ...
                  case Atype_String_Char: # array ist ein String
                    ...
                    array = STACK_0; # array wieder holen
                    { var reg1 uintB* ptr2 = &TheSstring(neuer_datenvektor)->data[0];
                      # alten in neuen Datenvektor kopieren:
                      if (len>0)
                        { var uintL index = 0;
                          var reg4 object datenvektor = array1_displace_check(array,len,&index);
                          var reg2 uintB* ptr1 = &TheSstring(datenvektor)->data[index];
                          var reg3 uintL count;
                          dotimespL(count,len, { *ptr2++ = *ptr1++; } );
                        }
                      # dann new_element anfügen:
      ...
      { var reg1 uintB initcount = 0; # Zähler
        ...
        if (initcount > 1) # Mehr als eine Initialisierung?
          { pushSTACK(TheSubr(subr_self)->name);
            ...
            fehler(error,GETTEXT("~: ambiguous, more than one initialisation specified"));
      }   }
      ...
      { initial_contents_depth--;
        ...
        if (!(eq(value1,*(localptr STACKop initial_contents_depth))))
          { # fehlerhafte Sequence seq noch in STACK_0.
            ...
            fehler(error,GETTEXT("~: ~ has not the correct length"));
          }
  ...
  local uintL test_displaced(eltype,totalsize)
    ...
    { # displaced-to überprüfen, muß ein Array sein:
      ...
      if_arrayp(displaced_to, ; ,
        { fehler_array_displaced_to(displaced_to);
        });
      {# Elementtyp von displaced_to bestimmen:
       ...
       if (!(eltype == displaced_eltype))
         { pushSTACK(displaced_to); # Wert für Slot DATUM von TYPE-ERROR
           ...
           fehler(type_error,GETTEXT("~: ~-argument ~ has not element type ~"));
         }
      }
       ...
       {# Überprüfen, ob angesprochenes Teilstück ganz in displaced-to paßt:
        ...
        if (!(displaced_index_offset+totalsize <= displaced_totalsize))
          { pushSTACK(S(Kdisplaced_to));
            ...
            fehler(error,GETTEXT("~: array-total-size + displaced-offset (= ~) exceeds total size ~ of ~-argument"));
       }  }
    ...
    { # fill-pointer war angegeben und /=NIL
      if (eq(STACK_2,S(t))) # T angegeben ->
        ...
        { pushSTACK(STACK_2); # Wert für Slot DATUM von TYPE-ERROR
          ...
          fehler(type_error,GETTEXT("~: fill-pointer ~ should be a nonnegative fixnum"));
        }
      else
      ...
      if (!nullp(STACK_2)) # fill-pointer angegeben?
        { if (!(rank==1)) # Rang muß 1 sein
            { pushSTACK(fixnum(rank));
              ...
              fehler(error,GETTEXT("~: ~ may not be specified for an array of rank ~"));
            }
       ...
       { eltype = eltype_code(STACK_5);
         ...
         if (!(eltype == (TheArray(STACK_6)->flags & arrayflags_atype_mask)))
           { pushSTACK(STACK_6); # Wert für Slot DATUM von TYPE-ERROR
             ...
             fehler(type_error,GETTEXT("~: array ~ has not element-type ~"));
       }   }
       else
         ...
         { # array muß Fill-Pointer haben:
           if (!(TheArray(STACK_6)->flags & bit(arrayflags_fillp_bit)))
             { pushSTACK(STACK_6);
               ...
               fehler(error,GETTEXT("~: array ~ has no fill-pointer"));
             }
           fillpointer = test_fillpointer(totalsize); # Fill-Pointer-Wert
         }
         else
           ...
           if (TheArray(array)->flags & bit(arrayflags_fillp_bit))
             if (!(TheArray(array)->dims[2] <= totalsize))
               ...
               { pushSTACK(fixnum(totalsize));
                 ...
                 fehler(error,GETTEXT("~: the fill-pointer of array ~ is ~, greater than ~"));
         }     }
       # Array modifizieren:
   ...
   {var reg2 uintL len = vector_length(seq);
    ...
    if (mposfixnump(STACK_0) && (posfixnum_to_L(STACK_0)<=len))
      ...
      { # Stackaufbau: seq, index.
        ...
        fehler(error,GETTEXT("Illegal START index ~ for ~"));
      }
  }}
   ...
   {var reg2 uintL len = vector_length(seq);
    ...
    if (mposfixnump(STACK_0) && (posfixnum_to_L(STACK_0)<=len))
      ...
      { # Stackaufbau: seq, index.
        ...
        fehler(error,GETTEXT("Illegal END index ~ for ~"));
      }
  }}

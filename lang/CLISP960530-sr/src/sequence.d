...
# kann GC auslösen
  ...
  local object valid_type(name)
    ...
    { # Unsere elementaren Sequence-Typen sind LIST, VECTOR, STRING, BIT-VECTOR.
      ...
      fehler(error, GETTEXT("There are no sequences of type ~"));
    }
...
# < ergebnis: Typdescriptor oder NIL
  ...
  local object get_seq_type(seq)
    ...
    { var reg3 object name;
      ...
      elif ((typecode(seq)&~imm_array_mask)==bvector_type) # Typ n, bedeutet (VECTOR (UNSIGNED-BYTE n))
    ...
    if (eq(STACK_1,unbound)) # :initial-element nicht angegeben?
      { if (!eq(STACK_0,unbound)) # :update ohne :initial-element -> Error
          { pushSTACK(S(make_sequence));
            ...
            fehler(error, GETTEXT("~: :update must not be specified without :initial-element"));
          }
...
# kann GC auslösen
  ...
  local void fehler_bad_length (object len,object concatenate_sym);
  local void fehler_bad_length (len,concatenate_sym)
    var object len;
    var object concatenate_sym;
    { pushSTACK(len);
      pushSTACK(concatenate_sym);
      //: DEUTSCH "~: Fehlerhafte Länge aufgetreten: ~"
      //: ENGLISH "~: bad length ~"
      //: FRANCAIS "~ : occurence d'une mauvaise longueur: ~"
      fehler(error, GETTEXT("~: bad length ~"));
       ...
       dotimesC(count,argcount,
         { NEXT(ptr); # typdescr überspringen
          {var reg1 object len = NEXT(ptr); # nächste Länge
           if (!(posfixnump(len)))
             fehler_bad_length(len,S(concatenate));
...
# (MAP-INTO result-sequence function {sequence}), CLtL2 S. 395
  { var reg7 object* args_pointer = rest_args_pointer STACKop 2;
   ...
   {var reg8 object* typdescr_pointer = args_end_pointer; # Pointer über die Typdescriptoren
    ...
    loop
      { # Test, ob eine weitere Iteration nötig:
        { var reg5 object* ptr1 = rest_args_pointer;
          ...
          var reg6 uintC count;
          dotimesC(count,argcount,
            { var reg3 object sequence = NEXT(ptr1);
              var reg2 object typdescr = NEXT(ptr2);
              var reg1 object pointer = NEXT(ptr2);
              # (SEQ-ENDTEST sequence pointer) :
    ...
    end_reached:
    { var reg1 object result = Next(args_pointer);
...
# CLTL S. 251, CLTL2 S. 397
  { # Stackaufbau: function, sequence, from-end, start, end, key, initial-value.
    ...
    { var reg1 object count = I_I_minus_I(STACK_(2+1),STACK_(3+1));
...
# > subr_self: Aufrufer (ein SUBR)
  ...
  global void fehler_both_tests()
    { pushSTACK(TheSubr(subr_self)->name);
      ...
      fehler(error, GETTEXT("~: Must not specify both arguments to :TEST and :TEST-NOT"));
    }
                ...
                until (bvi1==bvl)
                  { if (!(sbvector_btst(STACK_(0+1),bvi1))) # (sbit bv bvi1) abfragen
                      ...
                      {{pushSTACK(STACK_(6+3+1)); # sequence
                       ...
                       {var reg1 uintL bvi2 = bvi1+1; # bvi2 := bvi1+1
                        until (bvi2==bvl)
...
# (READ-CHAR-SEQUENCE sequence stream [:start] [:end]), cf. dpANS S. 21-26
  { # Stackaufbau: sequence, stream, start, end.
    ...
    if (eq(seq_type(STACK_0),S(string))) # Typname = STRING ?
      { var uintL len;
        var reg2 uintB* charptr = unpack_string(STACK_4,&len);
        # Ab charptr kommen len Zeichen.
        var reg3 uintL start = posfixnum_to_L(STACK_2);
        var reg4 uintL end = posfixnum_to_L(STACK_1);
        # Versuche, eine optimierte Lese-Routine aufzurufen:
        var reg1 uintB* endptr = read_schar_array(STACK_3,&charptr[start],end-start);
        if (!(endptr==NULL))
    ...
    until (eql(STACK_3,STACK_2)) # index = end (beides Integers) -> fertig
      { var reg1 object item = read_char(&STACK_4); # ein Element lesen
  ...
  { # Stackaufbau: sequence, stream, start, end.
    ...
    if (eq(seq_type(STACK_0),S(string))) # Typname = STRING ?
      { var uintL len;
        ...
        var reg1 uintB* endptr = write_schar_array(STACK_3,&charptr[start],end-start);
  ...
  { # Stackaufbau: sequence, stream, start, end.
    ...
    if (eq(seq_type(STACK_0),fixnum(8))) # Typname = (VECTOR (UNSIGNED-BYTE 8)) ?
      { var reg3 uintL start = posfixnum_to_L(STACK_2);
        var reg4 uintL end = posfixnum_to_L(STACK_1);
        var uintL index = 0;
        var reg5 object dv = array1_displace_check(STACK_4,end,&index);
        var reg2 uintB* byteptr = &TheSbvector(TheArray(dv)->data)->data[index];
        # Ab byteptr kommen end Bytes.
        # Versuche, eine optimierte Lese-Routine aufzurufen:
        var reg1 uintB* endptr = read_byte_array(STACK_3,&byteptr[start],end-start);
        if (!(endptr==NULL))
    ...
    # Stackaufbau: sequence, stream, index, end, typdescr, pointer.
    until (eql(STACK_3,STACK_2)) # index = end (beides Integers) -> fertig
      { var reg1 object item = read_byte(STACK_4); # ein Element lesen
        if (eq(item,eof_value)) break; # EOF -> fertig
        pushSTACK(STACK_5); pushSTACK(STACK_(0+1)); pushSTACK(item);
        funcall(seq_access_set(STACK_(1+3)),3); # (SEQ-ACCESS-SET sequence pointer item)
        # pointer := (SEQ-UPD sequence pointer) :
  ...
  { # Stackaufbau: sequence, stream, start, end.
    ...
    if (eq(seq_type(STACK_0),fixnum(8))) # Typname = (VECTOR (UNSIGNED-BYTE 8)) ?
      { var reg3 uintL start = posfixnum_to_L(STACK_2);
        ...
        # Ab byteptr kommen end Bytes.
        # Versuche, eine optimierte Schreib-Routine aufzurufen:
        var reg1 uintB* endptr = write_byte_array(STACK_3,&byteptr[start],end-start);
        if (!(endptr==NULL)) goto done;
      }
    # start- und end-Argumente subtrahieren:
    STACK_1 = I_I_minus_I(STACK_1,STACK_2); # (- end start), ein Integer >=0

...
# Deklarationen für SPVW:
#if !((defined(GNU) && !defined(RISCOS) && !defined(CONVEX)) || (defined(UNIX) && !defined(NO_ALLOCA) && !defined(SPARC)) || defined(WATCOM)) # siehe unten
  ...
  #endif
#elif (defined(UNIX) && !defined(NO_ALLOCA) && !defined(SPARC)) || defined(WATCOM)
  ...
  # Fehlermeldung, wenn zuwenig Platz für num_stack:
    ...
    local void arith_ueberlauf()
      { 
        ...
        fehler(storage_condition,GETTEXT( "stack overflow during bignum arithmetic"));
      }
  #
...
# Fehlermeldung wegen Division durch Null
  ...
  local void divide_0()
    { 
      ...
      fehler(division_by_zero,GETTEXT("division by zero"));
    }
...
# fehler_overflow();
  ...
  local void fehler_overflow()
    { 
      ...
      fehler(floating_point_overflow,GETTEXT("floating point overflow"));
    }
...
# fehler_underflow();
  ...
  local void fehler_underflow()
    { 
      ...
      fehler(floating_point_underflow,GETTEXT("floating point underflow"));
    }

# Stellt fest, ob Floating-Point-Unterlauf erlaubt ist
# underflow_allowed()
  #define underflow_allowed()  \
    (sym_nullp(S(inhibit_floating_point_underflow)))

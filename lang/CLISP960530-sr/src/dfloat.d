...
#   maybe_nan: Ergebnis unbestimmt, liefert IEEE-NaN
  ...
  #define double_to_DF(expr,ergebnis_zuweisung,maybe_overflow,maybe_subnormal,maybe_underflow,maybe_divide_0,maybe_nan)  \
    { var dfloatjanus _erg; _erg.machine_double = (expr);                \
      if ((_erg.explicit_ & ((uint64)bit(DF_exp_len+DF_mant_len)-bit(DF_mant_len))) == 0) # e=0 ? \
        { if ((maybe_underflow                                           \
               || (maybe_subnormal && !((_erg.explicit_ << 1) == 0))      \
              )                                                          \
              && underflow_allowed()                                     \
             )                                                           \
            ...
            && (((~_erg.explicit_) & ((uint64)bit(DF_exp_len+DF_mant_len)-bit(DF_mant_len))) == 0) # e=2047 ? \
           )                                                             \
        { if (maybe_nan && !((_erg.explicit_<<(64-DF_mant_len)) == 0))    \
        ...
        { ergebnis_zuweisung allocate_dfloat(_erg.explicit_); }           \
    }
#else
  #define double_to_DF(expr,ergebnis_zuweisung,maybe_overflow,maybe_subnormal,maybe_underflow,maybe_divide_0,maybe_nan)  \
    { var dfloatjanus _erg; _erg.machine_double = (expr);                 \
      if ((_erg.explicit_.semhi & ((uint32)bit(DF_exp_len+DF_mant_len-32)-bit(DF_mant_len-32))) == 0) # e=0 ? \
        { if ((maybe_underflow                                            \
               || (maybe_subnormal                                        \
                   && !(((_erg.explicit_.semhi << 1) == 0) && (_erg.explicit_.mlo == 0)) \
              )   )                                                       \
              && underflow_allowed()                                      \
             )                                                            \
            { fehler_underflow(); } # subnormal oder noch kleiner -> Underflow \
            ...
            && (((~_erg.explicit_.semhi) & ((uint32)bit(DF_exp_len+DF_mant_len-32)-bit(DF_mant_len-32))) == 0) # e=2047 ? \
           )                                                              \
        { if (maybe_nan && !(((_erg.explicit_.semhi<<(64-DF_mant_len)) == 0) && (_erg.explicit_.mlo==0))) \
            { divide_0(); } # NaN, also Singularität -> "Division durch 0" \
          else # Infinity                                                 \
          if (!maybe_overflow || maybe_divide_0)                          \
            { divide_0(); } # Infinity, Division durch 0                  \
        ...
        { ergebnis_zuweisung allocate_dfloat(_erg.explicit_.semhi,_erg.explicit_.mlo); }  \
    }
#endif
#endif
    ...
    { var reg2 dfloat x_ = TheDfloat(x)->float_value;
      ...
      if (uexp <= DF_exp_mid) # e<=0 ?
        ...
        { if (uexp > DF_exp_mid+DF_mant_len) # e > 52 ?
            ...
            { var reg1 uint64 mask = # Bitmaske: Bits 52-e..0 gesetzt, alle anderen gelöscht
                bit(DF_mant_len+1+DF_exp_mid-uexp)-1;
      ...
      if (uexp < DF_exp_mid) # x = 0.0 oder e<0 ?
        ...
        { if (uexp > DF_exp_mid+DF_mant_len) # e > 52 ?
            ...
            if (uexp > DF_exp_mid+1) # e>1 ?
              { var reg4 uint64 bitmask = # Bitmaske: Bit 52-e gesetzt, alle anderen gelöscht
                  bit(DF_mant_len+DF_exp_mid-uexp);
                var reg3 uint64 mask = # Bitmaske: Bits 51-e..0 gesetzt, alle anderen gelöscht
                  bitmask-1;
...
#if defined(FAST_DOUBLE) && !defined(I80Z86)
     ...
     {var reg8 signean sign = R_sign(x); # Vorzeichen
      ...
      {   var reg9 uintL exp = I_integer_length(x); # (integer-length x)
       ...
       {  var reg2 uintD* MSDptr;
          var reg5 uintC len;
          I_to_NDS_nocopy(x, MSDptr=,len=,_EMA_);
        ...
        { var reg4 uintD msd = *MSDptr++; # erstes Digit
         ...
         {var reg6 uintL shiftcount = exp % intDsize;
          #ifdef intQsize
          var reg3 uint64 mant = # führende 64 Bits
            (shiftcount==0

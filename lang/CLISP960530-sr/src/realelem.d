  ...
  #define defaultfloatcase(symbol, SF_statement,FF_statement,DF_statement,LF_statement, save_statement,restore_statement) \
    {var reg1 object def = Symbol_value(symbol); # Wert holen       \
     if (eq(def,S(short_float))) { SF_statement }                   \
       ...
       { set_Symbol_value(symbol,S(single_float)); # Wert korrigieren \
         save_statement                                             \
         # Warnung ausgeben:                                        \
         ...
         # )                                                        \
         { pushSTACK(OL(default_float_format_warnung_string_line_1)); \
           pushSTACK(O(newline_string));                              \
           pushSTACK(OL(default_float_format_warnung_string_line_2)); \
           pushSTACK(string_concat(3));                               \
         }                                                            \
         pushSTACK(symbol);                                         \
         pushSTACK(symbol);                                         \
...
# Generiert eine Funktion wie R_floor_I_R
  #define GEN_R_round(rounding)  \
    ...
    local void CONCAT3(R_,rounding,_I_R) PARM1(x, \
      var reg1 object x)                          \
      { if (R_rationalp(x))                       \
...
# Generiert eine Funktion wie R_ffloor_F_R
  #define GEN_R_fround(rounding)  \
    ...
    local void CONCAT3(R_f,rounding,_F_R) PARM1(x,                     \
      var reg1 object x)                                               \
      { if (R_rationalp(x))                                            \
...
# Generiert eine Funktion wie R_R_floor_I_R
  #define GEN_R_R_round(rounding)  \
    ...
    local void CONCAT3(R_R_,rounding,_I_R) PARM2(x,y, \
      var reg2 object x,                              \
      var reg1 object y)                              \
...
# Generiert eine Funktion wie R_R_mod_R
  #define GEN_R_R_mod(remainder,rounding)  \
    ...
    local object CONCAT3(R_R_,remainder,_R) PARM2(x,y, \
      var reg2 object x,                               \
      var reg1 object y)                               \
      { if (N_integerp(x) && N_integerp(y)) # beides Integers? \
...
# Generiert eine Funktion wie R_R_ffloor_F_R
  #define GEN_R_R_fround(rounding)  \
    ...
    local void CONCAT3(R_R_f,rounding,_F_R) PARM2(x,y,                           \
      var reg2 object x,                                                         \
      var reg1 object y)                                                         \
...
# kann keine GC auslösen!
  ...
  #define I_I_gleich(x_,y_,false_statement)  \
    { var reg1 object _x = (x_);                       \
      ...
      if (!eq(_x,_y))                                  \
        { if (!wbit_test(as_oint(_x) & as_oint(_y),bignum_bit_o)) { false_statement } \
  ...
  local boolean R_R_gleich(x,y)
    ...
    { if (R_rationalp(x))
        ...
        { if (R_rationalp(y))
            ...
            # x rational, y Float
            { if (!same_sign_p(x,y)) return FALSE; # verschiedene Vorzeichen?
              ...
              {SAVE_NUM_STACK # num_stack retten
               ...
               floatcase(y,
                 { # SF y entpacken:
                   ...
                   SF_decode(y, { goto no; },_EMA_,e=,mant=);

...
(defun \,-reader (stream char &aux (c (peek-char nil stream)))
  ...
  (cond ((null *backquote-level*)
         (error-of-type 'error
           #L{DEUTSCH "~S: Komma darf nur innerhalb von Backquote auftreten."
        ...
        ((zerop *backquote-level*)
         (error-of-type 'error
           #L{DEUTSCH "~S: Es dürfen nicht mehr Kommata als Backquotes auftreten."
  ...
  (if (atom skel)
    ...
    (cond ((eq (first skel) 'unquote-value)
          ...
          ((eq (first skel) 'splice)
           ; ,@form ist verboten
           (error-of-type 'error
             #L{DEUTSCH "Die Syntax ,@form ist nur innerhalb von Listen erlaubt."

...
# Bruno Haible 14.6.1995
# Marcus Daniels 11.3.1994
...
# fehler_bad_readtable();
  ...
  local void fehler_bad_readtable()
    { # *READTABLE* korrigieren:
      ...
      var reg2 object oldvalue = Symbol_value(sym);
      set_Symbol_value(sym,O(standard_readtable)); # := Standard-Readtable von Common Lisp
      # und Fehler melden:
      ...
      fehler(type_error, GETTEXT("The value of ~ was not a readtable. It has been reset."));
    }
...
# < readtable : die aktuelle Readtable
    ...
    #define get_readtable(zuweisung)  \
      { if (!sym_readtablep(S(readtablestern))) { fehler_bad_readtable(); }            \
        zuweisung Symbol_value(S(readtablestern));                                     \
      }
  #else # oder (optimierter):
    #define get_readtable(zuweisung)  \
      { if (!(sym_orecordp(S(readtablestern))                                                            \
...
# (GET-MACRO-CHARACTER char [readtable]), CLTL S. 362
  { # char überprüfen:
   ...
   {var reg1 object readtable = test_readtable_null_arg(); # Readtable
...
# (GET-DISPATCH-MACRO-CHARACTER disp-char sub-char [readtable]), CLTL S. 364
  { var reg2 object readtable = test_readtable_null_arg(); # Readtable
    var reg1 object* ptr = test_disp_sub_char(readtable);
...
# (READTABLE-CASE readtable), CLTL2 S. 549
  { var reg1 object readtable = popSTACK(); # Readtable
    if (!readtablep(readtable)) { fehler_readtable(readtable); } # überprüfen
...
# (SYSTEM::SET-READTABLE-CASE readtable value), CLTL2 S. 549
  { var reg4 object value = popSTACK();
    var reg5 object readtable = popSTACK(); # Readtable
    if (!readtablep(readtable)) { fehler_readtable(readtable); } # überprüfen
    # Symbol value in einen Index umwandeln durch Suche in der Tabelle O(rtcase..):
   {var reg1 object* ptr = &O(rtcase_0);
    ...
    pushSTACK(O(rtcase_2)); pushSTACK(O(rtcase_1)); pushSTACK(O(rtcase_0));
    pushSTACK(value);
...
# < TRUE, wenn /= NIL
# #define test_value(sym)  (!sym_nullp(sym))
  #define test_value(sym)  (!eq(NIL,Symbol_symvalue(sym)))
    ...
    { var reg2 object value = Symbol_value(symbol);
      ...
      if (posfixnump(value) &&
        ...
        { pushSTACK(value); # Wert für Slot DATUM von TYPE-ERROR
          ...
          pushSTACK(value);
          pushSTACK(symbol);
          set_Symbol_value(symbol,fixnum(10)); # Wert auf 10 setzen
          { const char *msg1,*msg2;
            //: DEUTSCH "Der Wert von ~ sollte eine ganze Zahl zwischen 2 und 36 sein, nicht ~." 
            //: ENGLISH "The value of ~ should be an integer between 2 and 36, not ~."
            //: FRANCAIS "La valeur de ~ doit être un nombre entier entre 2 et 36 et non ~."
            msg1 = GETTEXT("The value of ~ should be an integer between 2 and 36, not ~.");
            //: DEUTSCH "Er wird auf 10 gesetzt."
            //: ENGLISH "It has been reset to 10."
            //: FRANCAIS "Elle a été mise à 10."
            msg2 = GETTEXT("It has been reset to 10.");
            fehler3(type_error,msg1,NLstring,msg2);
          }
        }
    }
...
# fehler_charread(ch,&stream);
  ...
  local void fehler_charread(ch,stream_)
    ...
    { pushSTACK(*stream_); # Wert für Slot STREAM von STREAM-ERROR
      ...
      fehler(stream_error, GETTEXT("~ from ~: character read should be a string-char: ~"));
    }
...
# Case-Konversion:
  typedef uintB case_converter (uintB c);
  local uintB preserve_case (uintB c);
  local uintB preserve_case(c)
    var reg1 uintB c;
    { return c; }
...
# > stream: Stream
  ...
  local void fehler_eof_innen(stream_)
    var reg1 object* stream_;
    { pushSTACK(*stream_); # Wert für Slot STREAM von STREAM-ERROR
      if (sym_posfixnump(S(read_line_number))) # SYS::*READ-LINE-NUMBER* abfragen
        { pushSTACK(Symbol_value(S(read_line_number))); # Zeilennummer
          ...
          fehler(end_of_file, GETTEXT("~: input stream ~ ends within an object. Last opening parenthesis probably in line ~."));
        }
        else
        { pushSTACK(*stream_); # Stream
          pushSTACK(S(read));
...
# kann GC auslösen
  ...
  local void read_token_1(stream_,ch,scode)
    ...
    { # leere Token-Buffer holen, auf den STACK:
     ...
     {var reg8 boolean multiple_escape_flag = FALSE;
      var reg9 boolean escape_flag = FALSE;
      # Funktion zur Case-Umwandlung besorgen:
      var reg7 case_converter* case_convert;
      {var reg1 object readtable;
       get_readtable(readtable = );
       switch ((uintW)posfixnum_to_L(TheReadtable(readtable)->readtable_case))
         { case case_upcase:   case_convert = &up_case; break;
           case case_downcase: case_convert = &down_case; break;
           case case_preserve: case_convert = &preserve_case; break;
           default: NOTREACHED
      }  }
      goto char_read;
      loop
          ...
          switch(scode)
            { case syntax_illegal:
              ...
              case syntax_single_esc:
                ...
                if (scode==syntax_eof) # EOF erreicht?
                  { pushSTACK(*stream_); # Wert für Slot STREAM von STREAM-ERROR
                    ...
                    fehler(end_of_file, GETTEXT("~: input stream ~ ends within a token after single escape character"));
                  }
              escape:
                # nach Escape-Zeichen:
              ...
              case syntax_nt_macro:
                ...
                # als Groß-/Klein-Buchstabe ins Token übernehmen:
                {var reg1 uintB c = (*case_convert)(char_code(ch));
              ...
              case syntax_eof:
                ...
                if (multiple_escape_flag) # zwischen Multiple-Escape-Zeichen?
                  { pushSTACK(*stream_); # Wert für Slot STREAM von STREAM-ERROR
                    ...
                    fehler(end_of_file, GETTEXT("~: input stream ~ ends within a token after multiple escape character"));
                  }
      ...
      { var reg2 uintB* attrptr = attrptr0;
        ...
        dotimesL(count,len,
          { if (*attrptr >= a_letter)
              ...
              { var reg1 uintB c = *charptr; # Zeichen, muß 'A'-'Z','a'-'Z' sein
                if (c >= 'a') { c -= 'a'-'A'; }
         ...
         loop
           { # nächstes Zeichen
            ...
            {var reg2 uintB attr = *attrptr++; # dessen Attributcode
             if (attr==a_digit)
               { var reg1 uintB c = *charptr++; # Character (Digit, also '0'-'9','A'-'Z','a'-'z')
                 # Wert bestimmen:
                 var reg1 uintB wert = (c<'A' ? c-'0' : c<'a' ? c-'A'+10 : c-'a'+10);
...
# > O(token_buff_2): ihre Attributcodes
  ...
  local void upcase_token()
    { var reg3 object string = O(token_buff_1); # Semi-Simple-String
      var reg2 uintL len = TheArray(string)->dims[1]; # Fill-Pointer
      string = TheArray(string)->data; # Simple-String
     ...
     {var reg3 object macrodef = # Macro-Definition aus Tabelle holen
      ...
      if (nullp(macrodef)) # =NIL ?
        { pushSTACK(*stream_); # Wert für Slot STREAM von STREAM-ERROR
          ...
          fehler(stream_error, GETTEXT("~ from ~: ~ has no macro character definition"));
        }
        ...
        { pushSTACK(*stream_); # Stream als 1. Argument
          ...
          if (mv_count > 1)
            { pushSTACK(fixnum(mv_count)); # Wertezahl als Fixnum
              ...
              fehler(error, GETTEXT("~ from ~: macro character definition for ~ may not return ~ values, only one value."));
            }
          # höchstens 1 Wert.
         ...
         {var reg8 object arg; # Argument (Integer >=0 oder NIL)
              ...
              { var reg1 object nextch = read_char(stream_); # Character lesen
                if (eq(nextch,eof_value))
                  { pushSTACK(*stream_); # Wert für Slot STREAM von STREAM-ERROR
                    ...
                    fehler(end_of_file, GETTEXT("~: input stream ~ ends within read macro beginning to ~"));
                  }
          ...
          if (nullp(macrodef))
            ...
            { pushSTACK(*stream_); # Wert für Slot STREAM von STREAM-ERROR
              ...
              fehler(stream_error, GETTEXT("~ from ~: After ~ is ~ an undefined dispatch macro character"));
            }
          pushSTACK(*stream_); # Stream als 1. Argument
          ...
          if (mv_count > 1)
            { pushSTACK(fixnum(mv_count)); # Wertezahl als Fixnum
              ...
              { var const char *msg1,*msg2;
                ...
                msg1 = GETTEXT("~ from ~: dispatch macro character definition for ~ after ~ may not return ~ values,");
                //: DEUTSCH " sondern höchstens einen."
                //: ENGLISH " only one value."
                //: FRANCAIS ""
                msg2 = GETTEXT(" only one value.");
  ...
  local object read_internal(stream_)
    ...
    { wloop: # Schleife zum Überlesen von führendem Whitespace/Kommentar:
        ...
        { var reg1 uintL len = TheArray(O(token_buff_1))->dims[1]; # Länge des Token
          if (len > 1)
            ...
            { pushSTACK(*stream_); # Wert für Slot STREAM von STREAM-ERROR
              ...
              fehler(stream_error, GETTEXT("~ from ~: a token consisting only of dots cannot be meaningfully read in"));
            }
      ...
      { var uintWL base = get_read_base(); # Wert von *READ-BASE*
        ...
        var object string;
        var zahl_info info;
        var reg3 uintWL numtype = test_number_syntax(&base,&string,&info);
        if (!(numtype==0)) # Zahl?
         ...
         {var reg2 object pack = find_package(hstring);
          if (nullp(pack)) # Package nicht gefunden?
            { pushSTACK(copy_string(hstring)); # Displaced-String kopieren, Wert für Slot PACKAGE von PACKAGE-ERROR
              ...
              fehler(package_error, GETTEXT("~ from ~: there is no package with name ~"));
            }
          # Hilfs-String adjustieren:
          TheArray(hstring)->dims[0] = name_start_index; # Displaced-Offset
          TheArray(hstring)->totalsize =
            ...
            { # externes Symbol mit diesem Printnamen suchen:
              ...
              if (find_external_symbol(hstring,pack,&sym))
                ...
                { pushSTACK(pack); # Wert für Slot PACKAGE von PACKAGE-ERROR
                  ...
                  fehler(package_error, GETTEXT("~ from ~: ~ has no external symbol with name ~"));
            }   }
        }}
    } }
         ...
         {var reg1 object alistr = alist; # Liste durchlaufen
          ...
          if (!nullp(alistr))
            { fehler_badtable:
              ...
              fehler(error, GETTEXT("~: the value of ~ has been arbitrarily altered"));
            }
         }# Aliste alist ist OK
          pushSTACK(obj);
          {var reg1 object bad_reference =
...
# kann GC auslösen
  ...
  local object read_top(stream_,whitespace_p)
    ...
    var reg4 object whitespace_p;
    {
     ...
     #endif
      var reg7 object *whitespace_p_ptr;
      pushSTACK(whitespace_p); whitespace_p_ptr=&STACK_0;
      ...
      dynamic_bind(S(read_preserve_whitespace),*whitespace_p_ptr);
...
# Erst die allgemeine Funktion:
  ...
  local object read_delimited_list(stream_,endch,ifdotted)
    ...
    { var reg4 object ergebnis;
      var reg5 object *endch_ptr;
      var reg6 object *ifdotted_ptr;
      pushSTACK(endch); endch_ptr=&STACK_0;
      pushSTACK(ifdotted); ifdotted_ptr=&STACK_0;
      ...
      if (test_value(S(read_recursive_p))) # schon rekursiv?
        { ergebnis = read_delimited_list_recursive(stream_,*endch_ptr,*ifdotted_ptr); }
        else
  ...
  { var reg1 object* stream_ = test_stream_arg(STACK_1);
    ...
    if (test_value(S(read_suppress))) # *READ-SUPPRESS* /= NIL ?
      ...
      { get_buffers(); # zwei leere Buffer auf den Stack
        ...
        { var reg2 object string = copy_string(STACK_0);
...
# > STACK_0: sub-char
  ...
  local void fehler_dispatch_zahl()
    { pushSTACK(STACK_1); # Wert für Slot STREAM von STREAM-ERROR
      ...
      fehler(stream_error, GETTEXT("~ from ~: no number allowed between $ and $"));
    }
...
LISPFUNN(comment_reader,3) # liest #|
  { var reg1 object* stream_ = test_no_infix(); # n muß NIL sein
    ...
    fehler_eof:
      ...
      fehler(end_of_file, GETTEXT("~: input stream ~ ends within a comment #$ ... $#"));
    fertig:
      value1 = NIL; mv_count=0; skipSTACK(2); # keine Werte zurück
  }
      ...
      { var reg1 uintL font;
        if (mposfixnump(STACK_0) && ((font = posfixnum_to_L(STACK_0)) < char_font_limit))
          ...
          { pushSTACK(*stream_); # Wert für Slot STREAM von STREAM-ERROR
            ...
            fehler(stream_error, GETTEXT("~ from ~: font number ~ for character is too large, should be < ~"));
          }
      }
    # Font fertig.
          ...
          { var reg7 uintL hyphen = pos; # hyphen := pos
           ...
           {var reg10 uintL sub_len = hyphen-pos;
            ...
            { pushSTACK(*stream_); # Wert für Slot STREAM von STREAM-ERROR
              ...
              fehler(stream_error, GETTEXT("~ from ~: there is no character bit with name ~"));
            }
       ...
       {var reg1 object ch = name_char(hstring); # Character mit diesem Namen suchen
        if (nullp(ch))
          ...
          { pushSTACK(*stream_); # Wert für Slot STREAM von STREAM-ERROR
            ...
            fehler(stream_error, GETTEXT("~ from ~: there is no character with name ~"));
          }
        # gefunden
      ...
      switch (test_number_syntax(&base,&string,&info))
        { case 1: # Integer
          ...
          not_rational: # keine rationale Zahl
            ...
            fehler(stream_error, GETTEXT("~ from ~: token ~ after #$ is not a rational number in base ~"));
          default: NOTREACHED
        }
    }
  # UP für #B #O #X
  ...
  { var reg1 object* stream_ = test_stream_arg(STACK_2);
    ...
    if (nullp(STACK_0))
      { pushSTACK(*stream_); # Wert für Slot STREAM von STREAM-ERROR
        ...
        fehler(stream_error, GETTEXT("~ from ~: the number base must be given between $ and R"));
      }
   {var reg2 uintL base;
    ...
    if (mposfixnump(STACK_0) &&
      ...
      { pushSTACK(*stream_); # Wert für Slot STREAM von STREAM-ERROR
        ...
        fehler(stream_error, GETTEXT("~ from ~: The base ~ given between $ and R should lie between 2 and 36"));
      }
  }}
...
LISPFUNN(complex_reader,3) # liest #C
  { var reg3 object* stream_ = test_no_infix(); # n muß NIL sein
   ...
   {bad:
      ...
      fehler(stream_error, GETTEXT("~ from ~: bad syntax for complex number: #C~"));
  }}
  ...
  { var reg3 object* stream_ = test_stream_arg(STACK_2);
    ...
    {# Token kopieren und dabei in Simple-String umwandeln:
     ...
     fehler_dopp:
       ...
       fehler(stream_error, GETTEXT("~ from ~: token ~ after #: should contain no colon"));
  } }
  ...
  { var reg8 object* stream_ = test_stream_arg(STACK_2);
    ...
    if (token_escape_flag)
      { fehler_nur01:
        ...
        fehler(stream_error, GETTEXT("~ from ~: only zeroes and ones are allowed after #*"));
      }
       ...
       { # n angegeben, ein Integer >=0.
         ...
         if (n<len)
           { pushSTACK(*stream_); # Wert für Slot STREAM von STREAM-ERROR
             ...
             fehler(stream_error, GETTEXT("~ from ~: bit vector is longer than the explicitly given length ~"));
           }
         if ((n>0) && (len==0))
           { pushSTACK(*stream_); # Wert für Slot STREAM von STREAM-ERROR
             pushSTACK(STACK_(0+1)); # n
             ...
             fehler(stream_error, GETTEXT("~ from ~: must specify element of bit vector of length ~"));
           }
       }
      ...
      { # n angegeben, ein Integer >=0.
        ...
        if (n<len)
          { pushSTACK(*stream_); # Wert für Slot STREAM von STREAM-ERROR
            ...
            fehler(stream_error, GETTEXT("~ from ~: vector is longer than the explicitly given length ~"));
          }
        if ((n>0) && (len==0))
          { pushSTACK(*stream_); # Wert für Slot STREAM von STREAM-ERROR
            pushSTACK(STACK_(0+1)); # n
            ...
            fehler(stream_error, GETTEXT("~ from ~: must specify element of vector of length ~"));
          }
      }
    ...
    if (flags & strmflags_immut_B)
      { # Ergebnis-Array value1 immutabel machen:
        if (!array_simplep(value1))
          { var reg1 object dv = TheArray(value1)->data; # Datenvektor: Simple-Array
            if (!array_simplep(dv)) # Simple-Byte-Vektor?
              { TheArray(dv)->data = make_imm_array(TheArray(dv)->data); }
            TheArray(value1)->data = make_imm_array(dv);
          }
    ...
    { var reg4 object n = STACK_0;
      if (nullp(n)) # nicht angegeben?
        { pushSTACK(STACK_2); # Wert für Slot STREAM von STREAM-ERROR
          ...
          fehler(stream_error, GETTEXT("~ from ~: a number must be given between $ and $"));
        }
      # n ist ein Integer >=0
      if (!(posfixnump(n) && (posfixnum_to_L(n) < bit(oint_data_len-2))))
        # n ist zu groß
        { pushSTACK(STACK_2); # Wert für Slot STREAM von STREAM-ERROR
          pushSTACK(STACK_(1+1)); # sub-char
          pushSTACK(STACK_(0+2)); # n
     ...
     {var reg3 object label = # Internal-Label mit Nummer n
        type_data_object(system_type, bit(0) + (posfixnum_to_L(n)<<1) );
      ...
      bad_reftab: # Wert von SYS::*READ-REFERENCE-TABLE* ist keine Aliste
        ...
        fehler(error, GETTEXT("~ from ~: the value of ~ has been altered arbitrarily, it is not an alist: ~"));
    }}
   ...
   {var reg4 object lookup = lookup_label();
    if (consp(lookup))
      ...
      { pushSTACK(STACK_2); # Wert für Slot STREAM von STREAM-ERROR
        ...
        fehler(stream_error, GETTEXT("~ from ~: label #~= may not be defined twice"));
      }
      else
      ...
      {var reg3 object* stream_ = test_stream_arg(STACK_2);
       ...
       {var reg1 object new_cons = allocate_cons(); # neues Listen-Cons
        ...
        set_Symbol_value(S(read_reference_table),new_cons);
       }
...
LISPFUNN(not_readable_reader,3) # liest #<
  { var reg1 object* stream_ = test_stream_arg(STACK_2);
    ...
    fehler(stream_error, GETTEXT("~ from ~: objects printed as #<...> cannot be read back in"));
  }
...
LISPFUNN(syntax_error_reader,3) # liest #) und #whitespace
  { var reg1 object* stream_ = test_stream_arg(STACK_2);
    ...
    fehler(stream_error, GETTEXT("~ from ~: objects printed as $ in view of ~ cannot be read back in"));
  }
   ...
   {var reg1 object args = read_recursive_no_dot(stream_);
    ...
    if (atomp(args))
      { pushSTACK(*stream_); # Wert für Slot STREAM von STREAM-ERROR
        ...
        fehler(stream_error, GETTEXT("~ from ~: #S must be followed by the type and the contents of the structure, not ~"));
      }
    {var reg5 object name = Car(args); # Typ der Structure
     ...
     if (!symbolp(name)) # Typ muß ein Symbol sein !
       { pushSTACK(*stream_); # Wert für Slot STREAM von STREAM-ERROR
         ...
         fehler(stream_error, GETTEXT("~ from ~: the type of a structure should be a symbol, not ~"));
       }
     pushSTACK(name);
       ...
       {var reg4 object description = get(name,S(defstruct_description));
        if (eq(description,unbound)) # nichts gefunden?
          ...
          { pushSTACK(*stream_); # Wert für Slot STREAM von STREAM-ERROR
            ...
            fehler(stream_error, GETTEXT("~ from ~: no structure of type ~ has been defined"));
          }
        ...
        {var reg6 object constructor = # (svref description 2)
         ...
         if (nullp(constructor))
           { pushSTACK(*stream_); # Wert für Slot STREAM von STREAM-ERROR
             ...
             fehler(stream_error, GETTEXT("~ from ~: structures of type ~ cannot be read in, missing constructor function"));
           }
       ...
       { check_STACK();
         ...
         if (atomp(args))
           { dotted:
             ...
             fehler(stream_error, GETTEXT("~ from ~: a structure ~ may not contain a component \".\""));
           }
         {var reg4 object slot = Car(args);
          if (!symbolp(slot))
            { pushSTACK(*stream_); # Wert für Slot STREAM von STREAM-ERROR
              ...
              fehler(stream_error, GETTEXT("~ from ~: ~ is not a symbol, not a slot name of structure ~"));
            }
          if (nullp(Cdr(args)))
            { pushSTACK(*stream_); # Wert für Slot STREAM von STREAM-ERROR
              pushSTACK(*(stream_ STACKop -2)); # name
              ...
              fehler(stream_error, GETTEXT("~ from ~: missing value of slot ~ in structure ~"));
            }
  ...
  # > Stackaufbau: stream, sub-char, arg.
    ...
    local void fehler_closure_badchar()
      { pushSTACK(STACK_2); # Wert für Slot STREAM von STREAM-ERROR
        ...
        fehler(stream_error, GETTEXT("~ from ~: illegal syntax of closure code vector after #~Y"));
      }
      ...
      { var reg1 object obj = read_recursive_no_dot(stream_); # Objekt lesen
        if (!(consp(obj) && mconsp(Cdr(obj)))) # Länge >=2 ?
          { pushSTACK(*stream_); # Wert für Slot STREAM von STREAM-ERROR
            ...
            fehler(stream_error, GETTEXT("~ from ~: object #Y~ has not the syntax of a compiled closure"));
          }
        skipSTACK(3);
...
LISPFUNN(pathname_reader,3) # liest #"
  { test_no_infix(); # n muß NIL sein
   ...
   {var reg1 object string = # String lesen, der mit " anfängt
    ...
    { var reg2 object *string_;
      pushSTACK(string);
      string_ = &STACK_0;
      dynamic_bind(S(read_pathname_p),T);
      pushSTACK(*string_);
      funcall(L(parse_namestring),1); # (PARSE-NAMESTRING-FOR-READER string)
      dynamic_unbind();
      skipSTACK(1);
    }
   ...
   {var reg2 object recursive_p = popSTACK();
    ...
    if (eq(recursive_p,unbound) || nullp(recursive_p))
      ...
      { var reg4 object* stream_ = &STACK_0;
        var reg5 object* ch_ptr;
        pushSTACK(ch); ch_ptr=&STACK_0;
        # SYS::*READ-REFERENCE-TABLE* an die leere Tabelle NIL binden:
        dynamic_bind(S(read_reference_table),NIL);
        # SYS::*BACKQUOTE-LEVEL* an NIL binden:
        dynamic_bind(S(backquote_level),NIL);
       {var reg1 object obj = read_delimited_list(stream_,*ch_ptr,eof_value); # Liste lesen
        obj = make_references(obj); # Verweise entflechten
        dynamic_unbind();
        dynamic_unbind();
        skipSTACK(1);
   ...
   {var reg1 object ch = STACK_1; # char
    if (!charp(ch)) # muß ein Character sein !
      { pushSTACK(ch); # Wert für Slot DATUM von TYPE-ERROR
        ...
        fehler(type_error, GETTEXT("~: ~ is not a character"));
      }
   ...
   {var reg3 object peek_type = STACK_4;
    if (eq(peek_type,unbound) || nullp(peek_type))
      ...
      { pushSTACK(peek_type); # Wert für Slot DATUM von TYPE-ERROR
        ...
        fehler(type_error, GETTEXT("~: peek type should be NIL or T or a character, not ~"));
      }
    eof: # EOF liegt vor
...
# CLTL S. 380
  { # input-stream überprüfen:
    var reg3 object* stream_ = &STACK_3;
    test_istream(stream_);
   {var reg2 object stream = *stream_;
    ...
    { var reg1 signean status = stream_listen(stream);
      if (status < 0) # EOF ?
        { return_Values eof_handling(); }
     ...
     if (eq(arg,unbound))
       ...
       { if (posfixnump(arg) &&
           ...
           { pushSTACK(arg); # Wert für Slot DATUM von TYPE-ERROR
             ...
             fehler(type_error, GETTEXT("~: ~ argument should be an integer between 2 and 36, not ~"));
    }  }   }
     ...
     {var reg1 uintB c; # letztes gelesenes Character
      ...
      if (!junk_allowed)
        ...
        { pushSTACK(unbound); # "Wert" für Slot STREAM von STREAM-ERROR
          ...
          fehler(stream_error, GETTEXT("~: string ~ hasn't integer syntax"));
        }
...
# kann GC auslösen
  ...
  local void pr_hex6(stream_,obj)
    var reg2 object* stream_;
    var reg3 object obj;
    { var reg1 oint x = (as_oint(obj) >> oint_addr_shift) << addr_shift;
...
# kann GC auslösen
  ...
  local void pr_hex8(stream_,x)
    ...
    var reg1 uintP x;
    { write_schar(stream_,'#'); write_schar(stream_,'x'); # Präfix für "Hexadezimal"
     {var reg2 sintC k = (sizeof(uintP)-1)*8;
...
# fehler_print_readably(obj);
  ...
  local void fehler_print_readably(obj)
    ...
    { var reg1 object *obj_;
      ...
      pushSTACK(*obj_); # Wert für Slot OBJECT von PRINT-NOT-READABLE
      pushSTACK(*obj_);
      ...
      fehler(print_not_readable, GETTEXT("~: Despite of ~, ~ cannot be printed readably."));
    }
...
# fehler_print_case();
  ...
  local void fehler_print_case()
    ...
    { var reg1 object print_case = S(print_case);
      ...
      pushSTACK(S(print));
      set_Symbol_value(print_case,S(Kupcase)); # (setq *PRINT-CASE* ':UPCASE)
      { var const char *msg1,*msg2;
        //: DEUTSCH "~: Der Wert ~ von ~ ist weder ~ noch ~ noch ~."
        //: ENGLISH "~: the value ~ of ~ is neither ~ nor ~ nor ~."  
        //: FRANCAIS "~ : La valeur ~ de ~ n'est ni ~ ni ~ ni ~."
        msg1 = GETTEXT("~: the value ~ of ~ is neither ~ nor ~ nor ~.");
        //: DEUTSCH "Er wird auf ~ gesetzt."
        //: ENGLISH "It is reset to ~."
        //: FRANCAIS "Elle est remplacée par ~."
        msg2 = GETTEXT("It is reset to ~.");
        fehler3(type_error,msg1,NLstring,msg2);
      }
    }
  ...
  local void justify_start(stream_)
    ...
    { if (!(TheStream(*stream_)->strmtype == strmtype_pphelp))
        ...
        { # SYS::*PRIN-JBSTRINGS* an den Inhalt des Streams binden:
          dynamic_bind(S(prin_jbstrings),TheStream(*stream_)->strm_pphelp_strings);
          # SYS::*PRIN-JBMODUS* an den Modus des Streams binden:
          dynamic_bind(S(prin_jbmodus),TheStream(*stream_)->strm_pphelp_modus);
          # SYS::*PRIN-JBLPOS* an die Line-Position des Streams binden:
          dynamic_bind(S(prin_jblpos),TheStream(*stream_)->strm_pphelp_lpos);
...
# kann GC auslösen
  ...
  local void justify_empty_2(stream_)
    ...
    { var reg1 object stream = *stream_;
      ...
      set_Symbol_value(S(prin_jblocks),new_cons);
    }
...
#     und 1 mal LEVEL_END am Ende.
  #define LEVEL_CHECK(cnt)  { if (level_check(stream_)) { skipSTACK(cnt); return; } }
  #define LEVEL_END    level_end(stream_)
  #define LEVEL_CHECK1(item1) \
      { var auto object *item1_ptr;           \
        pushSTACK(item1); item1_ptr=&STACK_0; \
        LEVEL_CHECK(1);                       \
        item1=*item1_ptr;                     \
      }
  #define LEVEL_CHECK3(item1,item2,item3) \
      { var auto object *item1_ptr;           \
        var auto object *item2_ptr;           \
        var auto object *item3_ptr;           \
        pushSTACK(item1); item1_ptr=&STACK_0; \
        pushSTACK(item2); item2_ptr=&STACK_0; \
        pushSTACK(item3); item3_ptr=&STACK_0; \
        LEVEL_CHECK(3);                       \
        item1=*item1_ptr;                     \
        item2=*item2_ptr;                     \
        item3=*item3_ptr;                     \
      }
  #define LEVEL_END1 { LEVEL_END; skipSTACK(1); }
  #define LEVEL_END3 { LEVEL_END; skipSTACK(3); }
...
# pr_circle_(&stream,obj_,&pr_xxx);
# > obj: Objekt
# > pr_xxx: Ausgabe-Routine, die &stream und obj übergeben bekommt
# > stream: Stream
# < stream: Stream
# kann GC auslösen
  local void pr_circle_ (object* stream_, object *obj_, pr_routine* pr_xxx);
  local void pr_circle_(stream_,obj_,pr_xxx)
    var reg1 object* stream_;
    var reg3 object *obj_;
    var reg4 pr_routine* pr_xxx;
    { # Feststellen, ob Zirkularität:
      var reg2 circle_info* info;
      info = circle_p(*obj_);
...
# Nun kommen die einzelnen pr_xxx-Routinen:
  ...
  local_function pr_routine pr_instance;

# UP: Gibt ein Objekt auf einen Stream aus.
# prin_object_(&stream,obj_);
# > obj_: &Objekt
# > stream: Stream
# < stream: Stream
# kann GC auslösen
  local void prin_object_ (object *stream_,object *obj_);
  local void prin_object_(stream_,obj_)
    var reg2 object* stream_;
    var reg1 object* obj_;
    { restart_it:
      ...
      pr_circle_(stream_,obj_,&prin_object_dispatch);
    }
...
# < stream: Stream
# kann GC auslösen
  local void prin_object (object *stream_,object obj);
  local void prin_object(stream_,obj)
    var reg2 object* stream_;
    var reg1 object obj;
    {
      pushSTACK(obj);
      prin_object_(stream_,&STACK_0);
      skipSTACK(1);
    }

  local void prin_object_dispatch (object *stream_,object obj);
...
# < stream: Stream
# kann GC auslösen
  local void pr_symbol (object *stream_,object sym);
...
# < stream: Stream
# kann GC auslösen
  local void pr_symbol_part (object *stream_,object string);
  local void pr_symbol_part(stream_,string)
    var reg7 object* stream_;
    var reg8 object string;
    { # Feststellen, ob der Name ohne |...| außenrum ausgegeben werden kann:
      ...
      { # Brauche die Attributcodetabelle und die aktuelle Syntaxcodetabelle:
        ...
        var reg6 uintW rtcase; # readtable-case
        { var reg1 object readtable;
          get_readtable(readtable = ); # aktuelle Readtable
...
# < stream: Stream
# kann GC auslösen
  local void pr_like_symbol (object *stream_,object string);
...
# < stream: Stream
# kann GC auslösen
  local void pr_character (object *stream_,object ch);
    ...
    { # *PRINT-ESCAPE* abfragen:
      if (test_value(S(print_escape)) || test_value(S(print_readably)))
        ...
        { var reg2 uintL index = start;
          ...
          { var reg5 uintL index0 = index;
            loop
              { # Suche den nächsten #\" oder #\\ :
                string = STACK_0;
                while (len > 0)
                  { var reg1 uintB c = TheSstring(string)->data[index];
                    if ((c=='"') || (c=='\\')) break;
                    index++; len--;
                  }
                if (!(index==index0))
...
# < stream: Stream
# kann GC auslösen
  local void pr_string (object *stream_,object string);
...
# < ergebnis: Fixnum > 0
  ...
  local uintL get_indent_lists()
    { var reg1 object obj = Symbol_value(S(print_indent_lists));
      if (posfixnump(obj))
        { var reg2 uintL indent = posfixnum_to_L(obj);
          if (indent > 0)
            { return indent; }
        }
      # Defaultwert ist 1.
...
# < stream: Stream
# kann GC auslösen
  local void pr_list (object *stream_,object list);
...
# < stream: Stream
# kann GC auslösen
  local void pr_cons (object *stream_,object list);
...
# (unquote form)                               ,form
  ...
  local void pr_list_quote (object *stream_,object list);
  local void pr_list_quote(stream_,list) # list = (QUOTE object)
    var reg2 object* stream_;
    var reg1 object list;
    { var reg3 object *list_;
      pushSTACK(Car(Cdr(list))); # (second list) retten
      write_schar(stream_,'\''); # "'" ausgeben
      list_ = &STACK_0;
      INDENT_START(1); # um 1 Zeichen einrücken wegen "'"
      prin_object_(stream_,list_); # object ausgeben
      INDENT_END;
      skipSTACK(1);
    }

  local void pr_list_function (object *stream_,object list);
  ...
  local void pr_list_backquote (object *stream_,object list);
  local void pr_list_backquote(stream_,list) # list = (BACKQUOTE original-form [expanded-form])
    ...
    { var reg3 object *list_;
      ...
      INDENT_START(1); # um 1 Zeichen einrücken wegen '`'
      prin_object_(stream_,list_); # original-form ausgeben
      INDENT_END;
      dynamic_unbind();
      skipSTACK(1);
    }
  ...
  local void pr_list_bothsplice(stream_,list,ch)
    ...
    { var reg4 object *list_;
      ...
      if (consp(*list_) && eq(Car(*list_),S(unquote))
          && mconsp(Cdr(*list_)) && nullp(Cdr(Cdr(*list_)))
         )
        # ja -> noch die Form ausgeben:
        { INDENT_START(2); # um 2 Zeichen einrücken wegen ",@" bzw. ",."
          prin_object(stream_,Car(Cdr(*list_))); # Form ausgeben
          INDENT_END;
        }
        else
  ...
  local void pr_list_splice (object *stream_,object list);
  local void pr_list_splice(stream_,list) # list = (SPLICE object)
    var reg2 object* stream_;
    var reg1 object list;
    { pr_list_bothsplice(stream_,list,code_char('@')); }

  local void pr_list_nsplice (object *stream_,object list);
  local void pr_list_nsplice(stream_,list) # list = (NSPLICE object)
    var reg2 object* stream_;
    var reg1 object list;
    { pr_list_bothsplice(stream_,list,code_char('.')); }

  local void pr_list_unquote (object *stream_,object list);
...
# < stream: Stream
# kann GC auslösen
  local void pr_real_number (object *stream,object number);
...
# < stream: Stream
# kann GC auslösen
  local void pr_number (object *stream_,object number);
...
# < stream: Stream
# kann GC auslösen
  local void pr_array_nil (object *stream,object obj);
...
# < stream: Stream
# kann GC auslösen
  local void pr_bvector (object *stream_,object bv);
...
# < stream: Stream
# kann GC auslösen
  local void pr_vector (object *stream,object v);
  local void pr_vector(stream_,v)
    ...
    { # *PRINT-ARRAY* abfragen:
      if (test_value(S(print_array)) || test_value(S(print_readably)))
        ...
        { LEVEL_CHECK1(v);
          { var reg8 boolean readable = # Flag, ob Länge und Typ mit ausgegeben werden
...
# Zwei UPs zur Ausgabe eines Teil-Arrays:
  ...
  local void pr_array_elt_t (object *stream_,object obj,pr_array_info *info);
  ...
  local void pr_array_elt_bvector (object *stream_,object obj,pr_array_info *info);
  ...
  local void pr_array_elt_string (object *stream_,object obj,pr_array_info *info);
...
# < stream: Stream
# kann GC auslösen
  local void pr_array (object *stream_,object obj);
      ...
      if (test_value(S(print_array)) || test_value(S(print_readably)))
        ...
        {   LEVEL_CHECK1(obj);
         {  # Rang bestimmen und Dimensionen und Teilprodukte holen:
            ...
            var DYNAMIC_ARRAY(reg7,dims_sizes,array_dim_size,r); # dynamisch allozierter Array
...
# kann GC auslösen
  ...
  local void pr_structure_external(stream_,structure,function)
    var reg3 object* stream_;
    var reg4 object structure;
    var reg5 object function;
    { var reg1 uintC count = pr_external_1(*stream_); # Bindungen erstellen
      # (funcall fun Structure Stream SYS::*PRIN-LEVEL*) :
      pushSTACK(structure); # Structure als 1. Argument
      pushSTACK(*stream_); # Stream als 2. Argument
...
# < stream: Stream
# kann GC auslösen
  local void pr_structure (object *stream_,object structure);
  local void pr_structure(stream_,structure)
    var reg3 object* stream_;
    var reg10 object structure;
    { LEVEL_CHECK1(structure);
      # Typ der Structure bestimmen (vgl. TYPE-OF):
      { var reg10 object name = Car(TheStructure(structure)->structure_types);
        ...
        if (!eq(fun,unbound))
          ...
          { structure = STACK_1;
            ...
            if ((!test_value(S(print_circle)))
              ...
              { var reg1 object *fun_;
                var reg2 object *structure_;
                pushSTACK(fun); fun_=&STACK_0;
                pushSTACK(structure); structure_=&STACK_0;
                dynamic_bind(S(print_circle_table),unbound);
                pr_structure_external(stream_,*structure_,*fun_);
                dynamic_unbind();
                skipSTACK(2);
              }
              else
...
# < stream: Stream
# kann GC auslösen
  local void pr_machine (object *stream_,object obj);
...
# < stream: Stream
# kann GC auslösen
  local void pr_system (object* stream_,object obj);
...
# kann GC auslösen
  ...
  local void pr_record_ab(stream_,obj_,index,length)
    ...
    { var reg5 uintL len = Record_length(*obj_); # Länge des Record
      var reg6 uintL length_limit = get_print_length(); # *PRINT-LENGTH*-Begrenzung
      loop
...
# kann GC auslösen
  ...
  local void pr_record_descr(stream_,obj,name,readable,slotlist)
    ...
    { LEVEL_CHECK3(obj,name,slotlist);
      pushSTACK(obj);
      pushSTACK(name);
      pushSTACK(slotlist);
...
# < stream: Stream
# kann GC auslösen
  local void pr_orecord (object *stream_,object obj);
        ...
        { case Rectype_Hashtable:
          ...
          case Rectype_Loadtimeeval:
            ...
            { var reg1 object* obj_; 
              ...
              write_schar(stream_,'#'); write_schar(stream_,'.');
              INDENT_START(2); # um 2 Zeichen einrücken, wegen '#.'
              prin_object_(stream_,obj_); # form ausgeben
              INDENT_END;
              skipSTACK(1);
            } break;
          ...
          case Rectype_Fpointer:
            ...
            { if (test_value(S(print_readably))) { fehler_print_readably(obj); }
              LEVEL_CHECK1(obj);
             {var reg6 boolean validp = fp_validp(TheFpointer(obj));
              var reg1 uintP val = (uintP)(TheFpointer(obj)->fp_pointer); # Wert holen
              write_schar(stream_,'#'); write_schar(stream_,'<');
              INDENT_START(2); # um 2 Zeichen einrücken, wegen '#<'
              JUSTIFY_START;
              if (!validp)
          ...
          case Rectype_Socket_Server:
            ...
            { if (test_value(S(print_readably))) { fehler_print_readably(obj); }
              LEVEL_CHECK1(obj);
              pushSTACK(obj); # Socket-Server retten
             {var reg1 object* obj_ = &STACK_0; # und merken, wo es sitzt
              write_schar(stream_,'#'); write_schar(stream_,'<');
              INDENT_START(2); # um 2 Zeichen einrücken, wegen '#<'
              JUSTIFY_START;
              write_sstring_case(stream_,O(printstring_socket_server));
              {var reg5 uintL length_limit = get_print_length(); # *PRINT-LENGTH*-Begrenzung
               var reg4 uintL length = 0; # bisherige Länge := 0
               # auf Erreichen von *PRINT-LENGTH* prüfen:
               if (length >= length_limit) goto socket_server_end;
               JUSTIFY_SPACE; # Space ausgeben
               prin_object(stream_,TheSocketServer(*obj_)->port);
               length++; # bisherige Länge erhöhen
              }
              socket_server_end:
              JUSTIFY_END_ENG;
          ...
          default:
            ...
            fehler(serious_condition, GETTEXT("~: an unknown record type has been generated!"));
    }   }
...
# < stream: Stream
# kann GC auslösen
  local void pr_subr (object *stream_,object obj);
...
# < stream: Stream
# kann GC auslösen
  local void pr_fsubr (object *stream_,object obj);
...
# < stream: Stream
# kann GC auslösen
  local void pr_closure (object *stream_,object obj);
...
# < stream: Stream
# kann GC auslösen
  local void pr_cclosure (object *stream_,object obj);
    ...
    { # *PRINT-CLOSURE* abfragen:
      if (test_value(S(print_closure)) || test_value(S(print_readably)))
        ...
        { pr_other_obj(stream_,TheClosure(obj)->clos_name,
                       (TheSbvector(TheClosure(obj)->clos_codevec)->data[CCHD+4] & bit(4) # generische Funktion?
...
# < stream: Stream
# kann GC auslösen
  local void pr_cclosure_lang (object *stream_,object obj);
  local void pr_cclosure_lang(stream_,obj)
    ...
    { LEVEL_CHECK1(obj);
     ...
     {var reg1 object* obj_ = &STACK_0; # und merken, wo sie sitzt
      ...
      pr_circle_(stream_,&TheClosure(*obj_)->clos_codevec,&pr_cclosure_codevector);
...
# < stream: Stream
# kann GC auslösen
  local void pr_cclosure_codevector (object *stream_,object codevec);
  local void pr_cclosure_codevector(stream_,codevec)
    var reg2 object* stream_;
    var reg6 object codevec;
    { LEVEL_CHECK1(codevec);
...
# < stream: Stream
# kann GC auslösen
  local void pr_stream (object *stream_,object obj);
        ...
        switch (type)
          { case strmtype_sch_file:
            ...
            case strmtype_xsocket:
              # Socket-Stream
              JUSTIFY_SPACE;
              prin_object(stream_,TheStream(*obj_)->strm_xsocket_connect); # Verbindungsziel ausgeben
              break;
            #endif
            ...
            case strmtype_socket:
              ...
              { var object host=TheStream(*obj_)->strm_socket_host;
                if (!nullp(host))
                  prin_object(stream_,host);
                write_schar(stream_,':');
                prin_object(stream_,TheStream(*obj_)->strm_socket_port);
              }
              break;
            #endif
            default:
...
# < stream: Stream
# kann GC auslösen
  local void pr_instance (object *stream_,object obj);
...
# kann GC auslösen
  ...
  local void prin1a (object* stream_, object* obj_);
  local void prin1a(stream_,obj_)
    var reg2 object* stream_;
    var reg3 object* obj_;
  ...
  local void prin1b (object* stream_, object *obj_);
  local void prin1b(stream_,obj_)
    var reg3 object* stream_;
    var reg2 object* obj_;
         ...
         {var reg1 object circularities = # Zirkularitätentabelle
          ...
          if (nullp(circularities))
            ...
            { # Überlauf der GET_CIRCULARITIES-Routine behandeln:
              ...
              fehler(storage_condition, GETTEXT("~: not enough stack space for carrying out circularity analysis"));
            }
          else
  ...
  # UP: dasselbe mit Behandlung von *PRINT-CIRCLE* und *PRINT-PRETTY*
  # und SYS::*PRIN-STREAM* :
  local void prin1_ (object* stream_, object* obj_);
  local void prin1_(stream_,obj_)
    var reg1 object* stream_;
    var reg2 object* obj_;
...
# kann GC auslösen
  ...
  global void print(stream_,obj)
    ...
    { pushSTACK(obj); # Objekt retten
      ...
      prin1_(stream_,&STACK_0); # Objekt ausgeben
      skipSTACK(1);
    }
...
# > STACK_0: Stream
  ...
  local void write_up()
    { var reg2 object* argptr = args_end_pointer STACKop (1+print_vars_anz+1); # Pointer über die Keyword-Argumente
      var reg5 object* obj_ = &NEXT(argptr);
      ...
      {var reg1 object sym = first_print_var; # durchläuft die Symbole
       var reg3 uintC count;
       # sym won't move, because these are constant symbols
      ...
      {var reg1 object* stream_ = &NEXT(argptr); # nächstes Argument ist der Stream
       prin1_(stream_,obj_); # Objekt ausgeben
      }
      # Bindungen auflösen:
...
# vgl. CLtL2 S. 580
  { var reg2 boolean flag_fun = FALSE;
    var reg4 boolean flag_type = FALSE;
    var reg3 boolean flag_id = FALSE;
    { var reg1 object arg = popSTACK(); # :identity - Argument
      if (!(eq(arg,unbound) || nullp(arg))) { flag_id = TRUE; }
    }
    { var reg1 object arg = popSTACK(); # :type - Argument

...
#	     if $Verbose && $self->needs_linking();
	...
	if (defined $self->{PERL_SRC} and $self->{INSTALLDIRS} eq "perl") {
	    ...
	    $self->{INST_LIB} = $self->catdir("..","../..","lib");
	}
    }
    $self->{INST_ARCHLIB} ||= $self->catdir(".","../..","arch");
    $self->{INST_EXE} ||= $self->catdir('.','../..','bin');
	...
	if ($self->{INSTALLMAN1DIR} =~ /^(none|\s*)$/){
	    ...
	    $self->{INST_MAN1DIR} = $self->catdir('.','../..','man1');
	}
    }
	...
	if ($self->{INSTALLMAN3DIR} =~ /^(none|\s*)$/){
	    ...
	    $self->{INST_MAN3DIR} = $self->catdir('.','../..','man3');
	}
    }
...
');
    ...
    push(@otherfiles, qw[./../.. $(MAKE_APERL_FILE) $(INST_ARCHAUTODIR)/extralibs.all

...
with Special_Chars_App; use Special_Chars_App;

procedure Special_Chars is
...
Method_Result := DoMethodA(WI_SpecialChars,temp_Msg);

ClearTagList(temp_TagList);
AddTag(temp_TagList, MUIA_Window_Open, True);

SetAttrs_Result := SetAttrsA(WI_SpecialChars,temp_TagList);

...
with DviPrintGui_App; use DviPrintGui_App;

procedure DviPrintGui is
package Int_IO is new Integer_IO(Integer); use Int_IO;

running : Boolean := True;
Window_Opened : Integer;
...
Method_Result := DoMethodA(WI_dviprint,temp_Msg);

ClearTagList(temp_TagList);
AddTag(temp_TagList, MUIA_Window_Open, True);

SetAttrs_Result := SetAttrsA(WI_dviprint,temp_TagList);

SetAttrs_Result := GetAttr(MUIA_Window_Open,WI_dviprint,Window_Opened'Address);

if Window_Opened /= 0 then
   while running loop
           ClearMsg(temp_Msg);
   ...
   Put("Failed to open main window!!, MUI_Error #");
   Put(MUI_Error);
   New_Line;
end if;

Dispose_DviPrintGui_App;

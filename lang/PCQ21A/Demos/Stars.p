PROGRAM Sterne;

{$I "Include:Intuition/Intuition.i"}
{$I "Include:Graphics/GfxBase.i"}
{$I "Include:Graphics/Pens.i"}
{$I "Include:Graphics/DisplayInfo.i"}
{$I "Include:Utils/Random.i"}

CONST   MAX_STERNE = 42;
        MAX_GESCHW = 15;
        PI         = 3.14527;

TYPE    Star = Record
                 x,y :Integer;
                 sin :Real;
                 cos :Real;
                 d   :Integer;
                 v   :Integer;
               End;

VAR     Scr     :ScreenPtr;
        Win     :WindowPtr;
        Msg     :IntuiMessagePtr;
        Ende    :Boolean;
        Stars   :Array[1..MAX_STERNE] of Star;
        factor  :Real;
        col     :Short;
        dum     :Integer;



PROCEDURE newStern(num :Short);

BEGIN
  col:=RangeRandom(360);
  Stars[num].x := Scr^.Width shr 1;
  Stars[num].y := Scr^.Height shr 1;
  Stars[num].sin := sin(col*factor);
  Stars[num].cos := cos(col*factor);
  Stars[num].d := 0;
  Stars[num].v := RangeRandom(MAX_GESCHW)+2;
END;


PROCEDURE moveStern(num :Short);

BEGIN
  Stars[num].d:=Stars[num].d+Stars[num].v;
  Stars[num].x:=Round(Stars[num].d*Stars[num].sin)+Scr^.Width shr 1;
  Stars[num].y:=Round(Stars[num].d*Stars[num].cos)+Scr^.Height shr 1;
  {Inc(Stars[num].v);}
END;


PROCEDURE drawSterne;

BEGIN
  For dum:=1 to MAX_STERNE Do Begin
    If Stars[dum].v=0 Then Begin
      If RangeRandom(10)>4 Then
        newStern(dum);
    End Else If Stars[dum].d>Scr^.Width shr 1 Then Begin
      SetAPen(Win^.RPort,0);
      If WritePixel(Win^.RPort,Stars[dum].x,Stars[dum].y)=0 Then;
      Stars[dum].v:=0;
    End Else Begin
      SetAPen(Win^.RPort,0);
      If WritePixel(Win^.RPort,Stars[dum].x,Stars[dum].y)=0 Then;
      moveStern(dum);
      col:=(Stars[dum].d shl 5) Div Scr^.Height shr 1;
      If col>7 Then
        col:=7;
      SetAPen(Win^.RPort,col);
      If WritePixel(Win^.RPort,Stars[dum].x,Stars[dum].y)=0 Then;
    End;
  End;
END;


PROCEDURE initSterne;

BEGIN
  For dum:=1 to MAX_STERNE Do
    Stars[dum].x := Scr^.Width shr 1;
    Stars[dum].y := Scr^.Height shr 1;
    Stars[dum].sin := 0.0;
    Stars[dum].cos := 0.0;
    Stars[dum].d := 0;
    Stars[dum].v := 0;
  factor:=PI/180;
END;


PROCEDURE CleanUp(str:string; code : Integer);

BEGIN
  If Win<>Nil Then
    CloseWindow(Win);
  If (Scr<>Nil) then CloseScreen(Scr);
  Exit(code);
END;


PROCEDURE Init;

BEGIN
  Scr:=Nil;  Win:=Nil;
  Scr := OpenScreenTags(NIL,
                        SA_Depth,     3,
                        SA_DisplayID, HIRES_KEY,
                        TAG_END);
  If Scr=Nil Then CleanUp("No screen",20);

  Win:=OpenWindowTags(Nil,
                          WA_Flags, WFLG_BORDERLESS,
                          WA_IDCMP, IDCMP_MOUSEBUTTONS,
                          WA_CustomScreen, Scr,
                          TAG_DONE);
  If Win=Nil Then CleanUp("No window",20);

  initSterne;
  SetRGB4(@Scr^.SViewPort, 0, $0,$0,$0);
  SetRGB4(@Scr^.SViewPort, 1, $3,$3,$3);
  SetRGB4(@Scr^.SViewPort, 2, $6,$6,$6);
  SetRGB4(@Scr^.SViewPort, 3, $b,$b,$b);
  SetRGB4(@Scr^.SViewPort, 4, $c,$c,$c);
  SetRGB4(@Scr^.SViewPort, 5, $d,$d,$d);
  SetRGB4(@Scr^.SViewPort, 6, $e,$e,$e);
  SetRGB4(@Scr^.SViewPort, 7, $f,$f,$f);
END;



BEGIN
  Init;
  Ende:=false;

  Repeat
    drawSterne;
    Msg:=IntuiMessagePtr(GetMsg(Win^.UserPort));
    If Msg<>Nil Then Begin
      ReplyMsg(Address(Msg));
      Ende:=true;
    End;
  Until Ende;
  CleanUp(NIL,0);
END.


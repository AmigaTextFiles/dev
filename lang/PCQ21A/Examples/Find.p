...
{
    Removed procedure UpCase, will use UpperCase in pcq.lib.
    Also changed from SameName to Match. SameName is removed from
    pcq.lib. Match can handle all wildcards.
    11 May 1998.
...
begin
    ...
    if Stay then begin
        ...
        if fib_DirEntryType < 0 then begin { file }
            ...
            if Match(TestName, FileName) then
            Display(FB);

...
feature    
   ...
   make(sp: like start_position) is
      do
	 !!base_class_name.make(us_string,sp);
      end;
...
feature
   ...
   smallest_ancestor(other: TYPE): TYPE is
	 ...
	 elseif rto.is_expanded then
	    ...
	    if rto_bc.is_subclass_of(base_class) then
	       Result := Current;
	    else
	       Result := rto.smallest_ancestor(Current);
	    end;
...
feature {NONE}
   ...
   check_type is
	 -- Do some checking for type STRING to be runnable.
      local
	 bc: BASE_CLASS;
	 rc: RUN_CLASS;
	 rf: RUN_FEATURE;
	 ...
	 if nb_errors = 0 then
	    if bc.is_expanded then
	       error(start_position,"STRING must not be expanded.");
	    end;
	 end;
	 rc := run_class;
	 rf := rc.get_feature_with(us_capacity);
	 rf := rc.get_feature_with(us_count);

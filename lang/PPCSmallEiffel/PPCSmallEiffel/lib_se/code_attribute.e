...
feature {METHOD_INFO}
   ...
   store_in(storage: STRING) is
	 ...
	 i: INTEGER;
      do
	 append_u2(storage,constant_pool.idx_uft8_code);
	 check
	    program_counter > 0
	 end;
...
feature -- opcode feature list :
   ...
   opcode_invokevirtual(methodref_idx, stack_inc: INTEGER) is
      require
	 constant_pool.valid_index(methodref_idx)
      do
   ...
   opcode_instanceof(class_idx: INTEGER) is
      require
	 constant_pool.valid_index(class_idx)
      do
	 opcode(193,0);
	 add_u2(class_idx);
      end;
...
feature -- High level opcode calls :
   ...
   opcode_java_string2bytes_array is
	 ...
	 idx: INTEGER;
      do
      	 idx := constant_pool.idx_methodref3(fz_32,fz_33,fz_34);
   ...
   opcode_bytes_array2eiffel_string is
	 ...
	 opcode_astore(loc);
	 -- The new STRING :
	 opcode_new(cp.idx_eiffel_string_class);
	 -- Set count :
	 opcode_dup;
	 opcode_aload(loc);
	 opcode_arraylength;
...
feature -- Easy access to some Java basics :
   ...
   opcode_system_exit is
	 -- Assume the status code is already pushed.
      local
	 idx: INTEGER;
      do
	 idx := constant_pool.idx_methodref3(fz_36,fz_exit,fz_27);
   ...
   opcode_system_in is
	 ...
	 idx: INTEGER;
      do
	 idx := constant_pool.idx_fieldref3(fz_36,fz_37,fz_38);
   ...
   opcode_system_out is
	 ...
	 idx: INTEGER;
      do
	 idx := constant_pool.idx_fieldref3(fz_36,fz_39,fz_40);
	 opcode_getstatic(idx,1);
      end;
   ...
   opcode_system_err is
	 ...
	 idx: INTEGER;
      do
	 idx := constant_pool.idx_fieldref3(fz_36,fz_49,fz_40);
	 opcode_getstatic(idx,1);
      end;
   ...
   opcode_runtime_get_runtime is
      local
	 idx: INTEGER;
      do
	 idx := constant_pool.idx_methodref3(fz_53,fz_55,fz_56);
	 opcode_invokestatic(idx,1);
      end;

   opcode_runtime_trace_instructions(flag: BOOLEAN) is
      local
	 idx: INTEGER;
      do
	 opcode_runtime_get_runtime;
	 if flag then
	    opcode_iconst_1;
	 else
	    opcode_iconst_0;
	 end;
	 idx := constant_pool.idx_methodref3(fz_53,fz_52,fz_54);
	 opcode_invokevirtual(idx,-2);
      end;
...
feature {NONE}
   ...
   opcode(opcode_value, max_stack_increment: INTEGER) is
	 ...
	 opcode_value <= 255
      do
	 add_u1(opcode_value);
	 current_stack_level := current_stack_level + max_stack_increment;
	 if current_stack_level > max_stack then
	    max_stack := current_stack_level;
	 end;
      ensure
	 current_stack_level >= 0
      end;
...
feature {NONE}
   ...
   opcode_string2double(str: STRING) is
	 ...
	 opcode_ldc(idx);
	 idx := constant_pool.idx_methodref3(fz_62,fz_60,fz_61);
	 opcode_invokestatic(idx,0);
      end;
...
feature {NONE}
   ...
   push_strange_integer(i: INTEGER) is
	 ...
	 opcode_ldc(idx);
	 idx := cp.idx_methodref3(fz_81,fz_82,fz_83);
	 opcode_invokestatic(idx,0);
      end;

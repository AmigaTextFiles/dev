...
creation {TYPE_EXPANDED} from_other
...
feature
   ...
   jvm_method_flags: INTEGER is 
      do
	 if run_class.writable_attributes = Void then
	    Result := 9;
	 else
	    Result := 17;
	 end;
   ...
   jvm_push_default: INTEGER is
	 ...
	 rc: like run_class;
      do
	 rc := run_class;
	 if rc.current_type.is_basic_eiffel_expanded then
	    Result := rc.current_type.jvm_push_default;
	 else
   ...
   jvm_initialize_local(offset: INTEGER) is
	 ...
	 rc := run_class;
	 if rc.current_type.is_basic_eiffel_expanded then
	    rc.current_type.jvm_initialize_local(offset);
	 else
   ...
   jvm_write_local(offset: INTEGER) is
	 ...
	 rc := run_class;
	 if rc.current_type.is_basic_eiffel_expanded then
	    rc.current_type.jvm_write_local(offset);
	 else
...
feature {NONE}
   ...
   compute_mark(str: STRING): STRING is
      do
	 tmp_written_mark.copy(fz_expanded);
	 tmp_written_mark.extend(' ');
	 tmp_written_mark.append(str);
	 Result := unique_string.item(tmp_written_mark);
      end;

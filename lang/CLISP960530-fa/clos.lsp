...
(:import-from "SYSTEM"
  ...
  sys::block-name                                    ; in init.lsp definiert
...
; clos:generic-flet clos:generic-labels              ; in compiler.lsp behandelt
  ;; Export:
; clos::class    ; als Property in predtype.d und type.lsp, compiler.lsp benutzt
...
; Eine Instanz für ein Objekt der Metaklasse <standard-class> besorgen:
(defmacro std-allocate-instance (class)
  `(allocate-std-instance ,class (class-instance-slot-count ,class))
)
...
(defun (setf find-class) (new-value symbol &optional errorp environment)
  ...
  (setf (get symbol 'CLASS) new-value)
)
...
(eval-when (load eval)
  (do-all-symbols (s) (remprop s 'CLASS))
)

(defconstant empty-ht (make-hash-table :test #'eq :size 0))

(defstruct (class (:predicate nil) (:print-function print-class))
...
(defstruct (built-in-class (:inherit class) (:conc-name "CLASS-") (:print-function print-class))
)
(proclaim '(notinline built-in-class-p))

(defstruct (structure-class (:inherit class) (:conc-name "CLASS-") (:print-function print-class))
)

(defstruct (standard-class (:inherit class) (:conc-name "CLASS-") (:print-function print-class))
  direct-slots             ; Liste der neu hinzugekommenen Slots (als Plisten)
  slots                    ; Liste aller Slots (als Slot-Definitionen)
  instance-slot-count      ; Anzahl der Slots der direkten Instanzen + 1
  shared-slots             ; Simple-Vector mit den Werten aller Shared Slots
                         ...
                         (:DEFAULT-INITARGS
                          (let ((list (rest option)))
                            (when (and (consp list) (null (cdr list)) (listp (car list)))
                              ...
                              (warn 
                                    #L{
                                    DEUTSCH "~S ~S: Option ~S sollte als ~S geschrieben werden."
...
(defstruct (slot-definition
            ...
            (:constructor make-slot-definition (name allocation initargs location initer)))
...
(defun make-slotdef (&key name (allocation ':instance) (initargs '()) location (initer nil) (initform nil) (accessors '()) (readers '()) (writers '()) type documentation)
  ...
  (make-slot-definition name allocation initargs location initer)
)
  ...
  (let ((class (find-class name nil)))
    (if class
      ...
      (if (and (eq metaclass <standard-class>)
        ...
        (error-of-type 'error
          #L{
          DEUTSCH "~S: Klasse ~S kann nicht umdefiniert werden."
          ENGLISH "~S: Cannot redefine class ~S"
          FRANCAIS "~S : La classe ~S ne peut pas être redéfinie."
          }
          'defclass name
      ) )
      (progn
...
(defun make-instance-standard-class
  ...
  (let ((class (make-standard-class :classname name :metaclass metaclass)))
    ...
    (let ((ht (make-hash-table :test #'eq))
          (local-index 1) ; Index 0 wird von der Klasse belegt
          (shared-index 0))
      (mapc #'(lambda (slot)
      ...
      (setf (class-slot-location-table class) ht)
      (setf (class-instance-slot-count class) local-index)
  ...
  (let* ((superclasses ; Liste aller Oberklassen in irgendeiner Reihenfolge
    ...
    (mapc #'(lambda (D)
              (unless ; Ist (class-precedence-list D) Teil-Liste von L ?
                ...
                (warn 
                      #L{
                      DEUTSCH "(class-precedence-list ~S) und (class-precedence-list ~S) sind nicht verträglich."
...
(defun make-instance-structure-class
       (metaclass &key name (direct-superclasses '()) (slots '())
                  &allow-other-keys
       )
  ...
  (let ((class (make-structure-class :classname name :metaclass metaclass)))
    ...
    (setf (class-precedence-list class)
          (std-compute-cpl class
                           (add-default-superclass direct-superclasses <t>)
    )     )
    (setf (class-all-superclasses class)
          (std-compute-superclasses (class-precedence-list class))
    )
    (setf (class-slot-location-table class)
          (make-hash-table :test #'eq :initial-contents slots)
    )
    class
) )
        ...
        (setf (find-class name)
              (make-instance-structure-class <structure-class>
                ...
                :slots
                  (mapcan #'(lambda (slot)
                              (if (first slot)
                                (list (cons (first slot) (second slot)))
                            ) )
                          (svref descr 3)
                  )
) ) ) ) )     )
...
(macrolet ((def (&rest classes &aux (new (car (last classes))))
 ...
 ;(def <t>)
  (def <t> <character>)
  (def <t> <function>)
  (def     <function> <standard-generic-function>)
  (def <t> <hash-table>)
  (def <t> <package>)
  (def <t> <pathname>)
  #+LOGICAL-PATHNAMES
...
(defun bc-p (class)
  (or (built-in-class-p class) (eq class <standard-object>))
)
...
(defstruct (standard-method (:conc-name "STD-METHOD-") (:print-function print-std-method))
  ...
  gf                     ; die generische Funktion, zu der diese Methode
                         ; gehört (nur für den Bedarf von NO-NEXT-METHOD)
  initfunction           ; liefert, wenn aufgerufen, die Funktion
                         ; (nur für den Bedarf von ADD-METHOD)
)

; Bei NO-NEXT-METHOD muß die generische Funktion bekannt sein. Da allerdings
; im Prinzip Methoden nicht bestimmten generischen Funktionen zugehörig sind
; (wegen ADD-METHOD), müssen wir die Methode bei ADD-METHOD kopieren. Die
; Identität zweier Kopien derselben Methode stellen wir durch Blick auf
; std-method-initfunction fest. (Man könnte stattdessen auch die generische
; Funktion bei jedem Aufruf mitgeben, als erstes Argument an die effektive
; Methode, aber das ist sicher ineffizienter.)

(defun print-std-method (method stream depth)
  (declare (ignore depth))
    ...
    (loop
      (when (atom description)
        (error-of-type 'program-error
          ...
          FRANCAIS "~S ~S : la liste lambda manque."
          }
          caller funname
      ) )
      ...
      (let ((req-vars '())
        ...
        (do ()
          ...
          (let* ((item (pop specialized-lambda-list))
            ...
            (push (if (and (consp specializer-name)
                     ...
                     (let ((lambdabody-part1
                           ...
                           (lambdabody-part2
                             (if (eq caller 'generic-function)
                               body-rest
                               ; impliziter Block
                               `((BLOCK ,(block-name funname) ,@body-rest))
                          )) )
...
;                    sys::echo-stream-p sys::string-stream-p stringp symbolp
;                    vectorp
...
(let ((prototype-table (make-hash-table :test #'equal)))
                              ...
                              (proto-gf
                                (eval `(LET ((GF 'MAGIC))
                                         ...
                                         (%GENERIC-FUNCTION-LAMBDA (,@reqvars ,@(if restp '(&REST ARGS) '()))
                                           (DECLARE (INLINE APPLY))
                                           (APPLY 'INITIAL-FUNCALL-GF GF ,@reqvars ,(if restp `ARGS `'NIL))
                                       ) )
                             )) )
  ...
  (defun warn-if-gf-already-called (gf)
    (unless (gf-never-called-p gf)
      (warn 
       #L{
       DEUTSCH "Die generische Funktion ~S wird modifiziert, wurde aber bereits aufgerufen."
...
(defun initial-funcall-gf (gf &rest args)
  (install-dispatch gf)
  (apply gf args)
)
                ...
                `(LET ,ht-key-binding
                   (RETURN-FROM ,block-name
                     (,apply-fun
                      (OR ,em-expr
                          (,@setf-em-expr
                                (,apply-fun 'COMPUTE-EFFECTIVE-METHOD ',gf nil
                                            ,@apply-args
                      )   )     )
                      ,@apply-args
                 ) ) )
...
(defun compute-effective-method (gf probe &rest args)
  (let* ((signature (gf-signature gf))
         (req-anz (first signature))
         (req-vars (n-gensyms req-anz))
    ...
    (when (null methods)
      ...
      (if probe
          (error-of-type 'error
                         #L{
                         DEUTSCH ""
                         ENGLISH "No method for generic function: ~S"
                         FRANCAIS ""
                         }
                         gf)
          (return-from compute-effective-method
            (progn
              (install-dispatch gf)
              (apply #'compute-effective-method gf t args)
              ))))
    ...
    (when (null primary-methods)
      ...
      (if probe
          (error-of-type 'error
                         #L{
                         DEUTSCH ""
                         ENGLISH "No primary method for generic function: ~S"
                         FRANCAIS ""
                         }
                         gf)
          (return-from compute-effective-method

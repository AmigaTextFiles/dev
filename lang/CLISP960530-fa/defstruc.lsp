  ...
  (let ((constructorname (first descriptor))
    ...
    (let ((keying (or (member '&KEY arglist :test #'eq)
      ...
      (when keying
        (error-of-type 'program-error
          #L{
          DEUTSCH "~S ~S: Die Argumentliste für eine keywordfreie Konstruktorfunktion ~S darf kein ~S enthalten: ~S"
            ...
            (:PRINT-FUNCTION
               (let ((arg (second option)))
                 (when (and (consp arg) (eq (first arg) 'FUNCTION))
                   (warn 
                    #L{
                    DEUTSCH "~S: Bei :PRINT-FUNCTION ist FUNCTION bereits implizit.  Verwende daher ~S statt ~S."
                    ENGLISH "~S: Use of :PRINT-FUNCTION implicitly applies FUNCTION. Therefore using ~S instead of ~S."
                    FRANCAIS "~S : FUNCTION est déjà implicite avec :PRINT-FUNCTION.  C'est pourquoi ~S est utilisé au lieu de ~S."
                    }
       ...
       (LET ()
         (LET ,(append namesbinding (mapcar #'list slotdefaultvars slotdefaultfuns))
           ...
           (%PUT ',name 'DEFSTRUCT-DESCRIPTION
                 (VECTOR ,namesform ',type-option ',keyword-constructor ',slotlist
                         ,@slotdefaultvars
         ) )     )

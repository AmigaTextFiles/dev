...
static int fpe_stop;                    /* floating point exception */
      ...
      case 'C' : /* Call (load) program */
	    ...
	    *bp = islower(*bp) ? toupper(*bp) : *bp;
		  ...
		  *((char *)(strchr (tbuf,
                  ...
                  printf ("tbuf(%d) = '%s'\n", k, tbuf);
#endif
                  /*for (k--; isspace (tbuf[k]) && k > 2; k--) tbuf[k] = EOL;*/
...
#else
         fprintf (stderr,
		  "List not supported for this OS");
	 error (FILE_ERROR,
		0);
#endif
         break;
      ...
      case 'P' : /* Print to new file */
	    ...
	    if ((output = fopen (fname,
				 "w")) == NULL)
	    {
	       output = stdout;
	       fprintf (stderr,
			"Can't open for write : %s",
			fname);
#if defined(STRERROR)
	       fprintf (stderr,
			" : %s",
...
#else
         fprintf (stderr,
		  "Working directory not supported for this OS");
	 error (FILE_ERROR,
		0);
#endif
	 break;

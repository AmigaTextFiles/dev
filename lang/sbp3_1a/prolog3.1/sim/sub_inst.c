...
/******************************************************************************/
#ifdef AMIGA
/* Chk_Abort is called by Manx C on most I/O routines */
extern long Chk_Abort()

{
   long x;
   x = SetSignal(0L,SIGBREAKF_CTRL_C | SIGBREAKF_CTRL_D);
	    /* check for presence of ^C or ^D */

   if ( x &(SIGBREAKF_CTRL_C | SIGBREAKF_CTRL_D) == 0)
   {
      return(0);           /* neither ^C or ^D found */
   } /* if (x */
   ...
   /* must be a ^D then, handle it */
   if (x & SIGBREAKF_CTRL_D)
   {
      if (interrupt_code == 0)
      {
	 interrupt_code = 1;

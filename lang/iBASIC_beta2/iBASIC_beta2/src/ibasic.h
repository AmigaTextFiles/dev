/*
**  ibasic.h - a very basic BASIC interpreter
...
#include <setjmp.h>
#include <stddef.h>
// #include <ctype.h> // causes errors when loaded
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
enum {	NAME=1,NUMBER,STRING,LP,RP,COMMA,ADD,SUBS,MUL,DIV,MOD,
	...
	WHILE,FOR,TO,IF,ELSE,THEN,DIM,UBOUND,BYE,BREAK,RESUME, HELP, LOAD };
	
char	*kwd[]={ "AND","OR","FORMAT","SUB","END","RETURN","LOCAL","WHILE",
	"FOR","TO","IF","ELSE","THEN","DIM","UBOUND","BYE","BREAK","RESUME", "HELP", "LOAD", 0 };
...
{
if (n<1 || n>*mem) err("BOUNDS"); return mem+n;
}
...
initbasic(int comp) 
{ 
    pc=prg; sp=stk+STKSZ; csp=cstk+STKSZ; stabp=stab; compile=comp;
}
...
emit(int opcode())
{
    lmap[cpc]=lnum; prg[cpc++]=opcode;
}

/* ------------------------------------------------------------------------ */

inst(int opcode(), Val x)
{
emit(opcode); emit((Code)x);
}
...
RESUME_()
{
pc=opc? opc:pc; opc=pc; cpc=ipc; STEP;
}
...
DIV_()
{
if (!B) sp+=2,err("DIVISION BY ZERO"); A/=B; sp++; STEP;
}
...
MOD_()
{
if (!B) sp+=2,err("MODULUS OF ZERO"); A%=B; sp++; STEP;
}
...
FALSE_()
{
if (*sp++) pc++; else pc=prg+(int)*pc; STEP;
}
...
FOR_()
{
if (value[PCV] >= *sp) pc=prg+(int)*pc, sp++; else PCV; STEP;
}
...
LOADI_()
{
Val x=*sp++; x=*bound((Val*)value[PCV],x); *--sp=x; STEP;
}
...
STOREI_()
{
Val x=*sp++, i=*sp++; *bound((Val*)value[PCV],i)=x; STEP;
}
...
case BYE:		emit(BYE_); exit(0); break;
case GT:		expr(); emit(ECHO_); break;
	//case HELP:      printf("Not implemented yet!\n"); break;
case HELP:      iHelp(); break;
case LOAD:      printf("Not implemented yet!\n"); break;
...
void iHelp(void)
{
    ...
    puts( "	DIM	VAR(LENGTH)");
    puts( "	LOCAL	[VAR [, VAR ...]]");
    puts( "	SUB	NAME [PARAM [, PARAM ...]] ... END SUB");
    puts( "	NAME	[ARG [, ARG ...]]");
    puts( "	RETURN	[EXPRESSION]");	
    puts( "	IF	CONDITION ... [ELSE IF CONDITION ...] [ELSE ...] END IF");
    puts( "	IF	CONDITION THEN STATEMENT");
    puts( "	WHILE	CONDITION ... END WHILE");
    puts( "	FOR	VAR=INIT TO FINAL ... END FOR");
    puts( "	FORMAT	STRING [, EXPRESSION ...]");
    puts( "	RESUME");
    puts( "	BREAK");
    puts( "	LOAD    loads a source file  (not fully implemented yet)");
    puts( "	HELP    show this message");
    puts( "	BYE\n");

    puts( "	* '%' IN 'FORMAT' PRINTS THE NEXT ARGUMENT AS AN INTEGER");
    puts( "	* 'SUB' MAY NOT BE NESTED");
    puts( "	* 'LOCAL' MAY ONLY BE USED IN A SUBROUTINE\n");

    puts( "EXPRESSIONS:");
    puts( "	NUMBER, \" \", NAME, NAME(INDEX), NAME(ARG,...), UBOUND(VAR), (EXPRESSION)");
    
    puts( "	[EXPRESSION {*|/|\}] EXPRESSION");
    puts( "	[EXPRESSION {+|-}] EXPRESSION");
    puts( "	[EXPRESSION {=|<|>|<>|<=|=>}] EXPRESSION");
    puts( "	[EXPRESSION {AND|OR}] EXPRESSION\n");

    puts( "	* BOOLEAN VALUES ARE -1 FOR TRUE AND 0 FOR FALSE");
    puts( "	* 'AND' AND 'OR' ARE NOT SHORT-CIRCUITING");
    puts( "	* OPERATORS ARE LEFT-ASSOCIATIVE");
    puts( "	* STRINGS SHOULD ONLY BE USED FOR \"FORMAT\"\n");

    puts( "GENERAL NOTES:");
    puts( "	* VARIABLES ARE CASE-INSENSITIVE");
    puts( "	* ARRAY INDEXES START FROM 1\n");

    puts( "See file 'test.bas' for a syntax overview!");
    printf("\n%s", Reset);
}

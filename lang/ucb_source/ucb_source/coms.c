...
#ifdef AMIGA
    amiga_wait((unsigned int)getint(num));
#else
    if (NOT_THROWING) {
#ifdef WIN32
      win32_update_text();
...
#ifdef AMIGA
   char *foo = AllocVec(256L,MEMF_CLEAR);
   if (foo) {
      typestring(foo,args,255);
      Execute(foo,NULL,console);
      FreeVec(foo);
   } else
      ami_print("Shell failed: Unable to get memory.\n");
   return(UNBOUND);
#else
#ifdef mac

...
\ include? task-member.fth   member.fth   \ use parForth structures by commenting this out
\ include? :struct c_struct.fth           \ use parForth structures by commenting this out
include? smif{         smart_if.fth
include? file?         filefind.fth
include? see           see.fth          \ parForth separates see and (see) for seeing an xt
include? words.like    wordslik.fth
include? trace         trace.fth
\ include? ESC[    termio.fth           \ incompatible with AROS shell; causes echoing at terminal
\ include? HISTORY history.fth          \ use AROS shell history instead
include? SDAD          savedicd.fth
include? parForthExt.f parForthExt.f    \ parForth extensions; uses AROS library calls

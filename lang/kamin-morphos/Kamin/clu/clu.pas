...
type
   ...
   NAME = 0 .. MAXNAMES; (* a NAME is an index in printNames *)
   ...
   FUNTYPE = (NORMAL,CONSTRUCT,SELECTOR,SETTOR);
   FUNDEFREC = record
               funname: NAME;
               nextfundef: FUNDEF;
...
(* mkSetName - make name of settor corresponding to selector nm  *)
  ...
  begin
	 ...
	 for i:=1 to NAMELENG-4	do setname[i+4] := printNames[nm][i];
         ...
         begin
            with f^ do
               case ftype of
                  ...
                  CONSTRUCT: arity := lengthNL(cl^.clrep);
                  SELECTOR: arity := 1;
                  SETTOR: arity := 2
               end (* case and with *)
         end; (* arity *)
      ...
      begin (* checkArgs *)
		 ...
		 begin
			...
			then if actuals^.head^.vtype = PRIM
				 then typeError();
			if ftype = SELECTOR
			then if not isBound(nm.funpart, actuals^.head^.userval)
				 then typeError();
			if ftype = SETTOR
			then if not isBound(selname, actuals^.head^.userval)
				 then typeError()
		 end
      end; (* checkArgs *)

...
# Last modification date:   2005/07/17
#
...
__all__ = ['get_obj', 'get_src', 'posix_get_obj', 'posix_get_src', 'checkDepends', 'checkDepend',
           'log', 'execute', 'flexcat', 'flexcat_header', 'join_opts', 'fromposix', 'toposix']

get_obj = lambda n: os.path.splitext(n)[0]+'.o'
get_src = lambda n: os.path.splitext(n)[0]+'.c'

def posix_get_obj(n):
	return posixpath.splitext(str(n))[0]+'.o'
posix_get_src = lambda n: posixpath.splitext(str(n))[0]+'.c'
...
def flexcat_header(name, cd, sd, env):
    if  not env['quiet']:
...
if os.name == 'morphos':
    ...
    def toposix(path):
        ...
        return posixpath.normpath(drive + path)

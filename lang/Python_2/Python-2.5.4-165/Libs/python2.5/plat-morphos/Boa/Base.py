...
# Last modification date:   2006/08/03
#
        ...
        if isinstance(sources, list) or isinstance(sources, tuple):
            def f(l, x):
                if isinstance(x, list) or isinstance(x, tuple):
                    ...
                    l.append(resolvedash(x, self.builddir))
                else:
                    l.append(x)
            ...
            return (resolvedash(sources, self.builddir),)
        else:
            return (sources,)
...
class C_Object(Common):
    ...
    def __init__(self, name, source, *args, **kw):
        ...
        # replace an header '#' by the build dir
        l = []
        for path in self.INCLUDES:
            l.append(resolvedash(path, self.builddir))
        self.INCLUDES = l

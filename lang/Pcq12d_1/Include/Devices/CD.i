...
Type
 RMSF = Record
    ...
    Second,         { Seconds (0-59)         }
    Frame  : WORD;  { Frame   (0-74)         }
 end;
 RMSFPtr = ^RMSF;

 LSNMSF = Record
    MSF     : RMSF;         { Minute, Second, Frame  }
    LSN     : Integer;      { Logical Sector Number  }
 end;
 LSNMSFPtr = ^LSNMSF;
 ...
 *************************************************************************}
 ...
 CDXL = Record
    ...
    Buffer            : Address;       { data destination (word aligned) }
    Length,                            { must be even # bytes            }
    Actual            : Integer;       { bytes transferred               }
    IntData           : Address;       { interrupt server data segment   }
    IntCode           : Address;       { interrupt server code entry     }
 end;
 CDXLPtr = ^CDXL;

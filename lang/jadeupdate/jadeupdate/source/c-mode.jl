...
;;; Modified by Eric Van Zante 8-06-96.  Reduced indentation to more 
;;; acceptable level.

;;; Modified by Eric Van Zante 8-11-96.  Eliminated indent on colon.
;;; Made definition for { on the same line as int main
;;; { makes jade add a return and tab.
;;; Created definition for semi-colon.  Jade will now indent the line when a semi colon
;;; is entered.

;;; Modified by Eric Van Zante 1-20-97.  
;;; More appropriate use of return and indent involving semi-colons in C
;;; Jade is now consistent in how it treats spacing in C code.
;;; Tested and debugged 2-01-97
...
START and END except for the last line."
  ...
  (let
      ((max-width 0)
       ...
       tmp)
    (while (<= pos end)
      (setq tmp (char-to-glyph-pos (line-end pos)))
    ...
    (while (< pos end)
	  ...
	  (progn
	    (setq tmp (char-to-glyph-pos (match-start)))
	    (cond
	     ((> (pos-col tmp) max-width)
	      (delete-area (glyph-to-char-pos pos) (match-start)))
	     ((< (pos-col tmp) max-width)
...
(defun c-backward-exp (&optional number orig-pos no-blocks)
  ...
  (let
	...
	(unless (setq tmp (find-prev-regexp "[^\t\f ]" (prev-char 1 tmp)))

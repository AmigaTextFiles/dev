...
#if (defined(XP_AMIGA) && !defined(__SASC))
#define __USE_CLIB2_TIMEVAL
#include <sys/time.h>
#endif
...
PRMJ_LocalGMTDifference()
{
    ...
    return mktime(pLtime) - mktime(pGmtime);

#endif

#if defined(XP_UNIX) || defined(XP_WIN) || defined(XP_OS2) || defined(XP_BEOS) || (defined(XP_AMIGA) && defined(__SASC))
    struct tm ltime;
...
{
#if defined(XP_AMIGA) && defined(__SASC)
	JSInt64 s, us, ms2us, s2us, am2un;
	JSInt64 leapYearFudge;
	JSInt64 eight = JSLL_INIT(0, 8);
	JSInt64 two = JSLL_INIT(0, 2);
...
#endif
#if defined(XP_UNIX) || defined(XP_BEOS) || (defined(XP_AMIGA) && !defined(__SASC))
    struct timeval tv;
    JSInt64 s, us, s2us;
#endif /* XP_UNIX */
#ifdef XP_MAC
	...
	JSLL_UI2L(s, clock[0]);
	JSLL_UI2L(us, clock[1]);

	JSLL_MUL(s, s, s2us);
	JSLL_ADD(s, s, us);
	/* Amiga-time starts at 1978, unlike UNIX, 
	** which starts at 1970.  We must also   
	** add two extra days to account for two 
	** leap years.
	*/
	JSLL_UI2L(am2un, PRMJ_YEAR_SECONDS);
	JSLL_MUL(am2un, am2un, eight);
	JSLL_MUL(am2un, am2un, s2us);

	JSLL_UI2L(leapYearFudge, PRMJ_DAY_SECONDS);
	JSLL_MUL(leapYearFudge, leapYearFudge, two);
	JSLL_MUL(leapYearFudge, leapYearFudge, s2us);
	
	JSLL_ADD(s, s, am2un);
	JSLL_ADD(s, s, leapYearFudge);

	return s;
#elif defined(XP_AMIGA) && !defined(__SASC)
	gettimeofday(&tv, 0);
	JSLL_UI2L(s2us, PRMJ_USEC_PER_SEC);
...
{
#if defined(XP_UNIX) || defined(XP_WIN) || defined(XP_OS2) || defined(XP_MAC) || defined(XP_BEOS) || defined(XP_AMIGA)
    struct tm a;

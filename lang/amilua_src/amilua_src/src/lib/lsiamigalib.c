/*
** siamigalib.c, v 0.2 2005-11-21 Matthias Rustler
** Siamiga library
...
static int siamiga_gc(lua_State *L)
{
    ...
    if (siam->newmenu)
    {
        ...
        {
            struct NewMenu *menu = &siam->newmenu[i];
            if (menu->nm_Label != NM_BARLABEL) free(menu->nm_Label);
            free(menu->nm_CommKey);
        }
        free(siam->newmenu);
    }
    return 0;
...
static int sigadget_get(lua_State *L)
{
    ...
    switch (sig->type)
    {
        ...
        case INTEGER_KIND:
            ...
            break;
        case STRING_KIND:
            changed = GT_GetGadgetAttrs(sig->gad, win, NULL, GTST_String, (ULONG)&result, TAG_END);
...
static int sipicture_load(lua_State *L)
{
    ...
    GetDTAttrs(o, PDTA_DestBitMap, (ULONG)&dbm, PDTA_BitMapHeader, (ULONG)&bmh, TAG_END);
    if ((dbm == NULL) || (bmh == NULL))
    {
        DisposeDTObject(o);
        luaL_error(L, "Destbitmap or Bitmapheader is Null");
    }
...
static int sipicture_save(lua_State *L)
{
    ...
    if (ncols)
    {
        GetDTAttrs(o, PDTA_ColorRegisters, (ULONG)&cmap, PDTA_CRegs, (ULONG)&cregs, TAG_END);

...
;;$Header: /usr/local/cvsroot/slib/glob.scm,v 1.11 1998/12/16 20:32:15 radey Exp $
;;$Name:  $
...
(define (glob:caller-with-matches pat proc)
  ...
  (let* ((toks (glob:pattern->tokens pat))
	 ...
	 (matcher (glob:make-matcher toks char=? char<=?)))
    (lambda (str)
...
(define (replace-suffix str old new)
  (let* ((f (glob:substitute?? (list "*" old) (list "*" new)))
	 (g (lambda (st)
	      (or (f st)

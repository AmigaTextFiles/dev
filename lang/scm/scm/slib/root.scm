  ...
  (letrec
	...
	(cond ((procedure? prec) prec)
		 ...
		 (>= count (- prec))))
	      (else
	       (lambda (x0 f0 x1 f1 count)
		 (< (min (abs f0) (abs f1)) prec)))))
       (bracket-iter
	(lambda (xlo flo xhi fhi count)
	  (if (stop? xlo flo xhi fhi count)
	      (if (> (abs flo) (abs fhi)) xhi xlo)
	      (let tail ((del (- (/ flo (- fhi flo)))))
		(let* ((xnew (+ xlo (* del (- xhi xlo))))
		       (fnew (f xnew)))
		  (cond ((< 0.1 del 0.9)	;accept secant step
			 (if (positive? fnew)
			     (bracket-iter xlo flo xnew fnew (+ count 1))
			     (bracket-iter xnew fnew xhi fhi (+ count 1))))
			(else			;bisection step
			 (tail 1/2)))))))))
    ...
    (let ((f0 (f x0))
	  (f1 (f x1)))
      (cond ((<= f0 0 f1)
	     (bracket-iter x0 f0 x1 f1 0))
	    ((<= f1 0 f0)
	     (bracket-iter x1 f1 x0 f0 0))
	    (must-bracket? #f)
	    (else
	     (let secant-iter ((x0 x0)
	       ...
	       (cond ((stop? x0 f0 x1 f1 count)
		     ...
		     ((<= f0 0 f1)
		      (bracket-iter x0 f0 x1 f1 count))
		     ((>= f0 0 f1)
		      (bracket-iter x1 f1 x0 f0 count))
		     ((= f0 f1) #f)
		     (else
		      (let ((xnew (+ x0 (* (- (/ f0 (- f1 f0))) (- x1 x0)))))
			(secant-iter xnew (f xnew) x0 f0 (+ count 1)))))))))))

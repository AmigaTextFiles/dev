...
uses exec, intuition, graphics, utility, vartags;
...
procedure TR_SetString(p : pTR_Project; id : Longint; txt : String);
begin
    TR_SetAttribute(p,id,0,Longstr(txt));
end;

procedure TR_SetText(p : pTR_Project; id : Longint; txt : string);
begin
    TR_SetAttribute(p,id,TRAT_Text,Longstr(txt));
end;

procedure TR_SetWindowTitle(p : pTR_Project; thetitle : string);
begin
    TR_SetAttribute(p,0,TRWI_Title,Longstr(thetitle));
end;
    ...
    IF TritonBase <> NIL THEN begin
        ...
        ExitProc := @CloseTritonLibrary;
         ...
         MessageBox('FPC Pascal Error',
                    'Can''t open triton.library version ' + 
                    VERSION +
                    chr(10) + 

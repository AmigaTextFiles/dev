{
    $Id: dos.pp,v 1.8 1998/08/19 14:52:52 carl Exp $
...
{
   History:
   I have made an temporary fix for GetEnv('path')).
   Now you get a string with all directories in your
   path env. You can now do a fsearch('myfile',GetEnv('Path')).
   This fix is just a tempfix, can be removed in the next
   release. (or fixed).

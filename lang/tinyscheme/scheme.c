...
scheme *scheme_init_new() {
  scheme *sc=(scheme*)safe_alloc(sizeof(scheme));
  if(!scheme_init(sc)) {
    free(sc);
    return 0;
  } else {
...
scheme *scheme_init_new_custom_alloc(func_alloc malloc, func_dealloc free) {
  scheme *sc=(scheme*)safe_alloc(sizeof(scheme));
  if(!scheme_init_custom_alloc(sc,safe_alloc,free)) {
    free(sc);
    return 0;
  } else {
    return sc;
...
int scheme_init(scheme *sc) {
 return scheme_init_custom_alloc(sc,safe_alloc,free);
}

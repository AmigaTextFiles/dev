...
dosave(p) char *p;{
    ...
    fwrite(&start,   sizeof(char *), 1, fd);
    fwrite(&length,  sizeof(unsigned int), 1, fd);

    for (len = 0; len < length; len += dlen) {
	dlen = ((length - len) > 512) ? 512 : (length - len);
	if (dlen != fwrite(start+len, dlen, 1, fd)) {
		cant_happen(23);
		}
	}
...
# endif
	...
	fread(&test, sizeof(int), 1, fd);
	fread(&start, sizeof(char *), 1, fd);
	fread(&length, sizeof(unsigned int), 1, fd);
...
# endif
    	    ...
    	    for (len = 0; len < length; len += dlen) {
		dlen = ((length - len) > 512) ? 512 : (length - len);
		if (dlen != fread(start + len, dlen, 1, fd)) {
			cant_happen(23);
			}
		}
# ifdef ENVSAVE

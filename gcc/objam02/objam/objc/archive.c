...
static int objc_read_class (struct objc_typed_stream* stream, OCClass** class);
static int objc_write_use_common (struct objc_typed_stream* stream, unsigned int key);
static int objc_write_register_common (struct objc_typed_stream* stream, unsigned int key);
static int objc_write_class (struct objc_typed_stream* stream, struct objc_class* class);
...
{
  if ((val&_B_VALUE) == val)
    { buf[0] = val|_B_SINT; return 1; }
  else
    { buf[0] = _B_NINT|0x01; buf[1] = val; return 2; }
}


int
objc_write_unsigned_char (struct objc_typed_stream* stream, unsigned char value)
{
...
{
  if (val >= 0) return __objc_code_unsigned_char (buf, val);
  else { buf[0] = _B_NINT|_B_SIGN|0x01; buf[1] = -val; return 2; }
}
...
{
  if (val <= 0xffU) return __objc_code_unsigned_char (buf, val);
  else 
  {
    buf[0] = _B_NINT|0x02;
    buf[1] = val/0x100;
...
{
  if (val > 0) return __objc_code_unsigned_short (buf, val);
  if (val > -0x7f) /* val > -128 */ return __objc_code_char (buf, val);
  else 
  {
...
{
  if (val < 0x10000) return __objc_code_unsigned_short (buf, val%0x10000);
...
{
  if (val >= 0) return __objc_code_unsigned_int (buf, val);
  if (val > -0x7f) return __objc_code_char (buf, val);
  else 
  {
...
int
objc_write_string (struct objc_typed_stream* stream, const unsigned char* string, unsigned int nbytes)
{
  unsigned char buf[sizeof(unsigned int)+1];
  int len = __objc_code_unsigned_int (buf, nbytes);
  
  if ((buf[0]&_B_CODE) == _B_SINT) buf[0] = (buf[0]&_B_VALUE)|_B_SSTR;
  else /* _B_NINT */ buf[0] = (buf[0]&_B_VALUE)|_B_NSTR;
...
objc_read_char (struct objc_typed_stream* stream, char* val)
{
  ...
  if (len != 0)
  {
    if ((buf & _B_CODE) == _B_SINT) (*val) = (buf & _B_VALUE);
    else if ((buf & _B_NUMBER) == 1)
    {
      len = (*stream->read)(stream->physical, val, 1);
      if (buf&_B_SIGN) (*val) = -1*(*val);
    }
    else __objc_archiving_fatal("Expected 8bit signed int, got %dbit int", (int)(buf&_B_NUMBER)*8);
  }
  return len;
}
...
objc_read_unsigned_char (struct objc_typed_stream* stream, unsigned char* val)
{
  ...
  {
    if ((buf & _B_CODE) == _B_SINT) (*val) = (buf & _B_VALUE);
    else if ((buf & _B_NUMBER) == 1) len = (*stream->read)(stream->physical, val, 1);
    else __objc_archiving_fatal("Expected 8bit unsigned int, got %dbit int", (int)(buf&_B_NUMBER)*8);
  }
  return len;
}
...
objc_read_short (struct objc_typed_stream* stream, short* value)
{
  ...
  if ((len = (*stream->read)(stream->physical, buf, 1)))
  {
    ...
    else
    {
      ...
      if (nbytes > sizeof (short)) __objc_archiving_fatal("Expected short, got bigger (%dbits)", nbytes*8);
...
objc_read_string (struct objc_typed_stream* stream, char** string)
{
  ...
  if ((len = (*stream->read)(stream->physical, buf, 1)))
  {
      ...
      case _B_SSTR:
	{
	  ...
	  if (key) hash_add (&stream->stream_table, (void*)key, *string);
	...
	break;
      ...
      default:
	__objc_archiving_fatal("Expected string, got opcode %c\n", (buf[0]&_B_CODE));
    }
  }
...
objc_read_selector (struct objc_typed_stream* stream, SEL* selector)
{
  ...
  if ((len = (*stream->read)(stream->physical, buf, 1)))
  {
    ...
    if (buf[0] == (_B_EXT|_BX_SEL)) /* selector! */
    {
      ...
      __objc_xfree (selector_name);

      /* register */
      if (key) hash_add (&stream->stream_table, (void*)key, (unsigned char *)(*selector));
    }
    else if ((buf[0]&_B_CODE) == _B_UCOMM)
    {
      if (key) __objc_archiving_fatal("Cannot register use upcode...",0);
    ...
    else __objc_archiving_fatal("Expected selector, got opcode %c", buf[0]);
  }
  return len;
}
...
__objc_skip_type (const char* type)
{
  switch (*type)
  {
    ...
    case _C_ARY_B:
      ...
      if (*type == _C_ARY_E) return ++type;
      else __objc_archiving_fatal("Cannot parse typespec: %s", (int)type);
      break;
...
objc_read_type(TypedStream* stream, const char* type, void* data)
{
  ...
  switch(c = *type)
  {
    ...
    case _C_CLASS:
      return objc_read_class (stream, (OCClass**)data);
      break;
...
objc_read_types(TypedStream* stream, const char* type, ...)
{
  ...
  for (c = type; *c; c = __objc_skip_type(c))
  {
	...
	break;
      ...
      case _C_CLASS:
	res = objc_read_class(stream, va_arg(args, OCClass**));
	break;

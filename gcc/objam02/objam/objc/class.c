...
OCClass* (*_objc_lookup_class)(const char* name) = 0;
...
void
__objc_add_class_to_hash(OCClass* class)
{
  OCClass* h_class;
...
{
  node_ptr node;
  OCClass* object_class = objc_get_class ("Object");
...
OCClass*
class_pose_as (OCClass* impostor, OCClass* super_class)
{
  ...
  {
    OCClass **subclass = &(super_class->subclass_list);
    BOOL super_is_base_class = NO;

    /* move subclasses of super_class to impostor */
    while (*subclass)
      {
	OCClass *nextSub = (*subclass)->sibling_class;

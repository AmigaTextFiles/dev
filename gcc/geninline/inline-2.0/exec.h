...
AddMemList (BASE_PAR_DECL unsigned long size,unsigned long attributes,unsigned long pri,APTR base,UBYTE *name)
{
  BASE_EXT_DECL
  ...
  register unsigned long d2 __asm("d2") = pri;
...
AddTask (BASE_PAR_DECL struct Task *task,APTR initPC,APTR finalPC)
{
  ...
  : "r" (a6), "r" (a1), "r" (a2), "r" (a3)
  : "a0","a1","a2","a3","d0","d1");
  *(char *)a1 = *(char *)a1;  *(char *)a2 = *(char *)a2;  *(char *)a3 = *(char *)a3;
  return _res;
}
static __inline void 
...
static __inline APTR 
AllocPooled (BASE_PAR_DECL unsigned long memSize,APTR poolHeader)
{
  BASE_EXT_DECL
  register APTR  _res  __asm("d0");
  register struct ExecBase *a6 __asm("a6") = BASE_NAME;
...
static __inline APTR 
CreatePrivatePool (BASE_PAR_DECL unsigned long requirements,unsigned long puddleSize,unsigned long puddleThresh)
{
  BASE_EXT_DECL
  register APTR  _res  __asm("d0");
  register struct ExecBase *a6 __asm("a6") = BASE_NAME;
  ...
  register unsigned long d2 __asm("d2") = puddleThresh;
...
static __inline void 
DeletePrivatePool (BASE_PAR_DECL APTR poolHeader)
{
  BASE_EXT_DECL
  register struct ExecBase *a6 __asm("a6") = BASE_NAME;
  register APTR a0 __asm("a0") = poolHeader;
...
static __inline void 
FreePooled (BASE_PAR_DECL APTR memory,APTR poolHeader)
{
  BASE_EXT_DECL
  register struct ExecBase *a6 __asm("a6") = BASE_NAME;
  ...
  : "r" (a6), "r" (a1), "r" (a0)
  : "a0","a1","d0","d1");
  *(char *)a1 = *(char *)a1;  *(char *)a0 = *(char *)a0;
}
static __inline void 
...
InitStruct (BASE_PAR_DECL APTR initTable,APTR memory,unsigned long size)
{
  ...
  : "r" (a6), "r" (a1), "r" (a2), "r" (d0)
  : "a0","a1","a2","d0","d1");
  *(char *)a1 = *(char *)a1;  *(char *)a2 = *(char *)a2;
}
static __inline void 
...
static __inline ULONG 
Procure (BASE_PAR_DECL struct Semaphore *semaport,struct Message *bidMsg)
{
  BASE_EXT_DECL
  register ULONG  _res  __asm("d0");
  register struct ExecBase *a6 __asm("a6") = BASE_NAME;
  register struct Semaphore *a0 __asm("a0") = semaport;
  register struct Message *a1 __asm("a1") = bidMsg;
...
static __inline void 
StackSwap (BASE_PAR_DECL APTR newSize,APTR newSP,APTR newStack)
{
  BASE_EXT_DECL
  register struct ExecBase *a6 __asm("a6") = BASE_NAME;
  register APTR d0 __asm("d0") = newSize;
  register APTR d1 __asm("d1") = newSP;
  register APTR a0 __asm("a0") = newStack;
...
Supervisor (BASE_PAR_DECL unsigned long (*userFunction)())
{
  ...
  register ULONG  _res  __asm("d0");
  register struct ExecBase *a6 __asm("a6") = BASE_NAME;
  register unsigned long (*a5)() __asm("a3") = userFunction;
  __asm __volatile ("movel a5,sp@-; movel a3,a5; jsr a6@(-0x24); movel sp@+,a5"
  : "=r" (_res)
  : "r" (a6), "r" (a5)
  : "a0","a1","a3","d0","d1");
  *(char *)a5 = *(char *)a5;
...
static __inline void 
Vacate (BASE_PAR_DECL struct Semaphore *semaport)
{
  BASE_EXT_DECL
  register struct ExecBase *a6 __asm("a6") = BASE_NAME;
  register struct Semaphore *a0 __asm("a0") = semaport;

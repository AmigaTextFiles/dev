...
intex=''
    ...
    IF ConSize~='' then do
        IF ~open('console',ConSize,'W') THEN EXIT 20
        else nop
    end
    else
    ...
    if offen=0 then Address command "sys:rexxc/rx MakeXRefFile"
    ...
    do forever
        ...
        if li.mystack='?' then do
            raus=3
            li.mystack=' '
            CALL Print('StructReader®')
            CALL Print('©1996 by Stefan Guttmann')
        ...
        if (raus~=2)&(raus~=3) then do
            ...
            do until index(mystruct,',')=0
                ...
                if qui=TRUE then do 
                    ...
                    CALL Print('Please enter address in hex or <enter> for Offsets')

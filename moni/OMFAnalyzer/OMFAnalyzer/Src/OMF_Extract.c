    ...
    if(i == segment_number)
      {
        ...
        if(current_segment->lconst_length == 0 || current_segment->lconst_data == NULL)
          {
        ...
        result = CreateBinaryFile(file_path,current_segment->lconst_length,current_segment->lconst_data);
        if(result == 1)

...
static void *DecodeRecordSUPER(unsigned char *,int,int,unsigned char *,int,int *);
static void *DecodeRecordGeneral(unsigned char *,int,int,int *);
static void *DecodeRecordExperimental(unsigned char *,int,int,int *);

static struct subrecord_SuperReloc2 *DecodeSuperReloc2(unsigned char *,unsigned char *,int,BYTE *,int *);
static struct subrecord_SuperReloc3 *DecodeSuperReloc3(unsigned char *,unsigned char *,int,BYTE *,int *);
static struct subrecord_SuperInterseg1 *DecodeSuperInterseg1(unsigned char *,unsigned char *,int,BYTE *,int *);
static struct subrecord_SuperInterseg212 *DecodeSuperInterseg212(unsigned char *,BYTE,unsigned char *,int,BYTE *,int *);
static struct subrecord_SuperInterseg1324 *DecodeSuperInterseg1324(unsigned char *,BYTE,unsigned char *,int,BYTE *,int *);
static struct subrecord_SuperInterseg2536 *DecodeSuperInterseg2536(unsigned char *,BYTE,unsigned char *,int,BYTE *,int *);

static void AddReloc(BYTE,BYTE,WORD,WORD);
static void AddInterseg(BYTE,BYTE,WORD,WORD,WORD,WORD);
...
/****************************************************************/
struct omf_body_record *DecodeOneRecord(unsigned char *data, int segment_num, int file_offset, int segment_offset, unsigned char *lconst, int lconst_length, int *record_length_rtn)
{
  struct omf_body_record *current_record;

  /* Allocation mémoire */
  current_record = (struct omf_body_record *) calloc(1,sizeof(struct omf_body_record));
  ...
  if(current_record->operation_code == 0x00)
    ...
    current_record->record_data = DecodeRecordSUPER(data,file_offset,segment_offset,lconst,lconst_length,record_length_rtn);
  ...
  else
    {
      ...
      printf("Error : Unknow Record Operation Code %02X for Segment #%d at offset %d.\n",current_record->operation_code,segment_num,file_offset);
...
char *GetSUPERType(BYTE Type)
{
  ...
  if(Type == 0x00)
    ...
    sprintf(type_txt,"SUPER INTERSEG%d",Type-1);
  else
...
static void *DecodeRecordEND(unsigned char *data, int file_offset, int segment_offset, int *record_length_rtn)
{
  ...
  /* Allocation mémoire */
  current_record = (struct record_END *) calloc(1,sizeof(struct record_END));
  if(current_record == NULL)
    {
      printf("Error : Impossible to allocate memory to process Segment Body Record.\n");
      return(NULL);
...
static void *DecodeRecordCONST(unsigned char *data, int file_offset, int segment_offset, int *record_length_rtn)
{
  ...
  /* Allocation mémoire */
  current_record = (struct record_CONST *) calloc(1,sizeof(struct record_CONST));
  if(current_record == NULL)
    {
      printf("Error : Impossible to allocate memory to process Segment Body Record.\n");
      return(NULL);
  ...
  /* Allocation mémoire */
  current_record->data = (unsigned char *) calloc(current_record->ByteCnt,sizeof(unsigned char));
...
static void *DecodeRecordALIGN(unsigned char *data, int file_offset, int segment_offset, int *record_length_rtn)
{
  ...
  /* Allocation mémoire */
  current_record = (struct record_ALIGN *) calloc(1,sizeof(struct record_ALIGN));
  if(current_record == NULL)
    {
      printf("Error : Impossible to allocate memory to process Segment Body Record.\n");
      return(NULL);
...
{  
  ...
  /* Allocation mémoire */
  current_record = (struct record_ORG *) calloc(1,sizeof(struct record_ORG));
  if(current_record == NULL)
    {
      printf("Error : Impossible to allocate memory to process Segment Body Record.\n");
      return(NULL);
...
static void *DecodeRecordRELOC(unsigned char *data, int file_offset, int segment_offset, int *record_length_rtn)
{
  ...
  /* Allocation mémoire */
  current_record = (struct record_RELOC *) calloc(1,sizeof(struct record_RELOC));
  if(current_record == NULL)
    {
      printf("Error : Impossible to allocate memory to process Segment Body Record.\n");
      return(NULL);
  ...
  /* Conserve ce RELOC */
  AddReloc(current_record->ByteCnt,current_record->BitShiftCnt,(WORD)current_record->OffsetPatch,(WORD)current_record->OffsetReference);
...
static void *DecodeRecordINTERSEG(unsigned char *data, int file_offset, int segment_offset, int *record_length_rtn)
{
  ...
  /* Allocation mémoire */
  current_record = (struct record_INTERSEG *) calloc(1,sizeof(struct record_INTERSEG));
  if(current_record == NULL)
    {
      printf("Error : Impossible to allocate memory to process Segment Body Record.\n");
      return(NULL);
  ...
  AddInterseg(current_record->ByteCnt,current_record->BitShiftCnt,(WORD)current_record->OffsetPatch,(WORD)current_record->OffsetReference,current_record->FileNum,current_record->SegNum);
...
static void *DecodeRecordUSING(unsigned char *data, int file_offset, int segment_offset, int *record_length_rtn)
{
  ...
  /* Allocation mémoire */
  current_record = (struct record_USING *) calloc(1,sizeof(struct record_USING));
  if(current_record == NULL)
    {
      printf("Error : Impossible to allocate memory to process Segment Body Record.\n");
      return(NULL);
...
static void *DecodeRecordSTRONG(unsigned char *data, int file_offset, int segment_offset, int *record_length_rtn)
{
  ...
  /* Allocation mémoire */
  current_record = (struct record_STRONG *) calloc(1,sizeof(struct record_STRONG));
  if(current_record == NULL)
    {
      printf("Error : Impossible to allocate memory to process Segment Body Record.\n");
      return(NULL);
...
static void *DecodeRecordGLOBAL(unsigned char *data, int file_offset, int segment_offset, int *record_length_rtn)
{
  ...
  /* Allocation mémoire */
  current_record = (struct record_GLOBAL *) calloc(1,sizeof(struct record_GLOBAL));
  if(current_record == NULL)
    {
      printf("Error : Impossible to allocate memory to process Segment Body Record.\n");
      return(NULL);
...
static void *DecodeRecordGEQU(unsigned char *data, int file_offset, int segment_offset, int *record_length_rtn)
{
  ...
  /* Allocation mémoire */
  current_record = (struct record_GEQU *) calloc(1,sizeof(struct record_GEQU));
  if(current_record == NULL)
    {
      printf("Error : Impossible to allocate memory to process Segment Body Record.\n");
      return(NULL);
...
static void *DecodeRecordMEM(unsigned char *data, int file_offset, int segment_offset, int *record_length_rtn)
{
  ...
  /* Allocation mémoire */
  current_record = (struct record_MEM *) calloc(1,sizeof(struct record_MEM));
  if(current_record == NULL)
    {
      printf("Error : Impossible to allocate memory to process Segment Body Record.\n");
      return(NULL);
...
static void *DecodeRecordEXPR(unsigned char *data, int file_offset, int segment_offset, int *record_length_rtn)
{
  ...
  /* Allocation mémoire */
  current_record = (struct record_EXPR *) calloc(1,sizeof(struct record_EXPR));
  if(current_record == NULL)
    {
      printf("Error : Impossible to allocate memory to process Segment Body Record.\n");
      return(NULL);
...
static void *DecodeRecordZEXPR(unsigned char *data, int file_offset, int segment_offset, int *record_length_rtn)
{
  ...
  /* Allocation mémoire */
  current_record = (struct record_ZEXPR *) calloc(1,sizeof(struct record_ZEXPR));
  if(current_record == NULL)
    {
      printf("Error : Impossible to allocate memory to process Segment Body Record.\n");
      return(NULL);
...
static void *DecodeRecordBEXPR(unsigned char *data, int file_offset, int segment_offset, int *record_length_rtn)
{
  ...
  /* Allocation mémoire */
  current_record = (struct record_BEXPR *) calloc(1,sizeof(struct record_BEXPR));
  if(current_record == NULL)
    {
      printf("Error : Impossible to allocate memory to process Segment Body Record.\n");
      return(NULL);
...
static void *DecodeRecordRELEXPR(unsigned char *data, int file_offset, int segment_offset, int *record_length_rtn)
{
  ...
  /* Allocation mémoire */
  current_record = (struct record_RELEXPR *) calloc(1,sizeof(struct record_RELEXPR));
  if(current_record == NULL)
    {
      printf("Error : Impossible to allocate memory to process Segment Body Record.\n");
      return(NULL);
...
static void *DecodeRecordLOCAL(unsigned char *data, int file_offset, int segment_offset, int *record_length_rtn)
{
  ...
  /* Allocation mémoire */
  current_record = (struct record_LOCAL *) calloc(1,sizeof(struct record_LOCAL));
  if(current_record == NULL)
    {
      printf("Error : Impossible to allocate memory to process Segment Body Record.\n");
      return(NULL);
...
static void *DecodeRecordEQU(unsigned char *data, int file_offset, int segment_offset, int *record_length_rtn)
{
  ...
  /* Allocation mémoire */
  current_record = (struct record_EQU *) calloc(1,sizeof(struct record_EQU));
  if(current_record == NULL)
    {
      printf("Error : Impossible to allocate memory to process Segment Body Record.\n");
      return(NULL);
...
static void *DecodeRecordDS(unsigned char *data, int file_offset, int segment_offset, int *record_length_rtn)
{
  ...
  /* Allocation mémoire */
  current_record = (struct record_DS *) calloc(1,sizeof(struct record_DS));
  if(current_record == NULL)
    {
      printf("Error : Impossible to allocate memory to process Segment Body Record.\n");
      return(NULL);
...
static void *DecodeRecordLCONST(unsigned char *data, int file_offset, int segment_offset, int *record_length_rtn)
{
  ...
  /* Allocation mémoire */
  current_record = (struct record_LCONST *) calloc(1,sizeof(struct record_LCONST));
  if(current_record == NULL)
    {
      printf("Error : Impossible to allocate memory to process Segment Body Record.\n");
      return(NULL);
...
static void *DecodeRecordLEXPR(unsigned char *data, int file_offset, int segment_offset, int *record_length_rtn)
{
  ...
  /* Allocation mémoire */
  current_record = (struct record_LEXPR *) calloc(1,sizeof(struct record_LEXPR));
  if(current_record == NULL)
    {
      printf("Error : Impossible to allocate memory to process Segment Body Record.\n");
      return(NULL);
...
static void *DecodeRecordENTRY(unsigned char *data, int file_offset, int segment_offset, int *record_length_rtn)
{
  ...
  /* Allocation mémoire */
  current_record = (struct record_ENTRY *) calloc(1,sizeof(struct record_ENTRY));
  if(current_record == NULL)
    {
      printf("Error : Impossible to allocate memory to process Segment Body Record.\n");
      return(NULL);
...
static void *DecodeRecordcRELOC(unsigned char *data, int file_offset, int segment_offset, int *record_length_rtn)
{
  ...
  /* Allocation mémoire */
  current_record = (struct record_cRELOC *) calloc(1,sizeof(struct record_cRELOC));
  if(current_record == NULL)
    {
      printf("Error : Impossible to allocate memory to process Segment Body Record.\n");
      return(NULL);
  ...
  current_record->ByteCnt = data[1];
  current_record->BitShiftCnt = data[2];
  memcpy(&current_record->OffsetPatch,&data[3],sizeof(WORD));
  memcpy(&current_record->OffsetReference,&data[5],sizeof(WORD));

  /* Conserve ce RELOC */
  AddReloc(current_record->ByteCnt,current_record->BitShiftCnt,current_record->OffsetPatch,current_record->OffsetReference);
...
static void *DecodeRecordcINTERSEG(unsigned char *data, int file_offset, int segment_offset, int *record_length_rtn)
{
  ...
  /* Allocation mémoire */
  current_record = (struct record_cINTERSEG *) calloc(1,sizeof(struct record_cINTERSEG));
  if(current_record == NULL)
    {
      printf("Error : Impossible to allocate memory to process Segment Body Record.\n");
      return(NULL);
  ...
  memcpy(&current_record->OffsetReference,&data[6],sizeof(WORD));

  /* Conserve ce INTERSEG */
  AddInterseg(current_record->ByteCnt,current_record->BitShiftCnt,current_record->OffsetPatch,current_record->OffsetReference,current_record->FileNum,current_record->SegNum);
...
/*****************************************************/
static void *DecodeRecordSUPER(unsigned char *data, int file_offset, int segment_offset, unsigned char *lconst, int lconst_length, int *record_length_rtn)
{
  int offset, length;
  BYTE page;
  ...
  /* Allocation mémoire */
  current_record = (struct record_SUPER *) calloc(1,sizeof(struct record_SUPER));
  if(current_record == NULL)
    {
      printf("Error : Impossible to allocate memory to process Segment Body Record.\n");
      return(NULL);
  ...
  if(current_record->RecordType == 0x00)
    {
      ...
      while(1)
        {
          /* Récupère les données */
          current_SuperReloc2 = DecodeSuperReloc2(&data[1+sizeof(DWORD)+1+offset],lconst,lconst_length,&page,&length);
  ...
  else if(current_record->RecordType == 0x01)
    {
      ...
      while(1)
        {
          /* Récupère les données */
          current_SuperReloc3 = DecodeSuperReloc3(&data[1+sizeof(DWORD)+1+offset],lconst,lconst_length,&page,&length);
  ...
  else if(current_record->RecordType == 0x02)  /* 2 */
    {
      ...
      while(1)
        {
          /* Récupère les données */
          current_SuperInterseg1 = DecodeSuperInterseg1(&data[1+sizeof(DWORD)+1+offset],lconst,lconst_length,&page,&length);
  ...
  else if(current_record->RecordType >= 0x03 && current_record->RecordType <= 0x0D)  /* 13 */
    {
      /** Super Interseg 2-12 **/
      offset = 0;
      page = 0x00;
      while(1)
        {
          /* Récupère les données */
          current_SuperInterseg212 = DecodeSuperInterseg212(&data[1+sizeof(DWORD)+1+offset],current_record->RecordType-1,lconst,lconst_length,&page,&length);
  ...
  else if(current_record->RecordType >= 0x0E && current_record->RecordType <= 0x19)  /* 25 */
    {
      ...
      while(1)
        {
          /* Récupère les données */
          current_SuperInterseg1324 = DecodeSuperInterseg1324(&data[1+sizeof(DWORD)+1+offset],current_record->RecordType-13,lconst,lconst_length,&page,&length);
  ...
  else if(current_record->RecordType >= 0x1A && current_record->RecordType <= 0x25)  /* 37 */
    {
      ...
      while(1)
        {
          /* Récupère les données */
          current_SuperInterseg2536 = DecodeSuperInterseg2536(&data[1+sizeof(DWORD)+1+offset],current_record->RecordType-24,lconst,lconst_length,&page,&length);
...
static void *DecodeRecordGeneral(unsigned char *data, int file_offset, int segment_offset, int *record_length_rtn)
{
  ...
  /* Allocation mémoire */
  current_record = (struct record_General *) calloc(1,sizeof(struct record_General));
  if(current_record == NULL)
    {
      printf("Error : Impossible to allocate memory to process Segment Body Record.\n");
      return(NULL);
...
static void *DecodeRecordExperimental(unsigned char *data, int file_offset, int segment_offset, int *record_length_rtn)
{
  ...
  /* Allocation mémoire */
  current_record = (struct record_Experimental *) calloc(1,sizeof(struct record_Experimental));
  if(current_record == NULL)
    {
      printf("Error : Impossible to allocate memory to process Segment Body Record.\n");
      return(NULL);
...
/****************************************************************/
static struct subrecord_SuperReloc2 *DecodeSuperReloc2(unsigned char *data, unsigned char *lconst, int lconst_length, BYTE *page_rtn, int *length_rtn)
{
  int i;
  struct subrecord_SuperReloc2 *current_SuperReloc2;

  /* Allocation mémoire */
  current_SuperReloc2 = (struct subrecord_SuperReloc2 *) calloc(1,sizeof(struct subrecord_SuperReloc2));
  if(current_SuperReloc2 == NULL)
    {
      printf("Error : Impossible to allocate memory for SubRecord SuperReloc2.\n");
      return(NULL);
    }
      ...
      for(i=0; i<current_SuperReloc2->nb_address; i++)
        {
          ...
          if(current_SuperReloc2->OffsetPatch[i] > lconst_length)
            {
              printf("  Error : Bad Reference Address for Offset %04X.\n",current_SuperReloc2->OffsetPatch[i]);
              current_SuperReloc2->OffsetReference[i] = 0xFFFF;   /* Erreur */
            }
          else
            memcpy(&current_SuperReloc2->OffsetReference[i],&lconst[current_SuperReloc2->OffsetPatch[i]],sizeof(WORD));

          /* Conserve ce RELOC */
          AddReloc(current_SuperReloc2->ByteCnt,current_SuperReloc2->BitShiftCnt,current_SuperReloc2->OffsetPatch[i],current_SuperReloc2->OffsetReference[i]);
...
/****************************************************************/
static struct subrecord_SuperReloc3 *DecodeSuperReloc3(unsigned char *data, unsigned char *lconst, int lconst_length, BYTE *page_rtn, int *length_rtn)
{
  int i;
  struct subrecord_SuperReloc3 *current_SuperReloc3;

  /* Allocation mémoire */
  current_SuperReloc3 = (struct subrecord_SuperReloc3 *) calloc(1,sizeof(struct subrecord_SuperReloc3));
  if(current_SuperReloc3 == NULL)
    {
      ...
      for(i=0; i<current_SuperReloc3->nb_address; i++)
        {
          ...
          memcpy(&current_SuperReloc3->OffsetReference[i],&lconst[current_SuperReloc3->OffsetPatch[i]],sizeof(WORD));

          /* Conserve ce RELOC */
          AddReloc(current_SuperReloc3->ByteCnt,current_SuperReloc3->BitShiftCnt,current_SuperReloc3->OffsetPatch[i],current_SuperReloc3->OffsetReference[i]);
        }
...
/**********************************************************************/
static struct subrecord_SuperInterseg1 *DecodeSuperInterseg1(unsigned char *data, unsigned char *lconst, int lconst_length, BYTE *page_rtn, int *length_rtn)
{
  int i;
  struct subrecord_SuperInterseg1 *current_SuperInterseg1;

  /* Allocation mémoire */
  current_SuperInterseg1 = (struct subrecord_SuperInterseg1 *) calloc(1,sizeof(struct subrecord_SuperInterseg1));
  if(current_SuperInterseg1 == NULL)
    {
      ...
      for(i=0; i<current_SuperInterseg1->nb_address; i++)
        {
          ...
          memcpy(&current_SuperInterseg1->OffsetReference[i],&lconst[current_SuperInterseg1->OffsetPatch[i]],sizeof(WORD));
          current_SuperInterseg1->SegNum[i] = (WORD) lconst[current_SuperInterseg1->OffsetPatch[i]+2];

          /* Conserve ce INTERSEG */
          AddInterseg(current_SuperInterseg1->ByteCnt,current_SuperInterseg1->BitShiftCnt,current_SuperInterseg1->OffsetPatch[i],current_SuperInterseg1->OffsetReference[i],current_SuperInterseg1->FileNum,current_SuperInterseg1->SegNum[i]);
        }
...
/**************************************************************************/
static struct subrecord_SuperInterseg212 *DecodeSuperInterseg212(unsigned char *data, BYTE FileNum, unsigned char *lconst, int lconst_length, BYTE *page_rtn, int *length_rtn)
{
  int i;
  struct subrecord_SuperInterseg212 *current_SuperInterseg212;

  /* Allocation mémoire */
  current_SuperInterseg212 = (struct subrecord_SuperInterseg212 *) calloc(1,sizeof(struct subrecord_SuperInterseg212));
  if(current_SuperInterseg212 == NULL)
    {
      ...
      for(i=0; i<current_SuperInterseg212->nb_address; i++)
        {
          ...
          memcpy(&current_SuperInterseg212->OffsetReference[i],&lconst[current_SuperInterseg212->OffsetPatch[i]],sizeof(WORD));
          current_SuperInterseg212->SegNum[i] = (WORD) lconst[current_SuperInterseg212->OffsetPatch[i]+2];

          /* Conserve ce INTERSEG */
          AddInterseg(current_SuperInterseg212->ByteCnt,current_SuperInterseg212->BitShiftCnt,current_SuperInterseg212->OffsetPatch[i],current_SuperInterseg212->OffsetReference[i],current_SuperInterseg212->FileNum,current_SuperInterseg212->SegNum[i]);
        }
...
/****************************************************************************/
static struct subrecord_SuperInterseg1324 *DecodeSuperInterseg1324(unsigned char *data, BYTE SegNum, unsigned char *lconst, int lconst_length, BYTE *page_rtn, int *length_rtn)
{
  int i;
  struct subrecord_SuperInterseg1324 *current_SuperInterseg1324;

  /* Allocation mémoire */
  current_SuperInterseg1324 = (struct subrecord_SuperInterseg1324 *) calloc(1,sizeof(struct subrecord_SuperInterseg1324));
  if(current_SuperInterseg1324 == NULL)
    {
      ...
      for(i=0; i<current_SuperInterseg1324->nb_address; i++)
        {
          ...
          memcpy(&current_SuperInterseg1324->OffsetReference[i],&lconst[current_SuperInterseg1324->OffsetPatch[i]],sizeof(WORD));

          /* Conserve ce INTERSEG */
          AddInterseg(current_SuperInterseg1324->ByteCnt,current_SuperInterseg1324->BitShiftCnt,current_SuperInterseg1324->OffsetPatch[i],current_SuperInterseg1324->OffsetReference[i],current_SuperInterseg1324->FileNum,current_SuperInterseg1324->SegNum);
        }
...
/****************************************************************************/
static struct subrecord_SuperInterseg2536 *DecodeSuperInterseg2536(unsigned char *data, BYTE SegNum, unsigned char *lconst, int lconst_length, BYTE *page_rtn, int *length_rtn)
{
  int i;
  struct subrecord_SuperInterseg2536 *current_SuperInterseg2536;

  /* Allocation mémoire */
  current_SuperInterseg2536 = (struct subrecord_SuperInterseg2536 *) calloc(1,sizeof(struct subrecord_SuperInterseg2536));
  if(current_SuperInterseg2536 == NULL)
    {
  ...
  if((current_SuperInterseg2536->Count & 0x80) == 0x80)
  ...
  else
    {
      ...
      /* Opérations >> ou << = Toujours à 0x0F  */
      current_SuperInterseg2536->BitShiftCnt = 0x0F;
      ...
      for(i=0; i<current_SuperInterseg2536->nb_address; i++)
        {
          ...
          memcpy(&current_SuperInterseg2536->OffsetReference[i],&lconst[current_SuperInterseg2536->OffsetPatch[i]],sizeof(WORD));

          /* Conserve ce INTERSEG */
          AddInterseg(current_SuperInterseg2536->ByteCnt,current_SuperInterseg2536->BitShiftCnt,current_SuperInterseg2536->OffsetPatch[i],current_SuperInterseg2536->OffsetReference[i],current_SuperInterseg2536->FileNum,current_SuperInterseg2536->SegNum);
        }
...
/*******************************************************/
static void AddReloc(BYTE ByteCnt, BYTE BitShiftCnt, WORD OffsetPatch, WORD OffsetReference)
{
  struct omf_reloc *current_reloc;

  /* Allocation d'une structure Reloc */
  current_reloc = (struct omf_reloc *) calloc(1,sizeof(struct omf_reloc));
...
/*************************************************************/
static void AddInterseg(BYTE ByteCnt, BYTE BitShiftCnt, WORD OffsetPatch, WORD OffsetReference, WORD FileNum, WORD SegNum)
{
  struct omf_interseg *current_interseg;

  /* Allocation d'une structure Reloc */
  current_interseg = (struct omf_interseg *) calloc(1,sizeof(struct omf_interseg));

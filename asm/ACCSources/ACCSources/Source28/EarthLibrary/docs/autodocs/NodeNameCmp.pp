...
NAME
	NodeNameCmp -- Compare two names by the tn_Name field

SYNOPSIS
	result = NodeNameCmp(node2,node1)
	d0,ccr               a2    a1

FUNCTION
	Compare two nodes using the node name as the comparison
	criterion. Note that this comparison will be case sensitive.
...
SEE ALSO
	NodeNameICmp(), NodeValueCmp(), InitLibraryHook()

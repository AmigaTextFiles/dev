...
NAME
	ISDIGIT -- Test whether or not a character is a decimal digit.
...
FUNCTION
	Test whether or not a charater is a decimal digit.

INPUTS
	If using the macro _ISDIGIT, then register a6 must contain the
	base address of earth.library.

	This is not necessary with ISDIGIT, since this macro moves the
	address into a6 prior to doing _ISDIGIT.
...
RESULT
	If the character is a digit then the zero flag is reset,
	Else the zero flag is set.

NOTES
	This is just about the FASTEST possible way of testing whether
	or not a character is a decimal digit. If a6 already contains
	the base address of earth.library then use _ISDIGIT, else use
	ISDIGIT.

	Note that the ISDIGIT macro supports your SETDATA and SETA6
...
SEE ALSO
	ISALPHA, ISALNUM, ISCNTRL, ISGRAPH, ISLOWER, ISPRINT,

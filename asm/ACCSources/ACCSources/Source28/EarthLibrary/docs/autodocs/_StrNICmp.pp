...
NAME
	_StrNImp -- Compare initial part of two strings with case
		insensitivity.

SYNOPSIS
	comparison = _StrNICmp(string1,string2,length)
	ccr                    a0      a1      d0

FUNCTION
	Compare the initial part of two strings alphabetically.
	Only the first <length> bytes are actually compared, the rest
	are ignored. Note that this is a case insensitive comparison.

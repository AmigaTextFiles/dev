...
NAME
	ISALNUM -- Test whether or not a character is alphanumeric
...
FUNCTION
	Test whether or not a charater is alphanumeric.

INPUTS
	If using the macro _ISALNUM, then register a6 must contain the
	base address of earth.library.

	This is not necessary with ISALNUM, since this macro moves the
	address into a6 prior to doing _ISALNUM.
...
RESULT
	If the character is alphanumeric then the zero flag is reset,
	Else the zero flag is set.

NOTES
	This is just about the FASTEST possible way of testing whether
	or not a character is alphanumeric. If a6 already contains the
	base address of earth.library then use _ISALNUM, else use
	ISALNUM.

	Note that the ISALNUM macro supports your SETDATA and SETA6

...
;********** Copperlist **********
	...
	lea spritePalette,a1				; Palette Sprites
	move.w #COLOR16,d0					; COLOR16=$01A0
	moveq #4-1,d1						; 16 Farben
_palette:
	move.w d0,(a0)+
	addq.w #2,d0
	move.w (a1)+,(a0)+
...
_loop:
	...
	movem.w d0,-(sp)
	move.w #SPRITE_Y-1,d0				; ($2C+8-1)  ; auf die Zeile warten
	bsr _waitRaster
	move.w #SPRITE_Y,d0					; ($2C+8)	 ; und auf die nächste Zeile warten
	bsr _waitRaster
	movem.w (sp)+,d0
	...
	move.w #((VSTART<<8)!(HSTART)),SPR0POS(a5)						;move.w #((SPRITE_Y&$FF)<<8)!(((SPRITE_X-1)&$1FE)>>1),SPR0POS(a5)
	move.w #((VEND<<8)!(SPTCTL)),SPR0CTL(a5)						;move.w #(((SPRITE_Y+SPRITE_DY)&$FF)<<8)!((SPRITE_Y&$100)>>6)!(((SPRITE_Y+SPRITE_DY)&$100)>>7)!((SPRITE_X-1)&$1),SPR0CTL(a5)	
...
_waitSpriteMiddle:
	...
	move.w #(VSTART<<8)!(((SPRITE_X+9-1)&$1FE)>>1),SPR0POS(a5)		;move.w #((SPRITE_Y&$FF)<<8)!(((SPRITE_X+9-1)&$1FE)>>1),SPR0POS(a5)
...
_waitSpriteThreequarters:
	move.l VPOSR(a5),d0
	lsr.l #8,d0
	and.w #$01FF,d0
	cmpi.w #SPRITE_Y+((SPRITE_DY>>1)+(SPRITE_DY>>2)),d0
	blt _waitSpriteThreequarters

	;move.w #(((SPRITE_Y+SPRITE_DY)&$FF)<<8)!((SPRITE_Y&$100)>>6)!(((SPRITE_Y+SPRITE_DY)&$100)>>7)!((SPRITE_X+9-1)&$1),SPR0CTL(a5)
	;move.w #((VEND<<8)!(SPTCTL)),SPR0CTL(a5)
	move.w #(VEND<<8)!((SPRITE_Y&$100)>>6)!(((SPRITE_Y+SPRITE_DY)&$100)>>7)!((SPRITE_X+9-1)&$1),SPR0CTL(a5)
...
In diesem Programmbeispiel geht es um die Anzeige von Spritedaten ohne 
Verwendung der DMA, also nur durch die CPU. Dadurch werden die Spriteregister
SPRxPOS, SPRxCTL, SPRxDATA und SPRxDATB verwendet, wobei x die Spritenummer
zwischen 0 und 7 ist.

SPRxPOS und SPRxCTL sind die zwei Steuerwörter am Anfang der Spritedefinition
mit den Angaben zur horizontalen und vertikalen Anfangsposition, sowie der Höhe.
Die Sprite-Bitplanedaten werden in die Register SPRxDATA und SPRxDATB geladen,
was normalerweise die DMA macht. Aus diesen Registern werden die Daten dann
über die Viedologik zur Anzeige gebracht.

Da die Daten in den Registern gewisserweise festgeschrieben sind, wird jede
Zeile das gleiche Muster angezeigt. Um in einer Zeile eine Änderung der Daten
zu bekommen, wäre eine Synchronisation mit der Zeile erforderlich um dann eine
gezielte Änderung der Daten vorzunehmen. Dies ist praktischweise am Besten mit
dem Copper möglich in dem dann in der Copperliste die SPRx-Register mit den
Daten geladen werden.

Zu beachten wäre noch, dass ein Schreiben eines Wertes in SPRxCTL die Anzeige
beendet und die Auslösung der Anzeige durch ein Schreiben in SPRxDATA erfolgt.

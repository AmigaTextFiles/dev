...
; "Vector balls" mit 16 x 16 Pixel großen BOBs in vier Farben.
...
SIDE=100
DEPTH=150					; Abstand zwischen dem Beobachter und dem Bildschirm
NBBOBS=35
ROTATEX=1
ROTATEY=2
ROTATEZ=1
TRANSLATEZ=200				; Abstand zwischen dem Beobachter und dem Ursprung des Koordinatensystems
...
_maskRows:
	moveq #(BOB_DX>>4)-1,d1					; (320/16)-1 Schleifenzähler Spalten
...
_loop:
	...
	; Debuggen: Hintergrundfarbe zu Beginn der Schleife auf rot ändern
...
_swapBuffers:
	...
	addi.l #DISPLAY_DX>>3,d0			; 40 Bytes nächste Bitebene
	...
	move.l backBuffer,BLTDPTH(a5)		; Ziel = backbuffer
	move.w #((DISPLAY_DEPTH*DISPLAY_DY)<<6)!(DISPLAY_DX>>4),BLTSIZE(a5)	; 2*256 Zeilen und 320/16=20 Wörter Breite
	WAIT_BLITTER

	; Wenden Sie eine Drehung um die Z-Achse auf die Punkte an

	move.w angleZ,d0			; Winkel Z
	lea sinus,a0				; Tabellenanfang Sinuswerte
	move.w (a0,d0.w),d1			; den Sinuswert zum Winkel in d1 sin(x)
	lea cosinus,a0				; Tabellenanfang Cosinuswerte
	move.w (a0,d0.w),d0			; den Cosinuswert zum Winkel in d0 cos(x)
	lea bobs3D,a0				; Anfangsadrese Bobs3d mit 3 Koordinaten X,Y,Z
	lea bobs2D,a1				; Anfangsadrese Bobs2d mit 2 Koordinaten X,Y
	moveq #NBBOBS-1,d2			; 35-1 für alle Bobs
_rotateZ:
	move.w (a0),d3				; d3=X
	move.w d3,d4				; d4=Kopie X

	muls d0,d3					; X*COS
	swap d3						; X*COS * 2^16	
	rol.l #2,d3					; D3=X*COS * 2^14 

	muls d1,d4					; X*SIN
	swap d4						; X*SIN * 2^16
	rol.l #2,d4					; D4=X*SIN * 2^14 

	move.w 2(a0),d5				; d5=Y
	move.w d5,d6				; d6=Kopie Y

	muls d0,d5					; Y*COS
	swap d5						; Y*COS	* 2^16
	rol.l #2,d5					; D5=Y*COS * 2^14 

	muls d1,d6					; Y*SIN	
	swap d6						; Y*SIN * 2^16 
	rol.l #2,d6					; D6=Y*SIN * 2^14 
	...
	move.w 4(a0),(a1)+
	lea 6(a0),a0
	dbf d2,_rotateZ				; für alle Bobs wiederholen

	; Wenden Sie eine Drehung um die Y-Achse auf die Punkte an
	
	move.w angleY,d0			; Winkel Y
	lea sinus,a0				; Tabellenanfang Sinuswerte
	move.w (a0,d0.w),d1			; den Sinuswert zum Winkel in d1 sin(x)
	lea cosinus,a0				; Tabellenanfang Cosinuswerte
	move.w (a0,d0.w),d0			; den Cosinuswert zum Winkel in d0 cos(x)
	lea bobs2D,a0				; Anfangsadrese Bobs2d mit 2 Koordinaten X,Y
	lea bobs2D,a1				; Anfangsadrese Bobs2d mit 2 Koordinaten X,Y
	moveq #NBBOBS-1,d2			; 35-1 für alle Bobs
_rotateY:
	move.w (a0),d3				; d3=X
	...
	move.w 4(a0),d5				; d5=Z
	move.w d5,d6				; d6=Kopie Z

	muls d0,d5					; Z*COS
	swap d5						; Z*COS	* 2^16
	rol.l #2,d5					; D5=Z*COS * 2^14

	muls d1,d6					; Z*SIN
	swap d6						; Z*SIN * 2^16
	rol.l #2,d6					; D6=Z*SIN * 2^14
	...
	lea bobs2D,a0				; Anfangsadrese Bobs2d mit 2 Koordinaten X,Y				
	moveq #NBBOBS-1,d0			; 35-1 für alle Bobs
_project:
	move.w 4(a0),d1				; nächste Bobwerte
	addi.w #TRANSLATEZ,d1		; 200 - Abstand zwischen dem Beobachter und dem Ursprung des Koordinatensystems

	move.w (a0),d2				; aktueller Bobwert X-Pos
	muls #DEPTH,d2				; X-Pos*150 (150 = Abstand zwischen dem Beobachter und dem Bildschirm)
	divs d1,d2					; (X-Pos*150)/200
	addi.w #DISPLAY_DX>>1,d2	; +320/2 - zur Mitte des Bildschirms verschieben
	move.w d2,(a0)+				; aktueller Bobwert X-Pos (Perspektive)

	move.w (a0),d2				; aktueller Bobwert Y-Pos
	muls #DEPTH,d2				; Y-Pos*150 (150 = Abstand zwischen dem Beobachter und dem Bildschirm)
	divs d1,d2					; (Y-Pos*150)/200
	addi.w #DISPLAY_DY>>1,d2	; +320/2 - zur Mitte des Bildschirms verschieben
	move.w d2,(a0)+				; aktueller Bobwert Y-Pos (Perspektive)

	lea 2(a0),a0				; nächster Bob
	dbf d0,_project				; für alle Bobs wiederholen

	; Debuggen: Linien zwischen den BOBs ziehen, die einen Würfel bilden (vor dem Sortieren tun!).

	movea.l backBuffer,a2		; A2 = Adresse der bitplane

	lea bobs2D+27*3*2,a0		; A0 = Adresse der Koordinaten (X, Y) von Punkt A
	lea 6(a0),a1				; A1 = Adresse der Koordinaten (X, Y) von Punkt B
	bsr _drawLine				; Drawline mit CPU	
	movea.l a1,a0				; 	
	lea 6(a1),a1				; 	
	bsr _drawLine				; Drawline mit CPU	
	movea.l a1,a0				; 	
	...
	; Sortieren Sie die Punkte nach abnehmender Tiefe (d.h. von am weitesten entfernt bis am nächsten).

	lea bobs2D+(NBBOBS-1)*3*2+2*2,a0	; den Anfang ans Ende setzen
	moveq #NBBOBS-2,d0			; Anzahl Bobs - äussere Schleife
	moveq #NBBOBS-2,d1			; Anzahl Bobs - innere Schleife Startwert
_bubbleSortBackward:
	lea bobs2D+2*2,a1			; Adresse Anfang des Bereichs
	move.w d1,d2				; Anzahl Bobs - innere Schleife (Kopie
_bubbleSortForward:				; wird pro Durchlauf jeweils um 1 reduziert)		
	move.w (a1),d3				; aktuellen Wert nach d3
	cmp.w (a0),d3				; mit dem letzten Wert vergleichen
	bge _bubbleSortSkip			; wenn größer, (i.O.) dann überspringen 
	move.w (a0),(a1)			; ansonsten, Wert vom Ende an den Anfang kopieren
	move.w d3,(a0)				; Wert vom Ende ersetzen
	move.l -4(a1),d3			; neuen Wert in d3
	move.l -4(a0),-4(a1)		; Wert vom Ende an den Anfang kopieren
	move.l d3,-4(a0)			; Wert vom Ende ersetzen
_bubbleSortSkip:
	lea 3*2(a1),a1				; neue Adresse in a1 - vorwärts
	dbf d2,_bubbleSortForward	; wiederholen bis alle fertig
	subq.w #1,d1				; nächsten Durchlauf um 1 reduziert 
	lea -3*2(a0),a0				; neue Adresse in a1 - rückwärts
	dbf d0,_bubbleSortBackward	; wiederholen bis alle fertig

	; Winkel animieren (Drehung im Uhrzeigersinn)

	move.w angleX,d0					; alten Winkelwert X in d0
	subi.w #ROTATEX<<1,d0				; X-Geschwindigkeit*2 subtrahieren
	bge _angleXNoUnderflow				; wenn größer, gleich 0, i.O.
	addi.w #360<<1,d0					; ansonsten auf Startwert zurücksetzen
_angleXNoUnderflow:
	move.w d0,angleX					; neuen Winkelwert X speichern

	move.w angleY,d0					; alten Winkelwert Y in d0
	subi.w #ROTATEY<<1,d0				; Y-Geschwindigkeit*2 subtrahieren
	bge _angleYNoUnderflow				; wenn größer, gleich 0, i.O.
	addi.w #360<<1,d0					; ansonsten auf Startwert zurücksetzen
_angleYNoUnderflow:
	move.w d0,angleY					; neuen Winkelwert Y speichern

	move.w angleZ,d0					; alten Winkelwert Z in d0
	subi.w #ROTATEZ<<1,d0				; Z-Geschwindigkeit*2 subtrahieren
	bge _angleZNoUnderflow				; wenn größer, gleich 0, i.O.
	addi.w #360<<1,d0					; ansonsten auf Startwert zurücksetzen
_angleZNoUnderflow:
	move.w d0,angleZ					; neuen Winkelwert Z speichern

	; BOBs zeichnen

	lea bobs2D,a0						; Adresse Bob
	moveq #NBBOBS-1,d2					; Anzahl Bobs-1 Schleifen
_drawBobs:
	moveq #0,d1							; d1 zurücksetzen
	move.w (a0)+,d0						; aktuelle X-Position des Bobs
	subi.w #BOB_DX>>1,d0				; (16/2) subtrahieren
	move.w d0,d1						; Kopie
	...
	move.w d0,BLTCON0(a5)				; fertigen Wert iN BPLCON0
	lsr.w #3,d1							; multiplizieren * 8
	and.b #$FE,d1						; nur gerade Bytes zulassen
	move.w (a0)+,d0						; aktuelle Y-Position des Bobs
	subi.w #BOB_DY>>1,d0				; (16/2) subtrahieren
	mulu #DISPLAY_DEPTH*(DISPLAY_DX>>3),d0	; 2*(320/8)=80 Bytes
	add.l d1,d0							; das richtige Byte in der Zeile finden
	move.l backBuffer,d1				; Adresse backBuffer
	add.l d1,d0							; richtige Adresse im backpuffer finden
	...
	move.w #(DISPLAY_DX-(BOB_DX+16))>>3,BLTCMOD(a5)	; (320-(16+16))/8 = 288/8 = 36   
	move.w #(DISPLAY_DX-(BOB_DX+16))>>3,BLTDMOD(a5) ; (320-(16+16))/8 = 288/8 = 36   
	move.l #bob,BLTAPTH(a5)				; Quelle A - Bob
	move.l #bobMask,BLTBPTH(a5)			; Quelle B - Maske
	move.l d0,BLTCPTH(a5)				; Quelle C - backBuffer (Hintergrund)
	move.l d0,BLTDPTH(a5)				; Ziel D - backBuffer 
	move.w #(BOB_DEPTH*(BOB_DY<<6))!((BOB_DX+16)>>4),BLTSIZE(a5)	; 5*16 Zeilen und (16+16)/16=2 Wörter Breite 
	WAIT_BLITTER
	lea 2(a0),a0						; nächste Bobdaten
	dbf d2,_drawBobs					; wiederholen bis alle fertig

	; Debugging: Hintergrundfarbe am Ende der Anzeige auf grün ändern
...
;---------- Linienführung in einer Bitplane (auf der CPU) ----------

; Eingang(s) :
;	A0 = Adresse der Koordinaten (X, Y) von Punkt A
;	A1 = Adresse der Koordinaten (X, Y) von Punkt B
;	A2 = Adresse der bitplane
; Ausgang(s) :
;	(keine)
; Bemerkung :
; Diese Routine ist eine einfache Faktorisierung. Sie verwendet Register,
; ohne sie zu erhalten, und verweist auf Konstanten. Keine Optimierung,
; nur zum Debuggen.
...
_DXPositive:
	...
	move.w #-DISPLAY_DEPTH*(DISPLAY_DX>>3),d4		; D4 = Inkrement Y
	sub.w 2(a1),d2
	bge _DYPositive
	neg.w d2
	neg.w d4
...
angleY:				DC.W 0
angleZ:				DC.W 0
bobs3D:													; X,Y,Z Koordinaten der Bobs
					DC.W 0, 0,SIDE
					DC.W 0, 0, 3*SIDE/4
					DC.W 0, 0, SIDE/2
					DC.W 0, 0, SIDE/4
					...
					DC.W 0, (3*SIDE)/4, 0
					DC.W 0, SIDE, 0
					DC.W SIDE/2, SIDE/2, -SIDE/2				; Offset +27 Würfelkanten
					DC.W SIDE/2, -SIDE/2, -SIDE/2
					DC.W -SIDE/2, -SIDE/2, -SIDE/2		; 30
					DC.W -SIDE/2, SIDE/2, -SIDE/2
					DC.W SIDE/2, SIDE/2, SIDE/2
					DC.W SIDE/2, -SIDE/2, SIDE/2
					DC.W -SIDE/2, -SIDE/2, SIDE/2
					DC.W -SIDE/2, SIDE/2, SIDE/2		; 35	; Offset +34 Würfelkanten
bobs2D:
					BLK.W NBBOBS*3,0		; Anzahl Bobs * 3 = 35*3=105
...
cosinus:
	...
	dc.w $3F61, $3F86, $3FA6, $3FC2, $3FD8, $3FEA, $3FF6, $3FFE		end	Programmbeschreibung:Vor der Hauptschleife wird zuerst 2x Speicher angefordert für jeweils einenBildschirm der Größe 320x256 mit 2 Bitebenen. Die Speicherbereiche sind mit backpuffer und frontpuffer bezeichnet. Der Frontpuffer beinhaltet dabei dasfertige Bild. In den Backpuffer wird das nächste Bild gezeichnet. Dann wird dieCopperliste erstellt und die Bitplanepointer werden auf den Speicherbereich"frontpuffer" gesetzt.Als nächstes wird mit der CPU die Maske des Bobs erstellt, bevor als nächstesdie Hauptschleife beginnt. In der Hauptschleife wird zuerst auf das Endes desangezeigten Bildes gewartet und dann werden die Zeiger von backpuffer und frontpuffer ausgetauscht und der neue backpuffer (das eben angezeigte Bild)komplett im Speicher gelöscht.Dann werden die Punkte rotiert und anschließend auf den Monitor durch dieBerechnung der Perspektive projeziert.Dann werden 12 Linien mit der CPU gezeichnet und zwar zwischen den 8 Eckeneines gedachten Würfels von den BoB-Ecken.Es handelt sich dabei um die Punkte:  Pkt. A0			Pkt. A1	
* bobs2D+27*3*2,a0	bobs2D+28*3*2,a1
* bobs2D+28*3*2,a0	bobs2D+29*3*2,a1
* bobs2D+29*3*2,a0	bobs2D+30*3*2,a1
* bobs2D+27*3*2,a0	bobs2D+30*3*2,a1

* bobs2D+31*3*2,a0	bobs2D+32*3*2,a1
* bobs2D+32*3*2,a0	bobs2D+33*3*2,a1
* bobs2D+33*3*2,a0	bobs2D+34*3*2,a1
* bobs2D+31*3*2,a0	bobs2D+34*3*2,a1

* bobs2D+27*3*2,a0	bobs2D+31*3*2,a1
* bobs2D+28*3*2,a0	bobs2D+32*3*2,a1
* bobs2D+29*3*2,a0	bobs2D+33*3*2,a1
* bobs2D+30*3*2,a0	bobs2D+34*3*2,a1

* lea 6(a0),a1 entspricht lea(1*3*2)(a0),a1

* Als nächstes werden die Punkte mit dem Bubblesort Algorithmus sortiert.
* Danach liegen die am weitesten entfernten Punkte in der Liste hinten und
* die nächsten am Anfang.

* Dann werden die Winkel für die nächste Rotation vorberechnet und gespeichert
* und die Bobs in einer Schleife der Reihe nach gezeichnet.

* Es fehlt noch das Konzept des Linienzeichnens. (mit CPU)
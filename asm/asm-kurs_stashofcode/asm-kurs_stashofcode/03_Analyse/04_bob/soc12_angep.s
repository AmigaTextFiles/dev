...
_bitplanes:
	...
	addq.w #2,d0						; Registeradresse 2 Bytes addieren, ergibt BPL2PTH, usw. BPLxPTH
...
_checkerDrawLines:
	...
	lea DISPLAY_DX>>3(a2),a2			; 320/8=40 - nächste Zeile in der Bitebene
	dbf d5,_checkerDrawLines			; wiederholen, bis alle Zeilen fertig
_checkerSkipBitplane:
	lea DISPLAY_DY*(DISPLAY_DX>>3)(a1),a1	; 256*40 Bytes, nächste Bitebene
	...
	lea 15*(DISPLAY_DX>>3)(a0),a0		; 15*(320/8)=600 zur Adresse hinzufügen
	dbf d1,_checkerDrawRows				; wiederholen bis alle 16 Reihen fertig sind
	
	; Kopieren des Hintergrunds in den Front- und Backpuffer.
...
_copyBackground:
	...
	lea DISPLAY_DY*(DISPLAY_DX>>3)(a0),a0		; 256*40 , nächste Bitebene
	lea DISPLAY_DY*(DISPLAY_DX>>3)(a1),a1		; 256*40 , nächste Bitebene
	lea DISPLAY_DY*(DISPLAY_DX>>3)(a2),a2		; 256*40 , nächste Bitebene
...
_swapBuffers:
	...
	addi.l #DISPLAY_DY*(DISPLAY_DX>>3),d0	; 256*40 Bytes nächste Bitebene
	...
	mulu #DISPLAY_DX>>3,d0				; 320/8=40 Bytes d0=d0*40
...
_clearBob:
	...
	move.w #(BOB_DY<<6)!(DISPLAY_DX>>4),BLTSIZE(a5)	; 64 Zeilen * 320/16=20 Wörter Breite
	WAIT_BLITTER
	lea DISPLAY_DY*(DISPLAY_DX>>3)(a0),a0	; 256*40 , nächste Bitebene
	lea DISPLAY_DY*(DISPLAY_DX>>3)(a1),a1	; 256*40 , nächste Bitebene
	dbf d0,_clearBob						; wiederholen bis alle fertig

	; Bewegen des BOBs, indem Sie ihn von den Rändern abprallen lassen.
...
_moveBobNoOverflowX:					;
	...
	move.w d0,bobY+2					; aktuelle Y-Position des Bobs kopieren, für spätere Löschung
...
_moveBobNoOverflowY:
	...
	move.w bobY,d0						; aktuelle Y-Position des Bobs
	mulu #DISPLAY_DX>>3,d0				; 320/8=40 Bytes, d0=d0*40 Zeile in der Bitebene finden
	add.w d1,d0							; das richtige Byte in der Zeile finden

	lea bob,a0							; Adresse Bob
	lea mask,a1							; Adresse Maske
	movea.l backBuffer,a2				; Adresse backBuffer
	add.l d0,a2							; richtige Bob-Adresse im backpuffer finden
	...
	move.w #(DISPLAY_DX-(BOB_DX+16))>>3,BLTCMOD(a5)		; (320-(64+16))/8 = 240/8 = 30   
	move.w #(DISPLAY_DX-(BOB_DX+16))>>3,BLTDMOD(a5)		; (320-(64+16))/8 = 240/8 = 30
	move.w #BOB_DEPTH-1,d0				; über alle Bitebenen
_drawBobBitplanes:
	move.l a0,BLTAPTH(a5)				; Quelle A - Bob
	move.l a1,BLTBPTH(a5)				; Quelle B - Maske
	move.l a2,BLTCPTH(a5)				; Quelle C - backBuffer (Hintergrund)
	move.l a2,BLTDPTH(a5)				; Ziel D - backBuffer 
	move.w #(BOB_DY<<6)!((BOB_DX+16)>>4),BLTSIZE(a5)	; 64 Zeilen und (64+16)/16=5 Wörter Breite 
	WAIT_BLITTER
	addi.l #BOB_DY*(BOB_DX>>3),a0			; addieren 64 * (64/8) 
	addi.l #BOB_DY*((BOB_DX+16)>>3),a1		; addieren 64 * (80/8)	Maske ist ein Wort größer, als der Bob
	addi.l #DISPLAY_DY*(DISPLAY_DX>>3),a2	; addieren 256 * (320/8), nächste Bitebene
	dbf d0,_drawBobBitplanes				; wiederholen bis alle fertig
...
Zu allererst haben wir einen Lowres-Screen 320x256 mit 5 Bitebenen, 
halten jedoch 3x den Speicher dafür vor und zwar für den Hintergrund,
den Frontbuffer und Backpuffer. Der Hintergrund bleibt zu allen Zeiten
in der Hauptschleife unverändert. Der Frontbuffer (besser viewbuffer)
ist der Bereich der angezeigt wird, nachdem zuvor in den backpuffer
(besser drawbuffer) gezeichnet wurde.

Die Copperliste ist kurz und beinhaltet nur die Standardregister für
Screen- und Displayeinstellung, die Bitplanepointer und die Palette.
Es werden die Bitplanepointer für den frontbuffer geladen und die
Copperliste gestartet. Auch werden die DMA-Kanäle eingeschaltet.

Das Hauptprogramm beginnt mit der Erstellung eines Hintergrundbildes,
einem Muster aus 16x16 Quadraten. Dies erfolgt durch das Kopieren von
Bitmustern in Schleifen in die einzelnen Bitebenenen des Hintergrunds.

Anschließend wird dieses Muster, also der Hintergrund durch eine einfache
Kopie durch den Blitter in den Front- und Backpuffer kopiert.

Die Hauptschleife wartet zunächst auf das Ende des Displays an Zeile $2c+256
und tauscht dann die Anfangsadressen von Front- und Backpuffer aus, 
bevor als nächstes die Bitplanepointer in der Copperliste getauscht
werden. Die Zeile mit move.l d0,frontBuffer sagt, das hier immer der
frontbuffer angezeigt wird, der zuvor mit dem BOB beschrieben wurde.

Idee:
Da die Adressen feststehen könnte dieser Wechsel auch durch einen
Copperjmp im Wechsel erfolgen, d.h. 1) nur Copperlist 1 und
2) über Copperlist 1 zu Copperlist 2 un dann 3) wieder nur Copperlist 1.

Während also das fertige Bild angezeigt wird, entsteht zeitgleich das 
nächste Bild.

Als nächstes wird mit einem Blitt der Größe 64 Zeilen x 20 Wörter, ab der Zeile
wo sich der Bob zuvor befand, der Hintergrund zurück in den Backpuffer kopiert.
Somit entspricht der Backpuffer wieder dem Hintergrund.
(bobY+2 ist dabei (oldBobY) die Zeile, die zuvor gespeichert wurde.)

Im nächsten Teil wird die BobX und BoxY-Position ermittelt unter 
Berücksichtigung der Ränder.

Als nächstes erfolgt das Kopieren des Bobs auf den Hintergrund an die neue
Position durch die Cookie-Cut Funktion. D=A+bC
Diesmal 64 Zeilen und 4 Wörter breit. Nein, halt 5 Wörter breit, weil wir noch
ein Wort mehr blitten, welches wir zu alles Null maskieren um es dann zu 
verschieben. Dadurch werden links vom Bob beim Verschieben Nullen eingefügt.
Den zusätzlichen Wortblitt gehen wir dann mit -2 in BPLxMOD wieder zurück um
wieder an der richtigen Adresse des Bobs zu stehen. 

Die Bobdaten 4 Wörter breit und die Bob-Maske 5 Wörter breit liegen dafür
bereits verfügbar im Speicher und werden den Kanälen A und B zugewiesen, weil
bei diesen ein Verschiebungswert von 0 bis 15 Pixel eingestellt werden kann.
Der Verschiebungswert ist bei beiden natürlich gleich und ergibt sich aus der
Restberechnung bei der Wordadressenermittlung der BobX-Position.

BLTCON0 wird mit dem Wert $0FF2 geladen
$0Fxx - alle Kanäle
Minterm $F2 = D=A+bC

	move.w #$FFFF,BLTAFWM(a5)			; alles passiert
	move.w #$0000,BLTALWM(a5)			; alles gelöscht 
	move.w #-2,BLTAMOD(a5)				; Modulo Bob   (4 Wörter breit)
	move.w #0,BLTBMOD(a5)				; Modulo Maske (5 Wörter breit)

	move.w #(BOB_DY<<6)!((BOB_DX+16)>>4),BLTSIZE(a5)	; 64 Zeilen und 64+16/16=5 Wörter Breite 

Der Blitt ist ein Wort breiter als der Bob aufgrund der Maskierung und
Verschiebung.

Modulo ist entsprechend der Rest bezogen auf die Bitplanebreite.

	move.w #(DISPLAY_DX-(BOB_DX+16))>>3,BLTCMOD(a5)		; (320-(64+16))/8 = 240/8 = 30   
	move.w #(DISPLAY_DX-(BOB_DX+16))>>3,BLTDMOD(a5)		; (320-(64+16))/8 = 240/8 = 30

Der Original-Minterm für Cookie-Cut laut HRM, wäre:
Minterm $CA			D =  AB + aC

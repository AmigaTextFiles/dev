...
; Lezione11b.s - Erste Verwendung des neuen startup2.s und eines Interrupts.
...
START:
	...
	move.w	#$c020,$9a(a5)		; INTENA - aktivieren interrupt "VERTB" per
								; Level 3 ($6c), was einmal pro Frame
								; (an der Zeile $00) generiert wird.		
	;move.w	#$4000,$9a(a5)		; Test des Bit14  Master Enable - deaktiviert alle Interrupts			
...
die Schleife, wenn der Elektronenstrahl die Zeile $00 erreicht und führt 
die Routine MT_MUSIC aus und kehrt zurück um die einfache Schleife auszuführen.
Anstelle dieser abscheulichen Maus-Warteschleife hätten wir auch eine 
Routine zur Berechnung eines Fraktals, die mehrere Sekunden dauern kann,
während die Musik "zeitgemäß" und synchronisiert spielen würde, einsetzen 
können ohne die Berechnung des Fraktals zu stören. Es verlangsamt nur das 
Wenige, was es zum Abspielen der Musik in jedem Frame braucht.
...
Das "wartet" ein wenig, bevor es die Kontrolle über die Hardware übernimmt.
Für eine Berechnung der zu wartenden Zeit wird 50 als 1 Sekunde angenommen 
da wir das Vblank nutzen. Also 150 sind 3 Sekunden. Wenn Ihr Programm 
jedoch eine ziemlich große und komprimierte Datei ist, um es entpacken,
braucht es eine oder zwei Sekunden. Ansonsten können Sie es auch auf einem
niedrigen Wert belassen. 
Wenn Sie stattdessen die unkomprimierte Datei gespeichert haben, und wenn 
Sie von der Diskette gestartet haben, würde die Ausführung beginnen bevor
das Lesen vom Laufwerk beendet wurde und ein von 5 Mal kommt es vor, das am 
Ende das Dos im totalen Koma ist.
Um dies zu vermeiden, berechnen Sie immer den Zeitverlust mit der "waitdisk" -
Schleife beim Auspacken und das Programm startet nach mindestens 3 Sekunden
nach dem Ende des Uploads.
...
; Lezione11h2.s	- Routine, die ausgeblendete Balken erzeugt - BENUTZEN SIE DIE 
			; RECHTE MAUSTASTE UM DIE STANGENHÖHE ZU ERHÖHEN.
...
changecop:
	...
	cmp.b	#$24,barlen	; Andernfalls überprüfen Sie, ob wir bereits bei $24 sind
	beq.s	noadd		; In diesem Fall springt es zu noadd
	addq.b	#1,barlen	; oder vergrößern Sie die Leiste (BARLEN)
noadd:
	LEA	copcols,a0		; Adresse Puffer in copperlist
	MOVE.w	#LINEE-1,d0	; Anzahl der Zeilen des loop
...
LineeLoop:
	move.w	(a1)+,6(a0)	; Kopieren Sie die Farbe aus der Tabelle in die copperliste
	...
	cmp.l	#FINETABColBarra,PuntatTemporaneo	; sind wir am Endes des tab?
	bne.s	NonRipartire		; wenn nicht, weiter mit NonRipartire
	lea	TABColoriBarra(pc),a1	; ansonsten ab dem ersten col!
NonRipartire:
	move.l	a1,PuntatTemporaneo	; und speichern Sie den Wert in Pun. vorübergehend
	moveq	#0,d1			; d1 zurücksetzen
AspettaSottoBarra:

...
; Lezione15c2.s		-  Farben gespeichert am Boden des Bildes mit AGAconv
...
; Diese Routine, die auch in meinem WORLD OF MANGA-Demo enthalten ist, wird benötigt
; um die 24-Bit-Palette zu lesen, die am unteren Rand des PIC RAW mit AGAconv 
; gespeichert ist. Es ist nicht optimiert, um die Operationen klarer zu machen.
; Grundsätzlich konvertiert es jede Farbe in 24-Bit, die im Format $00RrGgBb 
; kommt, wobei R = hohes nibble von ROT, r = niedriges nibble von ROT,
; G = hohes nibble von GRÜN usw., ins copperlistenformat AGA, das heißt
; in zwei Worten $0RGB mit hohen nibble und $0rgb mit niedrigen nibble.
; Beachten Sie, dass es auch verwendet werden kann, um Farben aus Tabellen
; der 24-Bit-Palette, die in diesem Format oder als Grundlage für Routinen
; gespeichert ist jeglicher Art in Bezug auf die Palette zu erhalten.

MettiColori:
	LEA	PICTURE+(10240*8),A0	; Adresse der Farbpalette
...
DaLongARegistri:		; Schleife, die die Farben $00RrGgBb.l in die 2 
			; Wörter $0RGB, $0rgb geeignet für die copperregister umwandelt.
...
Das Bild in RAW ist 1024 Bytes länger als es sein sollte. Tatsächlich ist die
Länge einer Lowres-Bitebene 10240, multipliziert mit 8 würde es 81920 ergeben, aber
das Bild ist 82944 lang. Diese 256 Langwörter (1024 Bytes = 512 Wörter = 256 Longs)
sind nichts anderes als die 24-Bit-Palette, die unten im Bild gespeichert ist.
Es ist gemacht mit kefcon für das nicht aga Bild. Die Möglichkeit, die Palette 
unten oder am Anfang eines Bildes zu speichern wird von AGAconv angegeben, einfach 
die Einstellungen einstellen (durch Drücken von Amiga + P) wie folgt:

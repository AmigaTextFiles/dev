...
; Lezione14-3a.s	** PRÄZISIONSTÖNE UND HALBTÖNE **
...
Der einzige unterschied ist, dass man jetzt nicht nur 7 Noten einer Skala
von verschiedenen Oktaven erzeugen kann, sondern auch die Noten der "schwarzen
...
größere Präzision zu gewährleisten:
In der Tat die zwei Wörter bezeichnen jeweils Zähler und Nenner des Bruchs, der
die Beziehung zwischen einer Note und dem C auf einer Skala angibt und in der
Tat ändert sich die Beziehung nicht.
Nehmen Sie zum Beispiel das SOL: In "Noten" ist der Wert 3/2 = 1.5,
in "Halbtönen" ist es gleich 14983/10000 = 1.4983. Wie Sie am Wert sehen
ist es fast das gleiche (der Unterschied ist vernachlässigbar).
In "Noten" habe ich über die "klassischen" Brüche berichtet, die in vielen 
Bücher über akustische Physik gefunden werden können, die den Vorteil haben, 
kleine und leicht zu merkende Zähler und Nenner zu haben.
Die Werte der "Halbtöne" haben stattdessen zusätzlich zu den Verhältnissen
aller Noten auf der Halbtonskala eine Genauigkeit von 4 Nachkommastellen. 
Es wird erreicht, indem mit sehr großen Zahlen multipliziert und dann durch 
...
Einmal muss der gewünschte Halbton ab DO1 angegeben werden.
Wenn wir also ein FA1 spielen wollen, müssen wir in d0.w = 5 setzen.
Zwischen dem DO1 und dem FA1 gibt es 5 Halbtöne Unterschied.
In der Musik ist 1 Ton = 2 Halbtöne, und jede Tonleiter hat 6 Töne = 12 Halbtöne.
Zwischen einer Note und der anderen liegt 1 Ton, ohne die Frequenzintervalle
zwischen MI und FA und zwischen SI und DO der Oktave, danach sind sie nur
noch gleich 1 Halbton.
...
nicht konstant, aber EXPONENTIEL mit Basis 2.
Also die Berechnung der Verhältnisse der Noten innerhalb einer Skala ist
dann so einfach wie es scheint: 
Es gilt zu wissen, dass das Intervall von Verhältnissen in einer Skala von 
12 Halbtönen gleich 1 ist (von 1 des ersten DO bis 2 des DO der nächsten Oktave).
Jeder Halbton ist 1/12 in der Abszissen-Achse eines kartesischen Graphen
(x, y) zum Nächsten, der die Funktion darstellt. exponentiell: Y = 2^X.
Betrachten Sie das Intervall 0 <= X <= 1, in dem wir uns befinden.
Ordne einen Kurvenzweig 2^0<=Y<=2^1 = 1<=Y<=2 zu. Jetzt werden alle 12
relativen Werte von Y beginnend von X = 0 berechnet. 
...
Dieser Dezimalwert muss in einen Bruch umgewandelt werden.
In der Tat MUSS es zu einem Bruch mit Zahlen zurückverfolgt werden 
(nicht dezimal), da der 68000 nur ganze Zahlen "simulieren" kann (d.h. ohne Komma).
Um zum Beispiel die Beziehung zwischen einem LA# und DO (= 1/1) zu kennen:

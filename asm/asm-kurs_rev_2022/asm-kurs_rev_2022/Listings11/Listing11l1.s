...
;  Listing11l1.s - ändere color0 und bplcon1 in jeder Zeile ($dff102)
...
; würden Sie bekommen so, wenig Optimierung das wäre es nicht wert ...
...
RullaLoop:
	move.w	(a0)+,d1			; nächster Tabellenwert Bplcon1 nach d1
	cmp.w	#-2,d1				; Ende Tabelle?
...
; erste Schleife, die der ntsc-Teil ist (erste $ff-Zeilen)
	...
	lea	FineTabCol(PC),a1		; Ende der Farbtabelle
...
initd:
	...
	lea	FineTabCol(PC),a1		; Beginne andernfalls am Ende der Farbtabelle
	move.w	-(a1),d1			; Lies die Farbe und gehe zurück
initc:
...
ContaNumLoop1:	dc.w	0		; Zähler Anzahl Loops	
Contatore1:	dc.w	0			; Zähler 1
Contatore2:	dc.w	0			; Zähler 2
...
TESTO:	     ;		  1111111111222222222233333333334
	...
	dc.b	'                                        ',0 ; 1
	dc.b	'    Questo listato cambia ad ogni       ',0 ; 2	Dieses Listing ändert mit jeder
	dc.b	'                                        ',0 ; 3
	dc.b	'    linea sia il color1 ($dff184),      ',0 ; 4	Zeile color1 ($dff184),
	dc.b	'                                        ',0 ; 5
	dc.b	'    che il bplcon1 ($dff102). Notate    ',0 ; 6	sowie die bplcon1 ($dff102). Notiz
	dc.b	'                                        ',0 ; 7
	dc.b	'    come si possano "unire" listati     ',0 ; 8	wie Sie Einträge "zusammenführen" können
	dc.b	'                                        ',0 ; 9
	dc.b	'    visti in precedenza in un solo      ',0 ; 10	zuvor in einem Effekt gesehen
	dc.b	'                                        ',0 ; 11
	dc.b	'    effetto. Si potrebbero cambiare     ',0 ; 12	Sie könnten 
	dc.b	'                                        ',0 ; 13
	dc.b	'    anche altri colori e i moduli per   ',0 ; 14   auch andere Farben und Modulo
	dc.b	'                                        ',0 ; 15
	dc.b	'    ogni linea, se avete voglia         ',0 ; 16   jede Zeile ändern, wenn Sie Lust dazu haben
	dc.b	'                                        ',0 ; 17

...
Waity2:
	...
	bsr.s	Scorri				; führt die Scroll-Routine aus

	bsr.w	CancellaSchermo		; Reinigt den Bildschirm
	bsr.w	Sine				; führt die Sinus-Bildlauffunktion aus
...
WBlit3:
	...
	move.w	#(64*63)+20,$58(a5)	; BLTSIZE (Blitter starten!)
								; lösche von Zeile 130 bis Zeile 193
	rts
...
von 2 Pixel. Dafür muss er 1 Wort weite Blitts durchführen. Jede Wortspalte
enthält 8 "Scheiben" mit einer Breite von 2 Pixeln. Also die Routine hat 2
verschachtelte Schleifen: Die innerste führt 8 Kopieraufrufe auf die ganze
Wortspalte aus, während die äußere die innere für alle Bildschirmwortspalten
wiederholt. So wählen Sie die "Scheiben" aus. Innerhalb des Wortes verwenden
wir eine Maske, die in D5 enthalten ist. Nach jedem Blitt wird die Maske
gedreht, um jedes Mal eine andere "Scheibe" auszuwählen. Da die aus einer
...
Tabelle gelesen wird. Die Bildschirmlöschroutine löscht nur den betroffenen
Teil aus der Zeichnung. Um zu berechnen, welcher Teil betroffen ist, muss man
die minimale und maximale y-Koordinate der "Scheiben" berücksichtigen, und
auch dass die "Scheiben" 20 Pixel hoch sind. Die erste zu löschende Zeile ist
also das minimale Y der "Scheiben", während die letzte Zeile die Summe des
maximalen Y und der Höhe der "Scheiben" ist.

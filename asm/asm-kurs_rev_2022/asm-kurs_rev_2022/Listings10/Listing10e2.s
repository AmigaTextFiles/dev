...
; Listing10e2.s		Löschen und Zeichnen mit coppermonitor
...
;	`#----' `-----'
	...
	move	#$ff0,$180(a5)		; gelber Bildschirm, bis Blitter beginnt								
...
WBlit_coppermonitor:
	...
	move.w	#$000,$180(a5)		; schwarzen Hintergrund zurücksetzen, wenn
								; der Blitter das Zeichnen abgeschlossen hat
	bra.s	mouse
...
für das Markieren der Geschwindigkeit (der benötigten Ausführungszeit) der
Routinen, die der Blitter und der Prozessor für die Arbeit verwendet.
Wenn die Löschroutine beginnt, wird der Bildschirm grün. Jetzt sind, sowohl der
Blitter als auch der Prozessor an der Löschoperation beteiligt. Wenn der
Prozessor die Löschroutine beendet hat, ist der Blitter noch nicht mit seiner
Arbeit fertig geworden. Aber der Prozessor ist bereits frei für andere
Aufgaben. Diese Situation wird hervorgehoben, indem der Bildschirm am Ende der
Routine "CancellaOggetto" mit einem dunkleren grün eingefärbt wird. An dieser
Stelle betritt der Prozessor die Routine "DisegnaOggetto". (während der Blitter
immer noch beschäftigt ist) Bevor der Zeichenvorgang beginnt, ist es notwendig,
auf den Blitter zu warten. Wenn der Blitter endlich fertig ist, ist der
Bildschirm rot. Nach Verlassen der Routine "DisegnaOggetto" erscheint der
Bildschirm in dunkelrot, das darauf wartet, dass der Blitter das Zeichnen 
beendet. Wenn dies geschieht, wird der Bildschirm wieder schwarz.

...
; Listing10l2.s		"Vorwärts / Rückwärts" -Animation mit dem Blitter
...
Animazione:
	...
	tst.b	Direzione			; Überprüfe die Markierungsrichtung
	beq.s	Avanti				; wenn Flag = 0 geht es weiter
	...
	CLR.B	Direzione			; Ja, dann die Richtung wechseln
AncoraIndietro:
	BRA.S	NonCambiare
...
Avanti:	LEA	FRAMETAB(PC),a0		; Tabelle mit den Bildern
	...
	MOVE.L	d0,4*4(a0)			; Stelle die erste Adresse auf den achten Platz

	CMP.L	#FRAME5,(A0)		; Haben wir den letzten Rahmen im Kopf?
	BNE.S	AncoraAvanti		; Nein, geh zurück
	MOVE.B	#-1,Direzione		; Ja, dann die Richtung wechseln
AncoraAvanti:
...
mit dem Blitter. Die Animation besteht aus 5 Frames. Diese Frames müssen
zuerst vom ersten bis zum letzten und unmittelbar danach in umgekehrter 
Reihenfolge gezeigt werden, bis sie zum Ersten zurückkehren. Die Reihenfolge
ist daher die folgende: 1-2-3-4-5-4-3-2-1-2-3- etc. Um diese Reihenfolge zu
erreichen, haben wir eine darauf basierende Animationsroutine. Im Status
eines Flags scrollt es die Adressen in der Tabelle vorwärts oder rückwärts
bis der erste oder der letzte Frame erreicht ist. Ist der erste Frame
erreicht wird der Flaggenstatus umgekehrt und bestimmt die Umkehrung der
Animationsrichtung. Die Zeichenroutine ist identisch mit der der in 
Listing10l1.s
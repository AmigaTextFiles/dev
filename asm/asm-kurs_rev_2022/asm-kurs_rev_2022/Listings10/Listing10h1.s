...
CollLoop:
	...
	lea	40*256(a0),a0			; Zeige auf die nächste Ebene auf dem Bildschirm
...
WBlit_coll2:
	...
	beq.s	SiColl				; wenn das Flag nicht Null ist, gibt
								; es eine Kollision zu melden.
								; ansonsten überprüfe die nächste bitplane

	dbra	d7,CollLoop			; wenn für keine bitplane eine
...
; Er hat die gleichen Abmessungen wie ein Blitt: Höhe 11, Breite 3 Wörter
; 3 Bitebenen
...
Zero-Flag des Blitters. Die Technik ist die Folgende: Wir führen einen Blitt
durch, der ein UND zwischen der Maske des Bobs und der Maske des Hintergrund
macht, aber ohne das Ausgabeergebnis zu schreiben. Wenn die UND-Verknüpfung als
Ergebnis eine Null für alle Bits des Blitts liefert, nimmt das Zero-Flag den
...
übereinstimmt, also gibt es keine Kollision.
Wenn stattdessen mindestens ein Bit der Maske mit einem Bits des Hintergrunds 
übereinstimmt, bedeutet dies dass es eine Kollision gab. In diesem Fall wie
beim UND zwischen diesen 2 Bits ist das Ergebnis 1, das ZERO-Flag nimmt den
Wert 0 an und an diesem können wir erkennen, dass es eine Kollision gegeben
hat.
...
Hintergrundmaske könnten wir die Kollision mit nur einem blitt zwischen den
Masken testen. Machen Sie es als Übung.
Beachten Sie auch, dass Sie vor dem Testen des Zero-Flags auf das Ende des
Blitts warten müssen.
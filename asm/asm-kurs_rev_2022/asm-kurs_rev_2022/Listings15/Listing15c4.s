...
; Listing15c4.s		Zweiter 24-Bit-Fade-Test. Nun die Tabelle
		; enthält die Farben, die bereits in niedrige und hohe nibbles
		; "konvertiert" wurden, um sie in die Coplist zu setzen.
...
MainFadeInOut:
	BSR.w	MettiColori2		; Ordnen Sie die Farben dieses frames
								; von der Tabelle nehmen (bereits konvertiert)
	BTST.b	#1,FlagFadeInOut	; Fade In ooder fade Out?
	BNE.S	FadeOut
FadeIn:
...
; Konvertieren niedriger Nibbles von $00RrGgBb (long) in die Farbe AGA $0rgb (word)
	...
	MOVE.B	1(A2),(a1)			; Hohes Byte der Farbe $00Rr0000 kopiert
								; in das Cop-Register für niedriges nibble
	ANDI.B	#%00001111,(a1)		; auswählen nur niedriges nibble ($0r)
	move.b	2(a2),d2			; Byte $0000Gg00 aus der 24-Bit-Farbe nehmen
	...
	move.b	3(a2),d3			; Byte $000000Bb aus der 24-Bit-Farbe nehmen
	ANDI.B	#%00001111,d3		; auswählen nur niedriges nibble ($0b)
	or.b	d2,d3				; "MISCHEN" der niedrigen nibble von grün und blau...
	move.b	d3,1(a1)			; Bilden des nachfolgenden Low-Bytes $gb zum Setzen
...
; Konvertieren hohe Nibbles von $00RrGgBb (long) in die Farbe AGA $0RGB (word)
	...
	MOVE.B	1(A2),d0			; Hohes Byte der Farbe $00Rr0000 in d0
	...
	move.b	d0,2(a1)			; das High-Byte $0R in das Farbregister kopieren
	move.b	2(a2),d2			; das Byte $0000Gg00 aus der 24-Bit-Farbe nehmen
	ANDI.B	#%11110000,d2		; auswählen nur hohes nibble ($G0)
	move.b	3(a2),d3			; das Byte $000000Bb aus der 24-Bit-Farbe nehmen
	...
	move.b	d3,1+2(a1)			; Bilden des letzten Low-Bytes $GB zum Setzen
								; im Farbregister nach dem Byte $0R für
...
MettiColori2:
	MOVE.L	ActualFadeTab(PC),A0	; Adresse Farbpalette in al
								; aktueller Zeiger der fade in der TAB
...
MettiBank:	; Schleife, die eine Bank von 32 Registern setzt

	move.l	(a0)+,d0			; Kopie der 2 word $0rgb0RGB in d0

; nibble hoch

	move.w	d0,(a1)				; Kopie $0RGB

; nibble niedrig

	swap	d0
	move.w	d0,(a2)				; Kopie $0rgb
	...
	dbra	d7,Fai8Banchi		; Konvertiert eine Bank mit 32 Farben von
...
Diesmal werden die Wörter direkt in der Tabelle COLORTABBY gespeichert
die Register, seit der Umwandlung von:

	$00RrGgBb	zu	$0rgb0RGB

da die Umwandlung von dem langen mit der 24-Bit-Farbe in das Wortpaar von 
Registern ebenfalls vorberechnet wird.
Auf diese Weise muss die Routine, die "verblasst", nur die Wörter kopieren,
ohne Konvertierung, und es ist schneller: Sie können mehr Operationen
ausführen während das Überblenden (Fade) erfolgt.

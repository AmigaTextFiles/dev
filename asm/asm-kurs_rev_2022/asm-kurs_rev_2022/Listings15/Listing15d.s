...
; Listing15d.s		Wir zeigen das erste Bild in 256-Farben-Einstellungen an 
				; mit dem 64-Bit-Pump-Burst (FMODE, $1fc = 3)
...
POINTB:
	...
	bsr.w	FaiAGACopB			; BLAUEN Farbton machen 
...
;*****************************************************************************
; Routine zum Erstellen von BLAUEN AGA-Farbtönen:
;
; d1 = Erste zu wartende Zeile (Wait, z.B. $2c07fffe pro Zeile Y=$2c)
; d5 = Anfangs Farbton ($00-$ff)
; d7 = Anzahl Zeilen zu erledigen
;*****************************************************************************
...
FaiAGALoopB:
	move.l	d1,(a0)+			; Lege das wait YYXXFFFE
	add.l	#$01000000,d1		; eine Zeile tiefer für die nächste
	move.l	d4,(a0)+			; BplCon3 - auswählen nibble hoch
	move.w	d2,(a0)+			; Register Color0
	addq.b	#1,d5				; "Hellt" die $Gg-Farbe leicht auf
	move.w	d5,d6				; Kopie in d6
	and.w	#%11110000,d6		; auswählen nur nibble hoch
	lsr.w	#4,d6				; an der richtigen Position, d.h. BLAU $xxB
	move.w	d6,(a0)+			; Wert für Color0 (nibble hoch)
	move.l	d3,(a0)+			; BplCon3 - auswählen nibble niedrig
	move.w	d2,(a0)+			; Register Color0
	move.w	d5,d6				; Farbe $xx in d6
	and.w	#%00001111,d6		; auswählen nur nibble niedrig - Position $xxB
	move.w	d6,(a0)+			; Farbe setzen in copperlist (nibble niedrig)
	dbra	d7,FaiAGALoopB
	rts
...
AgaCopEff1:
	dcb.l	99*5		; d.h.: 99 Zeilen * 5 long:
						; 1 für wait,
						; 1 für bplcon3
...
Beachten Sie in der copperliste, dass die Modulo -8 sind, da FMODE ($1fc) 64-Bit
ist. Versuchen Sie, den CNOP 0,8 vor dem PICTURE-Label mit einem schönen 
Semikolon zu entfernen und Sie werden feststellen, dass das Assemblieren des
Bildes zu einer Adresse die nicht auf 64 Bit ausgerichtet ist, dazu führt, das
sie eine vertikale Schnittansicht erhalten.
Ein weiteres Detail ist, dass bei aktivem Burst müssen Sie den Wert von 
DDFSTART und DDFSTOP auf normal hires einstellen, dh $003c und $00d4,
aber gleichzeitig bei lowres Wert, da "pumpt" es schnell .

	dc.w	$92,$0038	; DdfStart lowres, geeignet für HIRES mit burst
	dc.w	$94,$00d0	; DdfStop lowres, wie oben

Wir mögen den copper blauen Farbton im Hintergrund, richtig? Zumindest gehen
wir zu 256 + 99 angezeigte Gesamtfarben, d.h. 355.

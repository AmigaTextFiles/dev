...
; Zeiger auf sprites
	...
	MOVE.L	#BBUFFER,D0			; Adresse, an die die Sprites kopiert werden
...
Ploop:
	...
	bsr.w	sprite_copy			; Kopieren Sie das Sprite 21 Mal in den Puffer
...
Waity2:
	...
	btst	#2,$16(A5)			; rechte Taste gedrückt?
	beq.s	NonSpri
...
*****************************************************************************
; Routine, die die Sprites gemäß den Tabellen animiert
*****************************************************************************

doanim:
	subq.w	#1,countera			; waitcounter -1
	tst.w	countera			; sind wir bei 0 angekommen?
	bne.s	StessaAnim			; Wenn noch nicht, ok... andernfalls...
	move.l	actionptr(PC),a0	; Nächste Animation von der Tabelle!
	cmp.w	#-1,(a0)			; sind wir am Ende der Tabelle?
	bne.s	Nonripart1			; Wenn noch nicht, ok
	move.l	#action,actionptr	; ansonsten von vorn anfangen
	move.l	actionptr(PC),a0	; von der ersten Animation...
Nonripart1:
	move.w	(a0),countera		; Dauer der Animation
	...
	add.l	#14,actionptr		; Stellen Sie den Zeiger auf die nächste Animation
StessaAnim:
...
nomax2:
	...
	mulu.w	#68,d7				; aktuelles sprite*68 = offset für richtiges sprite
	lea	BBUFFER+7*68,a1			; Adresse letztes sprite
	sub.l	d7,a1				; Wir gehen, um das Sprite zu finden
	moveq	#0,d3				; 0 = nicht attacched
	moveq	#16,d2				; Höhe sprite
...
Copy_loop:
	...
	subq.w	#1,d0
	bne.s Copy_loop
	dbra d1,sprite_copy			; mache d1 Kopien
	rts
...
*****************************************************************************
; Tabelle SIN/COS für gekrümmte Flugbahnen
*****************************************************************************
...
; Farben der sprites - von color17 bis color31
	...
	dc.w	$1a2,$4d0		; Farben sprite 0,1 (grün)
	dc.w	$1a4,$ad8
	dc.w	$1a6,$ffe
	dc.w	$1a8,$4f0

...
; Listing14-2b.s	** SPIELE EINE HARMONISCHE IN VERSCHIEDENEN NOTEN 2 **
...
verdoppelt. Wenn also DO der ersten Oktave 131 Hz hat, so hat DO2 262 Hz, das
DO3 524 Hz usw. Innerhalb der Skala gibt es sehr genaue Zusammenhänge zwischen
den Frequenzen der 7 Noten: DO=1, RE=9/8, MI=5/4, FA=4/3, SOL=3/2, LA=5/3,
SI=15/8 (und das nächste DO = 2). Mit dieser Tabelle ist es sehr einfach
möglich die Frequenz jeder Note eines Oktavenbeginns von einer Note einer
Oktave zu berechnen.
Das Unterprogramm "Note2Per" möchte als Eingabeparameter: in das hohe Wort von
d0 die Note (von 0 für den DO bis 6 für den SI) und die Oktave (von 0 für die
erste bis 3 für die vierte) im Low-Word von d0 und in d1 die Länge des Samples
in Bytes. Es berechnet die in das AUDxPER-Register einzugebende Abtastperiode
und muss nur die Frequenz der Note DO1 und die gewünschte Note kennen.
Wie funktioniert es? Einfach: Beachten Sie zunächst die Beziehungen zwischen
den Noten in Bezug auf das DO. Jeder Note sind Wortdaten zugeordnet. Der erste
Wert gibt den Zähler des Bruchs an und der zweite den Nenner. Die Routine
verdoppelt zuallererst die Frequenz des DO1 um den Wert im tiefen Word von
Parameter d0, welcher die Oktave angibt, durch einfaches Verschieben nach links
(Multiplikation mit jeweils * 2). Durch Einfachverschiebung wird der Wert 131
(Frequenz des DO1) mit so vielen Bits wie der Wert in d0lo.w enthält nach links
verschoben. Dann, basierend auf den in d0hi.w angegebenen Wert, nimmt es ab
Noten + d0hi.w * 4 ein Longword mit dem Zähler im oberen Word und dem Nenner
des Bruchs im unteren Word, der das Verhältnis der Note angibt. Damit wird die
Frequenz der gewünschten Note durch Multiplikation mit dem Bruch berechnet.
Zuerst durch Multiplikation mit dem Zähler des Bruchs (High Word) und dann
dividieren wir alles durch den Nenner des Bruchs. (Low word)
...
dividiert durch das Produkt zwischen der Länge des Samples und der Frequenz der
Note.
...
Frequenz abzuspielen. Einfach, weil das Produkt von Länge und Frequenz durch
die Zeitkonstante geteilt werden muss und daher der Teiler der DIVU in einem
Word sein muss (glücklicherweise ohne Vorzeichen).
** In der Praxis schadet diese Einschränkung jedoch niemandem, da die
Lesegeschwindigkeit = Notenfrequenz * Samplelänge nicht 28836 Hz überschreiten
kann, ein Wert, der bequem in ein Wort passt:

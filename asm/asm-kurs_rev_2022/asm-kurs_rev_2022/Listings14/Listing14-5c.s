...
; Listing14-5c:  ** SPIELT SAMPLE SEHR LANG **
...
AttnFlags	equ	296
	...
	move.w	d7,$9c(a6)			; löschen Sie keine IRQ
...
PlayLongSample_restore:
	...
	move.w	#1<<7,d1
	lsl.w	d3,d1
	move.w	d1,$9c(a6)			; löscht alle IRQs der Stimme
	move.w	d1,$9a(a6)			; schaltet INT der Stimme aus
	move.w	d0,$96(a6)			; schaltet DMA der Stimme aus
	move.w	$1c(a6),d0
	...
	lea	.getvbr(pc),a5
	jsr	_LVOSupervisor(a6)
.No010:	move.l	oldlv4(pc),$70(a0)	; ... wiederherstellen alten Autovektor
.NoOFF:	lea	$dff000,a6
	add.w	d3,d3				; d3=d3*2: drückt word Offset aus
...
Lv4IRQ:	
	move.w	d3,-(sp)
	link	a0,#0	
.IRQLp:	pea	.irqlp(pc)			; push return für RTS auf dem stack
	moveq	#3,d3

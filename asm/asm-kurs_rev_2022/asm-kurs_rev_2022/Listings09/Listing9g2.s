...
1) In der Adressberechnung, um den Offset zwischen dem ersten Wort der Zeile Y
   und dem Anfang der Bitebene zu erhalten, müssen wir Y mit der Anzahl der
   Bitebenen multiplizieren, sowie der Anzahl der Wörter pro Zeile.   
   Bezüglich X, gibt es jedoch keine Unterschiede.
   ; OFFSET = (Y * (Anzahl der Wörter pro Zeile) * (Anzahl der Ebenen)) * 2

2) Die Höhe des Blitts ist gleich der Höhe des Bildes multipliziert mit der
   Anzahl der Bitebenen. Für die Breite gibt es jedoch keine Unterschiede.

Auch in Bezug auf die anderen Register, auch für das Modulo, gibt es keine
Unterschiede.

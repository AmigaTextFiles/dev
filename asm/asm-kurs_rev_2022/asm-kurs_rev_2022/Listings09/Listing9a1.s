...
; Listing9a1.s		Löschen von $10 Wörter mit dem BLITTER
...
eine Schleife von "clr.l (a0)+" zurückgesetzt werden.

In diesem Fall, assemblieren sie, ohne zu starten, und überprüfen erst mit
einem "M START" das unter dem Label $20 Bytes "$fe" vorhanden sind. An diesem
Punkt führen wir das Listing aus und aktivieren zum ersten Mal im Kurs den
Blitter. Danach wiederholen Sie "M START" und Sie werden bestätigen, dass diese
Bytes bis zum Label THEEND gelöscht wurden. In der Tat mit einem "M THEEND"
finden Sie immer seinen Platz. Die Löschoperation erfordert nur die Verwendung
des Kanals D. Außerdem müssen alle MINTERMS zurückgesetzt werden. Daher ist der
Wert, der im BLTCON0-Register geladen werden muss $0100.

Beachten Sie den Wert, der in dem BLTSIZE-Register geschrieben wird. Wir wollen
ein Rechteck, das 10 Wörter breit und eine Zeile hoch ist löschen. Wir müssen
die Breite immer in die Bits 0-5 von BLTSIZE und die Höhe in die Bits 6-15 von
BLTSIZE schreiben. Um die Höhe in die Bits 6-15 zu schreiben, können wir sie
6 Bits nach links verschieben, was einer Multiplikation mit 64 entspricht. Die
Größe des Rechtecks, das durch das BLTSIZE-Register zu löschen ist, verwendet
die folgende Formel:

In BLTSIZE = (Höhe * 64) + Breite ist der zu schreibende Wert.
...
Meisterwerken verwenden, müssen sie OwnBlitter am Anfang und DisownBlitter am
Ende hinzufügen, zusätzlich zum bekannten Deaktivieren und Aktivieren des
Multitasking.

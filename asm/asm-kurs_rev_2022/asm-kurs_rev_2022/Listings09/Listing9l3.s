...
; Listing9l3.s		Kopieren eines Rechtecks zwischen 2 sich überlappenden
			; Bereichen mit dem DESCENDING Modus.
...
; Verwenden des Descending-Modus, um die Kopie korrekt zu erstellen
;****************************************************************************
...
; ANMERKUNG DER UNTERSCHIED IM HINBLICK AUF DEN NORMALEN FALL IST:
; Die Adressen sind die Wörter, die sich am weitesten in der unteren rechten
; Ecke der Rechtecke befinden.
; Wenn Xa und Ya die Koordinate der oberen linken Ecke ist, ist die Koordinate
; Yb die Zeile, zu der die untere Zeile des Rechtecks ​​gehört:
; Yb = Ya + RECHTECKHÖHE
; Daher ist bei der Berechnung der Adresse der zu Y gehörende OFFSET gegeben
; durch: OFFSET_Y = (Yb * (Anzahl Wörter pro Zeile) * (Anzahl Ebenen)) * 2.
; Der Versatz relativ zu X wird stattdessen berechnet, indem dies beobachtet
; wird: Xa + RECHTECKBREITE ist die X-Koordinate des ersten Pixels des Wortes
; Es befindet sich außerhalb des Rechtecks ​​unmittelbar rechts. Der OFFSET
; dieses Wortes ist daher ((Xa + RECHTECKBREITE) / 16) * 2. Aber das
; interessiert uns nicht, sondern dasjenige, das ihm vorausgeht, oder das
; letzte Wort rechts vom Rechteck, dessen OFFSET ist daher: 
...
; Wie wir erklärten, gibt es keine Unterschiede im Vergleich zum normalen Fall!
...
Zonen. Die Adresse des Ziels ist größer als die der Quelle (auf dem Bildschirm
ist das Ziel niedriger) und deshalb benutzen wir den absteigenden Weg.
...
Berechnung der Adressen der Zeiger der DMA-Kanäle, die für die Berechnung 

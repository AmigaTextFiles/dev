...
Waity1:
	...
	beq.s	SaltaPulizia		; Wenn ja nicht "reinigen"
...
*																		*
*	BSR.s INITCOPPER ; vor dem Zeigen auf die Copperlist durchführen	*
*					 ; erstellen der copperlist (bestehend				*
					 ; aus WAIT und COLOR0)								*
*																		*
*	BSR.s CLRCOPPER	 ; ausführen um alte Balken zu löschen				*
*					 ; "Schwärzen" aller COLOR0 in der copperrlist		*
*					 ; HINWEIS: Sie können die Hintergrundfarbe ändern  *
*					 ; durch Änderung einwirken Hintergrund = $xxx		*
*																		*
...
initloop:
	...
	add.l	#$02000000,d1		; nächstes wait, 2 Zeilen tiefer warten
	dbra	d0,initloop
	rts
...
; DOBARS führt das "Scrollen" der farbigen Balken durch, einen nach dem anderen,
; Aufruf des Unterprogramms PUTBAR für jeden Balken
...
putloop:
	...
	addq.w	#8,a2				; zum nächsten Wert von color0 gehen
...
; Dies sind die Positionen der Balken relativ zueinander. Wie sie sehen können
...
barcopper:						; Hier wird die copperliste erstellt für
	dcb.w	coplines*4,0		; die Wirkung der Balken - in diesem Fall
								; werden 400 Wörter benötigt. (Coplines = 100)
...
Dieses Listing zeigt, wie Sie lange, aber regelmäßige Copperlisten mit 
Routinen "erstellen" können. Später werden wir sehen, das hinter den meisten
spektakulären Effekten sich oft kilometerlange Copperlisten verstecken.

Änderungsvorschlag: Um das Ganze etwas "flacher" zu machen, warten sie in
jeder Zeile und nicht alle zwei Zeilen. Ändern Sie einfach INITCOPPER:

	add.l	#$01000000,d1		; nächstes wait, warte 1 Zeile tiefer

Jetzt sind die Balken 13 Zeilen hoch und nicht 13 * 2 Zeilen!
Sie können sie auch alle 3 Zeilen warten lassen, aber dadurch gehen Sie zu weit
runter, aber versuchen Sie es trotzdem:

	add.l	#$03000000,d1		; nächstes wait, warte 3 Zeilen tiefer

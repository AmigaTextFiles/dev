...
; Listing8f.s	Eine FADE-Routine (dh Ein-und Ausblenden)
;				von und nach JEDER FARBE
; Drücken Sie abwechselnd die linke und rechte Taste, um die verschiedenen Optionen 
; Übergänge anzuzeigen
...
;	********** fünftes Fade: von FARBEN zu anderen verschiedenen Farben! *
...
* Label FaseDelFade als d0 für vorherige Routinen verwendet,				*
* = Moment der Überblendung, Multiplikator, in diesem Fall ist es jedoch    *
* notwendig, ihn zurückzusetzen. Einfach, um eine neue Überblendung		    *
* zu starten																*
...
dofade:
	cmp.w	#17,FaseDelFade		; Haben wir die letzte Phase erreicht? (16)?
	beq.s	FadeFinito
...
f2main:
	...
	addq.w	#2,a2				; das Gleiche gilt für die Ziel-Farbtabelle
	...
	clr.w	ColoreFinale		; die endgültige Farbe zurücksetzen

; BLAU

	move.w	(a1),d0				; aktuelle Farbe der Quell-Farbtabelle in d0
	move.w	(a2),d2				; Farbe der Ziel-Farbtabelle in d2
	and.l	#$00f,d0			; nur das BLAU von der Farbe der Quelltabelle auswählen 
...
SottFatto2:
	...
	move.w	(a1),d0				; aktuelle Farbe der Quell-Farbtabelle in d0
	move.w	(a2),d2				; Farbe der Ziel-Farbtabelle in d2
	and.w	#$00f,d0			; nur das BLAU von der Farbe der Quelltabelle auswählen
	and.w	#$00f,d2			; Farbe der Ziel-Farbtabelle
	cmp.w	d0,d2				; sind BLAU Quelle und Ziel gleich?
	bhi.b	SommaD1				; wenn d0>d2, Summe d1 + d0
...
OkBlu:
	move.w	d0,ColoreFinale		; das endgültige BLAU speichern

; GRÜN

	move.w	(a1),d0				; aktuelle Farbe der Quell-Farbtabelle in d0
	move.w	(a2),d2				; Farbe der Ziel-Farbtabelle in d2
	and.l	#$0f0,d0			; nur das GRÜN von der Farbe der Quelltabelle auswählen
...
SottFatto2v:
	moveq	#16,d1
	bsr.w	dodivu
	and.w	#$0f0,d1			; nur GRÜN auswählen
	move.w	(a1),d0				; aktuelle Farbe der Quell-Farbtabelle in d0
	move.w	(a2),d2				; Farbe der Ziel-Farbtabelle in d2
	and.w	#$0f0,d0			; nur das GRÜN von der Farbe der Quelltabelle auswählen
	and.w	#$0f0,d2			; Farbe der Ziel-Farbtabelle
	cmp.w	d0,d2				; sind GRÜN Quelle und Ziel gleich?
	bhi.b	SommaD1v			; wenn d0>d2, Summe d1 + d0
...
SommaD1v:
	add.w	d1,d0				; d0=d0+d1
OkVERDE:
	or.w	d0,ColoreFinale		; mit dem OR die grüne Komponente verbinden
...
SottFatto2r:
	moveq	#16,d1
	bsr.w	dodivu
	and.w	#$f00,d1			; nur ROT auswählen
	move.w	(a1),d0				; aktuelle Farbe der Quell-Farbtabelle in d0
	move.w	(a2),d2				; Farbe der Ziel-Farbtabelle in d2
	and.w	#$f00,d0			; nur das ROT von der Farbe der Quelltabelle auswählen
	and.w	#$f00,d2			; Farbe der Ziel-Farbtabelle
	cmp.w	d0,d2				; sind ROT Quelle und Ziel gleich?
	bhi.b	SommaD1r			; wenn d0>d2, Summe d1 + d0
...
SommaD1r:
	add.w	d1,d0				; d0=d0+d1
OkROSSO:
	or.w	d0,ColoreFinale		; mit dem OR die rote Komponente verbinden

; die Farbe in der copperliste eingeben!

	move.w	ColoreFinale(PC),(a0)	; und die endgültige Farbe in die copperliste legen!

ProssimoColore:
	dbra	d6,f2main			; Wiederholen für jede Farbe
...
Cend4:
	...
	dc.w $5b9,$6db,$7dc,$111,$222,$433,$b99,$644	; Farben! (grüner Ton)
;=----------
...
Funktionsprinzip ist daher komplexer als bei einer normalen Überblendung.
Verstehen Sie einfach, wie sie es benutzen können. Wenn Sie Ihr Gehirn braten

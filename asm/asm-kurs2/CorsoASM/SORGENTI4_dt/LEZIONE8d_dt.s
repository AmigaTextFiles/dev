...
; Lezione8d.s Eine FADE-Routine (dh Fade) von und nach SCHWARZ. ROUTINE Nr.2
...
* Um ein Einblenden von Weiß zu Farbe zu machen, müssen Sie in jedem *
...
;	Berechnen Sie die GRÜNE Komponente
	...
	OR.W	D4,D5		; Speichern Sie die GRÜNE Komponente in d5

;	Berechnen Sie die ROTE Komponente

	MOVE.W	(A0)+,D4	; Lesen Sie die Farbe aus der Tabelle
						; und auf die nächste Spalte zeigen
	AND.W	#$f00,D4	; Auswahl nur Komponente Rot ($RGB->$R00)
	MULU.W	D0,D4		; Mit der Fade-Phase multiplizieren (0-16)
	ASR.W	#4,D4		; Verschieben Sie 4 BITS nach rechts, dh dividieren Sie durch 16
	AND.W	#$f00,D4	; Auswahl nur Komponente Rot ($RGB->$R00)
	OR.W	D4,D5		; Speichern Sie die Farben ROT zusammen mit BLAU und GRÜN

	MOVE.W	D5,(A1)		; Und setzen Sie die endgültige $ 0RGB-Farbe in die copperliste
...
Diese Routine funktioniert genauso wie die vorherige, schreibt aber nicht die
Wortfarbe ein Byte nach dem anderen. Diese Routine eignet sich eher für Änderungen
wenn es AGA wird. Tatsächlich werden wir "AGhizzAta" in der AGA-Lektion sehen.
Das Thema der Multiplikation und Division der Komponenten R, G, B
wird für jede R, G, B Komponente auf ein Byte erweitert.

...
; Lezione8n.s - Punktdruckroutinen (Plot), optimiert durch Vorberechnung
...
Waity1:
	...
	bsr.s	PlotPIXP	; Drucken Sie den Punkt auf die Koordinate. X = d0, Y = d1
...
; Jetzt finden wir den vertikalen Versatz, das ist das Y, und nehmen den richtigen Wert 
; vorberechnet aus der Multab-Tabelle, deren Adresse in a1 steht
	...
	add.w	d1,d1		; Wir multiplizieren das Y mit 2 und finden den Versatz
						...
						; den ersten Wert der Tabelle an, der Null ist.
						; Wenn es 3 ist, nehmen wir den dritten Wert
						; der Tabelle, der jedoch das sechste Byte ist,
						; da wir 2 Bytes überspringen müssen, 1
						; Wort für jeden Wert in der Tabelle.
	add.w	(a1,d1.w),d0	; Wir addieren den rechten vertikalen Versatz,
...
Mit diesem Listing machen wir eine kleine Einführung in die Lektion der
Optimierungen, in der Tat "TABELLIAMO" eine Multiplikation. Diese Operation ist
sehr häufig im Code der schnellsten Demos oder in 3D-Spielen.
Unsere Pixeldruckroutine funktioniert einwandfrei, enthält jedoch eine
langsame Multiplikation. Die müssen wir unbedingt entfernen. Keine
Multiplikation mit einer Potenz von 2, kann nicht durch eine LSL ersetzt werden.
Wir haben es in der Druckroutine in Lektion 8b.s geschickt gemacht.
Die Möglichkeiten der Codierung sind jedoch unbegrenzt. Bedenken Sie die Situation,
die wir haben:

	mulu.w	#largschermo,d1
	add.w	d1,d0		; Offset vertikal bis horizontal

Die Bildschirmbreite ist in diesem Fall 40. In d1 haben wir jedes Mal einen anderen 
Wert, abhängig vom Y, aber wir wissen, dass es maximal von 0 bis 255 gehen kann.
Es gibt also 256 mögliche Ergebnisse, abhängig davon, ob einer der 256 
möglichen Werte von Y oder d1 auftritt. Diese 256 Ergebnisse, wenn wir jeweils 
einmal erhöhen, erhöht sich eine Zahl von 0 auf 255:

0,40,80,120,160,200	d.h.	40*0,40*1,40*2,40*3,40*4....

Stellen Sie sich vor, wir bereiten alle diese 256 möglichen Ergebnisse in einem 
mit Nullen zuvor vorbereiteten Raum "vor" :
...
PreCalcLoop
	move.w	d0,(a0)+		; Speichere das aktuelle Vielfache
	add.w	#LargSchermo,d0	; Wir addieren Bildschirmbreite, nächstes Vielfaches
	dbra	d7,PreCalcLoop	; Wir erstellen alle MulTab

Jetzt haben wir die Tabelle mit den "Ergebnissen" fertig. Aber wie "holen" wir
von der Tabelle jedes Mal das richtige Ergebnis? Im Eingang haben wir die Koordinate
Y oder eine Zahl von 0 bis 255. Wenn Y Null ist, nehmen Sie einfach den ersten Wert
von der Tabelle. Das ist das Wort $0000. Wenn stattdessen y = 1 wäre, müssen wir den 
zweiten Wert der Tabelle nehmen, der jedoch vom Anfang 2 Byte beträgt,
da seine Werte Worte sind. Ebenso, wenn wir das richtiges Ergebnis für die Koord. 
Y = 50 nehmen wollten. Das Ergebnis wäre das Fünfzigste Wort der Tabelle, in einem 
Abstand von 100 Bytes. Das alles ist nicht da. Ich schlag die Lösung vor. 
Berechnung des Offsets, dh des Abstands vom Anfang von der Tabelle multiplizieren 
Sie einfach das Y mit 2! Und da kann man es vervielfachen mit 2 durch add:

 	add.w	d1,d1

Wir sind immer ohne Multiplikation. Jetzt haben wir in d1 den Offset vom Anfang 
der Tabelle. Wir müssen es "nehmen" und es zu d0 hinzufügen. Das kann gemacht
werden mit einer einzigen Operation:
...
Wir werden dieses "Tabellensystem" immer öfter in den Listinga finden
die viele Berechnungen durchführen.

...
;	Lezione8p2b.s	Erweiterung des Zeichens in den Adressregistern

	SECTION	CondC,CODE

Inizio:
	move.l	#$ffffffff,a0	; dass "move.l #-1,a0"
	move.w	#$51a7,a0
stop:
	rts
...
In dieser Lektion werden wir uns mit einer anderen Besonderheit der 
Adressierung direkt zum Adressregister befassen.
...
Das Register A0 hat wie üblich den Wert $FFFFFFFF angenommen. Jetzt lass uns 
den zweiten Zug machen. Beachten Sie, dass ein 16-Bit-Wert in A0 geladen wird.
...
Dies geschieht, weil Sie beim Schreiben in ein Register ein WORT senden.
(Denken Sie daran, dass es NICHT möglich ist, ein einzelnes BYTE zu schreiben, 
d.h. die Anweisung MOVE.B xxxx,Ax ist NICHT erlaubt.)
LANGWORT durch eine Operation namens "Vorzeichenerweiterung", die besteht
beim Kopieren des höchstwertigen Bits des WORTES (dh Bit 15). Wie
Sie wissen, gibt das Vorzeichen eines WORD-Formatwerts in allen Bits des an
WORD hoch, um das gleiche Vorzeichen vom WORD-Wert zu erhalten
das LANGWORT. In der Praxis haben wir in unserem Fall:
...
In diesem Fall hat die Vorzeichenerweiterung den LONG WORD-Wert negativ gemacht:

...
; Lezione10c5.s	Reflektoreffekt auf interleaved Bildschirm
...
POINTBP:
	...
	move.w	d0,2(a1)
	swap	d0
	ADD.L	#40,d0			; INTERLEAVED! Höhe 1 Zeile 
	addq.w	#8,a1
	dbra	d1,POINTBP
...
ClearScreen:
	lea	BITPLANE1+100*40*5,a1	; zu löschende Adresse (plane1)
...
; von UND zwischen der Figur und einer Maske
;*****************************************************************************
...
Riflettore:
	lea	Figura+40*5,a0			; Adresse Figur
	lea	BITPLANE1+100*40*5,a1	; Adresse Ziel
...
WBlit2:
	...
	move.w	#(64*39*5)+4,$58(a5)	; BLTSIZE (Blitter starten!)
								; Breite 4 word
	rts							; Höhe 39 Zeilen
								; 5 planes
...
COPPERLIST:
	...
	dc.w	$102,0		; BplCon1
	dc.w	$104,0		; BplCon2
	dc.w	$108,160	; WERT MODULO = 2*20*(5-1)= 160
	dc.w	$10a,160
...
; Dies ist die Maske. Im interleaved Format besteht es aus 5 Ebenen
; 39 Zeilen hoch und 4 Wörter breit
...
In diesem Beispiel realisieren wir den "Reflektor" -Effekt mit einem 
interleaved Bildschirm. Die Technik ist die gleiche wie in Lektion10c4.s. Der 
einzige Unterschied ist dieses Mal, das wir einen interleaved Bildschirm 
verwenden. Daher können wir die Ebene zusammen blitten. Diese Tatsache (die 
wie wir wissen, die Geschwindigkeit erhöht) schafft uns aber Probleme mit 
der Maske. In der Tat, wenn wir alle Ebenen in einem abarbeiten, brauchen 
wir eine Maske für jede Ebene. In diesem Fall besteht unsere Maske (so wie
sie ist - interleaved) aus 5 gleichen Ebenen. In der Praxis ist sie wie die
Maske aus einer Ebene nur das wir jede Reihe 5 mal wiederholen. In diesem 
Fall zwingt uns das interleaved Format also eine 5-Ebenen-Maske zu verwenden, 
die mehr Speicher beansprucht als eine Maske von einer Bitplane.

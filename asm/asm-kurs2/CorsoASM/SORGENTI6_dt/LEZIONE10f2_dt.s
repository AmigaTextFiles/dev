...
; Lezione10f2.s	 Viele BOBs mit "Fake" und "Double Buffering" Hintergrund
...
START:
; Die ersten drei bitplanes sind auf die ScambiaBuffer-Routine ausgerichtet

; Lass uns die vierte Bitebene für (den Hintergrund) setzen
...
Ogg_loop:
	...
	addq.l	#8,a4				; Zeigen Sie auf das nächste Objekt
...
WBlit_coppermonitor:
	...
	bne.s	mouse				; wenn nicht, gehe zurück zur Maus:

	rts

;****************************************************************************
; Diese Routine tauscht die 2 Puffer durch den Austausch der Adressen der
; VIEW_BUFFER und DRAW_BUFFER Variablen aus.
...
ScambiaBuffer:
	move.l	draw_buffer(pc),d0			; Tausche den Inhalt 
	move.l	view_buffer(pc),draw_buffer	; der Variablen aus
	move.l	d0,view_buffer				; in d0 steht die Adresse
										; des neuen View-Puffers
									
; Aktualisieren Sie die copperliste, indem Sie auf die Bitebenen 
...
POINTBP:
	...
	move.w	d0,2(a1)
	swap	d0
	ADD.L	#40*256,d0	; + Bitplane Länge (hier 256 Zeilen)
	addq.w	#8,a1
	dbra	d1,POINTBP
...
UO_NoBounce2:
	btst	#15,d0				; Überprüfen Sie den linken Rand (X = 0)
...
DisegnaOggetto:
	move.l	draw_buffer(pc),a0	; Adresse Puffer Zeichnung
...
WBlit2:
	...
	move.w	d0,d1				; Kopierwert von BLTCON0,
	and.w	#$f000,d1			; Wählen Sie den Verschiebewert..
	...
	add.l	#4*31,a1			; nächstes Adressebene Bild
	add.l	#40*256,a0			; Adresse nächstes plane Ziel
	dbra	d7,DrawLoop
...
view_buffer	dc.l	BITPLANE1	; Puffer Anzeige
draw_buffer	dc.l	BITPLANE1b	; Puffer Design
...
In diesem Beispiel lösen wir das Problem des Beispiels lesson10f1.s durch
...
wird. Die Puffer werden bei jedem Vertical Blank ausgetauscht.
Die Adressen der 2 Puffer sind in 2 Variablen enthalten. Die Variable
draw_buffer enthält die Adresse des Puffers, der zum Zeichnen verwendet wird.
Die Routinen, die den Bildschirm ändern (Bildschirm löschen und Objekt 
zeichnen) liest die Zeichenpufferadresse aus draw_buffer und führen ihre 
Operationen darauf aus. Die Variable view_buffer enthält die aktuelle 
Pufferadresse die angezeigt wird.
Die ScambiaBuffer-Routine tauscht den Inhalt der 2 Variablen mit jedem 
vertical blank aus. Es stellt auch sicher, dass der richtige Puffer 
angezeigt wird (der, dessen Adresse in view_buffer gespeichert ist). 
In der Tat, um abwechselnd die 2 Puffer anzuzeigen, ist es erforderlich,
bei jedem frame die BPLxPT-register die auf die Bitebenen des
Anzeigepuffers zeigen in die copperliste zu bringen. Dies bedeutet,
dass die copperliste mit jedem vertical blank modifiziert wird, was von 
der ScambiaBuffer-Routine erledigt wird.

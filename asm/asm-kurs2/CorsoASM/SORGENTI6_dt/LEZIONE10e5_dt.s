...
; Lezione10e5.s		Löschung und Design mit Nasty Blitter
...
; 45 Zeilen hoch und 3 Bitplanes-Format
...
In diesem Beispiel zeigen wir den Effekt des Bits Nasty Blitter. Wie wir 
dieses Bit erklärt haben, wirkt es sich nur auf den Code im 
Speicherchip aus - und Prozessorinstruktions-Cache ist deaktiviert.
Um den Code in CHIP RAM zu laden, haben wir einen SECTION CODE_C angegeben.
Um den Cache zu deaktivieren (falls Sie einen haben), können Sie den CPU-Befehl 
des Betriebssystems verwenden (oder lesen Sie die Lektion über die Prozessoren
680x0 !!). An diesem Punkt (jetzt sind wir praktisch in der Verfassung von
einem Amiga 500), können Sie das Programm ausführen, indem Sie das Bit 10 von 
DMACON auf 0 oder 1 setzen. Ändern Sie dazu einfach das DMASET-Label.
Es steht am Anfang des Listing. Wenn Sie setzen:
...
Sie haben den Nasty Blitter (Bit 10) bei 0, und der Prozessor hat gelegentlich
der Vorrang.
Wenn Sie setzen:
...
Sie haben den Nasty Blitter (Bit 10) auf 1, und der Blitter hat immer den 
Vorrang. Sie können überprüfen, dass im zweiten Fall der Blitt schneller ist.
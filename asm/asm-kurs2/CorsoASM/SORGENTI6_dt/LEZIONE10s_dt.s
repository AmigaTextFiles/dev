...
; Lezione10s.s	Korrektes Füllen eines geschlossenen Polygons
...
fatto_bltcon1:
	...
	move.w	#12,$64(a5)		; BLTAMOD Breite 14 words (40-28=12)
	...
	move.l	a0,$54(a5)		; BLTDPT  Laden Sie den Parameter
	move.w	#(64*121)+14,$58(a5) ; BLTSIZE (Blitter starten!)
							; Breite 14 words
							; Höhe 121 Zeilen (1 plane)
	rts
...
DrawlineFill:
	cmp.w	d1,d3		; wenn D3 > D1 sind die Punkte bereits in der richtigen Reihenfolge
	bge.w	Ordinati
	exg.l	d1,d3		; ansonsten tauschen
	exg.l	d0,d2

Ordinati:
	sub.w	d1,d3		; D3 = DiffY (es ist sicherlich positiv)
...
DRAWL:
	...
	move.w	d0,d1		; .. kopiere sie nach D1 ..
	...
	move.l	a0,a1		; Kopiere die Adresse
	cmp.w	#7,d1		; die Bitnummer ist 7
	ble.s	NonCorreggi	; Wenn nicht, überspringen

	addq.w	#1,a1		; .. Zeigen Sie andernfalls auf das nächste Byte
	subq.w	#8,d1		; und mache die Bitnummer <7
NonCorreggi:
	not.b	d1			; umgekehrte Bitnummerierung
						; Diese Anweisung ist notwendig, weil
						; Die Bits sind innerhalb des Bytes nummeriert
						; von rechts nach links, während die Koordinaten
						; Pixel gehen von links nach rechts
	bchg	d1,(a1)		; Invertiere das erste Pixel der Linie

	move.w	d2,d1		; Kopie DX in D1
	addq.w	#1,d1		; D1=DX+1
	lsl.w	#$06,d1		; berechnet in D1 den Wert, der in BLTSIZE eingegeben werden soll
	addq.w	#2,d1		; addiert die Breite, gleich 2 Wörtern
	...
	btst	#6,2(a5)
WaitLine:
	btst	#$06,$02(a5); warte auf festen Blitter
	bne.s	WaitLine
	...
	or.w	#$0003,d5	; Bit 0 0 (aktiver Zeilenmodus), e
						; Bit 1 (spezielle Zeilen zum Füllen)
...
In diesem Beispiel sehen wir, wie das Problem der geschlossenen Linien 
gelöst werden kann. Wie wir in der Lektion erklärt haben, ändern wir die 
Linien-Verfolgungsroutine, so dass es von oben nach unten zeichnet. Dafür 
werden am Anfang die Y-Koordinaten der 2 Punkte verglichen, und falls
notwendig werden die 2 Punkte ausgetauscht. Auf diese Weise wird die 
Berechnung der Oktanten vereinfacht, da wir auf nur 4 Möglichkeiten beschränkt
sind. Weiterhin wird der erste Punkt der Linie mittels eines BCHG invertiert
um den Verfolgungseffekt aufzuheben. Zu diesem Zweck sind weitere 
Berechnungen zur Ermittlung der Bitanzahl und der richtige Adresse notwendig. 
Tatsächlich arbeitet das BCHG im Gegensatz zum Blitter mit einem Byte.
Wenn nötig, müssen Sie auf das untere Byte des Wortes von der Adresse zeigen.
Desweiteren ist es auch notwendig (mit dem NOT) die Bitnummerierung
zu invertieren, weil das BCHG die Bits von rechts nach links nummeriert.
Stattdessen werden die Koordinaten von links nach rechts nummeriert.
Die anderen Routinen sind identisch.
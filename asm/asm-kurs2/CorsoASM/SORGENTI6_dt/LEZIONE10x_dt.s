...
; Lezione10x.s	Effekt morphing!!!!!!! Coded by Executor/RAM JAM
...
In diesem Beispiel stellen wir eine Routine vor, die eine Animation mit 
einem Morph-Effekt ausführt. Dies ist eine der wichtigsten Techniken, 
verwendet im mythischen "State of the Art" von Spaceballs, vielleicht in  
einem der berühmtesten Demos aller Zeiten.
Mal sehen, wie diese Technik erreicht wird. Jeder Frame der Animation ist
mit einem einzigen verschmolzenen Polygon hergestellt. Um das Polygon zu 
zeichnen müssen wir die Kanten nachzeichnen und dann mit dem Blitter füllen.
Die erste Aufgaben wird von der Routine "DO_IT" und die zweite von der 
Routine "FILL" ausgeführt. Um die Animation zu erstellen, ändern Sie einfach 
die Form des Polygons für jedes Bild und dies geschieht durch das Ändern der
...
Speicher belegen. Darum verwenden wir eine andere Methode.
Wir speichern nur einige Frames und berechnen jedes Mal andere, indem Sie 
den Morph zwischen zwei der Frames machen, die wir gespeichert haben.
Die Animation ist daher in verschiedene Phasen unterteilt, in denen das 
Morphing zwischen einem Quellframe und einem Zielframe mit einer bestimmten
Anzahl der Frames gemacht wird.
Wenn der Zielrahmen erreicht ist, wechselt er zu einer neue Morph-Phase, 
die den Quellrahmen, den Zielrahmen und die Anzahl der Frames ändert.
Die Animationsschritte werden dabei in der Reihenfolge  gespeichert
ausgehend von der "STORY" -Adresse kommt der Übergang von einer Phase 
zur anderen. Die wird von der Routine "DO_ANIM" verwaltet.
...
ClearScreen:
	moveq	#5-1,d7			; 5 Bitplanes
	lea	BITPLANE1+100*40,a1	; Adresse Bereich der gelöscht werden soll (plane1)
...
WBlit1:
	...
	add.l	#256*40,a1			; Adresse nächste plane
	dbra	d7,Clearloop
	rts
	
;*****************************************************************************
; Diese Routine realisiert den Reflektoreffekt. Eine Operation wird ausgeführt
; durch ein UND zwischen der Figur und einer Maske
;*****************************************************************************
...
Riflettore:
	...
	lsr.w	#3,d2		; (entspricht einer Teilung durch 8)
	...
	add.w	d2,a0		; Summe zur Adresse der Bitebene, Finden
				; der richtigen Adresse in der Abbildung
...
SpostaMaschera:
	ADDQ.L	#2,TABXPOINT		; Zeigen auf das nächste Wort
	MOVE.L	TABXPOINT(PC),A0	; Adresse in TABXPOINT enthalten wird
								; nach a0 kopiert 
...
sein muss, im Gegensatz zu Lektion10c3.s. Da das verwendete System anders ist
und vor allem da es nicht genug Bitplanes gibt.
In diesem Beispiel sehen wir eine andere Technik, um den "Reflector"-Effekt zu 
realisieren. Dieses Mal haben wir ein Bild, das aus 5 Bitplanes mit der Breite
320  Pixel und 56 Zeilen besteht gespeichert ab der "Figura"-Adresse.
Die Tatsache, dass das Bild 5 Bitplanes hat, verhindert die Verwendung der 
vorherigen Technik, weil wir keine freie Bitplane haben, um die Maske separat 
zu zeichnen. Wir verwenden dann die folgende Technik: Wir führen eine UND-Operation 
zwischen der Maske und der Figur durch, und wir schreiben das Ergebnis in die 
Bitebenen des Bildschirms. Auf diese Weise werden nur die Bits der Figur auf 
den Bildschirm kopiert, die einem 1-Bit in der Maske entsprechen. 
...
Bildschirm arbeiten, müssen wir die Bitplanes eine nach der anderen abarbeiten.
Da die Maske nur zur Auswahl verwendet wird. Die Bits in der Abbildung sind für 
jede Bitebene immer gleich. Die Operation von AND wird natürlich von dem Blitter

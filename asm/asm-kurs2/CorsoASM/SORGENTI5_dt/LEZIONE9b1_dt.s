...
; Lezione9b1.s	BLITTED, in dem wir 8 Wörter in eine Null-Bit-Ebene kopieren.
...
Aspettasin:
	btst	#6,$bfe001	; Warten Sie, bis die linke-Maustaste gedrückt wird
	bne.s	Aspettasin
...
; In diesem Fall X = 32 und Y = 4.
	...
	move.w	#64*1+8,$58(a5)			; bltsize - Höhe 1 Linie,
									; Breite 8 words.
...
; Dies ist die "Figur", die mit einem blittata in die BITPLANE kopiert wird:
...
von der Marke "Figura_a_caso:" und überschreiben sie beginnend mit der Adresse
identifiziert durch das Label "BITPLANE:" Das Label Bitplane ist die Adresse 
einer Speicherzone, die eine Bitebene enthält.
Eigentlich kopieren wir die Daten in die BITPLANE + Offset, der von der Anfangsecke 
versetzt ist. Daher werden die Daten, die wir kopieren, auf dem Bildschirm angezeigt.
Um einen Kopiervorgang durchzuführen, müssen 2 DMA-Kanäle verwendet werden, eine 
zum Lesen und eine zum Schreiben. In diesem Fall benutzen wir den Kanal A zum
Lesen, und Kanal D zum Schreiben. Daher werden nur diese 2 Kanäle aktiviert,
indem die zugehörigen Bits im BLTCON0-Register auf 1 gesetzt werden. Um dem
Blitter mitzuteilen, dass er eine Kopie von Kanal A an den Kanal D senden soll
ist notwendig, das Byte, das die MINTERMS enthält, auf den Wert $f0 zu setzen.
Versuchen Sie, die Position zu ändern, in der die Figur gezeichnet wird, indem 
sie die Zieladresse der Blittata variieren. Wenden Sie das in der Lektion gemachte
Wissen an.

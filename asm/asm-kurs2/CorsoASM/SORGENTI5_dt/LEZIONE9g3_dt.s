...
; Lezione9g3.s	Wieder die Kacheln, aber diesmal mit dem INTERLEAVED-Bildschirm
...
POINTBP:
	...
	bsr.s	fillmem		; Führen Sie die "Kachel" Routine aus
...
FaiTutteLeRighe:
	moveq	#20-1,d6	; 20 Blöcke pro Zeile, tatsächlich
					; die Fliesen sind 16 Pixel breit,
					; das sind 2 Bytes, leitet es ab
					; das sind 320/16 = 20 für eine
					; horizontale Zeile.

; warte auf jede gezogene Zeile mit vblank.
WaitWblank:
...
; Es ist die gleiche Figur des Beispiels lezioine9f3.s nur dort war es
...
Format. Beachten Sie zunächst, die Figur der Kachel. In der 
Beispiellektion 9f3.s hatten wir die 2 separaten Bitebenen. Hier sind 
die Zeilen stattdessen gemischt.
In der Routine werden die Kacheln mit nur einem Blittata kopiert, während 
wir in der Lektion 9f3.s für jede Bitplane eine Blittata erstellen mussten.
Die Höhe des Blittings ist gleich dem Produkt der Höhe der Figur (15 Zeilen) 
und der Anzahl der Bitebenen (2), wie wir es in der Lektion erläutert haben.
Auch die Berechnung der Zieladresse ist (natürlich) anders. (Die Quelle ist 
fest und bleibt daher immer gleich). Die Kacheln der gleichen Zeile 
haben immer ein Wort Abstand zueinander. Wie gesehen unterscheidet sich
Interleaved nur durch die Anordnung der Zeilen im Unterschied zu den normalen
Bildern.
Der Unterschied liegt nach der internen Schleife, wenn wir das Ende einer
horizontalen Reihe von Kacheln erreicht haben und wir mit der folgende 
beginnen wollen.
Wenn wir mit Y die Zeile angeben, bei der wir zu blitten beginnen, müssen 
wir uns in der Zeile um Y + 16 bewegen.

In der inneren Schleife erhöhen wir den Zeiger jedes Mal um 2, um ihn  
jeweils ein Wort nach rechts zu bewegen. Am Ende der Schleife finden wir 
uns unmittelbar nach dem letzten Wort der aktuellen Zeile wieder, also das 
erste Wort von Ebene 2 der Zeile Y.
 
Zuerst müssen wir uns auf der Ebene 1 zur Linie Y + 1 bewegen. Dazu addieren
wir 40 (Anzahl der Bytes, die von einer Ebene einer Zeile belegt sind).
An diesem Punkt müssen wir weitere 15 Linien hinuntergehen.
Da eine Bitebene 40 Bytes belegt, haben wir jeweils zwei Ebenen Zeilen. 
Wir müssen 2 * 15 * 40 hinzufügen.
Natürlich können wir beide Mengen auf einmal hinzufügen und das
macht eine LEA 40 + 15 * 2 * 40 (A1), A1.
...
- Mit 40 addiert der Zeiger das mit (2) angegebene Wort.
- durch Hinzufügen von 2 * 40 * 15 bewegen wir uns 15 Zeilen nach unten und der 
  Zeiger zeigt auf das mit (3) bezeichnete Wort, welches das Wort ist, das wir 
  wollten.

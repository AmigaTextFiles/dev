...
; Lezione9c3.s	BLITTATA mit negativen MODULO
...
mouse:
	btst	#2,$dff016	; rechte Mausetaste gedrueckt?
	bne.s	mouse
...
; Dies ist die "Figur", die mit einer Blittata in die BITPLANE kopiert wird
...
In diesem Beispiel haben wir eine einzeilige hohe Figur, die wir kopieren
beginnend von einer bestimmten Zeile des Bildschirms, 10 Mal. Jedes 
Mal gehen wir eine Zeile runter. 
Natürlich könnten wir einfach eine Schleife von 10 Blittings machen, bei 
der wir jedes Mal die Zieladresse ändern. Es ist jedoch möglich, es mit 
nur eine Blittata die einen negativen Wert für das Quellmodulo hat, zu tun. 
Wie Sie wissen, wird der Wert des Modulo dem Inhalt der Adresse im 
Zeigerregister jedes Mal, wenn der Blitter eine Zeile beendet hinzugefügt.
Normalerweise setzt man im Modulo einen positiven Wert, der den Blitter 
erlaubt die Wörter, die nicht zum Rechteck gehören, zu "überspringen" und 
zu der nächsten Zeile zu gehen. Wenn das Modulo jedoch einen negativen Wert
hat, wird die Adresse die im Zeigerregister enthalten ist "zurückgegangen".
Insbesondere wenn die Blittata L words breit ist, und mischt eine Zeile 
mit dem im Zeiger enthaltenen Wert es wird um 2 * L erhöht (weil der Zeiger 
Bytes zählt, und 1 Wort = 2 Bytes ist).
...
verhalten wir uns stattdessen normal und dann werden die 10 Zeilen eine 
unter der anderen geschrieben.
...
Fall schreiben wir stattdessen die selbe Zeile mehrmals mit dem Blitter
in den Speicher.  

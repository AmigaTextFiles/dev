...
; Lezione9n1.s	Ladies and gentlemen, the SCROLLTEXT !!!!!
		; Left button to exit.
...
noreset
	SUB.B	#$20,D2		; ADDIERE 32 ZUM ASCII-WERT DES CHARAKTERS, IN
						; Möglichkeit, zum Beispiel, dass
						; SPACE ($20), in $00, das
						; Ausrufungszeichen ($21), in $01 ...
	ADD.L	D2,D2		; MULTIPLIZIERE MIT 2 DIE VORHERIGE NUMMER,
...
; Wir bewegen uns nach links, also benutzen wir den absteigenden Weg.
	...
	move.l	#bitplane+((21*(50+20))-1)*2,d0		; Adresse Quelle und
							; Ziel
ScorriLoop:
...
; Dies ist der Text. Mit 0 endet es. Die verwendete Schriftart enthält nur die
; Zeichen Großbuchstaben, Achtung!
...
COPPERLIST:
	...
	dc.w	$108,2		; Die Bitebene ist 42 Bytes breit, aber nur 40
						; Bytes sind sichtbar, dann das Modulo
						; Wert 42-40 = 2
	; dc.w $ 10a,2		; Wir verwenden nur eine Bitebene, dann BPLMOD2
...
In diesem Beispiel präsentieren wir einen der klassischsten Effekte der Demos:
Scrolltext. Dies ist ein Text, der auf dem Bildschirm von rechts nach links
scrollt und enthält in der Regel Grüße (Greeetings) Mandate von den Autoren 
der Demo zu den anderen Demo-Codern. Wie schafft man einen Scrolltext? 
Sie könnten daran denken, den gesamten Text auf einer Bitplane zu drucken der
größer als der sichtbare Bildschirm ist und dann scrollen Sie die Bitebene. 
Diese Technik hat den Nachteil, viel Speicher zu beanspruchen, weil sie 
eine Bitebene benötigt, die den gesamten Text enthält.
Wir verwenden eine andere Technik, die auf dem Blitter basiert, für die 
eine Bitebene ausreicht, die nur 16 Pixel breit (1 Wort) größer als der 
sichtbare Bereich ist. Also haben wir eine Spalte mit einem unsichtbaren Wort 
auf der rechten Seite des Bildschirms.
Nehmen wir an, wir drucken ein Zeichen im unsichtbaren Teil der Bitebene und
gleichzeitig schiebt man die Bitebene nach links mit dem Blitter. 
Es passiert, wie Sie es sich vorstellen können, dass der Charakter sich ein 
Pixel zu einem Zeitpunkt nach links bewegt. Beachten Sie, dass die 
Bitebene-Zeiger immer fest bleiben.
Wenn der Charakter komplett sichtbar ist, passiert das nach 16 Verschiebungen 
von 1 Pixel, da der Charakter 16 Pixel breit ist. Wir können das nächste 
Zeichen im unsichtbaren Teil des Bildschirms drucken. Die Zeichen, die am 
linken Rand ankommen, werden über die Maske vom Blitter gelöscht.
In der Praxis wird der Effekt unter Verwendung von 2 Routinen gewonnen.
Der erste "Printchar" befasst sich mit dem Drucken der Zeichen auf dem 
Bildschirm. Der Druckvorgang muss nur stattfinden, wenn das zuvor gedruckte 
Zeichen vorhanden ist und vollständig sichtbar werden, um eine Überlappung 
der 2 Zeichen zu vermeiden.
Da jedes Zeichen 16 Pixel breit ist, fließt der Text um ein Pixel nach.
Im Wesentlichen muss der Druck alle 16 Male auftreten, wenn die Routine
aufgerufen wird.
Dazu wird ein Zähler verwendet, der bei jedem Aufruf dekrementiert wird.
Wenn es den Wert 0 annimmt, wird das Zeichen gedruckt und der Zähler
auf 16 neu initialisiert. Der eigentliche Druck ist eine einfache Kopie mit 
dem Blitter (aufsteigend) und wird in der uns bekannten Weise hergestellt.
Die "Scroll" -Routine ist für das Scrollen des Textes mit der Verschiebung 
durch den Blitter nach links (also absteigend) in der Art, wie wir es 
im Beispiel lesson9m2.s gesehen haben verantwortlich. Da die Charaktere 20 
Zeilen groß sind, der gesamte Text belegt ein 20-Zeilen-High-Screen- "Band".
ist es notwendig, dieses "Band" oder ein hohes Rechteck 20 zu verschieben.
Die Linien und so breit wie jede Bitebene (einschließlich natürlich des 
UNSICHTBAREN Teils, um die Zeichen im sichtbaren Teil zu scrollen.
Die Maske des letzten Wortes löscht die Zeichen, bis sie die linke Kante 
erreichen.
Wir haben einen 1-Bitplane-Bildschirm verwendet, und die Farben erhalten
wir durch den copper (sind wir schlau?).

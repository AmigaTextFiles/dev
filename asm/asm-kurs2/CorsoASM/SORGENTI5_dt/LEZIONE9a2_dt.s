; Lezione9a2.s - Kopie von $10 Wörtern durch den BLITTER
...
Inizio:
	...
	jsr	-$1c8(a6)	; OwnBlitter, das gibt uns den exklusiven Zugang auf dem Blitter
					; verhindert, dass es vom Betriebssystem verwendet wird.
...
;	/         \
	...
	move.w	#(1*64)+$10,$dff058 ; BLTSIZE: definiert die Dimension des
					...
					; Weil die Höhe des Rechtecks in Bits 6-15 von 
...
Assembliert, ohne Jump, gesteuert mit dem Befehl des ASMONE "M SORG"
ab der SORG-Adresse gibt es im Speicher $10 Wörter, die verschiedene Werte
annehmen. Es ist die Quelle der Kopie, die von der Fläche ist.
Wir werden die Daten lesen. Überprüfen Sie dies auf gleiche Weise mit dem Befehl 
"M DEST" ausgehend von der DEST-Adresse werden $10 Wörter genullt.

Führen Sie an diesem Punkt das Beispiel aus.
Jetzt immer mit dem Befehl ASMONE "M" gehen und sehen sie, was im Speicher passiert ist:
Die Daten bei SORG blieben die gleichen wie zuvor.
Das ist normal, weil der Blitter einfach die Daten liest, ohne sie
zu bearbeiten. Stattdessen sind die Wörter beginnend mit der DEST-Adresse nicht mehr
zurückgesetzt, sondern haben die gleichen Werte wie die der Quelldaten angenommen.

Der Kopiervorgang erfordert die Verwendung eines Lese- und eines Schreib-Kanals.
In diesem Fall verwenden wir A zum Lesen und D (offensichtlich) zum Schreiben.
Um von Kanal A nach Kanal D zu kopieren, müssen die MINTERMS auf den Wert $F0 eingestellt 
werden. Daher ist der Wert, der in das BLTCON0-Register geladen werden soll, $09f0.

Beachten Sie, dass wir auch mit einem anderen Kanal zum Lesen (B oder C) hätten kopieren
können. Sie können es selbst in einer Übung versuchen. Die Änderungen sind 
sehr einfach:

- Aktivieren Sie den Kanal, den Sie anstelle von Kanal A verwenden möchten (Bit 8-11 von
  BLTCON0)

- Ändern Sie den Wert der MINTERMS (Bit 0-7 von BLTCON0), um eine Kopie anzuzeigen
  von dem Kanal, den Sie in Kanal D verwenden möchten.
  Um von Kanal B nach D zu kopieren, ist der korrekte Wert $CC, während zum Kopieren
  von C nach D der richtige Wert $AA ist.

- Schreiben Sie die Startadresse der zu kopierenden Daten statt in den Zeiger für
  Kanal A (BLTAPT) in den Kanalzeiger, den Sie verwenden möchten. Die Adressen der 
  Register BLTBPT und BLTCPT werden in der Lektion gezeigt.

...
; Lezione9i4.s	BOB mit "falschem" Hintergrund
...
;	Wir zielen auf die vierte Bitebene (den Hintergrund)
	...
	move.l	#SfondoFinto,d0	; Adresshintergrund
	move.w	d0,30(a0)		; Der Hintergrund ist Bitplane 4
	swap	d0	
	move.w	d0,26(a0)		; Schreibe ein Wort hoch
...
Waity1:
	...
	bsr.w	CancellaOggetto		; Entferne den Bob von der alten
								; Lage
...
MuoviOggetto:
	...
	cmp.w	#Lowest_Floor,d1	; Überprüfen Sie die Unterkante
	blt.s	UO_NoBounce1

	subq.w	#1,d3				; Entfernen Sie die Geschwindigkeitserhöhung
	neg.w	d3					; Ändern Sie das Geschwindigkeitszeichen dy
								; Bewegungsrichtung umkehren
	move.w	#Lowest_Floor,d1	; vom Rand weggehen
UO_NoBounce1:

	cmp.w	#Right_Side,d0		; Überprüfen Sie die rechte Kante
...
DisegnaOggetto:
	...
	move.w	d0,d1			; Kopie
	and.w	#$000f,d0		; Sie wählen die ersten 4 Bits, weil sie gehen
							; in den Shifter von Kanal A eingefügt
	lsl.w	#8,d0			; Die 4 Bits werden in das hohe Halbbyte verschoben
	lsl.w	#4,d0			; des word...
	or.w	#$09f0,d0		; ...Recht, das BLTCON0-Register einzugeben
	...
	add.l	d1,a0			; Summe zur Adresse der Bitebene, Finden
							; die richtige Zieladresse

	move.l	a0,IndirizzoOgg		; speichert die Adresse des
							; Ziel für die Routine
							; der Stornierung
...
WBlit2:
	...
	add.l	#4*31,a1			; Bild der nächsten Adressebene
...
CancellaOggetto:
	...
	move.l	IndirizzoOgg(PC),a0	; Lesen Sie die Zieladresse erneut
...
BPLPOINTERS:
	...
	dc.w	$194,$040			; sind die gleichen Werte
	...
	dc.w	$190,$345	; color 8 - Pixel bei 1 des Hintergrunds
...
Den Effekt haben wir aber mit einem Trick erhalten, der die Leistung stark einschränkt. 
Der Trick ist Folgender: Wir verwenden 4 Bitebenen, die ersten 3 zum Zeichnen des
Bobs und die vierte für den Hintergrund. Der Hintergrund und der Bob haben daher 
getrennte Ebenen. Damit der Bob über dem Hintergrund angezeigt wird, wird die Bitebene 
erstellt. Der Hintergrund wirkt sich nicht auf die Farben des Bobs aus. Betrachten Sie
zum Beispiel das Pixel des Bobs, das gebildet wird, indem Ebene 1 = 0, Ebene 2 = 1, 
Ebene 3 = 1 genommen wird. Diese Bits der sich bewegenden Pixel werden mit der 
Ebene 4 überlagert. Entspricht es einem auf 0 gesetzten Bit, bildet sich für die 4 
Ebenen die Ebene 1 = 0, Ebene 2 = 1, Ebene 3 = 1, Ebene 4 = 0, die Farbe 6.
Wenn stattdessen ein auf 1 gesetztes Bit gefunden wird, bildet sich die 
Kombinationsebene 1 = 0, Ebene 2 = 1, Ebene 3 = 1, Ebene 4 = 1 bilden, und
definiert damit die Farbe 14. 
So ändern sich die Farben des Bob abhängig von der Hintergrundbereichskreuzung. Wir 
möchten stattdessen, dass der Bob immer auf dem gleichen Hintergrund erscheint.
Wir können diesen Effekt auf sehr einfache Weise simulieren, indem wir die Farben
in den Registern gleich machen, die sich nur in den Hintergrundbits unterscheiden. 
Zurück zum Beispiel, wenn sie den gleichen RGB-Wert in das COLOR06-Register und in 
COLOR14 eingeben. Damit ist der Wert des Bits der Ebene 4 egal und unser Pixel erscheint 
immer in der gleichen Farbe. Dasselbe machen wir für alle anderen Register (dh wir 
platzieren COLOR01 = COLOR09, COLOR02 = COLOR10, COLOR03 = COLOR11 usw.) So können wir 
das Problem lösen.
Der "transparente" Teil des Bobs ist derjenige, der die 3 Ebenen bei 0 hat. Dies zeigt 
Farbe 0 oder Farbe 8 an, abhängig vom Wert des Bits in der Ebene 4. Indem Sie diese 
beiden Farben unterschiedlich halten, können Sie den Hintergrund anzeigen:
Die Bits bei 0 des Hintergrunds werden in Farbe 0 angezeigt, während die bei 1 in der
Farbe 8 angezeigt werden.
Um gut zu verstehen, was passiert, versuchen Sie ein wenig die COLOR01-07 Register auf 
andere Werte als COLOR09-15 zu setzen: Sie werden den Trick sofort feststellen. 
Diese Technik hat den Nachteil, dass einige Farben "verschwendet" werden.
In der Tat sind wir gezwungen, gleiche RGB-Werte in einige Register zu schreiben und
verringern damit die Anzahl der anzeigbaren Farben. In diesem Beispiel verwenden wir 4
Bitplanes, aber wir können nur 8 Farben für den Bob und 2 für den Hintergrund verwenden.
Wir verschwenden daher 6 Farben. Wenn wir 3 Bitplanes für den Bob und 2 für den Hintergrund 
verwendet haben, könnten wir 8 + 4 = 12 Farben anzeigen, gegenüber den 32, die 
normalerweise durch 5 Bitplanes möglich sind. Wie Sie sehen, ist diese Technik daher
auch nicht ideal.
...
1) Wir verwenden das Make-up (bereits gesehen) das BLTLWM bei 0, um die
Wortspalte rechts vom Bob zu speichern;
2) Wir verwenden einen NICHT verschachtelten Bildschirm, um die Hintergrundplane von 
der  Bob Plane zu trennen.
3) In den vorherigen Beispielen berechnen wir die Adresse des Bob-Ziels, sowohl
in der Zeichenroutine wie auch in der Löschroutine. Eigentlich zwischen
der Zeichnung und dem anschließenden Löschung ändert der Bob seine Position nicht.
(Erst nach dem Löschen tut er es.) So ist die Berechnung immer gleich und wird
nur einmal gemacht. In diesem Beispiel machen wir genau das:
Die Berechnung erfolgt in der DisjectObject-Routine und wird in der Variablen 
AddressOgg gespeichert. Die Löschroutine liest einfach das Ergebnis aus der Variablen 
erneut und verwendet sie.
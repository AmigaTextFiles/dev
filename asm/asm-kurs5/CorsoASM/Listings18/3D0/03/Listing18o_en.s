...
SPEED = 10						 ; speed of movement of stars
...
START:
	...
	MOVE.W	#DMASET!$8200,$96(A6)	; DMA control data
...
; --------------------------
; PROJECTION + STAR CLIPPING
; --------------------------
; zu = -256
; S  = 2^24

	lea.l	mtk,a6					; ^ projection table (1/(ze-256))*2^24
	move.w	#160,a2					; Hx	
	move.w	#128,d6					; Hy


	lea.l	star(pc),a0				; ^ data stars
	lea.l	dest,a3					; ^ buffer destination
	lea.l	colstar,a1

	move.w	#numstars-1,d7			; number of stars (points)
	...
	move.l	a7,stack
	lea.l	tabella(pc),a7			; ^ table Y*40

centr	movem.w	(a0)+,d0-d2			; xe,ye,ze
	sub.w	d3,d2					; speed'
	and.w	d4,d2					; d4=range movement

	add.w	d2,d2					; -ze*2
	neg.w	d2						; z and is done because by mistake
									; I have memorized the z coordinate
									; inverted
	...
	bmi.s	nvis00					; clipping  horizontal
	cmp.w	a5,d0
	bgt.s	nvis00
	...
	bmi.s	nvis00					; clipping vertical
	cmp.w	a4,d1
	bge.s	nvis00

	move.w	d2,(a1)+				; color
	move.w	d0,(a3)+				; x
	lsr.w	#3,d0					; calculate point position in memory
	add.w	d1,d1
	add.w	0(a7,d1.w),d0
	move.w	d0,(a3)+				; y
...
; -------------------
	...
	lea.l	dest,a3					; ^ buffer pointer
	move.l	planes,a2				; ^ 1o bitplane
	lea.l	10240*1(a2),a6			; ^ 2o bitplane

; depth for point brightness selection
	move.w	#(-512+170)*2,d3
...
trace	move.w	(a1)+,d6			; color
	move.w	(a3)+,d0				; xs
	move.w	(a3)+,d1				; vertical position in bytes y
	not.w	d0
...
; trace point in one of the three colors depending on the depth
	move.l	a2,a5
	add.w	d1,a5
...
dest	ds.l	5000				; star buffer
colstar	ds.w	500					; star color buffer

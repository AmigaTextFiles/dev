...
Ploop:
	...
	MOVE.W	#DMASET,$96(a5)		; DMACON - abilita bitplane, copper
								; und sprites.
	move.l	#Newclist,$80(a5)	; Zeiger COP
...
DrawLine:
	...
	bgt.s	.y2gy1				; überspringen wenn positiv..
...
.y2gy1:
	...
	bge.s	.xdpos				; überspringen wenn positiv..
	addq.w	#2,d1				; ..andernfalls den Index verschieben
	neg.w	d2					; und mach den Unterschied positiv
.xdpos:
	moveq	#$f,d4				; Maske für die unteren 4 Bits
	and.w	d0,d4				; wähle sie aus in D4
	...
	lsr.w	#3,d0				; offset X:
								; An Bytes ausrichten (für BCHG)
	add.w	d0,a0				; ergänzt die Adresse
	...
	or.w	#$B00+DL_MInterns,d4	; fügt die Gelegenheit hinzu
								; Minterm (OR oder EOR)
...
.dygdx:
	...
	addx.w	d1,d1				; multiplizieren Sie den Index mit 2 und
								; fügt es gleichzeitig der Flagge hinzu
	...
	moveq	#6,d1				; Verschiebung und Prüfwert für
								; wait blitter 
...
; Wenn wir Zeilen für die Füllung ausführen möchten, setzt der Oktantencode das Bit auf 1
...
NewIRQ:
	btst.b	#5,$dff01f			; INTREQR - Bit 5, VERTB, ist gelöscht?
	beq.w	NOIRQ_6c			; Wenn ja, ist es kein "echtes" int VERTB!
	movem.l	d0-d7/a0-a6,-(sp)

; Lassen Sie uns nun den Generatorpunkt behandeln, der über den Bildschirm schwingt.
...
;	d1.w = Koordinate Y (0-255)
	...
	and.w	#%111,d2			; nur die ersten 3 Bits von X auswählen , d.h.
...
; Jetzt nehmen wir die Werte aus der Sintab und speichern sie für den nächsten Schritt
...
;	d1.w = Koordinate Y (0-255)
	...
	movem.l	(sp)+,d0-d7/a0-a6
NOIRQ_6c:
	move.w	#%1110000,$dff09c	; INTREQ - Anforderung löschen BLIT,VERTB und COPER
...
; Set color registers:
	...
	dc.w	$0180,$0000			; color0 - schwarz (Hintergrund)
	dc.w	$0182,$0ff0			; color1 - gelb (cursor und gunwriter)
	dc.w	$0184,$0fff			; color2 - weiß (Buchstabe gestoppt)

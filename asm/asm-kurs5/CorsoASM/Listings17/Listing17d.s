...
* Fähigkeit des Blitters. Denken Sie einfach daran, dass jeder
* Bildschirmpixel eine n-stellige binäre Zahl ist (wobei n die
...
* Pixel uns zusätzlich für einen gegebenen Frame interessieren.
* Zuerst nehmen Sie den Bob und den betroffenen Bildschirmbereich und
* führen ein UND durch und verwenden als Ziel einen zusätzlichen Bereich
* namens Carry. Sobald dies erledigt ist, nehmen sie den Bob und führen
* das XOR mit der entsprechenden Bildschirm-Bitebene aus.
* Verwenden Sie dann den Carry-Bereich weiter, als wenn es unser Bob 
* für jede Bitplane war.
* Es werden eindeutig 2 Carry-Bereiche benötigt!!!
************************************************************
...
MAIN:	
	...
	bne.b	.xok				; nein
	move.l	xr_ptr(pc),a3		; ja, den Zeiger zurücksetzen
.xok:
	cmp.l	yl_ptr(pc),a4		; Ende der Tabelle von y?
	bne.b	.yok				; nein
	move.l	yr_ptr(pc),a4		; ja, den Zeiger zurücksetzen
.yok:
	moveq	#0,d0				; löschen
...
ShadeBob:
	...
	add.w	d2,d2				; welches das Pixel an der Stelle 'x' enthält 

	andi.l	#$f,d0				; die vier hohen Bits von d0
	ror.l	#4,d0				; enthalten die Verschiebung für den Blitter 
	...
	move.l	#$0ba00000,d4		; Aktivieren Sie den dma der ACD-Kanäle, Verschiebung einstellen
...
.1
	adda.l	#8000,a1			; nächste bitplane
...
; so dass sie eine Größe von 1 Byte haben.
...
Copper:	
	dc.l	$1fc0000,$1060c00	; AGA zurücksetzen, falls vorhanden, 
								; sonst keine Bedeutung
	
		; DataFetch und Anzeigegröße einstellen

...
*Mem1 = AllocateMemoryBank(0, 64, 1 LSL 16) ; allocate first memory bank
                                            ; with a size of 64 bytes
                                            ; (public mem + clear mem)
*Mem2 = AllocateMemoryBank(1, 64, 1 LSL 16) ; allocate second memory bank
                                            ; with a size of 64 bytes
                                            ; (public mem + clear mem)

if *Mem1 and *Mem2

  FreeChip.l = AvailableMemory(2) / 1024 ; get free Chip-RAM in kb (/1024)
  PrintNumber(FreeChip.l)                ; and print it
  PrintN("k Chip-RAM free")

  FreeFast.l = AvailableMemory(1) / 1024 ; get free Fast-RAM in kb (/1024)

...
** directly from the factor code in evalute.c. The ones
** invoked via exec_function are marked as 'static'. if they
** are not static they are called from evaluate.c
*/
...
/*
** Following are the functions that deal with the built-in Basic functions.
** On entry to each of these, 'basicvars.current' points at the character
** after the function's number.
*/
...
/*
** 'fn_top' pushes the address of the end of the Basic program itself
** on to the Basic stack
*/
static void fn_top(void) {
...
static void (*function_table[])(void) = {
  ...
  fn_sum, fn_tan, fn_tempofn, fn_top, 			/* 3C..3F */
  fn_usr, fn_val, fn_verify, fn_vpos,  			/* 40..43 */
  fn_xlatedol						/* 44     */
};

...
static handler *oldfpe, *oldsegv, *oldint;
...
void init_errors(void) {
  ...
  if (basicvars.misc_flags.trapexcp) {	/* Want program to trap exceptions */
    oldfpe = signal(SIGFPE, handle_signal);
    oldsegv = signal(SIGSEGV, handle_signal);
    oldint = signal(SIGINT, handle_signal);
#ifdef TARGET_DJGPP
...
void show_options(void) {
  ...
  emulate_printf("The program starts at &%p and is %d bytes long.\r\nVariables start at &%p and occupy %d bytes. %d bytes of memory remain\r\n",

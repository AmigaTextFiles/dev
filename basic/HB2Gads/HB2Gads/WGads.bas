...
DIM f_gads&(0),m_gads&(16),ng(NewGadget_sizeof\2)
DIM AGadgetTags&(10),GadgetTags&(20),TempTags&(40),ATempTags&(10)
...
IF myscr&= 0 THEN
 Error.Trap 1
ELSE
	TAGLIST VARPTR(TempTags&(0)),TAG_DONE&
	vi&= GetVisualInfoA&(myscr&,VARPTR(TempTags&(0)))
...
'-----
DIM FboxTags&(20),RboxTags&(20)
TAGLIST VARPTR(FboxTags&(0)),GTBB_FrameType&,1,GT_VisualInfo&,vi&,TAG_DONE&
TAGLIST VARPTR(RboxTags&(0)),GTBB_Recessed&,1,GT_VisualInfo&,vi&,TAG_DONE&
'-----
'
'''''''''
Main.Menu
...
REDIM label$(0) :REDIM f_gads&(0)
FOR i= 0 TO 8 :READ parm(i) :NEXT :READ label$(0)
Create.Gadgets f_gad&,f_list&,f_gads&()
Gads.Window f_list&,1 :Add2.IDCMPFlags
...
STATIC gadgets,i,j
'-------
Gads.Window 0&,2 :Add1.IDCMPFlags
'-------
f_gad&= 0& :f_list&= 0&
f_gad&= CreateContext&(VARPTR(f_list&))
'-------
...
Process.WindowEvents f_gads&()
'-------
IF gid= 7 THEN CloseWindow act.win& :FreeGadgets f_list& :EXIT SUB
'-------
...
FOR i= 0 TO 5
 ...
 Create.Gadgets f_gad&,f_list&,f_gads&()
NEXT 
Gads.Window f_list&,0 :Add2.IDCMPFlags
Print.Notes 1
Process.WindowEvents f_gads&()
'-------
...
NEXT
'---
Gads.Window f_list&,0 :Add1.IDCMPFlags
Print.Notes 2
Process.WindowEvents f_gads&()
'---
...
SHARED FboxTags&(),RboxTags&()
'-------
SELECT CASE which
=0 :RESTORE Main_Boxes
 FOR i= 0 TO 2 :READ lef&,topp&,vidth&,height&
  DrawBevelBoxA rport&,lef&,topp&,vidth&,height&,VARPTR(FboxTags&(0))
 NEXT :Prin.T "G A D T O O L S  G A D G E T S",102,0,13
=1 :DrawBevelBoxA rport&,72,36,311,93,VARPTR(FboxTags&(0))
 DrawBevelBoxA rport&,72,133,311,28,VARPTR(FboxTags&(0))
=3 :DrawBevelBoxA rport&,52,55,366,88,VARPTR(FboxTags&(0))
=8 :SetAPen rport&,1 :RectFill rport&,11,41,415,121
 DrawBevelBoxA rport&,8,40,204,83,VARPTR(FboxTags&(0))
 DrawBevelBoxA rport&,214,40,204,83,VARPTR(FboxTags&(0))
 c(1) =12 :c(2) =210 :c(3) =218 :c(4) =415 :c(5)=42 :c(6) =121
 FOR i= 7 TO 10 :c(i)= 0 :NEXT
=16 :c(0)= 114 :c(1)= 111 :c(2)= 127 :c(3)= 295
 DrawBevelBoxA rport&,114,110,300,12,VARPTR(RboxTags&(0))
 DrawBevelBoxA rport&,114,126,300,12,VARPTR(RboxTags&(0))
END SELECT
EXIT SUB
'-------
Main_Boxes:
...
SHARED AGadgetTags&(),cy$(),GadgetTags&()
SHARED mx$(),ng(),viewlist&,topaz80(),shi1$()
'-----
...
SUB Gads.Window (glist&,BYVAL flags)
SHARED TempTags&(),ATempTags&(),viewlist&
'-----
wa.flags&= WFLG_GIMMEZEROZERO&+ WFLG_BORDERLESS&
IF flags= 1 THEN wa.flags&= wa.flags&+WFLG_CLOSEGADGET&
IF flags= 2 THEN
 TAGLIST VARPTR(ATempTags&(0)),TAG_DONE&
ELSE
 TAGLIST VARPTR(ATempTags&(0)),WA_Gadgets&,glist&,TAG_DONE&
END IF
'-----
TAGLIST VARPTR(TempTags&(0)), _
 WA_Left&, 84&, _
 WA_Top&, 23&, _
 WA_Width&, 472&, _
 WA_Height&, 175&, _
 ...
 WA_Flags&, wa.flags&, _
 WA_CustomScreen&, PEEKL(SYSTAB+12), _
TAG_MORE&, VARPTR(ATempTags&(0))
'-----
win&= OpenWindowTagList&(0,VARPTR(TempTags&(0)))
'-----
IF win&<> 0 THEN
 GT_RefreshWindow win&,0&
 act.win&= win&
 rport&= PEEKL(win&+rport)
ELSE
 IF glist&<> 0& THEN FreeGadgets glist&
 IF viewlist&<> 0& THEN Free.ViewList	viewlist&
 Error.Trap 7
...
SHARED c&(),cyi$(),gad&,mxi$()
SHARED terminated,viewlist&
'----------------
...
TAGLIST VARPTR(TempTags&(0)),item&,value&,TAG_DONE&
GT_SetGadgetAttrsA gadget&,act.win&,0&,VARPTR(TempTags&(0))
END SUB
...
=7 :txt$= "FAILED TO OPEN WINDOW" :et= 0
END SELECT
y= (80-LEN(txt$))\2
...
=0 :Prin.T "A: Cycle Gadget. B: Mutually Exclusive Gadget.",100,0,18
 ...
 Prin.T "A",200,77,84  :Prin.T "B",200,389,84
 Prin.T "C",200,77,100 :Prin.T "D",200,77,116
=1 :Bevel.Boxes 1

...
EXPORT PROC vdRunModule( finf:PTR TO vdFileInfo, command )
  ...
  IF finf.cfg.seglist[i]<>NIL
    ...
    StringF( args, 'file="\s" cmd=\s desc="\s" object=\d\n',
             finf.filename, ListItem(cmdstr,command), FilePart(finf.cfg.descfile), eargs )
    IF RunCommand( finf.cfg.seglist[i], 4096, args, StrLen(args))=0 THEN
      rc:=TRUE

  ELSE
    -> RUN ORDINARY SCRIPT-TYPE MODULE
    StringF( args, '"\s" file="\s" cmd="\s" desc="\s"', finf.cfg.module[i], finf.filename, ListItem(cmdstr,command), FilePart(finf.cfg.descfile) )
    ...
    IF rdargs:=ReadArgs( 'WZ/M', {args}, rda )
      WHILE args[i]
        ...
        IF subrdargs:=ReadArgs( 'CMD/A,ARG/F', subargs:=[0,0], rda )
          cfg.wizard[j].command   := estr( subargs[0] )
          cfg.wizard[j].arguments := estr( subargs[1] )
          cfg.wizard[j].seglist   := LoadSeg( subargs[0] )
          INC j
  ...
  IF (len:=GetVar('.modules', mem, StrMax(mem), GVF_BINARY_VAR)) > 0
    ...
    IF rda:=AllocDosObject( DOS_RDARGS, NIL )
      ...
      IF miscargs := New( SIZEOF vdMiscConfig )
        IF rdargs := ReadArgs( 'WIZDIR/K,EDITOR/K,EDARGS/K,TABU/F', miscargs, rda)

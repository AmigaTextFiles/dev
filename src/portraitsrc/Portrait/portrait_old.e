-> TODO
-> Status line for mode (so, eg. people know about the clone tool, etc. :-)
-> Hints for buttons
-> Project Window
-> Scrollers
-> Shapes window
...
CONST ID_RNDR="RNDR"
ENUM SEL=1,MENU
ENUM CONTINUOUS=0, LINE, RECTANGLE, FILL, CLEAR, CLONE, MAXDRAWMODE, SPREAD
...
DEF canvas=NIL:PTR TO canvas, cg=NIL:PTR TO colourgrid,
    wheel=NIL:PTR TO colorwheel, gpens=NIL:PTR TO INT, g=NIL:PTR TO gradientslider,
...
DEF labels=NIL:PTR TO tablabel, tabsgui=NIL:PTR TO LONG
...
PROC main() HANDLE
  ...
  IF (temprp:=AllocVec(MEMF_CLEAR,0))=NIL THEN Raise()
  InitRastPort(temprp)
  FOR i:=0 TO 11
    args[i]:=NIL
  ENDFOR
  ...
  IF (ifferror:=OpenIFF(iffhandle, IFFF_WRITE))=NIL
    IF (ifferror:=PushChunk(iffhandle, ID_WORK, ID_FORM, IFFSIZE_UNKNOWN))=NIL
      ...
      IF (ifferror:=PushChunk(iffhandle, NIL, ID_BODY, IFFSIZE_UNKNOWN))=NIL
        ...
        PopChunk(iffhandle)
      ENDIF
      IF (ifferror:=PushChunk(iffhandle, NIL, ID_RNDR, IFFSIZE_UNKNOWN))=NIL
        IF WriteChunkBytes(iffhandle, canvas.bitmap, 6)<>6
          ifferror:=IFFERR_WRITE
        ELSE
          i:=0
          WHILE (i<canvas.bitmap.depth) AND (ifferror=NIL)
            IF WriteChunkBytes(iffhandle, canvas.bitmap.planes[i], canvas.bitmap.bytesperrow*canvas.bitmap.rows)<>(canvas.bitmap.bytesperrow*canvas.bitmap.rows) THEN ifferror:=IFFERR_WRITE
          ENDWHILE
        ENDIF
        PopChunk(iffhandle)
      ENDIF
...
PROC gtrender(gl,vis,ta,x,y,xs,ys,win:PTR TO window) OF canvas
  ...
  self.horizgadget:=CreateGadgetA(SCROLLER_KIND,gl,[self.x,self.y+self.ys-10,self.xs-18,10,NIL,ta,HORIZ_ID,NIL,vis,NIL]:newgadget,
                                  [GTSC_ARROWS,16,GTSC_TOTAL,self.zwidth,GTSC_TOP,self.ox,GTSC_VISIBLE,xs-18,NIL])
  self.vertgadget:=CreateGadgetA(SCROLLER_KIND,self.horizgadget,[self.x+self.xs-18,self.y,18,self.ys-10,NIL,ta,VERT_ID,NIL,vis,NIL]:newgadget,
                                 [GTSC_ARROWS,8,GTSC_TOTAL,self.zheight,GTSC_TOP,self.oy,GTSC_VISIBLE,ys-10,PGA_FREEDOM,LORIENT_VERT,NIL])
  self.refresh(TRUE)
...
PROC refresh(resized=FALSE) OF canvas
  ...
  ENDIF
  IF self.ox>(self.xs-self.zwidth) THEN self.ox:=self.xs-self.zwidth
  IF self.oy>(self.ys-self.zheight) THEN self.oy:=self.ys-self.zheight
...
PROC message_test(imsg:PTR TO intuimessage,win:PTR TO window) OF canvas
  ...
  ->IF (imsg.mousex>=self.x) AND (imsg.mousey>=self.y) AND (imsg.mousex<(self.x+self.xs)) AND (imsg.mousey<(self.y+self.ys))
    IF ((imsg.class AND IDCMP_MOUSEMOVE)<>NIL) OR ((imsg.class AND IDCMP_MOUSEBUTTONS)<>NIL) OR ((imsg.class AND SCROLLERIDCMP)<>NIL) THEN RETURN TRUE
  /*ELSE
    self.butt:=NIL
  ENDIF*/
ENDPROC FALSE
...
PROC update() OF canvas
  DEF x,y,h,o:PTR TO colour,rxs,rys,pen,prog=FALSE
...
ENDPROC name
CHAR '$VER: portrait 1.2 (7.06.98) development version',0


REM Drawing a single pixel to the Python Shell window.
REM Compiled using ACE Basic Compiler, (C)David Benn.
REM Original idea copyright, (C)2007, B.Walker, G0LCU.
REM Written in 'baby BASIC' for kids to understand.
REM
REM Syntax:-
REM DrawPixel <x> <y> <colour><RETURN/ENTER>
REM
REM Where:-
REM 'x' is a value from 0 to 1280.
REM 'y' is a value from 0 to 1024.
REM 'colour' is a value from 0 to 7, (31).
REM
REM $VER: DrawPixel.b_Version_0.00.02_(C)2007_B.Walker_G0LCU.

REM Set up the necessary variables.
  LET x$="0"
  LET y$="0"
  LET colour$="0"
  LET x=0
  LET y=0
  LET colour=0
  LET n=0

REM Now get the values from the command line.
  LET x$=ARG$(1)
  LET y$=ARG$(2)
  LET colour$=ARG$(3)
  LET n=ARGCOUNT

REM Check for the correct number of arguments, do NOT allow an error.
  IF n<=2 THEN GOSUB setalltozero:
  IF n>=4 THEN GOSUB setalltozero:

REM Limit the pixel positions to the values shown.
  LET x=VAL(x$)
  IF x<=0 THEN LET x=0
  IF x>=1280 THEN LET x=1280
  LET y=VAL(y$)
  IF y<=0 THEN LET y=0
  IF y>=1024 THEN LET y=1024
  LET colour=VAL(colour$)
  IF colour<=0 THEN LET colour=0
  IF colour>=31 THEN LET colour=31

REM Now 'PLOT' the pixel to the screen.
  PSET (x,y),colour
  END

REM Do NOT allow an error and set all to zero.
setalltozero:
  LET x$="0"
  LET y$="0"
  LET colour$="0"
  RETURN

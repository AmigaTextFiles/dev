...
# SIMPLE radio button generation for PSEUDO-GUI usage.
# Original idea copyright, (C)2007, B.Walker, G0LCU.
# This IS SSLLOOWW but it works... :)
#
# $VER: Radio-Buttons.py_Version_0.00.11_(C)2007_B.Walker_G0LCU.
...
# Set up three radio buttons.
# Radio button one shown as a 'default' ON.
...
def main():
	...
	while 1:
		...
		checkmouse()
		# Set the bounds for EACH radiobutton!
		...
		# Whichever radio button is set generate a random number.
		# If any are NOT set then the number displayed will be the
		# last known number for that button!
		if rbone == 0:m = (1 + int(whrandom.random() * 100))
		if rbtwo == 0:n = (1 + int(whrandom.random() * 100))
		if rbthree == 0:o = (1 + int(whrandom.random() * 100))

/* ------------------------------------------------------------------------
   :Program.    DelUser.rexx
   :Contents.   removes an UMS user
   ...
   :Version.    $VER: DelUser.rexx 1.0 (13.1.96)
...
ArgsTemplate = "NAME/A,PWD,SERVER/K,USER/A"
args.SERVER = ""
...
if ~UMSDeleteUser(account,args.user) then do
  call CheckErr
  rc = 20; call logout
end

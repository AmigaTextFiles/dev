...
LIBRARY 'blur.wbbplugin', 1, 0, 'blur.wbbplugin 1.0 (7/5/99)' IS
...
PROC pluginInitInstance(tags:PTR TO tagitem) HANDLE
	...
	IF tags
		...
		h.passes	:= IF args[0] THEN Long(args[0]) ELSE 1
		IF h.passes > 1 THEN h.tempbuf := NewR(Mul(h.height, h.width))
		IF h.passes < 1 THEN eThrow(-1, 'Wrong number of passes: %ld', [h.passes])
	ELSE
		RETURN NIL
	ENDIF
...
PROC pluginFreeInstance(h:PTR TO handle)
	...
	IF h.tempbuf THEN Dispose(h.tempbuf)
...
PROC pluginGetAttr(h:PTR TO handle, attr, valueptr:PTR TO LONG)
	...
	CASE PLUGINTAG_COMMANDNAME
		valueptr[0] := 'PLUGIN_BLUR'	-> tooltypes command
		RETURN TRUE
	...
	CASE PLUGINTAG_DESC
		valueptr[0] := 'Blurs the input image'
		RETURN TRUE
...
PROC pluginDoAction(h:PTR TO handle, inbuf:PTR TO CHAR, outbuf:PTR TO CHAR, tags:PTR TO tagitem) HANDLE
	...
	IF inbuf <> NIL
		...
		ELSE
			...
			b2 := h.tempbuf
		ENDIF
		blur_buffer(inbuf, b1, h.width, h.height)
		FOR i := 0 TO h.passes-2
			blur_buffer(b1, b2, h.width, h.height)
			bx := b1
			b1 := b2
			b2 := bx
		ENDFOR
	...
	ELSE
		FOR i := 0 TO Mul(h.width, h.height)-1 DO outbuf[i] := 0
	ENDIF

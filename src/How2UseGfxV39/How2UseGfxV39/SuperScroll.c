/*  -------------------------------------------------------------------------
    ...
    Description: This is a short demo code that shows how we could
                 ...
                 The ball is  drawn transparent, check out masking and
                 ...
                 methods of image handling, without worriing of the destination
                 bitmap format ! This code generates the optimal bitmap format
                 for the images by itself - see the friend field in the AllocBitMap() !
                 
                 This is part 2 of my How2UseGfxV39 upload. 
                 It is dedicated to all that mean a A\Box will solve 
                 Amiga's problems. It won't till we do not support what we 
                 already have. What we need most is an efficient gfx system
                 that will take care of the hardware below.
                 Look what a Cirrus Blitter can do when running this demo
                 on EGS/EGSPlus. You can directly blit into visible bitmaps,
                 using WaiTOF() syncronisation and layered windows
                 (for real fast clipping EGSPlus Release 8 is required).
                 This makes smooth scrolling and blitting available
                 WITH the OS - much better and faster than you will ever be
                 by pokeing around in AGA registers.
                 (or maybe you want to try a clipped parallax scrolling by
                 doing you own code ? :)
    ...
    Date: 02.12.1996 (96/12/02)

    --------------------------------------------------------------------------  */

// ---------------------------------------------------------------------------
// if defined the window is opened on the WB, else a WB Close Screen is opened
// ---------------------------------------------------------------------------
#define PUBLIC_SCREEN
// ---------------------------------------------------------------------------
// if this is defined, we you my own DeltaScrollWindow() instead of ScrollLayer
// in this case the ball is switched off, this is basically for boards that do
// not support on board friend bitmaps (e.g. Cybergraphics) or the 2410 (yet)
// ---------------------------------------------------------------------------
//#define USE_DELTA_SCROLL


// ---------------------------------------------------------------------------
// as EGSPlus may now allocate SPECIALFMT BitMaps I have to define that flag
// ---------------------------------------------------------------------------
...
UBYTE bgpens[256],ballpens[256]; // 256 color array for the image and the ball

// ---------------------------------------------------------------------------

BOOL IsGfxBoard = FALSE;    // check if run of gfx board
BOOL verbose = FALSE;       // dump some output
...
void CleanUp()
{
    ...
    if (SuperBackGround) FreeBitMap(SuperBackGround);
...
void CreateBall(struct BitMap *DisplayMap)
{
    ...
    CopyMemQuick(MaskBuffer->Planes,BackupPlanes,8*sizeof(PLANEPTR));
    ...
    CopyMemQuick(BackupPlanes,MaskBuffer->Planes,8*sizeof(PLANEPTR));
    ...
    // here we convert the remaped chunky image to a friend bitmap of the window
    // we use a temporary rastport here where we install the previous allocated
    // ball bitmap. We use WriteChunkyPixels() here, so it will be converted
    // to the correct format ! This could also be a chunky format !
    // if we are on a gfxboard, this is just a plain copy, if we are on ECS/AGA,
    // it converts it to plane sources
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // I also don't need the tmpRastPort anymore, so I have to free its bitmap
    // ------------------------------------------------------------------
...
// ----------------------------------------------------------------------
// This is used to scroll the background around. It uses one on screen blit
// (usually the blitter is used) and 2 delta blits that refresh the resulting
// free areas. This makes 3 blits against one with ScrollLayer() - so if
// the SuperBitMap is available on the board ScrollLayer is much faster
// ----------------------------------------------------------------------
void DeltaScrollWindow(struct Window *win,int dx,int dy)
{
    int x_offset,y_offset,dstx,dsty,srcx,srcy;

// ----------------------------------------------------------------------
// as the source bitmap is located on absolut coords on the screen, the
// offset has to be calculated to window margins
// the dest is a RastPort and because of that transformed to the 
// window origin internally. Also Deltas can never be 0 (dx/dy).
// ----------------------------------------------------------------------

    x_offset = win->LeftEdge + win->BorderLeft;
    y_offset = win->TopEdge  + win->BorderTop;
    
    if (dx > 0 && dy > 0)
    {
        WaitBlit();
        BltBitMapRastPort(win->RPort->BitMap,x_offset + dx,y_offset + dy,win->RPort,0,0,
                          VISIBLE_WIDTH - dx, VISIBLE_HEIGHT - dy, 0xc0);

        WaitBlit();
        BltBitMapRastPort(SuperBackGround,x,y + VISIBLE_HEIGHT - dy,
                      win->RPort,0,VISIBLE_HEIGHT - dy,VISIBLE_WIDTH,dy,0xc0);

        WaitBlit();
        BltBitMapRastPort(SuperBackGround,x + VISIBLE_WIDTH - dx,y,
                      win->RPort,VISIBLE_WIDTH - dx,0,dx,VISIBLE_HEIGHT,0xc0);
    }
    if (dx > 0 && dy < 0)
    {
        WaitBlit();
        BltBitMapRastPort(win->RPort->BitMap,x_offset + dx,y_offset,win->RPort,0,-dy,
                          VISIBLE_WIDTH - dx, VISIBLE_HEIGHT - dy, 0xc0);

        WaitBlit();
        BltBitMapRastPort(SuperBackGround,x,y,
                      win->RPort,0,0,VISIBLE_WIDTH,-dy,0xc0);

        WaitBlit();
        BltBitMapRastPort(SuperBackGround,x + VISIBLE_WIDTH - dx,y - dy,
                      win->RPort,VISIBLE_WIDTH - dx,-dy,dx,VISIBLE_HEIGHT + dy,0xc0);
    }
    if (dx < 0 && dy > 0)
    {
        WaitBlit();
        BltBitMapRastPort(win->RPort->BitMap,x_offset ,y_offset + dy,win->RPort,-dx,0,
                          VISIBLE_WIDTH - dx, VISIBLE_HEIGHT - dy, 0xc0);

        WaitBlit();
        BltBitMapRastPort(SuperBackGround,x,y,
                      win->RPort,0,0,-dx,VISIBLE_HEIGHT - dy,0xc0);
    ...
    {
        WaitBlit();
        BltBitMapRastPort(win->RPort->BitMap,x_offset,y_offset,win->RPort,-dx ,-dy,
                          VISIBLE_WIDTH + dx, VISIBLE_HEIGHT + dy, 0xc0);
        ...
        WaitBlit();
        BltBitMapRastPort(SuperBackGround,x,y,
                          win->RPort,0,0,-dx,VISIBLE_HEIGHT,0xc0);
...
void ScrollBackGround()
{
    ...
    // ------------------------------------------------------------------
    // Now move the background - self explaining - ...
    // ------------------------------------------------------------------

#ifndef USE_DELTA_SROLL

    ScrollLayer(NULL,win->RPort->Layer,dx,dy);

#else

    DeltaScrollWindow(win,dx,dy);

#endif

    // ------------------------------------------------------------------
    // ... and blit the Green Ball over the new background
    // ------------------------------------------------------------------
    ...
    if (((gy + abs(dgy)) > (VISIBLE_HEIGHT - GREEN_HEIGHT)) || (gy < (abs(dgy)+5)))
    {
        dgy *= -1;
    }
}
...
// ----------------------------------------------------------------------
// this could be a custom screen (modify the TAGS if you like) ...
// ----------------------------------------------------------------------
...
// ----------------------------------------------------------------------
        ...
        if (win = OpenWindowTags(NULL,
                                ...
                                WA_SmartRefresh, TRUE,  // SuperBitMap MUST always be SmartRefresh, else no image
                                WA_DragBar, TRUE,
                                WA_CloseGadget, TRUE,
                                WA_DepthGadget,TRUE,
            ...
            while (!out)
            {
                ...
                if (message = (struct IntuiMessage *)GetMsg(win->UserPort))
                {
                    ...
                    switch(class)
            	    {
        		        ...
        		        case NEWSIZE:
                            // -------------------------------------------------
                            // the xor rects on a window move trashes my bitmap
                            // so I refresh it from my superbitmap
                            // This doesn't work, maybe the MoveWindow() destroys
                            // my superbitmap, too.
                            // -------------------------------------------------
